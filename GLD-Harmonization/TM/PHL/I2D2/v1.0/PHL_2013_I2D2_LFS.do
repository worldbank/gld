...
** Created	6/21/2013
** Modified	2/23/2017
** NUMBER OF HOUSEHOLDS	42389
** NUMBER OF INDIVIDUALS	201623
** EXPANDED POPULATION	98499055
**                                                                                                  **
******************************************************************************************************
*****************************************************************************************************/
...
** DIRECTORY
	local path "D:\__I2D2\Philippines\2013\LFS"


** LOG FILE
	log using "`path'\Processed\PHL_2013_I2D2_LFS.log", replace
...
** DATABASE ASSEMBLENT
	...
	use "`path'\Original\LFS JAN, APR 2013\LFS JAN2013.dta"
...
** YEARS OF EDUCATION COMPLETED
	gen byte educy=.
	replace educy=0 if j12c09_grade>=0 &  j12c09_grade<=260
	replace educy=7 if  j12c09_grade>=280 &   j12c09_grade<=330
	replace educy=13 if  j12c09_grade==350
	replace educy=16 if  j12c09_grade>500 &  j12c09_grade<800 & age>5
...
** EDUCATIONAL LEVEL 1
	...
	replace edulevel1=4 if  j12c09_grade>=310 &  j12c09_grade<=330
	replace edulevel1=5 if  j12c09_grade>=350 &  j12c09_grade<500
	replace edulevel1=7 if j12c09_grade>500 & age>5
...
** LABOR STATUS
	...
	replace lstatus=2 if  c38_lookw==1
...
** EMPLOYMENT STATUS
	gen byte empstat=.
	replace empstat=1 if c19_pclass==0 | c19_pclass==1 | c19_pclass==2
	replace empstat=2 if c19_pclass==6
	replace empstat=3 if c19_pclass==4
	replace empstat=4 if c19_pclass==3  | c19_pclass==5
...
** SECTOR OF ACTIVITY: PUBLIC - PRIVATE
	...
	replace ocusec=2 if lstatus==1 & c19_pclass!=2
...
** REASONS NOT IN THE LABOR FORCE
	...
	replace nlfreason=3 if c42_wynot==6 & age>15
...
** INDUSTRY CLASSIFICATION
	gen byte industry=.
	replace industry=1 if (c18_pkb>=100 &c18_pkb<=400)
	replace industry=2 if (c18_pkb>=500&c18_pkb<=1000) 
	replace industry=3 if (c18_pkb>=1011 & c18_pkb<=3299) 
	replace industry=4 if (c18_pkb>=3311 & c18_pkb<=3830) 
	replace industry=5 if c18_pkb>= 4100 & c18_pkb<=4390
	replace industry=6 if (c18_pkb>=4510 & c18_pkb<=4799)| (c18_pkb>=5610 & c18_pkb<=5630)
	replace industry=7 if (c18_pkb>=4911 & c18_pkb<=5590)| (c18_pkb>=5811 & c18_pkb<=6399) 
	replace industry=8 if (c18_pkb>=6411 & c18_pkb<=8299) 
	replace industry=9 if (c18_pkb>=8411)  & c18_pkb<=9103
	replace industry=10 if  (c18_pkb>=9200& c18_pkb<=9909)
...
** OCCUPATION CLASSIFICATION
	gen byte occup=floor(c16/1000)
	recode occup 0=10
	replace occup=99 if c16==930
...
** OCCUPATION CLASSIFICATION - SECOND JOB
	gen byte occup_2=floor(j02_otocc/10)
	recode occup_2 0=10
...
** DELETE MISSING VARIABLES
	...
	saveold "`path'\Processed\PHL_2013_I2D2_LFS.dta", replace
	saveold "D:\__CURRENT\PHL_2013_I2D2_LFS.dta", replace
