...
** NUMBER OF INDIVIDUALS	198657
** EXPANDED POPULATION	87834025
**                                                                                                  **
******************************************************************************************************
*****************************************************************************************************/
...
** DATABASE ASSEMBLENT
	...
	use "`path'\Original\LFS_JAN2007.dta"
...
** HOUSEHOLD IDENTIFICATION NUMBER
	gen idh= w_prv+ w_mun+ w_bgy+ w_ea+ w_shsn+ w_hcn
	sort idh
...
** INDIVIDUAL IDENTIFICATION NUMBER
	sort idh
	by idh: generate n1 = _n
	egen idp=concat(idh lc101_lno), p(-)
...
** RELATIONSHIP TO THE HEAD OF HOUSEHOLD
	...
	gen onehead=head==1
	bys idh: egen HEAD=sum(onehead)
	drop if HEAD!=1


** GENDER
	gen byte gender= lc06_sex
...
** YEARS OF EDUCATION COMPLETED
	gen byte educy=.
	replace educy=0 if lc09_grade==0 | lc09_grade==1
	replace educy=7 if lc09_grade==2 |  lc09_grade==3
	replace educy=13 if lc09_grade==4
	replace educy=16 if lc09_grade>5 & age>5
...
** LABOR STATUS
	...
	gen byte lstatus=3
	replace lstatus=1 if lc12_work==1 | lc13_job==1
	replace lstatus=2 if lc31_lookw==1
...
** REASONS NOT IN THE LABOR FORCE
	gen byte nlfreason=.
	replace nlfreason=1 if lc35_wynot==8
	replace nlfreason=2 if lc35_wynot==7
	replace nlfreason=3 if lc35_wynot==6 & age>15
	replace nlfreason=4 if lc35_wynot==3
	replace nlfreason=5 if lc35_wynot==1 | lc35_wynot==2 | lc35_wynot==4 | lc35_wynot==5 | lc35_wynot==9
...
** UNEMPLOYMENT DURATION: MONTHS LOOKING FOR A JOB
	gen byte unempldur_l=lc34_weeks/4.2
	label var unempldur_l "Unemployment duration (months) lower bracket"

	gen byte unempldur_u=lc34_weeks/4.2
	label var unempldur_u "Unemployment duration (months) upper bracket"


** INDUSTRY CLASSIFICATION
	gen byte industry=.
* MANY undefined labels in data file. Coding here taken from 2009 which seems similar
	replace industry=1 if lc17_pkb>=111&lc17_pkb<=699
	replace industry=2 if lc17_pkb>=1010&lc17_pkb<=1199
	replace industry=3 if lc17_pkb>=1511&lc17_pkb<=3999
	replace industry=4 if lc17_pkb>=4010&lc17_pkb<=4100
	replace industry=5 if lc17_pkb>=4510&lc17_pkb<=4549
	replace industry=6 if lc17_pkb>=5011&lc17_pkb<=5529
	replace industry=7 if lc17_pkb>=6011&lc17_pkb<=6420
	replace industry=8 if lc17_pkb>=6510&lc17_pkb<=7499
	replace industry=9 if lc17_pkb>=7511&lc17_pkb<=9309
	replace industry=10 if lc17_pkb>=9500&lc17_pkb<=9900
	replace industry=10 if industry==. & lc17_pkb!=.
...
**SURVEY SPECIFIC INDUSTRY CLASSIFICATION
	gen industry_orig=lc17_pkb
	replace industry_orig=. if lstatus!=1
	label var industry_orig "Original Industry Codes"


** OCCUPATION CLASSIFICATION
	gen byte occup=floor(lc15/1000)
	recode occup 0=10
	replace occup=99 if lc15==930
...
** HOURS WORKED LAST WEEK
	gen whours=lc20_phours
	label var whours "Hours of work in last week"


** WAGES
	gen double wage=lc26_pbasic
...
** DELETE MISSING VARIABLES
	...
	saveold "`path'\Processed\PHL_2007_I2D2_LFS.dta", replace
	saveold "D:\__CURRENT\PHL_2007_I2D2_LFS.dta", replace
