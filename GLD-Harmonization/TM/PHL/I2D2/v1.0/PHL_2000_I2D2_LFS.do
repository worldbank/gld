...
** INPUT DATABASES	Z:\_I2D2\_I2D2\Philippines\2001\Original\LFS JAN2000.dta
** RESPONSIBLE	Cristian Jara
** Created	4/4/2012
** Modified	2/6/2017
** NUMBER OF HOUSEHOLDS	37967
** NUMBER OF INDIVIDUALS	195553
** EXPANDED POPULATION	75541208
**                                                                                                  **
******************************************************************************************************
*****************************************************************************************************/
...
** DIRECTORY
	local path "D:\__I2D2\Philippines\2000\LFS"


** LOG FILE
	log using "`path'\Processed\PHL_2000_I2D2_LFS.log", replace
...
** DATABASE ASSEMBLENT
	...
	use "`path'\Original\LFS JAN2000.dta"
...
** HOUSEHOLD SIZE
	sort idh
	by idh: egen hhsize= count(lno) if rel>=1 & rel<8
...
** LABOR STATUS
	gen byte lstatus=3
	replace lstatus=1 if work==1 | job==1
	replace lstatus=2 if lokw==1
...
** EMPLOYMENT STATUS
	gen byte empstat=.
	replace empstat=1 if class==0 | class==1 | class==2 
	replace empstat=2 if class==6
	replace empstat=3 if class==4
	replace empstat=4 if class==3| class==5
...
** REASONS NOT IN THE LABOR FORCE
	gen byte nlfreason=.
	replace nlfreason=1 if qwnot==8
	replace nlfreason=2 if qwnot==7
	replace nlfreason=3 if qwnot==6 & age>10
	replace nlfreason=4 if qwnot==3
	replace nlfreason=5 if qwnot==1 | qwnot==2 | qwnot==4 | qwnot==5 | qwnot==9
...
** DELETE MISSING VARIABLES
	...
	saveold "`path'\Processed\PHL_2000_I2D2_LFS.dta", replace
	saveold "D:\__CURRENT\PHL_2000_I2D2_LFS.dta", replace
