...
<_Program name_>				NGA_2024_LFS_V02_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 17 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2026-06-10 </_Date created_>

-------------------------------------------------------------------------

<_Country_>						NGA </_Country_>
<_Survey Title_>				Nigeria Labour Force Survey </_Survey Title_>
...
<_Sample size (HH)_> 			17,627	[#] </_Sample size (HH)_>
<_Sample size (IND)_> 			90,170	[#] </_Sample size (IND)_>
...
use "`path_in_stata'/NLFS_2024Q1_HOUSEHOLD IDENTIFICATION.dta", clear
merge 1:m interview_key using "`path_in_stata'/NLFS_2024Q1_INDIVIDUAL.dta", nogen
gen qtr = 1
tempfile q1
save `q1'

use "`path_in_stata'/NLFS_2024Q2_HOUSEHOLD IDENTIFICATION.dta", clear
merge 1:m interview_key using "`path_in_stata'/NLFS_2024Q2_INDIVIDUAL.dta", keep(match) nogen
gen qtr = 2

append using `q1'

append using "`path_in_stata'\nlfs_2024q3_indiv.dta"
replace qtr = 3 if missing(qtr)

append using "`path_in_stata'\nlfs_2024q4_indiv.dta"
...
*<_int_year_>
	gen int_date = date(interviewstartdate, "YMD")
	format int_date %td

	gen int_year  = year(int_date)
...
/* <_subnatidsurvey_note>
	...
	decode urban, gen(urb)
	replace subnatidsurvey = urb
...
*<_hsize_>
	* Household size is derived from the individual roster because household
	* identification files are not available for all quarters.
	bysort year qtr hhid: gen hsize = _N
...
*<_relationharm_>
	gen relationharm = dc4
	recode relationharm (4 5= 3) (6/9 14 16 = 5) (10 11 = 4) (12 15 = 6) 
...
*<_marital_>
	gen byte marital = dc6
	recode marital (2 = 1) (5 = 4) (6 = 5) (7 = 2)
...
* Not in the questionnaire but implied from the data that min age is 15
* sum age if !missing(atw1)
...
*<_occup_isco_>
	...
	* Correction on the basis of the check done below
	replace occup_isco = "0110" if occup_isco == "0113" | occup_isco == "0111"
