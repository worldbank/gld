...
<_Program name_> 			BIH_2021_LFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
...
<_Sample size (HH)_> 			29,477 </_Sample size (HH)_>
<_Sample size (IND)_> 			75,330 </_Sample size (IND)_>
...
use "`path_in_stata'/lfs_raw_dlw_2021.dta", clear
...
--------------+--------------------------------------------
Valid   0     |      75150      99.52      99.52      99.52
        1     |        358       0.47       0.47     100.00
        2     |          3       0.00       0.00     100.00
        Total |      75511     100.00     100.00           
-----------------------------------------------------------
...
*/
	...
	*If household has no head:
	...
	-----------------------------------------------------------
				  |      Freq.    Percent      Valid       Cum.
	--------------+--------------------------------------------
	Valid   0     |          2       0.00       0.00       0.00
			1     |      68024      90.30      90.30      90.30
			2     |       5786       7.68       7.68      97.98
			3     |       1222       1.62       1.62      99.61
			4     |        239       0.32       0.32      99.92
			5     |         45       0.06       0.06      99.98
			6     |         12       0.02       0.02     100.00
			Total |      75330     100.00     100.00           
	-----------------------------------------------------------


	unique hhid if number_heads == 0
		Number of unique values of hhid is  1
		Number of records is  2

	There are 1 households (2 individuals) with no household head
	
	
	unique hhid if inrange(number_heads,2,999)
		Number of unique values of hhid is  2304
		Number of records is  7304

		
	There are 2304 households (7304 individuals) with more than one head
	*/
	...
	*(3) oldest member if still no head
	...
	bys hhid: replace relationharm = 1 if inrange(number_heads,0,99) & oldest == 1
	bys hhid: replace relationharm = 5 if relationharm == 1 & inrange(number_heads,0,99) & oldest != 1
...
*<_wage_no_compen_>
	*Some respondents agreed to report their exact salary, while others did not. For those who declined, a question about their salary range was asked instead, put them missing
	...
	replace wage_no_compen = . if incsum_q23 == 0
