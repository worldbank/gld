...
<_Program name_> 			BIH_2018_LFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
...
<_Data collection from_>		04/2018 </_Data collection from_>
<_Data collection to_>			04/2018 </_Data collection to_>
<_Source of dataset_> 			Agency of Statistics </_Source of dataset_>
<_Sample size (HH)_> 			7,722 </_Sample size (HH)_>
<_Sample size (IND)_> 			20,371 </_Sample size (IND)_>
...
use "`path_in_stata'/lfs_raw_dlw_2018.dta", clear
...
/* <_hhid_note>
	...
	gen brojclan_aux = string(brojclanova, "%02.0f")
...
*<_relationharm_>
	...
	*If household has no head:
	...
	--------------+--------------------------------------------
	Valid   1     |      20361      99.95      99.95      99.95
			2     |         10       0.05       0.05     100.00
			Total |      20371     100.00     100.00           
	-----------------------------------------------------------

	
	
	unique hhid if number_heads == 2
		Number of unique values of hhid is  4
		Number of records is  10

	There are 4 households (10 individuals) with more than 1 household head
	*/
	
	*(2) oldest member if still no head

	bys hhid (age) : gen oldest = 1 if (_n == _N)
	bys hhid: replace relationharm = 5 if relationharm == 1 & number_heads == 2 & oldest != 1
...
*<_industrycat_isic_year_>
	gen industrycat_isic_year = cond(djelatnost_ranije2  < 100, "0" + string(djelatnost_ranije2 , "%02.0f") + "0", string(floor(djelatnost_ranije2  / 10) * 100, "%04.0f"))
