...
<_Program name_> 			BIH_2015_LFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
...
<_Data collection to_>			04/2015 </_Data collection to_>
<_Source of dataset_> 			Agency of Statistics </_Source of dataset_>
<_Sample size (HH)_> 			8,519 </_Sample size (HH)_>
<_Sample size (IND)_> 			24,600 </_Sample size (IND)_>
...
use "`path_in_stata'/bih_lfs_raw_dlw_2015.dta", clear
...
*<_educat7_>
	...
	* 2. Primary incomplete (1–4/5 or 5–7/8 grades)
	replace educat7 = 2 if inlist(diploma, 2)

	* 3. Primary complete (completed elementary)
	replace educat7 = 3 if inlist(diploma, 3)

	* 4. Secondary incomplete (1–3 years of secondary)
	replace educat7 = 4 if inlist(diploma, 4, 5, 6)

	* 5. Secondary complete (4–5 years or completed high school)
	replace educat7 = 5 if inlist(diploma, 7, 8, 10)

	* 6. Higher than secondary but not university (specialization after secondary)
	replace educat7 = 6 if diploma == 9

	* 7. University incomplete or complete (all tertiary and postgraduate levels)
	replace educat7 = 7 if inlist(diploma, 11, 12, 13, 14, 15)
...
*<_lstatus_>
	...
	*ICLS-13 DEFINITION: "The employed comprise all persons above a specified age who, during a specified brief period, either:
	...
	replace lstatus = 1 if aktivnost == 1 | inlist(razlog,1,2) | kolikoods == 1 | naknadaods == 2
...
*<_industry_orig_2_>
	gen industry_orig_2 = pretezna_dj2
...
*<_lstatus_year_>
	...
	*** NLF ***
	replace lstatus_year = 3 if inrange(status_ranije,3,8)
