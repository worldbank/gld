...
<_Program name_> 			BIH_2020_LFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
...
<_Sample size (HH)_> 			Cannot determine </_Sample size (HH)_>
<_Sample size (IND)_> 			75,745 </_Sample size (IND)_>
...
* Date: [YYYY-MM-DD] - [Description of changes]
* Date: 2026-02-10 - Update vars survey
...
* Q1
use "`path_in_stata'/LFS_BA_Q1_2020_WB_ENG.dta", clear
tostring djelatnost_ranije2, replace
gen quarter = "Q1"


* Q2
append using "`path_in_stata'/LFS_BA_Q2_2020_WB_ENG.dta"
replace quarter = "Q2" if missing(quarter)

* Q3
append using "`path_in_stata'/LFS_BA_Q3_2020_WB_ENG.dta"
replace quarter = "Q3" if missing(quarter)

* Q4
append using "`path_in_stata'/LFS_BA_Q4_2020_WB_ENG.dta"
replace quarter = "Q4" if missing(quarter)
...
/* <_hhid_note>
	...
	*some demog vars have a lot of missings, use weights to create hhid
	rename weight weight_aux
	
	gen weight_q = calweight_ver11_korig
	replace weight_q = calweight_ver6 if quarter == "Q3"
	replace weight_q = calweight_ver7 if quarter == "Q4"
	gen weight_q_aux = string(weight_q) + quarter

	bys weight_q_aux: gen group = 1 if _n == 1

	replace group = sum(group)
	gen hhid = string(group, "%05.0f")
...
*<_pid_>
	...
	*Not Personal ID in raw data, we have to construct it:
	...
	bys hhid : gen sex_str = string(pol,"%02.0f")
	bys hhid : gen age_str = string(starost,"%03.0f")
	bys hhid : gen hhlink_str = string(odnos,"%02.0f")

	egen  pid = concat(hhid sex_str age_str hhlink_str counter)
...
*<_weight_>
	...
	bys quarter : gen q_pop = _N
	gen t_pop = _N
	gen weight = weight_q *(q_pop/t_pop)
...
*<_urban_>
	gen byte urban = .
	replace urban = 1 if tip_naselja == "G"
	replace urban = 0 if tip_naselja == "O"
...
*Q3 and Q4 have missings in this var
...
*<_relationharm_>
	...
	*If household has no head:
	...
	--------------+--------------------------------------------
	Valid   1     |      69636      91.93      91.93      91.93
			2     |       4869       6.43       6.43      98.36
			3     |       1011       1.33       1.33      99.70
			4     |        162       0.21       0.21      99.91
			5     |         55       0.07       0.07      99.98
			6     |         12       0.02       0.02     100.00
			Total |      75745     100.00     100.00           
	-----------------------------------------------------------

	*/
	
	
	* If more than one head, select the oldest as head
...
/*
1 No education
2 1 do 4/5 grades of elementary school
3 5/6 do 7/8 grades of elementary school
4 Compleated elementary school
5 One year
6 Two year
7 Three year
8 Four - five year
9 Specialization after secondary school
10 Compleated high school
11 University education - Bachelor
12 University education (old programe 4-4.5 year)
13 University education (old programe 5-6 year)
14 Postgraduate studies (M.A., master) 
*/
	destring diploma, replace
	gen byte educy = .
	replace educy = 0 if diploma == 1 // no education
	replace educy = 2 if diploma == 2 // The option says: "1-4 grades of elementary school", so we will assume it corresponds to grade 2.
	replace educy = 6 if diploma == 3 // The option says "4–7/8 grades of Elementary School", so we will assume grade 6
	replace educy = 9 if diploma == 4
	replace educy = 10 if diploma == 5
	replace educy = 11 if diploma == 6
	replace educy = 12 if diploma == 7
	replace educy = 13 if diploma == 8
	replace educy = 14 if diploma == 9
	replace educy = 16 if diploma == 10
	replace educy = 19 if diploma == 11
	replace educy = 20 if diploma == 12
	replace educy = 21 if diploma == 13
	replace educy = 22 if diploma == 14
...
*<_educat7_>
	...
	replace educat7 = 2 if inlist(diploma,2,3)
	replace educat7 = 3 if inlist(diploma,4,5)
	replace educat7 = 4 if inlist(diploma,6)
	replace educat7 = 5 if inlist(diploma,7,8,9,10,11)
	replace educat7 = 7 if inlist(diploma,12,13,14)
...
*<_lstatus_>
	...
	*ICLS-13 DEFINITION: "The employed comprise all persons above a specified age who, during a specified brief period, either:
	...
	replace lstatus = 1 if aktivnost == 1 | aktivnosta == 1 | aktivnostb == 1 | aktivnostc == 1 | aktivnostd == 1 | inlist(razlog,1,2) | kolikoods == 1 | naknadaods == 2
	
	***unemployed***
	
	*seeking work
	
	gen active = (trazili_posao == 1)
	
	*Available for work
	gen passive = (pocetak == 1)
	
	*Future starters: Have found a job which will start in no more than 3 months
	gen future_starter = (glavni_razlog == 1)
...
*<_underemployment_>
	gen byte underemployment = (zeli_da_radi == 1)
...
*<_nlfreason_>
	gen byte nlfreason1 = glavni_razlog if inrange(glavni_razlog,2,10)
	recode nlfreason1 (2 4 6 9 10 = 5) (3 = 2) (7 = 1) (8 = 3) (9 = 4)
...
*They use NACE 2 which is in line with ISIC rev 4 at 2 digits level
	gen industrycat_isic = cond(djelatnost2 < 100, "0" + string(djelatnost2, "%02.0f") + "0", string(floor(djelatnost2 / 10) * 100, "%04.0f"))
...
*<_occup_orig_>
	gen occup_orig = zanimanje if lstatus == 1
	label var occup_orig "Original occupation record primary job 7 day recall"
*</_occup_orig_>


*<_occup_isco_>
	gen occup_isco = string(zanimanje, "%04.0f")
...
*<_wage_no_compen_>
	*Some respondents agreed to report their exact salary, while others did not. For those who declined, a question about their salary range was asked instead, put them missing
	
	gen double wage_no_compen = iznosplate
	replace wage_no_compen = . if lstatus != 1
	replace wage_no_compen = . if iznosplate == 0
...
*<_whours_>
	gen whours = radili_ref if !missing(wage_no_compen)
	replace whours = . if radili_ref == 0
	replace whours = koliko_radite if missing(whours) & !missing(wage_no_compen)
...
*<_contract_>
	gen byte contract = ugovori if lstatus == 1
...
*<_healthins_>
	gen byte healthins = zdravstveno_p if lstatus == 1
...
*<_firmsize_l_>
	gen firmsize_l = zaposleni if lstatus == 1
...
*<_firmsize_u_>
	gen firmsize_u = zaposleni if lstatus == 1
...
*<_industry_orig_2_>
	gen industry_orig_2 = pretezna_dj2 if lstatus == 1 & !missing(empstat_2)
	label var industry_orig_2 "Original survey industry code, secondary job 7 day recall"
*</_industry_orig_2_>


*<_industrycat_isic_2_>
	gen industrycat_isic_2 = cond(pretezna_dj2 < 100, "0" + string(pretezna_dj2, "%02.0f") + "0", string(floor(pretezna_dj2 / 10) * 100, "%04.0f"))
...
*<_lstatus_year_>
	...
	*** employed ***
	replace lstatus_year = 1 if status_ranije  == 1
	
	*** unemployed ***
	replace lstatus_year = 2 if status_ranije  == 2
	
	*** NLF ***
	replace lstatus_year = 3 if inrange(status_ranije,3,7)
...
*<_nlfreason_year_>
	gen byte nlfreason_year = status_ranije
	recode nlfreason_year (1 2 = .) (3 = 1) (4 = 2) (5 = 3) (7 = 5) (6 = 4)
...
*<_empstat_year_>
	gen byte empstat_year = polozaj_ranije
...
*<_industry_orig_year_>
	gen industry_orig_year = djelatnost_ranije2  if lstatus_year == 1
	label var industry_orig_year "Original industry record main job 12 month recall"
*</_industry_orig_year_>


*<_industrycat_isic_year_>
	replace djelatnost_ranije2 = "" if djelatnost_ranije2 == "?72" 
	replace djelatnost_ranije2 = "" if djelatnost_ranije2 == "NUL" 
	destring djelatnost_ranije2, replace
	gen str4 industrycat_isic_year = ///
    cond(djelatnost_ranije2 < 100, ///
        "0" + string(djelatnost_ranije2, "%02.0f") + "0", ///
        string(floor(djelatnost_ranije2/10)*100, "%04.0f"))
