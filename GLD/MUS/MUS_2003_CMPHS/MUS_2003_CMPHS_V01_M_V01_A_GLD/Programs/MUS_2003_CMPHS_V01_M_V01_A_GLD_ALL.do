...
<_Program name_>				MUS_2003_CMPHS_V01_M_V01_A_GLD_ALL.do </_Program name_>
...
use "`path_in_stata'/CMHS2003.dta", clear
...
*<_hhid_>
    gen hhid = string(c01serialno, "%08.0f")
...
*<_subnatid1_>
    gen subnatid1 = ""
    replace subnatid1 = "1 - Port Louis"         if c03district == 11
    replace subnatid1 = "2 - Pamplemousses"      if c03district == 12
    replace subnatid1 = "3 - Riviere-du-Rempart" if c03district == 13
    replace subnatid1 = "4 - Flacq"              if c03district == 14
    replace subnatid1 = "5 - Grand Port"         if c03district == 15
    replace subnatid1 = "6 - Savanne"            if c03district == 16
    replace subnatid1 = "7 - Plaine-Wilhems"     if c03district == 17
    replace subnatid1 = "8 - Moka"               if c03district == 18
    replace subnatid1 = "9 - Black River"        if c03district == 19
    replace subnatid1 = "10 - Rodrigues"         if c03district == 1
...
*<_male_>
    gen byte male = .
    replace male = 1 if q15sex == 1
    replace male = 0 if q15sex == 2
...
*<_relationharm_>
    gen byte relationharm = q13reltohead
    replace relationharm = 4 if q13reltohead == 6
    replace relationharm = 5 if inlist(q13reltohead, 4, 5, 7, 8)
    replace relationharm = 6 if q13reltohead == 9
...
*<_relationcs_>
    gen byte relationcs = q13reltohead
    label var relationcs "Relationship to the head of household - Country original"
*</_relationcs_>


*<_marital_>
    gen byte marital = q16marstatus
...
*<_school_>
    gen byte school = (q23schoolatt == 1)
...
*<_educat7_>
    gen byte educat7 = .
   	...
   	* K-12 education
	replace educat7=1 if q24educlevel==1
	replace educat7=2 if q24educlevel>=11 & q24educlevel<=15
	replace educat7=3 if q24educlevel==16
	replace educat7=4 if q24educlevel>=21 & q24educlevel<=23
	* We set Forms 4, 5 and 6 as higher than secondary
	replace educat7=5 if q24educlevel>=24 & q24educlevel<=26
	
	
	* In ISCED, first digit = 5 means diploma
	gen qual1d = int(q26qualcourse/100)
...
*<_educat_orig_>
    gen educat_orig = q24educlevel
	replace educat_orig = q26qualcourse if !missing(q26qualcourse)
...
* Also, temporary absent were not asked unemployment questions, so survey logic assumes they are employed
    gen byte lstatus = .
    replace lstatus = 1 if q301work == 1 | q303awayfromwrk == 1
    replace lstatus = 2 if q301work == 2 & q304lookforwrk == 1 & q311availwrk == 1
    replace lstatus = 3 if (lstatus != 1 & q311availwrk == 2) | (lstatus != 1 & q304lookforwrk == 2)
...
*<_potential_lf_>
    ...
    replace potential_lf = 1 if (q304lookforwrk == 1 | q311availwrk == 1) & lstatus == 3
...
*<_nlfreason_>
    gen byte nlfreason = q312whynotavail if lstatus == 3
...
*<_unempldur_l_>
    gen byte unempldur_l = q307timelookwrk if lstatus == 2
    label var unempldur_l "Unemployment duration (months) lower bracket"
*</_unempldur_l_>


*<_unempldur_u_>
    gen byte unempldur_u = q307timelookwrk if lstatus == 2
...
*<_empstat_>
    gen byte empstat = q322employstat
...
*<_industry_orig_>
    gen industry_orig = q320aactivity
...
*<_industrycat_isic_>
    gen industrycat_isic = string(q320aactivity, "%02.0f") + "00" if !missing(q320aactivity)
    replace industrycat_isic = "" if inlist(q320aactivity, 4, 81, 38, 54, 82, 0)
...
*<_occup_orig_>
    gen occup_orig = string(q321occupation) if !missing(q321occupation)
...
*<_occup_isco_>
	...
	* THese are codes that are not in ISCO 88. Suspected to be typos because only 1 case each.
	replace occup_isco = "" if inlist(q321occupation, 45, 53, 57, 63, 66, 99)
...
*<_wage_no_compen_>
    gen double wage_no_compen = q3251ncome
