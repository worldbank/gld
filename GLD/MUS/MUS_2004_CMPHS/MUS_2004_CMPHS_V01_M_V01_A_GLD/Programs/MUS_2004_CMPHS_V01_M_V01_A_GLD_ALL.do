...
<_Program name_>				MUS_2004_CMPHS_V01_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 18 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2026-02-23 </_Date created_>

-------------------------------------------------------------------------

<_Country_>						MUS </_Country_>
<_Survey Title_>				Continuous Multi-Purpose Household Survey </_Survey Title_>
...
<_Source of dataset_> 			Statistics Mauritius </_Source of dataset_>
<_Sample size (HH)_> 			8,640 </_Sample size (HH)_>
<_Sample size (IND)_> 			32,626 </_Sample size (IND)_>
<_Sampling method_> 			Two-stage sampling design </_Sampling method_>
<_Geographic coverage_> 		National </_Geographic coverage_>
<_Currency_> 					Rupee </_Currency_>

-----------------------------------------------------------------------
<_ICLS Version_>				ICLS 13 </_ICLS Version_>
<_ISCED Version_>				ISCED 1976 </_ISCED Version_>
<_ISCO Version_>				ISCO 1988 </_ISCO Version_>
<_OCCUP National_>				National Classification of Occupations </_OCCUP National_>
<_ISIC Version_>				ISIC version 3 </_ISIC Version_>
<_INDUS National_>				National Standard Industrial Classification of Economic Activities </_INDUS National_>
-----------------------------------------------------------------------
<_Version Control_>
...
use "`path_in_stata'/CMPHS 04.dta", clear
...
/* <_hhid_note>
	...
	I am seeing a potential household ID issue in the 2004 file. The serialno (household ID) repeats across records that look like separate households. Specifically, after row 3, memserialno restarts at 1 and hhno changes, which usually signals a new household, but serialno stays at 3949. Later, the serialno sequence also skips 3950, jumping from 3949 to 3951. This makes me think serialno may need to increment to 3950 starting at row 4 (through the second-to-last row).
	
	Same issue for 3785! I will replace the data
</_hhid_note> */

	* Flag the start of a new household roster
	gen byte hh_start = (memserialno == 1)

	* Running household number (1,2,3,...)
	gen long hh_run = sum(hh_start)

	gen serialno_new = serialno
	replace serialno_new = 3950 in 14991
	replace serialno_new = 3950 in 14992
	replace serialno_new = 3950 in 14993
	replace serialno_new = 3950 in 14994
	replace serialno_new = 3950 in 14995
	replace serialno_new = 3950 in 14996
	replace serialno_new = 3950 in 14997

	replace serialno_new = 3786 in 14379
	replace serialno_new = 3786 in 14380
	replace serialno_new = 3786 in 14381
	replace serialno_new = 3786 in 14382
	replace serialno_new = 3786 in 14383

	drop if reltohead==.
	
	gen hhid_str = string(serialno_new, "%04.0f")
	gen hhno_str = string(hhno, "%02.0f")
	
	gen hhid = hhid_str + hhno_str
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>
	gen  pid_str = string(memserialno, "%02.0f")
	gen pid = hhid + pid_str
	
	duplicates tag pid, gen(dup)
	* There are 16 duplicate pairs
	
	* Do they share the same labor module responses?
	duplicates tag pid reltohead - hrswk, gen(x2)
	
	tab dup x2, mi
	* 1 pair (=2 obs) do not share the same responses in the labor module
	
	duplicates drop pid reltohead - hrswk, force

	* Inspect the odd pair. 
	list pid reltohead - hrswk if x2 == 0 & dup == 1
	
	* one of which has missing values for the labor module, we drop that!
	drop if pid == "37860603" & missing(hrswk)
	
	
	* There are duplicates in PID
...
*<_subnatid1_>
    gen subnatid1 = ""
    replace subnatid1 = "1 - Port Louis"         if district == 11
    replace subnatid1 = "2 - Pamplemousses"      if district == 12
    replace subnatid1 = "3 - Riviere-du-Rempart" if district == 13
    replace subnatid1 = "4 - Flacq"              if district == 14
    replace subnatid1 = "5 - Grand Port"         if district == 15
    replace subnatid1 = "6 - Savanne"            if district == 16
    replace subnatid1 = "7 - Plaine-Wilhems"     if district == 17
    replace subnatid1 = "8 - Moka"               if district == 18
    replace subnatid1 = "9 - Black River"        if district == 19
    replace subnatid1 = "10 - Rodrigues"         if district == 1
...
*<_relationharm_>
	gen byte relationharm=reltohead
	replace relationharm=4 if reltohead==6
	replace relationharm=5 if reltohead==4 |reltohead==5 |reltohead==7 | reltohead==8
	replace relationharm=6 if reltohead==9
...
*<_marital_>
	...
	recode marital (1=1) (3 4 =4)  (2=5) (5=2)
...
*<_school_>
	gen byte school=schoolatt
	recode school  (2/4=0) 
	replace school=. if age<6
...
*<_educat7_>
	* Here information is only provided for K-12 (up to Form 6, nothing to identify post-secondary)
	gen byte educat7=.
	replace educat7=1 if educattain==1 
	replace educat7=2 if educattain>=11 & educattain<=16
	replace educat7=3 if inlist(educattain, 16, 17)
	replace educat7=4 if educattain>=21 & educattain<=23
	replace educat7=5 if educattain>=24 & educattain<=28
	
	* Use ISCED from field of study vble
	gen qual1d = int(qualcourse/100)
...
*<_educat_orig_>
	gen educat_orig = educattain
	replace educat_orig = qualcourse if !missing(qualcourse)
...
*<_lstatus_>
	gen byte lstatus=.
	replace lstatus=1 if work==1 | activities==1 | awayfrwk==1
	replace lstatus=2 if work==2 & lookfrwk==1
	replace lstatus=3 if lstatus!=1 & availwk==2
	replace lstatus=. if age<12
...
*<_nlfreason_>
	gen byte nlfreason=whynotavail if lstatus==3
	recode nlfreason (1= 2) (2 = 1) (3= 4) (4 = 3)
...
*<_unempldur_l_>
	gen byte unempldur_l=timelkwk if lstatus==2
	label var unempldur_l "Unemployment duration (months) lower bracket"
*</_unempldur_l_>


*<_unempldur_u_>
	gen byte unempldur_u=timelkwk if lstatus==2
...
*<_empstat_>
	...
	recode empstat (3=1) (4 5=2) (1=3) (2=4) (6=5)
...
*<_ocusec_>
	gen byte ocusec=.
	replace ocusec=1 if estabname>=1 & estabname<=3
	replace ocusec=2 if estabname>=5 & estabname<=9
	replace ocusec=3 if estabname==4
...
*<_industry_orig_>
	gen industry_orig=string(activity, "%05.0f") if !missing(activity)
...
*<_industrycat_isic_>
	gen industry_2d = substr(industry_orig, 1, 2)
    gen industrycat_isic = industry_2d + "00" if !missing(industry_orig)
	* 5 total cases with code not mapped to ISIC
    replace industrycat_isic = "" if inlist(activity, 4520, 83221, 83223, 89109)
...
*<_occup_orig_>
    gen occup_orig = string(occupation) if !missing(occupation)
...
*<_wage_no_compen_>
	gen double wage_no_compen=paidempl
...
*<_firmsize_l_>
	gen byte firmsize_l=.
	replace firmsize_l=1 if nopersons==1
	replace firmsize_l=5 if nopersons==2
	replace firmsize_l=10 if nopersons==3
...
*<_firmsize_u_>
	gen byte firmsize_u=.
	replace firmsize_u=4 if nopersons==1
	replace firmsize_u=9 if nopersons==2
	replace firmsize_u=. if nopersons==3
