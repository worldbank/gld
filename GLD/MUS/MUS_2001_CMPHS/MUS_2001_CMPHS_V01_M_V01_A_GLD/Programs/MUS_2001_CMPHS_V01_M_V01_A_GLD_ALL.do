...
<_Program name_>				MUS_2001_CMPHS_V01_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 18 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2026-02-20 </_Date created_>
...
<_Sample size (IND)_> 			24,539 </_Sample size (IND)_>
...
use "`path_in_stata'/CMPHS 01.dta", clear
...
*<_relationharm_>
	...
	replace relationharm=5 if q103reltohead==4 |q103reltohead==5 |q103reltohead==7 | q103reltohead==8
...
*<_school_>
	gen byte school=(q107schoolatta==1)
	replace school=1 if q109schoolattv==1
...
*<_educat7_>
* edulevel variable only until secondary
* we can use the qualification variable to infer post-secondary
* this is based on ISCED-76, where first digit informs level of education
	gen byte educat7=.
	replace educat7=1 if q108edulevel==1
	replace educat7=2 if q108edulevel>=11 & q108edulevel<=15
	replace educat7=3 if q108edulevel==16
	replace educat7=4 if q108edulevel>=21 & q108edulevel<=23
	replace educat7=5 if q108edulevel>=24 & q108edulevel<=26
	
* In ISCED, first digit = 5 means diploma
	gen qual1d = int(q110qualcourse/100)
...
*<_educat_orig_>
	gen educat_orig = q108edulevel
	replace educat_orig = q110qualcourse
...
* Also, temporary absent were not asked unemployment questions, so survey logic assumes htey are employed
	gen byte lstatus=.
	replace lstatus=1 if q201didwork==1 | q202absent==1
	replace lstatus=2 if q201didwork==2 &  q203lookwork==1 & q211availwork==1
...
*<_potential_lf_>
	...
	replace potential_lf = 1 if (q203lookwork==1 | q211availwork==1) & lstatus == 3
...
*<_nlfreason_>
	gen byte nlfreason= q212whynotavail if lstatus==3
	recode nlfreason (2=1) (1=2) (3=4) (4=3)
	replace nlfreason=. if lstatus!=3|age<12
...
*<_unempldur_l_>
	gen byte unempldur_l=q207timelookwk if lstatus==2
	label var unempldur_l "Unemployment duration (months) lower bracket"
*</_unempldur_l_>


*<_unempldur_u_>
	gen byte unempldur_u=q207timelookwk if lstatus==2
...
{
*<_empstat_>
	gen byte empstat=q223empstatus
...
*<_ocusec_>
	gen byte ocusec=q214sector
	recode ocusec (1/3=1) (4=3)  (5/9=2) (0=.)
...
*<_industry_orig_>
	gen industry_orig=q221activity
...
*<_industrycat_isic_>
	gen industrycat_isic = string(q221activity, "%02.0f") + "00" if !missing(q221activity)
	* There seems to be a typo; 1 case each for code "04" and "81" which do not exist in ISIC
	replace industrycat_isic = "" if inlist(q221activity, 4, 81)
...
*<_occup_orig_>
	gen  occup_orig= string(q222occupation) if !missing(q222occupation)
...
*<_occup_isco_>
	...
	* THese are codes that are not in ISCO 88. Suspected to be typos because only 1 case each.
	replace occup_isco = "" if inlist(q222occupation, 45, 63, 85, 99)
	label var occup_isco "ISCO code of primary job 7 day recall"
*</_occup_isco_>


*<_occup_>
	gen byte occup=q222occupation if lstatus==1
...
*<_wage_no_compen_>
	gen double wage_no_compen= q227income
...
/* <_wage_total_note>
	...
	Use gross wages when availableqq and net wages only when gross wages are not available.
