...
<_Program name_>				MUS_1999_CMPHS_V01_M_V01_A_GLD_ALL.do </_Program name_>
...
<_Data collection from_>		04/1999 </_Data collection from_>
<_Data collection to_>			12/1999 </_Data collection to_>
<_Source of dataset_> 			Statistics Mauritius </_Source of dataset_>
<_Sample size (HH)_> 			4,726 </_Sample size (HH)_>
<_Sample size (IND)_> 			18,316 </_Sample size (IND)_>
...
<_OCCUP National_>				National Classification of Occupations 1999 </_OCCUP National_>
...
use "`path_in_stata'/CMHS1999.dta", clear
...
*<_pid_>
	gen mem_num = string(q201memserialno, "%02.0f")
...
*<_relationharm_>
	gen byte relationharm=q203reltohead
	replace relationharm=4 if q203reltohead==6
	replace relationharm=5 if q203reltohead==4 |q203reltohead==5 |q203reltohead==7 | q203reltohead==8
	replace relationharm=6 if q203reltohead==9
...
*<_school_>
	gen byte school=(q303schoolatta==1)
	replace school=1 if q307schoolattv==1
...
*<_educat7_>
	gen byte educat7=q304edulevel
	replace educat7 = 1 if q304edulevel == 1
	replace educat7 = 2 if inrange(q304edulevel, 101, 105)
	replace educat7 = 3 if q304edulevel == 106
	replace educat7 = 4 if inrange(q304edulevel, 201, 203)
	replace educat7 = 5 if inrange(q304edulevel, 204, 206)
	replace educat7 = 7 if inrange(q304edulevel, 301, 399)
	
	gen qual1d = int(q308qualcourse/100)
...
*<_educat_orig_>
	gen educat_orig = q304edulevel
	replace educat_orig = q308qualcourse if !missing(q308qualcourse)
...
* Also, temporary absent were not asked unemployment questions, so survey logic assumes htey are employed
	gen byte lstatus=.
	replace lstatus=1 if q401didwork==1 | q404aabsence==1
	replace lstatus=2 if q401didwork==2 &  q412alookjob==1 & q420availwk==1
...
*<_potential_lf_>
	...
	replace potential_lf = 1 if (q412alookjob==1 | q420availwk==1) & lstatus == 3
...
*<_nlfreason_>
	gen byte nlfreason= q421whynotavail if lstatus==3
...
*<_unempldur_l_>
	gen byte unempldur_l=q413timelookwk if lstatus==2
	label var unempldur_l "Unemployment duration (months) lower bracket"
*</_unempldur_l_>


*<_unempldur_u_>
	gen byte unempldur_u=q413timelookwk if lstatus==2
...
*<_industry_orig_>
	gen industry_orig=q406dactivity
...
*<_industrycat_isic_>
	gen industrycat_isic = string(q406dactivity, "%02.0f") + "00" if !missing(q406dactivity)
	* There seems to be a typo; 1 case each for code "04" and "81" which do not exist in ISIC
	replace industrycat_isic = "" if inlist(q406dactivity, 4, 81)
...
*<_occup_orig_>
	gen  occup_orig= string(q406eoccupation) if !missing(q406eoccupation)
...
*<_wage_no_compen_>
	gen double wage_no_compen= q406j1income
