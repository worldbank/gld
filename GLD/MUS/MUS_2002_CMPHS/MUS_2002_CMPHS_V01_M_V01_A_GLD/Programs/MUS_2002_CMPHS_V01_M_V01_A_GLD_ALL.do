...
<_Program name_>				MUS_2002_CMPHS_V01_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 18 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2026-02-22 </_Date created_>
...
<_Sample size (HH)_> 			6,300  </_Sample size (HH)_>
<_Sample size (IND)_> 			24,552 </_Sample size (IND)_>
...
use "`path_in_stata'/CMPHS 02.dta", clear
...
*<_int_year_>
	gen int int_year= 2002
	label var int_year "Year of the interview"
*</_int_year_>


*<_int_month_>
	gen byte int_month=c02month
...
*<_hhid_>
	gen hhid = string(c01serialno, "%05.0f")
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>
	gen mem_num = string(r1memserialno, "%02.0f")
	gen pid = hhid + mem_num
	isid pid
...
*<_subnatid1_>
	gen subnatid1 = ""
	replace subnatid1 = "1 - Port Louis"            if c03district == 1
	replace subnatid1 = "2 - Pamplemousses"         if c03district == 2
	replace subnatid1 = "3 - Riviere-du-Rempart"    if c03district == 3
	replace subnatid1 = "4 - Flacq"           	    if c03district == 4
	replace subnatid1 = "5 - Grand Port"            if c03district == 5
	replace subnatid1 = "6 - Savanne"               if c03district == 6
	replace subnatid1 = "7 - Plaine-Wilhems"        if c03district == 7
	replace subnatid1 = "8 - Moka"                  if c03district == 8
	replace subnatid1 = "9 - Black River"           if c03district == 9
	replace subnatid1 = "10 - Rodrigues"            if c03district == 10
...
*<_male_>
    gen byte male = .
    replace male = 1 if q105gender == 1
    replace male = 0 if q105gender == 2
...
*<_relationharm_>
    gen byte relationharm = q103reltohead
    replace relationharm = 4 if q103reltohead == 6
    replace relationharm = 5 if inlist(q103reltohead, 4, 5, 7, 8)
    replace relationharm = 6 if q103reltohead == 9
...
*<_relationcs_>
    gen byte relationcs = q103reltohead
    label var relationcs "Relationship to the head of household - Country original"
*</_relationcs_>


*<_marital_>
    gen byte marital = q106marstatus
...
*<_school_>
* There is a separate set of questions for K-12 and post-secondary
* q203 asks if currently attending k-12
* meanwhile, q205 currently attending post-secondary
	gen byte school=(q203schattps==1)
	replace school = 1 if q205schattoe == 1
...
*<_educat7_>
	gen byte educat7=.
	* K-12 education
	replace educat7=1 if q204educlevel==1
	replace educat7=2 if q204educlevel>=11 & q204educlevel<=15
	replace educat7=3 if q204educlevel==16
	replace educat7=4 if q204educlevel>=21 & q204educlevel<=23
	replace educat7=5 if q204educlevel>=24 & q204educlevel<=26
	
	* In ISCED, first digit = 5 means diploma
	gen qual1d = int(q206qualcourse/100)
...
*<_educat_orig_>
    gen educat_orig = q204educlevel
	replace educat_orig = q206qualcourse if !missing(q206qualcourse)
...
*<_lstatus_>
* Survey does not ask the reason for absence and whether there is still job attachment 
* Assume temporary absence since that is often the case
* Also, temporary absent were not asked unemployment questions, so survey logic assumes they are employed
    gen byte lstatus = .
    replace lstatus = 1 if q301workpastwk == 1 | q302absent == 1
    replace lstatus = 2 if q301workpastwk == 2 & q303lookforwork == 1 & q307availwork == 1
    replace lstatus = 3 if (lstatus != 1 & q307availwork == 2) | (lstatus != 1 & q303lookforwork == 2)
...
*<_potential_lf_>
    ...
    replace potential_lf = 1 if (q303lookforwork == 1 | q307availwork == 1) & lstatus == 3
...
*<_nlfreason_>
    gen byte nlfreason = q308whynotavail if lstatus == 3
    recode nlfreason (2=1) (1=2) (3=4) (4=3)
    replace nlfreason = . if lstatus != 3 | age < minlaborage | missing(age)
...
*<_unempldur_l_>
    gen byte unempldur_l = q306timelookwrk if lstatus == 2
    label var unempldur_l "Unemployment duration (months) lower bracket"
*</_unempldur_l_>


*<_unempldur_u_>
    gen byte unempldur_u = q306timelookwrk if lstatus == 2
...
*<_empstat_>
    gen byte empstat = q313empstatus
    recode empstat (3=1) (4 5=2) (1=3) (2=4) (6=5)
    replace empstat = . if lstatus != 1 | age < minlaborage | missing(age)
...
*<_industry_orig_>
    gen industry_orig = q311activity
...
*<_industrycat_isic_>
    gen industrycat_isic = string(q311activity, "%02.0f") + "00" if !missing(q311activity)
	* Mis-coding bec this does not exist in ISIC. 3 cases!
    replace industrycat_isic = "" if q311activity == 94 | q311activity == 4
...
*<_occup_orig_>
    gen occup_orig = string(q312occupation) if !missing(q312occupation)
...
*<_occup_isco_>
	...
	* THese are codes that are not in ISCO 88. Suspected to be typos because only 1 case each.
	replace occup_isco = "" if inlist(q312occupation, 43, 54, 75, 95, 99)
...
*<_wage_no_compen_>
    gen double wage_no_compen = q321income
    replace wage_no_compen = . if lstatus == 2 | lstatus == 3
