...
<_Program name_>				PHL_2016_LFS_V01_M_V06_A_GLD_ALL.do </_Program name_>
...
* Date: 2026-04-02 - Standardized January education from the detailed raw code and extended educat7 coding to all four rounds. January and April use the earlier PSCED 2008 split; July and October use the later code structure that matches 2017.
...
** RUN SETTINGS
	...
	local 	year 		"/Users/angelosantos/Downloads/PHL/`cty3'_`surv_yr'_LFS" // top data folder
...
* ouput
	local path_output `"`gld_data'/`cty3'_`surv_yr'_LFS_V01_M_V06_A_GLD_ALL.dta"'
...
/* <_subnatid1_note_>
	...
	Regions VI-A and VI-B appear coded differently in the data
	Region Negros Island makes an entry only in Q4. Since it only existed
	for a very brief time, it is here ignored, based on the province it
	is sent to the regions it belonged to.

</_subnatid1_note_> */
	gen byte 		subnatid1 = pufreg
	* Calarbazon is 41 in some quarters, 4 in others
	replace subnatid1 = 41 if pufreg == 4
	* Mimarropa is 42 in some quarters, 17 in others
	replace subnatid1 = 42 if pufreg == 17
	* Additionally, Negros Island recode as described above
	replace subnatid1 = 6 if prov == 45
	replace subnatid1 = 7 if prov == 46
...
*<_educat7_>
	/* <_educat7_note>
		...
		Education coding differs across rounds in 2016.

		January and April follow the earlier detailed PSCED 2008 structure.
		For these two rounds, post-secondary non-university is coded in the
		410-589 range, while 601-699 and 810+ are treated as tertiary.

		January needed special handling in the append template because the raw
		file contains both a coarse grouped variable (c09_grd) and a detailed
		one (j12c09_grade). We standardize January pufc07_grade from the
		detailed j12c09_grade so educat7 can use the same detailed ladder as
		April.

		July and October follow the later code structure that matches 2017.
		In those rounds, 410-500 correspond to K-12 grades, 510-520 correspond
		to grades 11-12, 601-699 are post-secondary non-university, and 701+
		are university or above.
	...
	gen byte educat7=.
		...
		* January and April follow the earlier detailed PSCED 2008 structure
		replace educat7 = 1 if (pufc07_grade == 0 | pufc07_grade == 10) & inlist(int_month,1,4)
		replace educat7 = 2 if inrange(pufc07_grade,210,260) & inlist(int_month,1,4)
		replace educat7 = 3 if pufc07_grade == 280 & inlist(int_month,1,4)
		replace educat7 = 4 if inrange(pufc07_grade,310,340) & inlist(int_month,1,4)
		replace educat7 = 5 if pufc07_grade == 350 & inlist(int_month,1,4)
		replace educat7 = 6 if inrange(pufc07_grade,410,589) & inlist(int_month,1,4)
		replace educat7 = 7 if inrange(pufc07_grade,601,699) & inlist(int_month,1,4)
		replace educat7 = 7 if pufc07_grade >= 810 & inlist(int_month,1,4)

		* July and October follow the later code structure used in 2017
		replace educat7 = 1 if pufc07_grade <= 110 & inlist(int_month,7,10)
		replace educat7 = 2 if ((inrange(pufc07_grade,110,160)) | inrange(pufc07_grade,410,450)) & inlist(int_month,7,10)
		replace educat7 = 3 if inlist(pufc07_grade,170,180,460) & inlist(int_month,7,10)
		replace educat7 = 4 if ((inrange(pufc07_grade,210,240)) | inrange(pufc07_grade,470,490)) & inlist(int_month,7,10)
		replace educat7 = 5 if inlist(pufc07_grade,250,500) & inlist(int_month,7,10)
		replace educat7 = 6 if ((inrange(pufc07_grade,510,520)) | inrange(pufc07_grade,601,699)) & inlist(int_month,7,10)
		replace educat7 = 7 if inrange(pufc07_grade,701,950) & inlist(int_month,7,10)
		replace educat7 = 3 if pufc07_grade == 191 & inlist(int_month,7,10)
		replace educat7 = 5 if pufc07_grade == 192 & inlist(int_month,7,10)
