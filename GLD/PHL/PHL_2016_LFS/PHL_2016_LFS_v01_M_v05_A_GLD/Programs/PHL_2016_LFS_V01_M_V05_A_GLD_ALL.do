...
** RUN SETTINGS
	...
	local 	gld 		"`year'\\`cty3'_`surv_yr'_LFS_v01_M_v05_A_GLD"
...
* ouput
	local path_output `"`gld_data'\\`cty3'_`surv_yr'_LFS_V01_M_V05_A_GLD_ALL.dta"'
...
/* <_educat7_note>
	...
	The data has some issues. In January the variable is 7 categories
	In April it is many categories, but in line with questionnaire
	July and October have data that does not seem to fit anything
	The latter two are not done
	...
	* run for January
	...
	replace educat = 1 if pufc07_grade == 0 & int_month == 1
	replace educat = 2 if pufc07_grade == 1 & int_month == 1
	replace educat = 3 if pufc07_grade == 2 & int_month == 1
	replace educat = 4 if pufc07_grade == 3 & int_month == 1
	replace educat = 5 if pufc07_grade == 4 & int_month == 1
	replace educat = 7 if pufc07_grade == 5 & int_month == 1
	replace educat = 6 if pufc07_grade == 60 & int_month == 1

	replace 	educat7=1 if (pufc07_grade==0 | pufc07_grade == 10)  & int_month == 4 	// "No education" and "Preschool" -> "No Education"
	replace 	educat7=2 if (pufc07_grade>=210 & pufc07_grade<=260) & int_month == 4	// Grades 1-7 to "Primary Incomplete"
