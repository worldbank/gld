...
<_Program name_>				PHL_2023_LFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 18 <_Application_>
<_Author(s)_>					World Bank Jobs Group </_Author(s)_>
<_Date created_>				2025-10-16 </_Date created_>
...
<_Sample size (HH)_> 			393364 </_Sample size (HH)_>
<_Sample size (IND)_> 			1626506 </_Sample size (IND)_>
...
* Date: 2026-04-02 - Updated educat7 to use the detailed post-2020 education ladder across all available months, removed the inherited grouped 2019 cutoffs, and restored grade for August to December from pufc07_grade.
...
local root    "/Users/angelosantos/Downloads/PHL/PHL_2023_LFS"
local country "PHL"
local year    "2023"
local survey  "LFS"
local vermast "V01"
...
tempfile tempdata
save `tempdata'
* Load separate data (w/urban data)
* For context: we received the data in csv format for Jan - Dec 2023, however, the August to December rounds do not have the urban variable. Then we heard back from the country team and gave us the revised data but in Stata. 

foreach m of local months2 {
	use "`path_in_stata'/LFS `m' 2023.dta", clear
	ds
	
	foreach v of varlist `r(varlist)' {
		capture tostring `v', replace force usedisplayformat
	}
...
* August to December keep the education code in pufc07_grade, while
* January to July already renamed it to grade during CSV import.
capture replace grade = pufc07_grade if missing(grade) & !missing(pufc07_grade)

* Convert all to numeric so we can assign value labels
	foreach var of varlist pufreg pufpwgtprv - ftwork1  {
...
label variable pufurbrural "2015Urban-RuralFIES"
label values pufurbrural pufurb2015lbl


* Variable: pufsvymo_vs1 - Survey Month
label variable pufsvymo "Survey Month"
label variable month "Month last worked"
label variable year "Year last worked"
...
label values month pufsvymolbl
...
label var provmun "Province and municipality of employment"
* Variable: sex
...
* Variable: pufc11a_arrangement_vs1 - C11A - Working Arrangement
label variable arrangement "C11A - Working Arrangement"
label define arrangement 1 "Working in the default place of work (not at home)" 2 "Telecommuting / Work from home" 3 "Home-based work" 4 "On job rotation" 5 "On a mixed mode working arrangement" 6 "On reduced hours" 
label values arrangement arrangement
...
label variable ftwork1 "C31-First Time to Look for Work"
label define ftwork1 1 "yes" 2 "no" 
label values ftwork1 ftwork1
...
save "`path_in_stata'/PHL_LFS_2023.dta", replace
...
*/
use "`path_in_stata'/PHL_LFS_2023.dta", clear
...
*<_urban_>
	gen byte urban = (pufurb2015 == 1) | (pufurbrural == 1)
...
*<_subnatid1_>
	sdecode pufreg, gen(regtext)
	tostring pufreg, gen(regcode)
	
	* Beginning August 2023, ARMM code is changed to 19 from 15. 
	* I just use 19 here for all, then use 15 for subnatid_prev for consistency

	replace regcode = "19" if regcode == "15"
	
	gen subnatid1 = regcode + " - " + regtext if !missing(pufreg)
...
*<_subnatid1_prev_>
	...
	replace subnatid1_prev = "15 - Autonomous Region in Muslim Mindanao (ARMM)" if subnatid1 == "19 - Autonomous Region in Muslim Mindanao (ARMM)"
...
*<_educat7_>
	/*Please refer to the "Education_Levels.md" for a detailed discussion on classification of how each level is classified and why,
		available in the github repository.

		For 2023, the raw education codes follow the PSCED 2017 structure and are
		coded directly. */

	gen byte educat7=.
	replace educat7 = 1 if grade <= 2000 		// less than primary to "no education"
	replace educat7 = 2 if grade == 10002
	replace educat7 = 2 if grade == 10010
	replace educat7 = 2 if inrange(grade, 10011, 10015)
	replace educat7 = 3 if grade == 10020
	replace educat7 = 3 if inrange(grade, 10016, 10018)
	replace educat7 = 4 if grade == 24002
	replace educat7 = 4 if inrange(grade, 24010, 24014)
	replace educat7 = 5 if inrange(grade, 24015, 35013)
