...
** FILES
	local round1 `"`stata'\LFS JAN2005.dta"'
	local round2 `"`stata'\LFS APR2005.dta"'
...
*** set up the codebook template
	iecodebook template ///
		...
		, clear replace surveys(JAN2005 APR2005 JUL2005 OCT2005) /// survey names
...
*** append the dataset
	iecodebook append ///
		...
		, clear surveys(JAN2005 APR2005 JUL2005 OCT2005)	 /// survey names
		generate(round)										//  generate a factor round variable
	}
	else {
*** use the single file
	use `"`round1'"', clear
...
** HOUSEHOLD WEIGHTS
	...
	gen double wgt= fwgt/ 4
	* // for 2005, inverse scaling by [no. rounds]*[10 000], only for first two rounds. See "weights_methodology.Rmd"
	replace 	wgt = wgt / 10000 	if round == 1 | round == 2
...
** OCCUPATION CLASSIFICATION
	* in 2005, raw variable is numeric
...
** CONTRACT
	gen byte contract=.
	replace contract=0 if c10_cnwr==2 | c10_cnwr==8
	replace contract=1 if c10_cnwr==1
