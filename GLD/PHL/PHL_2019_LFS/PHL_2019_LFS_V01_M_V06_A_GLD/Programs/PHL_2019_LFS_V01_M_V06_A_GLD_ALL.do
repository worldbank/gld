...
<_Program name_>				PHL_2019_LFS_V01_M_V06_A_GLD_ALL.do </_Program name_>
...
* Date: 2026-04-02 - Filled educat7 for July and October using the same broad 2019 education ladder across all four rounds, and mapped the detailed 100xx primary codes so 10011-10015 align with grouped code 10010 and 10016-10018 align with grouped code 10020.
* 2023/02/24 - Update educat7
...
*<_educat7_>
	/*Please refer to the "Education_Levels.md" for a detailed discussion on classification of how each level is classified and why,
		...
		For 2019, January and April use grouped PSCED 2017-style codes, while July
		and October use a more detailed unlabeled version of the same broad ladder.
		The first digit remains informative across all four rounds: 0 is early
		childhood/no schooling, 1 primary, 2 lower secondary, 3 upper secondary, 4
		post-secondary non-tertiary, 5 short-cycle tertiary, and 6+ bachelor or
		higher.

		For the 100xx primary block, weighted distributions suggest that grouped code
		10010 in the labelled rounds corresponds most closely to detailed codes
		10011-10015 in July/October, while grouped code 10020 corresponds more
		closely to detailed codes 10016-10018. The mapping below uses that
		aggregation.

		Code 24002 is labelled only as IPed / Madrasah / SPED. For 2019 we treat
		this as secondary incomplete rather than primary complete, so the 2xxxx
		prefix remains consistent with the lower-secondary block used in the rest of
		the 2019 ladder.

		Weighted distributions in July and October also suggest that 24015 aligns
		more closely with the grouped 24020 block than with the grouped 24010 block,
		so 24015 is treated as secondary complete rather than secondary incomplete. */

	gen byte educat7=.
	replace educat7 = 1 if pufc07_grade <= 2000 		// less than primary to "no education"
	replace educat7 = 2 if pufc07_grade == 10002
	replace educat7 = 2 if pufc07_grade == 10010
	replace educat7 = 2 if inrange(pufc07_grade, 10011, 10015)
	replace educat7 = 3 if pufc07_grade == 10020
	replace educat7 = 3 if inrange(pufc07_grade, 10016, 10018)
	replace educat7 = 4 if pufc07_grade == 24002
	replace educat7 = 4 if inrange(pufc07_grade, 24010, 24014)
	replace educat7 = 5 if inrange(pufc07_grade, 24015, 35013)
	replace educat7 = 6 if inrange(pufc07_grade, 40000, 59999)
	replace educat7 = 7 if inrange(pufc07_grade, 60000, 99999)
