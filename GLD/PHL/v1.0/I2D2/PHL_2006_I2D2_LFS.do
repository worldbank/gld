...
** NUMBER OF INDIVIDUALS	203795
** EXPANDED POPULATION	85027819
**                                                                                                  **
******************************************************************************************************
*****************************************************************************************************/
...
** DATABASE ASSEMBLENT
	...
	use "`path'\Original\LFS JAN2006.dta"
...
** HOUSEHOLD SIZE
	sort idh
	by idh: egen hhsize= count(idd) if c05_rel>=1 & c05_rel<9
	label var hhsize "Household size"


** RELATIONSHIP TO THE HEAD OF HOUSEHOLD
	gen byte head=c05_rel
	recode head (7=4) (9/11=6)(4 5 6 8=5)
...
** MARITAL STATUS
	gen byte marital=.
	replace marital=1 if c08_ms==2
	replace marital=4 if c08_ms==4
	replace marital=5 if c08_ms==3
	replace marital=2 if c08_ms==1
...
** YEARS OF EDUCATION COMPLETED
	gen byte educy=.
	replace educy=0 if c09_grade==0 | c09_grade==1
	replace educy=7 if c09_grade==2 |  c09_grade==3
	replace educy=13 if c09_grade==4
	replace educy=16 if c09_grade>5 & age>5
...
** EVER ATTENDED SCHOOL
	gen byte everattend=.
	replace everatten=1 if edulevel2==2 | edulevel2==3 | edulevel2==4
	replace everatten=0 if edulevel2==1
...
** LABOR STATUS
	gen byte lstatus=3
	replace lstatus=1 if c12_work==1 | c13_job==1
	replace lstatus=2 if c31_lookw==1
...
** EMPLOYMENT STATUS
	gen byte empstat=.
	replace empstat=1 if c24_pclas==0 | c24_pclas==1 | c24_pclas==2 | c24_pclas==5
	replace empstat=2 if c24_pclas==6
	replace empstat=3 if c24_pclas==4
	replace empstat=4 if c24_pclas==3
...
** REASONS NOT IN THE LABOR FORCE
	...
	replace nlfreason=3 if c35_wynot==6 & age>15
...
** UNEMPLOYMENT DURATION: MONTHS LOOKING FOR A JOB
	...
	replace unempldur_l=. if age<lb_mod_age & age!=.
	label var unempldur_l "Unemployment duration (months) lower bracket"

	gen byte unempldur_u=c34_weeks/4.2
	replace unempldur_u=. if age<lb_mod_age & age!=.
...
** OCCUPATION CLASSIFICATION
	gen byte occup=floor(c15/10)
	recode occup 0=10
	replace occup=99 if c15==9
...
** WAGES TIME UNIT
	gen byte unitwage=1 if lstatus==1
...
** DELETE MISSING VARIABLES
	...
	saveold "`path'\Processed\PHL_2006_I2D2_LFS.dta", replace
	saveold "D:\__CURRENT\PHL_2006_I2D2_LFS.dta", replace
