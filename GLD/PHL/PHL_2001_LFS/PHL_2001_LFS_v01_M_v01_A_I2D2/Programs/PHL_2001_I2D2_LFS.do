...
** Created	4/4/2012
** Modified	24/5/2021
** NUMBER OF HOUSEHOLDS	39,716
** NUMBER OF INDIVIDUALS	207,457
...
** VALUES
	...
	local cases  	777698		// 207457 (Jan) + 190830 (APR) + 190074 (Jul) + 189337 (Oct) (Source: ILO from PSA)
...
*** append the dataset
	iecodebook append ///
		...
		, clear surveys(JAN2001) generate(round) // survey names
	}
	else {
*** use the single file
	use `"`round1'"', clear
...
** INDUSTRY CLASSIFICATION
	...
	gen byte industry=.
	replace industry=1 if c16_pkb >= 1 & c16_pkb <= 9		// Agriculture
	replace industry=2 if c16_pkb == 10 | c16_pkb == 11		// Mining
	replace industry=3 if c16_pkb>=15 & c16_pkb <= 39		// Manufacturing
	replace industry=4 if c16_pkb==40 | c16_pkb==41			// Public Utility Services
	replace industry=5 if c16_pkb==45						// Construction
	replace industry=6 if c16_pkb >= 50 & c16_pkb <= 55		// Commerce
	replace industry=7 if c16_pkb >= 60 & c16_pkb <= 64		// Transport + Communication
	replace industry=8 if c16_pkb >= 65 & c16_pkb <= 74		// Financial + Business Services
	replace industry=9 if c16_pkb == 75						// Public Administration
	replace industry=10 if c16_pkb>=76 & c16_pkb <= 99 		// this includes education for now.
