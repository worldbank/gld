...
** INPUT DATABASES	LFS JAN2007
** RESPONSIBLE	 World Bank Jobs Group
** Created	4/4/2012
** Modified	24/5/2021
** NUMBER OF HOUSEHOLDS	(~200,000 est PSA website)
...
** FILES
	local round1 `"`stata'\LFS JAN2007.dta"'
	local round2 `"`stata'\LFS APR2007.dta"'
	local round3 `"`stata'\LFS JUL2007.dta"'
	local round4 `"`stata'\LFS OCT2007.dta"'

** VALUES
	local n_round 	4			// numer of survey rounds
	local cases  	798840		// 198627 (Jan) + 202256 (APR) + 196085 (Jul) + 201872 (Oct)
...
*** set up the codebook template
	iecodebook template ///
		...
		, clear replace surveys(JAN2007 APR2007 JUL2007 OCT2007) /// survey names
...
*** append the dataset
	iecodebook append ///
		...
		generate(round)										// create a factor var called "round" to identify data source
	}
	else {
*** use the single file
	use `"`round1'"', clear
...
** HOUSEHOLD WEIGHTS
	...
	* // inverse scaling by [no. rounds]*[10 000], only for first round. See "weights_methodology.Rmd"
	replace 	wgt = wgt / 10000 	if round == 1
