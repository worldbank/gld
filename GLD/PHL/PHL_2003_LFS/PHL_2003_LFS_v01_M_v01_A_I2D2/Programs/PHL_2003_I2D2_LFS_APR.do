...
** LOG FILE
	log using `"`id_data'\\rounds\\`cty3'_`surv_yr'_I2D2_LFS_APR.log"', replace
...
** DELETE MISSING VARIABLES
	...
	foreach var of varlist urb - round {
	qui sum `var'
	scalar sclrc = r(mean)
	if sclrc==. {
...
** MISSING VALUES
	*Declare varlist which cannot contain missings
	...
	save `"`id_data'\\rounds\\`cty3'_`surv_yr'_I2D2_LFS_APR.dta"', replace
