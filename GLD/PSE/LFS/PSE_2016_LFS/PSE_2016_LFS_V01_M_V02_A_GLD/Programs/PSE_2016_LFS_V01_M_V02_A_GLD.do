...
<_Program name_>				[PSE_2016_LFS_V01_M_V02_A_GLD] </_Program name_>
...
<_Sample size (HH)_> 				[13,968] </_Sample size (HH)_>
<_Sample size (IND)_> 				[94,036] </_Sample size (IND)_>
...
* Try to use the file
use "`path_in_stata'/LFS16Q1.dta", clear

merge 1:1 IDSAM HR0A using "`path_in_stata'/LFS_Q1_2016_weights.dta"
    drop if _merge == 1
    drop RW
    rename weight RWP
    drop _merge
...
* Q2
use "`path_in_stata'/LFS16Q2.dta", clear
merge 1:1 IDSAM HR0A using "`path_in_stata'/LFS_Q2_2016_weights.dta"
    drop if _merge == 1
    drop RW
    rename weight RWP
    drop _merge
...
* Q3
use "`path_in_stata'/LFS16Q3.dta", clear
merge 1:1 IDSAM HR0A using "`path_in_stata'/LFS_Q3_2016_weights.dta"
    drop if _merge == 1
    drop RW
    rename weight RWP
    drop _merge
...
* Q4
use "`path_in_stata'/LFS16Q4.dta", clear
merge 1:1 IDSAM HR0A using "`path_in_stata'/LFS_Q4_2016_weights.dta"
    drop if _merge == 1
    drop RW
    rename weight RWP
    drop _merge
...
* Annual file 
use "`path_in_stata'/LFS16_Y.dta", clear
* Keep only one district per household (no differences)
bys IDSAM : egen sd = sd(ID06)
quietly : summ sd
assert `r(mean)' == 0
bys IDSAM :  gen runner = _n 
keep if runner == 1
decode ID06, gen(oldvar_label)
gen governorate = string(ID06) + " - " + oldvar_label
...
*<_industrycat_isic_>
	gen indus_help1=string(PW37, "%02.0f")
	gen str4 indus_help2= indus_help1 + "00"
	gen industrycat_isic = indus_help2
...
*<_occup_isco_>
	gen str4 occup_isco= string(PW39) + "00"
	replace occup_isco="0100" if occup_isco=="100"
	replace occup_isco="0200" if occup_isco=="200"
	replace occup_isco="0300" if occup_isco=="300"
...
*<_healthins_>
	gen byte healthins = .
	replace healthins=1 if PW471==1 | PW472==1
...
*<_industrycat_isic_2_>
	gen indus_help1=string(PW66, "%02.0f")
	gen str4 indus_help2= indus_help1 + "00"
	gen industrycat_isic_2 = indus_help2
...
*<_occup_isco_2_>
	gen str4 occup_isco_2= string(PW68) + "00"
	replace occup_isco_2="0100" if occup_isco_2=="100"
	replace occup_isco_2="0200" if occup_isco_2=="200"
	replace occup_isco_2="0300" if occup_isco_2=="300"
	replace occup_isco_2="" if occup_isco_2==".00"
	replace occup_isco_2="" if occup_isco_2=="-100"
