...
<_Program name_>				[PSE_2005_LFS_V01_M_V01_A_GLD] </_Program name_>
...
<_Sample size (HH)_> 				[15,327] </_Sample size (HH)_>
<_Sample size (IND)_> 				[68,852] </_Sample size (IND)_>
...
* Try to use the file
use "`path_in_stata'/LFS05Q1.dta", clear
gen quarter=1
append using  "`path_in_stata'/LFS05Q2.dta"
replace quarter=2 if quarter==.
append using  "`path_in_stata'/LFS05Q3.dta"
replace quarter=3 if quarter==.
append using  "`path_in_stata'/LFS05Q4.dta"
...
/* <_hhid_note>
	...
	gen  pid_1 = string(person, "%03.0f")
	gen hhid_1 = string(idsam, "%07.0f")
		
	*gen hhid = string(idsam, "%06.0f") + "_" + string(quarter)
		 * --- Step 1: Sort and find duplicates within each household-person-quarter
bysort hhid_1 pid_1 quarter: gen n_dups = _N
gen dupflag = n_dups > 1

* --- Step 2: Identify which records belong to the "duplicate household" (second+ entries)
bysort hhid_1 pid_1 quarter (pr1): gen in_split = dupflag & _n > 1

* --- Step 3: Mark households that contain any duplicates
bysort hhid_1: egen anysplit = max(in_split)

* --- Step 4: Create a new household id (string)
clonevar hhid_str=hhid_1
gen strL hhid_new = hhid_str

* --- Step 5: For duplicate households, assign new hhid to second household group
* If you want just "_new" you can use that. 
replace hhid_new = hhid_str + "_99" if anysplit & in_split
gen hhid = hhid_new 
...
replace occup_isco="5000" if occup_isco=="5300"
	* Check that no errors --> using our universe check function, count should be 0 (no obs wrong)
	* https://github.com/worldbank/gld/tree/main/Support/Z%20-%20GLD%20Ecosystem%20Tools/ISIC%20ISCO%20universe%20check
	preserve 
