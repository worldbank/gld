...
<_Program name_>				[PSE_2024_LFS_V01_M_V01_A_GLD] </_Program name_>
...
<_Sample size (HH)_> 				[9,706] </_Sample size (HH)_>
<_Sample size (IND)_> 				[60,242] </_Sample size (IND)_>
...
* Q1
use "`path_in_stata'/new/LFS24Q1.dta", clear
gen quarter=1
save `q1'

* Q2

use "`path_in_stata'/new/LFS24Q2.dta", clear
gen quarter=2
save `q2'

* Q3
use "`path_in_stata'/new/LFS24Q3.dta", clear
gen quarter=3
save `q3'

* Q4
use "`path_in_stata'/new/LFS24Q4.dta", clear
gen quarter=4
save `q4'


* Annual file 
use "`path_in_stata'/LFS24_Y.dta", clear
...
append using `q3'
append using `q4'
merge m:1 IDSAM using "`governorate'", assert(match) nogen
...
*<_weight_>
	...
	**using code from previous years does not work. so we use an the population 15 plus to compute an scalar that project what 10 to 14 should be. 

	* 1) Compute your micro 15+ total using relative weights
	summarize RWP if inrange(Pr1,15,999)
	scalar micro_15plus = r(N)*r(mean)

	* 2) Define the official 15+ control (West Bank, 2024)
	scalar report_wap_15plus = 2163574

	* 3) Single scalar to align your relative weights
	scalar adj_scalar = report_wap_15plus / micro_15plus

	* 4) Apply scalar to RWP
	gen weight = RWP * adj_scalar
...
*<_weight_q_>
	* Each quarter shall have the weight to expand to the same annual population 
	* (have no quarterly to compare to)
	...
	gen weight_q = weight*(1/help_w_4)
	drop help_w_*
...
*<_educat7_>
	gen byte educat7 = Pr4
	recode educat7 2=1 3=2 4=3  8=6 9=7 10=7
...
*are UN agencies private ? there is no option for foregin 
	gen byte ocusec_2 = PW77
	recode ocusec_2 (2 4 5 6 7 8=2) (3=1)
