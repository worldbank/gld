...
<_Date created_>				2024-3-21 </_Date created_>
-------------------------------------------------------------------------
<_Country_>						Cameroon (CMR) </_Country_>
<_Survey Title_>				Third survey on employment and the informal sector in Cameroon (EESI3) </_Survey Title_>
...
<_Geographic coverage_> 		EESI covers the whole national territory which comprises 
								...
								the ten administrative regions of the country. </_Geographic coverage_>
...
<_OCCUP National_>				MEPC </_OCCUP National_>
...
* harmonized output in a single file
	 ...
	 *use "`path_in_stata'\eesi3ind.dta.dta", clear
	 use "C:\Users\IrIs_\OneDrive - Georgetown University\World Bank\GLD\CMR\CMR_2021_EESI\CMR_2021_EESI_V01_M\Data\Stata\eesi3ind.dta", clear
...

{
	...
	replace region="Centre Yaounde" if s0g1==4
	replace region="East" if s0g1==5
	replace region="Far North" if s0g1==6
	replace region="Littoral Douala" if s0g1==7
...
Note that 2021 has a newly added variable in the questionnaire, g11a, which is the
number of persons 18 years of age or above. But this variable is not actually in the
dataset.
...
329 observations have both answered in months (m11am) and in years (m11an). We
only took their answers in months for this variable since people only answered in 
years if the duration exceeds 2 years.

*<_migrated_years_note_>*/  

	gen migrated_years=.
	replace m11an=. if m11an==0
	replace m11am=. if m11am==0
	
	replace migrated_years=round(m11am/12,0.1) if !mi(m11am)&mi(m11an)
	replace migrated_years=m11an if !mi(m11an)&mi(m11am) 
...
*<_migrated_reason_>
	...
	recode migrated_reason (1 2=3) (4=2) (7 8=1) (3 6 9 10 96=5) 
...
*<_lstatus_>
	gen byte lstatus=.
	replace lstatus=1 if se2==1|inrange(se3,1,9)|inrange(se5,1,7)|se6==1
	replace lstatus=2 if lstatus==.&sch1a==1&sch1c==1
	replace lstatus=3 if !mi(sch2b1)&lstatus==.
...
1)available but not searching or (sch1c==1|sch2b2==1)&sch1a!=1
2)searching but not immediately available to work or sch1a==1&inrange(sch1c,2,4)

</_potential_lf_note_>*/

	gen potential_lf=.
	replace potential_lf=1 if [(sch1c==1|sch2b2==1)&sch1a!=1] | [sch1a==1&inrange(sch1c,2,4)]
	replace potential_lf=0 if [sch1a==1&(sch1c==1|sch2b2==1)] | [inrange(sch1c,2,4)&sch1a!=1]
...
*<_nlfreason_>
	gen byte nlfreason=sch2b1
	recode nlfreason (12=1) (6=4) (1/4 7/11 13=5)
...
c1y indicates the year a given respondent was unemployed;
c1m is the month a given respondent was unemployed.

*<_unempldur_l_note_>*/

	gen unemp_year=2021-c1y if !mi(c1y)
	gen unemp_month=int_month-c1m if unemp_year==0
	replace unemp_month=. if unemp_month<0
	replace unemp_month=1 if unemp_month==0
	
	replace unemp_year=unemp_year-1 if c1m>int_month&unemp_year!=0
	replace unemp_month=12-c1m+int_month if c1m>int_month&unemp_year!=0
	replace unemp_month=int_month-c1m if c1m<int_month&unemp_year!=0
	
	gen byte unempldur_l=unemp_year*12+unemp_month
...
*<_empstat_>
	...
	recode empstat (1/5 9 10=3) (6 7=4) (8=2) (11=5)   
...
*<_ocusec_>
	gen byte ocusec=ap4
	recode ocusec (2=3) (3 4 6 7 8=2) (5=4)
