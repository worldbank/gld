...
/* -----------------------------------------------------------------------
<_Program name_>				CMR_2005_EESI_V01_M_V01_A_GLD_ALL.do </_Program name_>
...
<_Study ID_>					CMR_2005_EESI_v01_M </_Study ID_>
<_Data collection from (M/Y)_>	[02/2005] </_Data collection from (M/Y)_>
<_Data collection to (M/Y)_>	[10/2005] </_Data collection to (M/Y)_>
...
use "`path_in_stata'\eesi1ind_hh.dta", clear
* Keep only those for whom we have the relevant data (aged 10+ in 99.9% of cases)
keep if !missing(nzd)

* There are two cases of repeat in HH numbering that are clearly different people, correct here 
* and subsequently same in the other file
replace m1 = 9 if nzd == 122 & nmen == 78 & m1 == 4 & m5 == 14
replace m1 = 9 if nzd == 154 & nmen == 64 & m1 == 5 & m5 == 21

tempfile eesi_1
save "`eesi_1'"

use "`path_in_stata'\eesi1ind.dta", clear

* Make same correction as above 
replace m1 = 9 if nzd == 122 & nmen == 78 & m1 == 4 & m5 == 14
replace m1 = 9 if nzd == 154 & nmen == 64 & m1 == 5 & m5 == 21

* Then merge with tempfile file
merge 1:1 nzd nmen m1 using "`eesi_1'", assert(master match) nogen

* There are six observations that have no nzd, nmen, cannot assign them. Have no ap info. Drop
quietly: count 
local count_before `r(N)'
drop if missing(nzd) & missing(nmen)
quietly: count
assert `r(N)' == `count_before' - 6 
...
*<_hhid_>
	gen helper_1 = string(nzd,  "%03.0f")
	gen helper_2 = string(nmen, "%03.0f")
	egen hhid = concat(helper_1 helper_2)
	label var hhid "Household id"
*</_hhid_>


*<_pid_>
	gen helper_3 = string(m1, "%02.0f")
	egen pid=concat(hhid helper_3)
...

{
	...
	replace urb="Urban" if urb=="urbain"
	replace urb="Rural" if urb=="rural"
	}
...
There are households without heads, use the next in line per HH line number as head if missing

*<_relationharm_note_>*/

	gen byte relationharm=m2
	recode relationharm (7 = 6)
	bys hhid :  egen min_number = min(m1)
	replace relationharm = 1 if (min_number == m1) & (min_number > 1)
...
*<_educat7_>
	gen byte educat7=.
	replace educat7 = 1 if m15 == 0
	replace educat7 = 2 if m15 == 1 & m17 == 1
	replace educat7 = 3 if m15 == 1 & m17 == 2
	replace educat7 = 4 if m15 == 2 | m15 == 4
	replace educat7 = 5 if m15 == 3 | m15 == 5
	replace educat7 = 7 if m15 == 6
...
*<_educat_orig_>
	...
	* Create it from combination of m15 (level of instruction) and m17 highest diploma
	* Rewrite m15 labels as the accents appear as missing
	label define m15 0 "non scolarise" 1 "primaire" 2 "secondaire 1er cycle secondaire" 3 "secondaire 2nd cycle secondaire" 4 "secondaire 1er cycle technique" 5 "secondaire 2nd cycle technique" 6 "superieur" 9 "donne manquante", replace
	label define m17 0 "donnee manquante" 1 "aucun diplome" 2 "cepe/cep/fslc" 3 "bepc/cap/gceol" 4 "probatoire/bp" 5 "bac/gceal/bep" 6 "bts/dut/deug" 7 "licence" 8 "matrise/dea/mba/master" 9 "doctorat/phd", replace
	
	decode m15, gen(helper_ed_orig_1)
	decode m17, gen(helper_ed_orig_2)
	
	egen educat_orig = concat(helper_ed_*), punct(" - ")
...
{	
*<_lstatus_>
	* SE section not shared, use the derived variable "actif" already in the data
	* Note that we only use the "chomeur BIT" (unemployed ILO) as unemployed, else
	* is seen as NLF
	gen byte lstatus=.
	replace lstatus=1 if actif == 1
	replace lstatus=2 if actif == 2
	replace lstatus=3 if inrange(actif, 3, 9)
...
*<_industrycat10_>
	* Industry information coded as NACAM Ancien in 2005, moslty possible based on 2-digits
	* except for special cases:
	* --> 192 (printing, to communications) and 
	* --> 322 (réparation of machines/professional equipment says classif document vs.
	*          réparation of personal goods say data labels, to Commerce)
	* --> 421 (sanitation, to public utilities)
	
	gen ap2_1dig=floor(ap2/10)
	gen industrycat10=ap2_1dig
	recode industrycat10 (1/5 = 1) (6/7 = 2) (8/28 = 3) (29 = 4) (30 = 5) (31/33 = 6) (34/35 = 7) (36/38 = 8) (39 = 9) (40/44 = 10)
	
	replace industrycat10 = 7 if ap2 == 192
	replace industrycat10 = 3 if ap2 == 322
	replace industrycat10 = 4 if ap2 == 421
...
Unclear what thes classification used is. The documents shared do not seem
to match it. It is roughly correct (codes starting with 1 are agriculture)
but I cannot see a code 1111 (the most common one) explained.
...
We have accurate data for 6,071 people at monthly level and 1,167 annually. The category
data is present for 4,498 people monthly, 880 annually.
...
*<_wage_no_compen_note_>*/
    ...
    * to missing if value is 0.
	 gen wage13m=ap13a2*1000 if ap13a1==1 
	 gen wage13a=ap13a2*1000 if ap13a1==2 
     ...
     * Create salary categories based on winsor values
     gen salary_catm=.
     replace salary_catm=1 if inrange(wage13m_w, 1, 23499)
     replace salary_catm=2 if inrange(wage13m_w, 23500, 46999)
     ...
     * Fill it first with values that are accurate
     replace wage_no_compen=wage13m if ap13a1==1 
	 replace wage_no_compen=wage13a if ap13a1==2
...
*<_unitwage_>
	gen byte unitwage=.
	replace unitwage=5 if ap13a1==1|inrange(ap13b,1,7)
	replace unitwage=8 if ap13a1==2|inrange(ap13b,8,14)
...
*<_whours_>
	gen whours=ap10c
	replace whours=. if lstatus!=1 | ap10c==0 | ap10c==999
...
*<_firmsize_l_>
	...
	recode firmsize_l (4=6) (5=11) (6=21) (7=51) (8=101) (9=501) (0=.)
...
*<_firmsize_u_>
	...
	recode firmsize_u (3=5) (4=10) (5=20) (6=50) (7=100) (8=500) (0 9=.)
...
*<_empstat_2_>
	...
	recode empstat_2 (1/5=1) (6=3) (7=4) (8/9=2) (10=5) (99=.) 
...
*<_wage_no_compen_2_>
	 gen wage10m=as10a2*1000 if as10a1==1 
	 gen wage10a=as10a2*1000 if as10a1==2 
     ...
     * Create salary categories based on winsor values
     gen salary_catm2=.
     replace salary_catm2=1 if inrange(wage10m_w, 1, 23499)
     replace salary_catm2=2 if inrange(wage10m_w, 23500, 46999)
     ...
     * Fill it first with values that are accurate
     replace wage_no_compen_2=wage10m if as10a1==1 
	 replace wage_no_compen_2=wage10a if as10a1==2
...
*<_unitwage_2_>
	gen byte unitwage_2=.
	replace unitwage_2=5 if as10a1==1|inrange(as10b,1,7)
	replace unitwage_2=8 if as10a1==2|inrange(as10b,8,14)
...
*<_firmsize_l_2_>
	...
	recode firmsize_l_2 (4=6) (5=11) (6=21) (7=51) (8=101) (9=501) (0=.)
