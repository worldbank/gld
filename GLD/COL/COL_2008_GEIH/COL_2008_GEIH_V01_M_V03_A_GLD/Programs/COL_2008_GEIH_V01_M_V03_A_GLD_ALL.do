...
<_Program name_>				COL_2008_GEIH_V01_M_V01_A_GLD
...
<_Survey Year_>					2008
<_Study ID_>					DANE GEIH 2008 </_Study ID_>
...
<_Sample size (HH)_> 				221,988 </_Sample size (HH)_>
<_Sample size (IND)_> 				823,814
...
* Date: 2022-05-08 - Update contract variable
...
*** append monthly data
clear all
use "`path_in_stata'\GEIH_2008_1.dta"

forvalues i=2/12 {
    append using "`path_in_stata'\GEIH_2008_`i'.dta", force
}
...
duplicates tag directorio secuencia_p orden, gen(asserter)
assert asserter == 0
drop asserter
...
merge 1:1 directorio secuencia_p orden using "`general_2008'", force assert(master match) keep(match) nogen

save "`path_in_stata'\data_2008_final.dta", replace
*/
use "`path_in_stata'\data_2008_final.dta", clear
...
/* <_subnatidsurvey_note>
	...
	*egen subnatidsurvey = concat(subnatid2 urban), p(" ")
	gen subnatidsurvey = ""
	replace subnatidsurvey = subnatid3 if real(substr(subnatid3,1,3)) >= 5 ///
    & real(substr(subnatid3,1,3)) <= 76 //main 23 cities
...
*<_lstatus_>
	...
	replace lstatus=2 if dsi==1 & lstatus!=1
	replace lstatus=3 if inrange(p7430,1,2) & lstatus!=1
