...
* Date: 2022-29-05 - Changes from I2D2 to GLD
...
local path_output "Z:\GLD-Harmonization\582018_AQ\COL\COL_1999_ENH\COL_1999_ENH_v01_M_v01_A_GLD\Data\Harmonized"
...
*<_pid_>
	gen helper_n=string(nrolinea,"%02.0f")
	gen helper_1=string(100)
	egen pid = concat(helper_h helper_1 helper_n)
...
*<_hsize_>
	rename parentco relacion
	gen a=1 if (relacion!=8 & relacion!=19)
...
*<_educy_>
	gen byte educy = .
	replace educy=0 if niveduc==100 | niveduc==200 | niveduc==201
	replace educy=niveduc-300 if niveduc>=300 & niveduc<=305
	replace educy=niveduc-400+5 if niveduc>=400 & niveduc<=411
	replace educy=niveduc-500+11 if niveduc>=500 & niveduc<=509
	replace educy=. if age<ed_mod_age & age!=.
	replace educy=age if age < educy & (age != . & educy != .)
...
{
*<_lstatus_>
	gen byte lstatus = 1 if fzatrab==1 | actremu==1 | teniaemp==1 | actfamil==1
...
*<_unempldur_l_>
	gen byte unempldur_l= int(tpobus/4) if tpobus!=0 & tpobus!=99
...
*<_unempldur_u_>
	gen byte unempldur_u= int(tpobus/4) if tpobus!=0 & tpobus!=99
...
/* <_unitwage_note>
	...
	gen byte unitwage = periodoy
	replace unitwage = 5 if periodoy==1
	replace unitwage = 3 if periodoy==2
	replace unitwage = 10 if periodoy==3
	replace unitwage = 2 if periodoy==4
	replace unitwage = 1 if periodoy==5
	replace unitwage = . if periodoy==9 | periodoy==0
...
/* <_wage_total_note>
	...
	*Paid employees
	*For ENH make wages monthly
	gen wage_m = .
	replace wage_m = ingtraef*30 if unitwage == 1
	replace wage_m = ingtraef*4 if unitwage == 2
	replace wage_m = ingtraef*2 if unitwage == 3
	replace wage_m = ingtraef*0.5 if unitwage == 4
	replace wage_m = ingtraef if unitwage == 5
	replace wage_m = ingtraef*(1/3) if unitwage == 6
	replace wage_m = ingtraef/24 if unitwage == 7
	replace wage_m = ingtraef/12 if unitwage == 8
	replace wage_m = ingtraef*8 if unitwage == 9
	replace wage_m = . if unitwage == 10

	gen wage_principal_1 = ingtraef
	replace wage_principal_1 = ingtraef + wage_extra if wage_extra!=.

	*Self-employed, employer and others
	*Wage for employers and self-employed workers
	count if inganan!=. & (empstat!=3 & empstat!=4 & empstat!=5)
	gen wage_self = inganan if empstat==3 | empstat==4 | empstat==5

	*Replace wage for self-employed, employers, others
	replace wage_principal_1 = wage_self if (empstat==3 | empstat==4 | empstat==5 | inganan!=.) & wage_self!=.
	...
	*Replace wage for self-employed, employers, others
	replace wage_principal_2 = wage_self if (empstat==3 | empstat==4 | empstat==5 | inganan!=.) & wage_self!=.
...
save "`path_output'/COL_1999_ENH_V01_M_V01_A_GLD_ALL.dta", replace
