...
<_Program name_>				COL_2009_GEIH_V01_M_V04_A_GLD
...
*** append monthly data
clear all
use "`path_in_stata'\GEIH_2009_1.dta"

forvalues i=2/12 {
    append using "`path_in_stata'\GEIH_2009_`i'.dta", force
}
...
merge 1:1 directorio secuencia_p orden mes using "`general_2009'", force assert(master match) keep(match) nogen

save "`path_in_stata'\data_2009_final.dta", replace
*/

use "`path_in_stata'\data_2009_final.dta", clear
...
local letters "secuencia_p orden"
	...
	egen hhid_gld = concat(helper_d helper_secuencia_p)
