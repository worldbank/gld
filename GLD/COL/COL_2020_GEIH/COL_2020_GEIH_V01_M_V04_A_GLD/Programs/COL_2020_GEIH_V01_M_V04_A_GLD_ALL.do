...
<_Program name_>				COL_2020_GEIH_V01_M_V03_A_GLD_ALL
...
<_Sample size (HH)_> 				229935 </_Sample size (HH)_>
<_Sample size (IND)_> 				747822  </_Sample size (IND)_>
...
<_INDUS National_>				ISIC REV 4 COLOMBIA (2020) </_INDUS National_>
...
clear all
use "`path_in_stata'\GEIH_2020_1.dta"

forvalues i=2/12 {
    append using "`path_in_stata'\GEIH_2020_`i'.dta", force
}
rename *, lower

** amazonia annual data append
*for data transformation use convert file in programs folder in original data folder
append using "`path_in_stata'\amazonia\amazonia_2020.dta", force

***San andrés
append using "`path_in_stata'\SA_2020.dta", force

* Convert month into a number
destring mes, gen(mes_num)

* Save tempfile
tempfile general_2020
save `general_2020'
...
* Save tempfile
tempfile migration_2020
save `migration_2020'


* Start with full set
use "`general_2020'", clear

merge 1:1 directorio secuencia_p orden mes_num using "`migration_2020'",  nogen
...
merge 1:1 directorio secuencia_p orden mes_num using "`general_mig'", force nogen keep(2 3) 

save "`path_in_stata'\data_2020_final.dta", replace
*/
use "`path_in_stata'\data_2020_final.dta", clear
...
*<_weight_>
	...
	replace weight = fex_c/12 if missing(weight)
	replace weight = fex_c_2011/12 if missing(weight)
...
*<_subnatid2_>
	...
	replace subnatid2 = "88 - San Andres"                if area == "88" 
