...
<_Program name_>				COL_2020_GEIH_V01_M_V02_A_GLD_ALL
...
local path_output "Z:\GLD-Harmonization\582018_AQ\COL\COL_2020_GEIH\COL_2020_GEIH_v01_M_V02_A_GLD\Data\Harmonized\"
...
clear all
use "`path_in'\GEIH_2020_1.dta"

forvalues i=2/12 {
    append using "`path_in'\GEIH_2020_`i'.dta", force
}
rename *, lower

** amazonia annual data append
*for data transformation use convert file in programs folder in original data folder
append using "`path_in'\amazonia\amazonia_2020.dta", force

***San andrés
append using "`path_in'\SA_2020.dta", force
...
bys directorio : egen min_month = min(mes_num) if (dup > 0 & !missing(dup))
...
replace first_two = "99" if mes_num == min_month & (dup > 0 & !missing(dup))
...
*destring directorio_unique, replace
destring directorio_unique , replace
format %16.0g directorio_unique
drop directorio
rename directorio_unique directorio
...
merge 1:1 directorio secuencia_p orden using "`migration_2020'",  nogen
...
save "`path_output'\COL_2020_GEIH_V01_M_V02_A_GLD_ALL.dta", replace
