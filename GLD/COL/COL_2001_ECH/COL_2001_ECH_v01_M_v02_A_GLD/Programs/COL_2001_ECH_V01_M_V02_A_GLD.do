...
local path_output "Z:\GLD-Harmonization\582018_AQ\COL\COL_2001_ECH\COL_2001_ECH_V01_M_V02_A_GLD\Data\Harmonized"
...
use "`path_in'/ECH_2001_7.dta"

forvalues i=8/12 {
    append using "`path_in'\ECH_2001_`i'.dta", force
}

save "`path_in'/ECH_2001.dta", replace
...
*<_minlaborage_>
	...
	*assumption based on assumption survey could be 12 , could be 10 , now in 12d2 is 06
...
/* <_wage_total_note>
	...
	*In-kind income (housing) QI.17
	destring valor30b, replace
	gen wage_house = valor30b if lstatus==1
	...
	*Paid employees
	...
	replace wage_principal_1 = valor28 + wage_extra if wage_extra!=.
	...
	*Replace wage for self-employed, employers, others
	replace wage_principal_1 = wage_self if (empstat==3 | empstat==4 | empstat==5 | valor28!=.) & wage_self!=.
	...
	*Replace wage for self-employed, employers, others
	replace wage_principal_2 = wage_self if (empstat==3 | empstat==4 | empstat==5 | valor28!=.) & wage_self!=.
...
*<_healthins_>
	destring p6f, force replace
	gen byte healthins= p6f
...
save "`path_output'/COL_2001_ECH_V01_M_V02_A_GLD_ALL.dta", replace
