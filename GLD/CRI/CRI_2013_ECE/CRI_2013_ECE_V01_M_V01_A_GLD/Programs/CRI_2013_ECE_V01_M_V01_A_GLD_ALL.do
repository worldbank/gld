...
<_Program name_>				CRI_2013_ECE_V01_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					StataNow 19 </_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2026-08-07 </_Date created_>
...
<_Study ID_>					CR-INEC-ECE-2013 </_Study ID_>
...
<_Sample size (HH)_> 				30,036 </_Sample size (HH)_>
<_Sample size (IND)_> 				100,796 </_Sample size (IND)_>
...
use "`path_in_stata'/I Trimestre 2013.dta", clear
append using "`path_in_stata'/II Trimestre 2013.dta"
append using "`path_in_stata'/III Trimestre 2013.dta"
append using "`path_in_stata'/IV Trimestre 2013.dta"
...
*<_isco_version_>
gen str9 isco_version = "isco_2008"
...
* Reviewer rule: enforce one head per household-period.
...
label var relationharm "Relationship to head - Harmonized"
...
label var relationcs "Relationship to head - Country original"
*</_relationcs_>
...
label var ed_mod_age "Age education module begins"
*</_ed_mod_age_>
...
replace educy = . if Amos_educacion==99
label var educy "Years of completed education"
*</_educy_>


*<_educat7_>
* Note: Approved questionnaire ladder.
gen byte educat7 = .
...
replace educat_orig = . if Educacion_nivel_grado==99
label var educat_orig "Level of education - country original"
*</_educat_orig_>
...
label define lblvocational 0 "No" 1 "Yes"
...
tostring EducacionNoregular_codigo, gen(vocational_field_orig) format(%02.0f)
replace vocational_field_orig = "" if vocational!=1 | EducacionNoregular_codigo==99
label var vocational_field_orig "Vocational training field - country original"
*</_vocational_field_orig_>
...
label var minlaborage "Minimum age for labor questions"
*</_minlaborage_>
...
{
*<_lstatus_>
* Note: ICLS-13 indicators validated against the raw questionnaire tree in all 100,796 records.
gen byte lstatus = .
...
label var lstatus "Labor market status"
...
*<_underemployment_>
gen byte underemployment = Subempleado if lstatus==1 & inlist(Subempleado,0,1)
...
*<_nlfreason_>
gen byte nlfreason = .
replace nlfreason = 2 if lstatus==3 & Desea_trabajar==5
replace nlfreason = 3 if lstatus==3 & Desea_trabajar==2
replace nlfreason = 4 if lstatus==3 & Desea_trabajar==3
replace nlfreason = 5 if lstatus==3 & inlist(Desea_trabajar,1,4,6,7,8,9)
...
*<_empstat_>
gen byte empstat = .
replace empstat = 1 if lstatus==1 & Posicion_empleo==1
replace empstat = 2 if lstatus==1 & Posicion_empleo==4
replace empstat = 3 if lstatus==1 & Posicion_empleo==3
replace empstat = 4 if lstatus==1 & Posicion_empleo==2
label var empstat "Employment status primary job"
...
*<_ocusec_>
gen byte ocusec = .
replace ocusec = 1 if lstatus==1 & Sector_institucional==1
replace ocusec = 2 if lstatus==1 & Sector_institucional==2
label var ocusec "Sector primary job"
...
*<_industry_orig_>
gen long industry_orig = Cod_rama_publicacion
replace industry_orig = . if !(lstatus==1) | Cod_rama_publicacion==0
label var industry_orig "Industry country original"
*</_industry_orig_>


*<_industrycat_isic_>
gen str4 industrycat_isic = ""
replace industrycat_isic = string(Cod_rama_publicacion,"%04.0f") if lstatus==1 & Cod_rama_publicacion!=0 & !missing(Cod_rama_publicacion)
label var industrycat_isic "ISIC industry classification"
*</_industrycat_isic_>


*<_industrycat10_>
gen byte industrycat10 = .
gen byte __div = floor(Cod_rama/100) if lstatus==1 & Cod_rama!=0
replace industrycat10 = 1 if inrange(__div,1,3)
replace industrycat10 = 2 if inrange(__div,5,9)
replace industrycat10 = 3 if inrange(__div,10,33)
replace industrycat10 = 4 if inrange(__div,35,39)
replace industrycat10 = 5 if inrange(__div,41,43)
replace industrycat10 = 6 if inrange(__div,45,47)
replace industrycat10 = 7 if inrange(__div,49,53)
replace industrycat10 = 6 if inrange(__div,55,56)
replace industrycat10 = 7 if inrange(__div,58,63)
replace industrycat10 = 8 if inrange(__div,64,82)
replace industrycat10 = 9 if inrange(__div,84,84)
replace industrycat10 = 10 if inrange(__div,85,99)
drop __div
label var industrycat10 "1 digit industry classification"
label define lblindustrycat10 1 "Agriculture" 2 "Mining" 3 "Manufacturing" 4 "Public utilities" 5 "Construction" 6 "Commerce" 7 "Transport and Communications" 8 "Financial and Business Services" 9 "Public Administration" 10 "Other Services, Unspecified"
...
*<_occup_orig_>
gen long occup_orig = Cod_ocupacion
replace occup_orig = . if !(lstatus==1) | Cod_ocupacion==9800
label var occup_orig "Occupation country original"
*</_occup_orig_>


*<_occup_isco_>
gen str4 occup_isco = ""
replace occup_isco = string(Cod_ocupacion,"%04.0f") if lstatus==1 & Cod_ocupacion!=9800 & !missing(Cod_ocupacion)
label var occup_isco "ISCO occupation classification"
*</_occup_isco_>


*<_occup_>
gen byte occup = .
replace occup = real(substr(occup_isco,1,1)) if lstatus==1 & occup_isco!="" & substr(occup_isco,1,1)!="0"
replace occup = 10 if lstatus==1 & substr(occup_isco,1,1)=="0"
...
*<_whours_>
gen double whours = Horas_efectivas if lstatus==1
replace whours = . if Horas_efectivas==999 | whours>168
label var whours "Primary weekly hours"
*</_whours_>
...
*<_healthins_>
gen byte healthins = Condicion_aseguramiento if lstatus==1 & inlist(Condicion_aseguramiento,0,1)
label var healthins "Employment has health insurance"
...
*<_socialsec_>
gen byte socialsec = Condicion_aseguramiento if lstatus==1 & inlist(Condicion_aseguramiento,0,1)
label var socialsec "Employment has social security"
label define lblsocialsec 0 "Without social security" 1 "With social security"
...
*<_firmsize_l_>
gen byte firmsize_l = .
replace firmsize_l = 1 if lstatus==1 & Tamano_empresa==1
replace firmsize_l = 4 if lstatus==1 & Tamano_empresa==2
replace firmsize_l = 10 if lstatus==1 & Tamano_empresa==3
replace firmsize_l = 30 if lstatus==1 & Tamano_empresa==4
...
*<_firmsize_u_>
gen byte firmsize_u = .
replace firmsize_u = 3 if lstatus==1 & Tamano_empresa==1
replace firmsize_u = 9 if lstatus==1 & Tamano_empresa==2
replace firmsize_u = 29 if lstatus==1 & Tamano_empresa==3
...
*<_empstat_2_>
gen byte empstat_2 = .
replace empstat_2 = 1 if Empleo_secundario==1 & Posicion_secundario==1
replace empstat_2 = 2 if Empleo_secundario==1 & Posicion_secundario==4
replace empstat_2 = 3 if Empleo_secundario==1 & Posicion_secundario==3
replace empstat_2 = 4 if Empleo_secundario==1 & Posicion_secundario==2
label var empstat_2 "Employment status secondary job"
label define lblempstat_2 1 "Paid employee" 2 "Non-paid employee" 3 "Employer" 4 "Self-employed" 5 "Other, workers not classifiable by status"
...
*<_ocusec_2_>
gen byte ocusec_2 = .
replace ocusec_2 = 1 if Empleo_secundario==1 & Sector_institucional_secundario==1
replace ocusec_2 = 2 if Empleo_secundario==1 & Sector_institucional_secundario==2
label var ocusec_2 "Sector secondary job"
label define lblocusec_2 1 "Public Sector, Central Government, Army" 2 "Private, NGO" 3 "State owned" 4 "Public or State-owned, but cannot distinguish"
label values ocusec_2 lblocusec_2
*</_ocusec_2_>


*<_industry_orig_2_>
gen long industry_orig_2 = Cod_actividad_sec
replace industry_orig_2 = . if !(Empleo_secundario==1) | Cod_actividad_sec==0
label var industry_orig_2 "Industry secondary country original"
*</_industry_orig_2_>


*<_industrycat_isic_2_>
gen str4 industrycat_isic_2 = ""
replace industrycat_isic_2 = string(Cod_actividad_sec,"%04.0f") if Empleo_secundario==1 & Cod_actividad_sec!=0 & !missing(Cod_actividad_sec)
label var industrycat_isic_2 "ISIC secondary classification"
*</_industrycat_isic_2_>


*<_industrycat10_2_>
gen byte industrycat10_2 = .
gen byte __div2 = floor(Cod_actividad_sec/100) if Empleo_secundario==1 & Cod_actividad_sec!=0
replace industrycat10_2 = 1 if inrange(__div2,1,3)
replace industrycat10_2 = 2 if inrange(__div2,5,9)
replace industrycat10_2 = 3 if inrange(__div2,10,33)
replace industrycat10_2 = 4 if inrange(__div2,35,39)
replace industrycat10_2 = 5 if inrange(__div2,41,43)
replace industrycat10_2 = 6 if inrange(__div2,45,47)
replace industrycat10_2 = 7 if inrange(__div2,49,53)
replace industrycat10_2 = 6 if inrange(__div2,55,56)
replace industrycat10_2 = 7 if inrange(__div2,58,63)
replace industrycat10_2 = 8 if inrange(__div2,64,82)
replace industrycat10_2 = 9 if inrange(__div2,84,84)
replace industrycat10_2 = 10 if inrange(__div2,85,99)
drop __div2
label var industrycat10_2 "1 digit industry secondary"
label define lblindustrycat10_2 1 "Agriculture" 2 "Mining" 3 "Manufacturing" 4 "Public utilities" 5 "Construction" 6 "Commerce" 7 "Transport and Communications" 8 "Financial and Business Services" 9 "Public Administration" 10 "Other Services, Unspecified"
...
*<_occup_orig_2_>
gen long occup_orig_2 = Cod_ocupacion_sec
replace occup_orig_2 = . if !(Empleo_secundario==1) | Cod_ocupacion_sec==9800
label var occup_orig_2 "Occupation secondary country original"
*</_occup_orig_2_>


*<_occup_isco_2_>
gen str4 occup_isco_2 = ""
replace occup_isco_2 = string(Cod_ocupacion_sec,"%04.0f") if Empleo_secundario==1 & Cod_ocupacion_sec!=9800 & !missing(Cod_ocupacion_sec)
label var occup_isco_2 "ISCO secondary classification"
*</_occup_isco_2_>


*<_occup_2_>
gen byte occup_2 = .
replace occup_2 = real(substr(occup_isco_2,1,1)) if Empleo_secundario==1 & occup_isco_2!="" & substr(occup_isco_2,1,1)!="0"
replace occup_2 = 10 if Empleo_secundario==1 & substr(occup_isco_2,1,1)=="0"
label var occup_2 "1 digit occupation secondary"
label define lbloccup_2 1 "Managers" 2 "Professionals" 3 "Technicians" 4 "Clerks" 5 "Service and market sales workers" 6 "Skilled agricultural" 7 "Craft workers" 8 "Machine operators" 9 "Elementary occupations" 10 "Armed forces" 99 "Others"
...
label var unitwage_2 "Secondary wage unit"
label define lblunitwage_2 1 "Daily" 2 "Weekly" 3 "Every two weeks" 4 "Bimonthly" 5 "Monthly" 6 "Trimester" 7 "Biannual" 8 "Annually" 9 "Hourly" 10 "Other"
...
replace whours_2 = . if Horas_efectivas_secs==999 | whours_2>168
label var whours_2 "Secondary weekly hours"
*</_whours_2_>
...
*<_wage_total_2_>
gen double wage_total_2 = Ingreso_bruto_secundario*12 if Empleo_secundario==1 & Ingreso_bruto_secundario<99999999
label var wage_total_2 "Total secondary wage"
*</_wage_total_2_>
...
replace njobs = 2 if lstatus==1 & F1==2
replace njobs = 3 if lstatus==1 & F1==3
