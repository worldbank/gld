...
/* -----------------------------------------------------------------------
<_Program name_> 				ARM_2023_LFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata SE 18 <_Application_>
<_Author(s)_>					Wolrd Bank Job's Group </_Author(s)_>
<_Date created_>				2025-01-28 </_Date created_>
-------------------------------------------------------------------------
...
<_Study ID_>					ARM_2023_LFS_V01_M </_Study ID_>
...
<_Source of dataset_> 			The Armenian 2022 data is downloaded from 
...
<_Sample size (HH)_> 			7,707  </_Sample size (HH)_>
<_Sample size (IND)_> 		    27,007 </_Sample size (IND)_>
...
* Date: 2025-01-28 - Update lstatus to drop absent responders
...
use "`path_in_stata'/ARM_2023_LFS_raw_public.dta", clear

* Rename variables with industry letter codes
...
*<_weight_q_>
	egen weight_q = rowtotal(WeightsCalib_1 WeightsCalib_2 WeightsCalib_3 WeightsCalib_4)
...
16 households (34 observations) in the raw data set do not have household heads. 
Their household IDs are:

001012 002415 002442 003009 003083 003159 003180 003449 005014
005206 006088 006619 009012 010471 012183 012617

During the harmonization, we assign a household head unique by choosing:
- the oldest male (if unique in the HHs) or
- the oldest male (if no male and unique in the HHs).

*<_relationharm_note_>*/

	* Denote HHs with no head
	bys ID_HH : egen min_rel = min(B4)

	* Within those, denote 1 for oldest male
	bys ID_HH : egen max_age = max(age)
	gen oldest_male = (min_rel != 1) & B3 == 1 & age == max_age

	* Now find oldest female
	gen oldest_fem = (min_rel != 1) & B3 == 2 & age == max_age

	* Count instances
	bys ID_HH : egen single_om = total(oldest_male)
	bys ID_HH : egen single_fm = total(oldest_fem)

	* Replace if we have one oldest male if just one
	replace B4 = 1 if min_rel != 1 & oldest_male == 1 & single_om == 1

	* Update min_rel
	drop min_rel 
	bys ID_HH : egen min_rel = min(B4)

	* Now replace with oldest female
	replace B4 = 1 if min_rel != 1 & oldest_fem == 1 & single_fm == 1

	gen byte relationharm = B4
	recode relationharm (6 = 3) (7 = 4) (8 9 10=5) (11 12 = 6)
...
*<_educy_>
	gen byte educy=.
	replace educy = 0 if  B7 == 1 | B7 == 2
	replace educy = 4 if  B7 == 3
	replace educy = 9 if  B7 == 4
	replace educy = 12 if B7 == 5
	replace educy = 11 if B7 == 6
	replace educy = 15 if B7 == 7
	replace educy = 16 if B7 == 8
	replace educy = 18 if B7 == 9 | B7 == 10
	replace educy = 20 if B7 == 11
...
/*<_lstatus_note>
	...
	We do have 329 people who have lstatus info but nothing else. These are people
	who are absent (for the most part) and someone else is responding for them.
	
	We will drop those who are stated as absent for more than 3 months (C10 = 4)
	or if absent for less than 3 months (C10 = 3) but intend to stay in "that
	settlement" for more than 3 months.
	
	Admittedly, it is unclear (neither in Armenian when translated) whether this
	is the settlement where the person is answering or the one the are in (since
	absent from this). Answers align with C11 refering to wher the absent is.

*<_lstatus_note>*/

	gen byte lstatus = .
	replace lstatus = 1 if D1==1 | inrange(D3,1,5) | D4==1 | D5==1
	replace lstatus = 2 if lstatus == . & inrange(J9,1,3) & J16 == 1
...
*<_occup_>
	 gen occup = E2_9group
	 replace occup = . if lstatus !=1 | age < minlaborage
...
27.7% of total non-missing wage values were imputed using this method.
*<_wage_no_compen_note_>*/
    ...
    * to missing if value is 0.
     ...
     * Create salary categories based on winsor values (note, values updated in questionnaire, seemingly not in labels)
     gen salary_cat=.
...
*<_unitwage_>
	* We denote the time the wage_no_compen info relates to, not E16.
	gen byte unitwage = 5
...
*<_firmsize_u_>
	...
	recode firmsize_u (1=5) (2=9) (3=19) (4=49) (5=99) (6 7 =.)
...
*<_underemployment_year_>
	...
	replace underemployment_year = . if age < minlaborage
