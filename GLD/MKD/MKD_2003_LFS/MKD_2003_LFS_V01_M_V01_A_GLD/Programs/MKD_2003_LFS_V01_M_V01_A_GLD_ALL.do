...
<_Program name_>				MKD_2003_LFS_V01_M_V01_A_GLD_ALL.do </_Program name_>
...
<_Sample size (HH)_> 			5770 </_Sample size (HH)_>
<_Sample size (IND)_> 			19638 </_Sample size (IND)_>
...
use "`path_in_stata'/wb_lfs2003.dta.dta", clear
...
*<_pid_>
	destring persono, replace
	gen personstr= string(persono, "%02.0f")
...
*<_relationharm_>
	destring nositel1, replace
	gen byte relationharm=nositel1
	recode relationharm ///
		(1 = 1)  /// Head of household
		(2 = 2)  /// Spouse/partner
		(3 = 3)  /// Children (Son/Daughter)
		(5 6 = 4)  /// Parents (Father/Mother; Father-in-law/Mother-in-law)
		(4 7 8 = 5)  /// Other relatives (Son-in-law/Daughter-in-law; Brother/Sister; Other relative)
		(9 = 6)  /// Not related
...
*<_school_>
	gen byte school= inrange(q79, 2, 7)
...
*<_educat7_>
* There is the raw variable educlvl but we do not know what the equivalent categories are as they are only numbers!
* There is variable q14 but not sure how to match (q14 = 1, which is "low", or 2 "unqualified" with educat7 categories. We can match perfectly for secondary, hgiher and university but not lower)
	gen byte educat7= .
...
*<_unempldur_l_>
	* Cannot be determined because we do not have date of interview, but only month and year since indiv. started looking
...
*<_occup_isco_>
	...
	* There is a code = 0900 which is not in ISCO; set this to missing
	replace occup_isco = "" if occup_isco == "0900" | occup_isco == "00"
...
*<_contract_>
* This is not available. asked if written or oral agreement but not the same
	gen byte contract=.
...
*<_firmsize_l_>
	gen byte firmsize_l = .
	replace firmsize_l = q19a if q19==1 & inrange(q19a,1,10)    // 1–10 exact
...
*<_firmsize_u_>
	gen byte firmsize_u = .
	replace firmsize_u = q19a if q19==1 & inrange(q19a,1,10)    // 1–10 exact
