...
<_Program name_>				MKD_2019_LFS_V01_M_V01_A_GLD_ALL.do </_Program name_>
...
<_Sample size (HH)_> 			16124 </_Sample size (HH)_>
<_Sample size (IND)_> 			49979 </_Sample size (IND)_>
...
use "`path_in_stata'/raw_lfs_mkd_dlw_2019.dta", clear

* Convert all numeric-looking strings to numeric (keep others as-is)
ds, has(type string)
local s `r(varlist)'

foreach v of local s {
    quietly replace `v' = strtrim(`v')
    quietly replace `v' = subinstr(`v', char(160), "", .)    // remove non-breaking space
    quietly replace `v' = "" if inlist(upper(`v'),"NA","N/A",".","-","NULL")  // common missings
    capture destring `v', replace ignore(" ,.") dpcomma      // handle 1 234,56 and 1,234.56
}
...
*<_marital_>
	...
	recode marital (1 = 2) (2 = 1) (3 = 5) (5 = 3)
...
*<_lstatus_>
	* Choose ulogars variable because this is consistent with the definition and this is used in the official reports
	...
	recode lstatus (0 = .)
	/*
	gen lstatus = 1 if Q1 == 1 | Q1A == 1 | Q2 == 1 | Q3 == 1 | inrange(Q4, 1, 7)| Q5 == 1

	* Restrictions on employed: absent more than 3 months and only getting paid less than half
	replace lstatus = . if Q5 == 1 & inrange(Q7, 3, 5) & Q8 != 1

	* Restrictions on employed: Not certain will be recalled to work
	replace lstatus = . if Q9 == 2
	
	tab lstatus ulogars
	* There is one individual tagged by ulogars as employed but missing data on hours worked/temporary work 
	* we will not code this as employed bec there is no basis!
	...
	* Unemployed: looked for work in the past month OR found a job already but not yet started and available in the next two weeks
	replace lstatus = 2 if inlist(q60, 1, 3) & q69 == 1
	
	* There are people who are looking and available to work -- no clear basis for this!
	tab lstatus ulogars
...
*<_industrycat_isic_>
	gen industrycat_isic = substr(industry_orig, 1, 2) + "00" if !missing(SDejnos1)
