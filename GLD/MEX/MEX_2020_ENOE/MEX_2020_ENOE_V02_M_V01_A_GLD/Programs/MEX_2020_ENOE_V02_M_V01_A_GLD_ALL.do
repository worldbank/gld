...
* Rename variables for append all quarters
* harmonized output in a single file

* Due to COVID-19, 2020-Q2 survey was named ETOE (Encuesta Telefónica de Ocupación y Empleo). It has the same questions as ENOE survey.
* Q2 data is only available at the monthly level and is stored in .dbf files.

/*
/*

	local f: dir "`path_in_other'" files "*.dbf"
	foreach file of local f {
		import dbase using "`path_in_other'/`file'", clear
		
		// put varnames to lowecase
		foreach var of varlist * {
			rename `var' `=strlower("`var'")'
		} 
		
		local f_name = substr("`file'",1,length("`file'")-4)
		display "`f_name'"
	...
	use `hogn0620', clear
	destring est_d, replace
	save `hogn0620', replace
	
	use `sdemn0620', clear
	destring est_d, replace
	save `sdemn0620', replace
	
	use `vivn0620', clear
	destring est_d, replace
	save `vivn0620', replace
	
	*** COE1 ***
	use `coe1t0420', clear
	append using  `coe1t0520'
	append using  `coe1n0620'
	destring r_def, replace
	replace tipo = "2" if missing(tipo) //phone interview in apr and may; june has both: phone and face to face
	save "`path_in_stata'\ETOE_COE1T220.dta", replace 
	
	*** COE2 ***
	use `coe2t0420', clear
	append using  `coe2t0520'
	append using  `coe2n0620'
	replace tipo = "2" if missing(tipo) //phone interview in apr and may; june has both: phone and face to face
	save "`path_in_stata'\ETOE_COE2T220.dta", replace 
	
	*** HOG ***
	use `hogt0420', clear
	append using  `hogt0520' 
	append using  `hogn0620'
	destring r_def, replace
	replace tipo = "2" if missing(tipo) //phone interview in apr and may; june has both: phone and face to face
	save "`path_in_stata'\ETOE_HOGT220.dta", replace 
	
	*** SDEM ***
	use `sdemt0420', clear
	append using  `sdemt0520'
	append using  `sdemn0620'
	destring r_def, replace
	replace tipo = "2" if missing(tipo) //phone interview in apr and may; june has both: phone and face to face
	save "`path_in_stata'\ETOE_SDEMT220.dta", replace 
	
	*** VIV ***
	use `vivt0420', clear
	append using  `vivt0520'
	append using  `vivn0620'
	replace tipo = "2" if missing(tipo) //phone interview in apr and may; june has both: phone and face to face
	save "`path_in_stata'\ETOE_VIVT220.dta", replace
*/
	
*Append all  quarters

	
	tempfile mex_ind_master
	forvalues q = 1(1)4 {
		
		*** COE1 ***
		display "Merge COE1 - T`q':"
		
		capture confirm file "`path_in_stata'\ENOE_COE1T`q'20.dta"
		if _rc==0 {
			use "`path_in_stata'\ENOE_COE1T`q'20.dta", clear
		}
		else {
			capture confirm file "`path_in_stata'\ETOE_COE1T`q'20.dta"
			if  _rc==0 {
					use "`path_in_stata'\ETOE_COE1T`q'20.dta", clear
			}
			
			else {
				use "`path_in_stata'\ENOEN_COE1T`q'20.dta", clear
			}	
		}
			
	* Due COVID-19: From 2020-Q2 INEGI applied phone interviews every month to a subsample of ENOE 2020-Q1. 
	* Hence there are duplicates on individulas within quarters. 
	* Is necesary to create a longer hhid and pid.
	* In Q2 they only did phone interviews
	* In Q3 and Q4 they did both phone and face to face interviews
		...
		if `q' == 1 {
			...
			* Add d_sem: Weekly distribution
			* vars in string (q2), convert them to int	
			foreach	v of varlist d_sem ent con v_sel n_hog h_mud n_ren tipo {
				destring `v', replace 
			}
			
			*to string but with specific format
			tostring (ent v_sel n_ren), gen(ent_str v_sel_str n_ren_str) format("%02.0f")
			tostring con, gen(con_str) format("%05.0f")
			tostring (n_hog h_mud tipo), gen(n_hog_str h_mud_str tipo_str) format("%01.0f")
			tostring d_sem , gen(d_sem_str) format("%03.0f")
			
			egen person = concat(ent_str con_str v_sel_str d_sem_str tipo_str n_hog_str h_mud_str n_ren_str)
		}
		else {
			* Add mes_cal: Calendar month
			* Add tipo: Interview type
			tostring (ent v_sel n_ren mes_cal), gen(ent_str v_sel_str n_ren_str mes_cal_str) format("%02.0f")
			tostring con, gen(con_str) format("%05.0f")
			tostring (n_hog h_mud tipo), gen(n_hog_str h_mud_str tipo_str) format("%01.0f")

			egen person = concat(ent_str con_str v_sel_str mes_cal_str tipo_str n_hog_str h_mud_str n_ren_str)
		}
	...
	*** COE2 ***
		display "Merge COE2 - T`q':"
		
		capture confirm file "`path_in_stata'\ENOE_COE2T`q'20.dta"
		if _rc==0 {
			use "`path_in_stata'\ENOE_COE2T`q'20.dta", clear
		}
		else {
			capture confirm file "`path_in_stata'\ETOE_COE2T`q'20.dta"
			if  _rc==0 {
					use "`path_in_stata'\ETOE_COE2T`q'20.dta", clear
			}
			
			else {
				use "`path_in_stata'\ENOEN_COE2T`q'20.dta", clear
			}	
		}		
	...
	*** SDEMT ***
		display "Merge SDEMT - T`q':"

		capture confirm file "`path_in_stata'\ENOE_SDEMT`q'20.dta"
		if _rc==0 {
			use "`path_in_stata'\ENOE_SDEMT`q'20.dta", clear
		}
		else {
			capture confirm file "`path_in_stata'\ETOE_SDEMT`q'20.dta"
			if  _rc==0 {
					use "`path_in_stata'\ETOE_SDEMT`q'20.dta", clear
			}
			
			else {
				use "`path_in_stata'\ENOEN_SDEMT`q'20.dta", clear
			}	
		}
		...
		* Create HH level for merging with that information
		if `q' == 1 {
			...
			egen casa_long = concat(ent_str con_str v_sel_str d_sem_str tipo_str n_hog_str h_mud_str)
			egen casa_short = concat(ent_str con_str v_sel_str d_sem_str tipo_str)
		}
		else {
			egen casa_long = concat(ent_str con_str v_sel_str mes_cal_str tipo_str n_hog_str h_mud_str)
			egen casa_short = concat(ent_str con_str v_sel_str mes_cal_str tipo_str)
	...
	*** VIV ***
		display "Merge VIV - T`q':"

		capture confirm file "`path_in_stata'\ENOE_VIVT`q'20.dta"
		if _rc==0 {
			use "`path_in_stata'\ENOE_VIVT`q'20.dta", clear
		}
		else {
			capture confirm file "`path_in_stata'\ETOE_VIVT`q'20.dta"
			if  _rc==0 {
					use "`path_in_stata'\ETOE_VIVT`q'20.dta", clear
			}
			
			else {
				use "`path_in_stata'\ENOEN_VIVT`q'20.dta", clear
			}	
		}
		...
		if `q' == 1 {
			...
			* Add d_sem: Weekly distribution
			* vars in string (q2), convert them to int	
			foreach	v of varlist d_sem ent con v_sel tipo {
				destring `v', replace 
			}
			...
			*to string but with specific format
			tostring (ent v_sel), gen(ent_str v_sel_str) format("%02.0f")
			tostring con, gen(con_str) format("%05.0f")
			tostring tipo, gen(tipo_str) format("%01.0f")
			tostring d_sem, gen(d_sem_str) format("%03.0f")
			...
			* Add mes_cal: Calendar month
			* Add tipo: Interview type
			tostring (ent v_sel mes_cal), gen(ent_str v_sel_str mes_cal_str) format("%02.0f")
			tostring con, gen(con_str) format("%05.0f")
			tostring (tipo), gen(tipo_str) format("%01.0f")
	...
	*** HOG ***
		display "Merge HOG - T`q':"
		
		capture confirm file "`path_in_stata'\ENOE_HOGT`q'20.dta"
		if _rc==0 {
			use "`path_in_stata'\ENOE_HOGT`q'20.dta", clear
		}
		else {
			capture confirm file "`path_in_stata'\ETOE_HOGT`q'20.dta"
			if  _rc==0 {
					use "`path_in_stata'\ETOE_HOGT`q'20.dta", clear
			}
			
			else {
				use "`path_in_stata'\ENOEN_HOGT`q'20.dta", clear
			}	
		}
		...
		if `q' == 1 {
			...
			* Add d_sem: Weekly distribution
			* vars in string (q2), convert them to int	
			foreach	v of varlist d_sem ent con v_sel n_hog h_mud tipo {
				destring `v', replace 
			}
	...
	***first job
		...
		*There are some vars in string (q2), convert them to int	
		destring p4a, replace
	...
	*** Rename vars for append (extended questionnarie: q1) ***
		if `q' > 1  {
			local old_varname = "p5f1 p5f10 p5f11 p5f12 p5f13 p5f14 p5f15 p5f2 p5f3 p5f4 p5f5 p5f6 p5f7 p5f8 p5f9 p5f99 p3l p3i p5 p5a p5b_thrs p5d_thrs"
			local new_varname = "p5g1 p5g10 p5g11 p5g12 p5g13 p5g14 p5g15 p5g2 p5g3 p5g4 p5g5 p5g6 p5g7 p5g8 p5g9 p5g99 p3q p3j p5a p5b p5c_thrs p5e_thrs"
			*rename
			rename (`old_varname') (`new_varname')
			
			capture confirm variable fac_np
			if _rc == 0 {
				rename fac_np fac
			}
			else {
		...
		if `q' > 2 {
			rename (est_d_tri t_loc_tri)(est_d t_loc)
		}
		
		
		*keep selected variables (used to create GLD vars)
		
		*extended Q
		if `q' == 1 {
			keep ent v_sel n_ren con n_hog h_mud r_def d_mes p4a p7c p3 p7a fac upm est_d t_loc cd_a loc par_c eda sex e_con cs_p17 cs_p12 cs_p13_1 cs_p13_2 cs_p15 cs_ad_mot scian* isic* p1 p1a1 p1a2 p1b p1c p1d p1e p2_1 p2_2 p2_3 p2_4 p2e p3g1_1 p3g2_1 p3g3_1 p3g4_1 p3h p3d p4c p4d2 p6b2 p5c_thrs p5e_thrs p5a p4 p5g1 p5g2 p5g3 p5g4 p5g5 p5g6 p5g7 p5g8 p5g9 p5g10 p5g11 p5g12 p5g13 p5g14 p5g15 p5g99 p3j p3m5 p3m4 p3i p3q p3g_tot p7 p7e t_tra p2b p8a n_ent *_str sinco* isco*
		}
		else {
			keep ent v_sel n_ren con n_hog h_mud r_def d_mes p4a p7c p3 p7a fac upm est_d t_loc cd_a loc par_c eda sex e_con cs_p17 cs_p12 cs_p13_1 cs_p13_2 cs_p15 cs_ad_mot scian* isic* p1 p1a1 p1a2 p1b p1c p1d p1e p2_1 p2_2 p2_3 p2_4 p2e p3g1_1 p3g2_1 p3g3_1 p3g4_1 p3h p3d p4c p4d2 p6b2 p5c_thrs p5e_thrs p5a p4 p5g1 p5g2 p5g3 p5g4 p5g5 p5g6 p5g7 p5g8 p5g9 p5g10 p5g11 p5g12 p5g13 p5g14 p5g15 p5g99 p3j p3q p3g_tot p7 t_tra p2b p8a n_ent *_str sinco* isco*
		}
		...
		* Create quarter variable
		...
		if `q' == 2 {
			qui destring cd_a cs_ad_mot cs_p12 cs_p13_1 cs_p13_2 cs_p15 cs_p17 d_mes e_con eda loc n_ent p1 p1a1 p1a2 p1b p1c p1d p1e p2_1 p2_2 p2_3 p2_4 p2b p2e p3d p3g1_1 p3g2_1 p3g3_1 p3g4_1 p3g_tot p3h p3j p3q p4 p4c p4d2 p5a p5c_thrs p5e_thrs p5g1 p5g10 p5g11 p5g12 p5g13 p5g14 p5g15 p5g2 p5g3 p5g4 p5g5 p5g6 p5g7 p5g8 p5g9 p5g99 p6b2 p7 p8a par_c sex t_loc upm, replace
		}
		...
		* Append the data to a master dataset
		if `q' == 1 {
			...
			* For subsequent files, append them to the master dataset
			...
			quie append using `mex_ind_master'
			save `mex_ind_master', replace
		}

		display "q`q' appended"
	}
