...
<_Sample size (HH)_> 			[109335] </_Sample size (HH)_> 
<_Sample size (IND)_> 			[390018] </_Sample size (IND)_> 
...
* harmonized output in a single file	
	use "`path_in'\VIVT119.dta",clear
	drop p1-p3
	destring loc mun est ageb t_loc cd_a upm d_sem n_pro_viv ent con v_sel n_ent per, replace
	merge 1:m ent con v_sel using "`path_in'\HOGT119.dta", nogen
	merge 1:m ent con v_sel n_hog using "`path_in'\SDEMT119.dta"
	drop if _merge==1
	drop _merge
	merge 1:1 ent con v_sel n_hog n_ren using "`path_in'\COE1T119.dta", nogen
	merge 1:1 ent con v_sel n_hog n_ren using "`path_in'\COE2T119.dta", nogen
...
***first job
	...
	merge m:1 scian using "Z:\GLD-Harmonization\582018_AQ\MEX\MEX_2019_LFS\MEX_2019_LFS_v01_M\Data\Stata\SCIAN_18_ISIC_4.dta", keep(master match) nogen
...
***then first job
	tostring p3, gen(sinco) force
	merge m:1 sinco using "Z:\GLD-Harmonization\582018_AQ\MEX\MEX_2019_LFS\MEX_2019_LFS_v01_M\Data\Stata\SINCO_11_ISCO_08.dta", keep(master match) nogen
	rename sinco sinco_1
	
***then second job
	tostring p7a, gen(sinco) force
	merge m:1 sinco using "Z:\GLD-Harmonization\582018_AQ\MEX\MEX_2019_LFS\MEX_2019_LFS_v01_M\Data\Stata\SINCO_11_ISCO_08.dta", keep(master match) nogen
...
save "`path_output'\MEX_2019_ENOE_V01_M_V01_A_GLD.dta", replace
