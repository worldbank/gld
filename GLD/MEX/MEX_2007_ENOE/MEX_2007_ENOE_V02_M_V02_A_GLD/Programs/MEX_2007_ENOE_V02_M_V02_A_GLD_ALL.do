...
<_Program name_>				[MEX_2007_ENOE_V02_M_V02_A_GLD_ALL.do] </_Program name_>
<_Application_>					[STATA-18] <_Application_>
<_Author(s)_>					[The World Bank Jobs Group] </_Author(s)_>
<_Date created_>				2012-12-13 </_Date created_>
...
<_Sample size (HH)_> 			[127,566] </_Sample size (HH)_>
<_Sample size (IND)_> 			[561,652] </_Sample size (IND)_>
...
<_OCCUP National_>				[CMO ] </_OCCUP National_>
<_ISIC Version_>				[Rev.4] </_ISIC Version_>
<_INDUS National_>				[SCIAN 2004] </_INDUS National_>
...
* Date: [2024-12-13] - [Update on arrangement process, isc, isic, njobs, hsize, ...]
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe1t`quarter'07.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe2t`quarter'07.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/sdemt`quarter'07.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/hogt`quarter'07.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/vivt`quarter'07.dta"
...
* Merge in the CMO to ISCO codes
...
* Save the file
save "`path_in_stata'/ENOE_2007.dta", replace
*/	

use "`path_in_stata'/ENOE_2007.dta", clear
...
*<_hsize_>
	bysort hhid quarter: gen hsize = _N if inrange(sdem_par, 101, 305) | inrange(sdem_par_c,501,623)
	* Domestic workers (codes 400), Guests (700s) and non-defined (999) are not counted
...
*<_relationharm_>
	* Code different in Q1,2 than Q3,4 for things after codes 305
	...
	replace relationharm = 4 if inrange(sdem_par_c, 601, 601)
	replace relationharm = 5 if inrange(sdem_par_c, 602, 623)
	replace relationharm = 6 if (inrange(sdem_par_c, 401, 461) | inrange(sdem_par_c, 701, 999) | inrange(sdem_par_c, 501, 503))
...
*<_whours_>
	...
	* Generate unified vars - values differ if extended or basic survey
	...
	replace hlp_whours_usual_bin = coe1_p5c if inlist(quarter, 1, 3, 4)
	replace hlp_whours_usual_bin = coe1_p5d if quarter == 2

	gen hlp_whours_lw = .
	replace hlp_whours_lw = coe1_p5b_thrs if inlist(quarter, 1, 3, 4)
	replace hlp_whours_lw = coe1_p5c_thrs if quarter == 2

	gen hlp_whours_nw = .
	replace hlp_whours_nw = coe1_p5d_thrs if inlist(quarter, 1, 3, 4)
	replace hlp_whours_nw = coe1_p5e_thrs if quarter == 2
...
foreach num of numlist 1/15 99 {
	...
	replace hlp_wmonths_`num' = 1 if !mi(coe1_p5f`num') & inlist(quarter, 1, 3, 4)
	replace hlp_wmonths_`num' = 1 if !mi(coe1_p5g`num') & quarter == 2
...
*<_contract_>
	* Contract is 3J in extended, 3I in basic questionnaire 
	gen byte contract = .
	replace contract = coe1_p3i if inlist(quarter,1,3,4)
	replace contract = coe1_p3j if quarter == 2
...
*<_union_>
	* Only in extended questionnaire
	gen byte union = coe1_p3i if quarter == 2
	recode union (2 = 0) (9 = .)
...
*<_firmsize_l_>
	...
	* Firmsize for employees is 3Q in extended questionnaire, 3L in basic 
	gen helper_fs = .
	replace helper_fs = coe1_p3l if inlist(quarter,1,3,4)
	replace helper_fs = coe1_p3q if quarter == 2
