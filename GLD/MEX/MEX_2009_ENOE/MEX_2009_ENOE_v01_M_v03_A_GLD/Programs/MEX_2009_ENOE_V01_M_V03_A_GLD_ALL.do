...
<_Program name_>				[MEX_2009_ENOE_V01_M_V03_A_GLD_ALL.do] </_Program name_>
...
local path_output "Z:\GLD-Harmonization\582018_AQ\MEX\MEX_2009_ENOE\MEX_2009_ENOE_V01_M_V03_A_GLD\Data\Harmonized"

*----------1.3: Database assembly------------------------------*

* All steps necessary to merge datasets (if several) to have all elements needed to produce
* harmonized output in a single file
		use "`path_in'\COE1T109.dta", clear
	...
	use "`path_in'\COE2T109.dta", clear
	tostring (ent v_sel n_ren), gen(ent_str v_sel_str n_ren_str) format("%02.0f")
	tostring con, gen(con_str) format("%05.0f")
	tostring (n_hog h_mud), gen(n_hog_str h_mud_str) format("%01.0f")
	...
	* Overwrtwrite temp file
	...
	use "`path_in'\SDEMT109.dta", clear
	tostring (ent v_sel n_ren), gen(ent_str v_sel_str n_ren_str) format("%02.0f")
	tostring con, gen(con_str) format("%05.0f")
	tostring (n_hog h_mud), gen(n_hog_str h_mud_str) format("%01.0f")
...
*<_empstat_>
	...
	replace empstat = 1 if p3a == 1 & p3h == 1
	replace empstat = 2 if p3a == 1 & inlist(p3h,2,3)

	replace empstat = 1 if (p3a == 2 & p3b == 2) & p3h == 1  // p3b can only be 2 if p3a is 2, so it is redundant but for clarity
	replace empstat = 2 if (p3a == 2 & p3b == 2) & inlist(p3h,2,3)
...
*<_firmsize_u_>
	...
	label var firmsize_u "Firm size (upper bracket) primary job 7 day recall"*/
     ...
     replace firmsize_u = 501 if p3q==11
...
*<_firmsize_l_2_>
	*gen byte firmsize_l_2 = p7e
     ...
     * Add employer
    /* replace firmsize_l_2 = 1 if p3g_tot == 1
     replace firmsize_l_2 = 2 if inrange(p3g_tot, 2, 5)
     replace firmsize_l_2 = 6 if inrange(p3g_tot, 6, 10)
     replace firmsize_l_2 = 11 if inrange(p3g_tot, 11, 15)
     replace firmsize_l_2 = 16 if inrange(p3g_tot, 16, 20)
     replace firmsize_l_2 = 21 if inrange(p3g_tot, 21, 30)
     replace firmsize_l_2 = 31 if inrange(p3g_tot, 31, 50)
     replace firmsize_l_2 = 51 if inrange(p3g_tot, 51, 100)
     replace firmsize_l_2 = 101 if inrange(p3g_tot, 101, 250)
     replace firmsize_l_2 = 251 if inrange(p3g_tot, 251, 500)
     replace firmsize_l_2 = 501 if inrange(p3g_tot, 501, 999999)*/
...
*<_firmsize_u_2_>
	*gen byte firmsize_u_2 = p7e
	...
	gen byte firmsize_u_2 = .
     ...
     replace firmsize_u_2 = 501 if p7e==11

     * Add self-employed
     replace firmsize_u_2 = 1 if empstat_2 == 4

     * Add employer
    /* replace firmsize_u_2 = 1 if p3g_tot == 1
     replace firmsize_u_2 = 5 if inrange(p3g_tot, 2, 5)
     replace firmsize_u_2 = 10 if inrange(p3g_tot, 6, 10)
     replace firmsize_u_2 = 15 if inrange(p3g_tot, 11, 15)
     replace firmsize_u_2 = 20 if inrange(p3g_tot, 16, 20)
     replace firmsize_u_2 = 30 if inrange(p3g_tot, 21, 30)
     replace firmsize_u_2 = 50 if inrange(p3g_tot, 31, 50)
     replace firmsize_u_2 = 100 if inrange(p3g_tot, 51, 100)
     replace firmsize_u_2 = 250 if inrange(p3g_tot, 101, 250)
     replace firmsize_u_2 = 500 if inrange(p3g_tot, 251, 500)
     replace firmsize_u_2 = 501 if inrange(p3g_tot, 501, 999999)*/
...
save "`path_output'\MEX_2009_ENOE_V01_M_V03_A_GLD_ALL.dta", replace
