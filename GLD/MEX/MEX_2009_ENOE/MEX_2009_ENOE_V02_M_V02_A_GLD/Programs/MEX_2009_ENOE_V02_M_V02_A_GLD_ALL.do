...
<_Program name_>				[MEX_2009_ENOE_V02_M_V02_A_GLD_ALL.do] </_Program name_>
...
<_Sample size (HH)_> 			[125,779] </_Sample size (HH)_>
<_Sample size (IND)_> 			[547,464] </_Sample size (IND)_>
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe1t`quarter'09.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe2t`quarter'09.dta"
...
forvalues quarter = 1/4 {
	...
	* For some reason, Q1 data is string, others numeric for some variables, change:
	if `quarter' == 1 {
		
		use "`path_in_stata'/sdemt`quarter'09.dta"
		cap gen quarter = `quarter'
		destring est_d t_loc, replace
		
	}
	else {
		
		quietly: append using "`path_in_stata'/sdemt`quarter'09.dta"
...
forvalues quarter = 1/4 {
	...
	* For some reason, Q1 data is string, others numeric for some variables, change:
	if `quarter' == 1 {
		...
		use "`path_in_stata'/hogt`quarter'09.dta"
		destring est_d t_loc, replace
		
	}
	else {
		
		quietly: append using "`path_in_stata'/hogt`quarter'09.dta"
...
forvalues quarter = 1/4 {
	...
	* For some reason, Q1 data is string, others numeric for some variables, change:
	if `quarter' == 1 {
		...
		use "`path_in_stata'/vivt`quarter'09.dta"
		destring est_d t_loc, replace
		
	}
	else {
		
		quietly: append using "`path_in_stata'/vivt`quarter'09.dta"
...
* Save the file
save "`path_in_stata'/ENOE_2009.dta", replace
*/	

use "`path_in_stata'\ENOE_2009.dta", clear
