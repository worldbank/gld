...
<_Program name_>				[MEX_2012_ENOE_V02_M_V02_A_GLD_ALL.do] </_Program name_>
<_Application_>					[STATA-18] <_Application_>
<_Author(s)_>					[The World Bank Jobs Group] </_Author(s)_>
<_Date created_>				2012-12-11 </_Date created_>
...
<_Sample size (HH)_> 			[125,785] </_Sample size (HH)_>
<_Sample size (IND)_> 			[536,626] </_Sample size (IND)_>
...
<_OCCUP National_>				[CMO Q1, Q2, SINCO 2011 Q3, Q4] </_OCCUP National_>
<_ISIC Version_>				[Rev.4] </_ISIC Version_>
<_INDUS National_>				[SCIAN 2004 Q1, Q2, SCIAN 2007 Q3, Q4] </_INDUS National_>
-----------------------------------------------------------------------
<_Version Control_>
...
* Date: [2024-12-12] - [Update on arrangement process, isc, isic, njobs, hsize, ...]
...
local exclude "cd_a ent con v_sel n_hog h_mud n_ren loc t_loc t_loc_tri t_loc_men fac fac_tri fac_men mun est est_d ageb ur n_ent r_def upm n_pro_viv per tipo quarter est_d_tri est_d_men"

*** COE1 ***
clear
forvalues quarter = 1/4 {
	
	quietly: append using "`path_in_stata'/coe1t`quarter'12.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe2t`quarter'12.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/sdemt`quarter'12.dta"
	cap gen quarter = `quarter'
	replace quarter = `quarter' if mi(quarter)
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/hogt`quarter'12.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/vivt`quarter'12.dta"
...
merge 1:1 cd_a ent con v_sel n_hog h_mud n_ren per using "`coe1'", assert(match master) nogen
merge 1:1 cd_a ent con v_sel n_hog h_mud n_ren per using "`coe2'", assert(match master) nogen
...
merge m:1 cd_a ent con v_sel per             using "`viv'", assert(match using) keep(match) nogen
merge m:1 cd_a ent con v_sel per n_hog h_mud using "`hog'", assert(match using) keep(match) nogen

count
assert `r(N)' == `num_rows'

* Merge in the SINCO to ISCO codes - first and second job (Q1 and Q2) - CMO USED
tostring coe1_p3, gen(cmo) format("%04.0f")
...
tostring coe2_p7a, gen(cmo) format("%04.0f")
merge m:1 cmo using "`path_in_stata'/CMO_09_ISCO_08.dta", keep(master match) nogen
...
*Merge in SCIAN codes - first and second job - (Q 1&2 - SCIAN 04)
gen scian = coe1_p4a
merge m:1 scian using "`path_in_stata'/SCIAN_04_ISIC_4.dta", keep(master match) nogen
...
*Merge in SCIAN codes - first and second job - (Q 3&4 - SCIAN 07)
gen scian = coe1_p4a
...
/* <_weight_note>
	...
	Weight is fac, but at quarter level, need to annualise.
...
*<_hsize_>
	bysort hhid quarter: gen hsize_q34 = _N if sdem_par_c<400 | inrange(sdem_par_c,600,999)
	replace hsize_q34 = . if inrange(quarter, 3, 4)
	bysort hhid quarter: gen hsize_q12 = _N if inrange(sdem_par, 101, 305) | inrange(sdem_par_c,501,623)
	replace hsize_q12 = . if inrange(quarter, 1, 2)
	gen hsize = hsize_q12
	replace hsize = hsize_q34 if missing(hsize)
	drop hsize_*
	* Domestic workers (codes 600), Guests (700s) and non-defined (999) are not counted
	* Codes different in Q1, Q2
...
*<_relationharm_>
	* Code different in Q1,2 than Q3,4 for things after codes 305
	gen relationharm = .
	...
	* Q3/Q4 
	replace relationharm = 4 if inrange(sdem_par_c, 401, 402) & inrange(quarter, 3, 4)
	replace relationharm = 5 if inrange(sdem_par_c, 403, 424) & inrange(quarter, 3, 4)
	replace relationharm = 6 if inrange(sdem_par_c, 501, 999) & inrange(quarter, 3, 4)

	* Q1/Q2 
	replace relationharm = 4 if inrange(sdem_par_c, 601, 601) & inrange(quarter, 1, 2)
	replace relationharm = 5 if inrange(sdem_par_c, 602, 623) & inrange(quarter, 1, 2)
	replace relationharm = 6 if (inrange(sdem_par_c, 401, 461) | inrange(sdem_par_c, 701, 999) | inrange(sdem_par_c, 501, 503)) & inrange(quarter, 1, 2)
