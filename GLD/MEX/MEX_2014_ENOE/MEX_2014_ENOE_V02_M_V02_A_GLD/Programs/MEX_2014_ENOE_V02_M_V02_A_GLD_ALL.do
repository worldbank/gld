...
<_Program name_>				[MEX_2014_ENOE_V02_M_V02_A_GLD_ALL.do] </_Program name_>
<_Application_>					[STATA-18] <_Application_>
<_Author(s)_>					[The World Bank Jobs Group] </_Author(s)_>
<_Date created_>				2022-12-05 </_Date created_>
...
<_Sample size (HH)_> 			[126,803] </_Sample size (HH)_>
<_Sample size (IND)_> 			[540,117] </_Sample size (IND)_>
...
* Date: [2024-12-02] - [Update based on new series since 2020 (assembly, ISIC/ISCO, ...)]
...
forvalues quarter = 1/4 {
	...
	use "`path_in_stata'/coe1t`quarter'14.dta", clear
	rename *, lower
	tempfile d`quarter'
	save "d`quarter'", replace
	
}

* Append files
clear
append using d1 d2 d3 d4
...
forvalues quarter = 1/4 {
	...
	use "`path_in_stata'/coe2t`quarter'14.dta", clear
	rename *, lower
	tempfile d`quarter'
	save "d`quarter'", replace
...
forvalues quarter = 1/4 {
	...
	use "`path_in_stata'/sdemt`quarter'14.dta", clear
	rename *, lower
	tempfile d`quarter'
	save "d`quarter'", replace
...
forvalues quarter = 1/4 {
	...
	use "`path_in_stata'/hogt`quarter'14.dta", clear
	rename *, lower
	cap tostring t_loc, replace force
	tempfile d`quarter'
	save "d`quarter'", replace
...
forvalues quarter = 1/4 {
	...
	use "`path_in_stata'/vivt`quarter'14.dta", clear
	rename *, lower
	cap tostring t_loc, replace force
	tempfile d`quarter'
	save "d`quarter'", replace
...
* Save the file
save "`path_in_stata'/ENOE_2014.dta", replace
*/	

use "`path_in_stata'\ENOE_2014.dta", clear	
...
*<_wage_no_compen_>
	...
	* categories to estimate that of others. Only one salary zone (border zone
	* introduced Jan 2019) - https://www.banxico.org.mx/SieInternet/consultarDirectorioInternetAction.do?sector=10&accion=consultarCuadro&idCuadro=CL289&locale=es
	
	preserve 
		
		* First store minimum salaries 
		summ sdem_salario if sdem_zona == 1
		local min_sal `r(mean)'
			
		gen     salary_cat = .
		replace salary_cat = 1 if wage_no_compen < `min_sal'
		replace salary_cat = 2 if wage_no_compen == `min_sal'
		replace salary_cat = 3 if wage_no_compen > `min_sal'   & wage_no_compen <= `min_sal'*2
		replace salary_cat = 4 if wage_no_compen > `min_sal'*2 & wage_no_compen <= `min_sal'*3
		replace salary_cat = 5 if wage_no_compen > `min_sal'*3 & wage_no_compen <= `min_sal'*5
		replace salary_cat = 6 if wage_no_compen > `min_sal'*5 & wage_no_compen <= `min_sal'*10
		replace salary_cat = 7 if wage_no_compen > `min_sal'*10 & !mi(wage_no_compen)
	...
	* the minimum salary (coe2_p6c == 2)
	summ sdem_salario if sdem_zona == 1
	local min_sal `r(mean)'
	replace wage_no_compen = `min_sal' if mi(wage_no_compen) & sdem_zona == 1 & coe2_p6c == 2
