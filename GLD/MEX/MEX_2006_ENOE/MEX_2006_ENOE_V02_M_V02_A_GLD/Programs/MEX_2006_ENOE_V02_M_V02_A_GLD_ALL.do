...
<_Program name_>				[MEX_2006_ENOE_V02_M_V02_A_GLD_ALL.do] </_Program name_>
...
<_Sample size (HH)_> 			[127,790] </_Sample size (HH)_>
<_Sample size (IND)_> 			[567,951] </_Sample size (IND)_>
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe1t`quarter'06.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe2t`quarter'06.dta"
...
* Sepcial case, var cs_p14_c is double in Q3
use "`path_in_stata'/sdemt306.dta"
tostring cs_p14_c, replace
gen quarter = 3

foreach quarter of numlist 1 2 4 {
	
	quietly: append using "`path_in_stata'/sdemt`quarter'06.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/hogt`quarter'06.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/vivt`quarter'06.dta"
...
* Save the file
save "`path_in_stata'/ENOE_2006.dta", replace
*/	

use "`path_in_stata'/ENOE_2006.dta", clear
...
*<_whours_>
	...
	* Generate unified vars - values differ if extended or basic survey
	...
	replace hlp_whours_usual_bin = coe1_p5c if inrange(quarter, 3, 4)
	replace hlp_whours_usual_bin = coe1_p5d if inrange(quarter, 1, 2)

	gen hlp_whours_lw = .
	replace hlp_whours_lw = coe1_p5b_thrs if inrange(quarter, 3, 4)
	replace hlp_whours_lw = coe1_p5c_thrs if inrange(quarter, 1, 2)

	gen hlp_whours_nw = .
	replace hlp_whours_nw = coe1_p5d_thrs if inrange(quarter, 3, 4)
	replace hlp_whours_nw = coe1_p5e_thrs if inrange(quarter, 1, 2)
...
foreach num of numlist 1/15 99 {
	...
	replace hlp_wmonths_`num' = 1 if !mi(coe1_p5f`num') & inrange(quarter, 3, 4)
	replace hlp_wmonths_`num' = 1 if !mi(coe1_p5g`num') & inrange(quarter, 1, 2)
...
*<_contract_>
	* Contract is 3J in extended, 3I in basic questionnaire 
	gen byte contract = .
	replace contract = coe1_p3i if inlist(quarter, 3, 4)
	replace contract = coe1_p3j if inlist(quarter, 1, 2)
...
*<_union_>
	* Only in extended questionnaire
	gen byte union = coe1_p3i if inlist(quarter, 1, 2)
	recode union (2 = 0) (9 = .)
...
*<_firmsize_l_>
	...
	* Firmsize for employees is 3Q in extended questionnaire, 3L in basic 
	gen helper_fs = .
	replace helper_fs = coe1_p3l if inlist(quarter, 3, 4)
	replace helper_fs = coe1_p3q if inlist(quarter, 1, 2)
