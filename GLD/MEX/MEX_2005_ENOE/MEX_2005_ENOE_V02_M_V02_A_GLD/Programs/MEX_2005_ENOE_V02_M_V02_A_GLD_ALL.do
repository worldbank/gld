...
<_Program name_>				[MEX_2005_ENOE_V02_M_V02_A_GLD_ALL.do] </_Program name_>
<_Application_>					[STATA-18] <_Application_>
<_Author(s)_>					[The World Bank Jobs Group] </_Author(s)_>
<_Date created_>				2012-12-15 </_Date created_>
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe1t`quarter'05.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/coe2t`quarter'05.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/sdemt`quarter'05.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/hogt`quarter'05.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/vivt`quarter'05.dta"
...
* Save the file
save "`path_in_stata'/ENOE_2005.dta", replace
*/	

use "`path_in_stata'/ENOE_2005.dta", clear
...
*<_whours_>
	* Var differs if extended or basic questionnaire - 2005 all extended
	* Generate unified vars - values differ if extended or basic survey
	gen hlp_whours_usual_bin = .
	*replace hlp_whours_usual_bin = coe1_p5c if inrange(quarter, 1, 4)
	replace hlp_whours_usual_bin = coe1_p5d /// if quarter == 1

	gen hlp_whours_lw = .
	*replace hlp_whours_lw = coe1_p5b_thrs if inrange(quarter, 1, 4)
	replace hlp_whours_lw = coe1_p5c_thrs /// if quarter == 1

	gen hlp_whours_nw = .
	*replace hlp_whours_nw = coe1_p5d_thrs if inrange(quarter, 1, 4)
...
*<_wmonths_>
* Var changes if extended or basic questionnaire, unify - 2005 all extended
foreach num of numlist 1/15 99 {
	
	gen hlp_wmonths_`num' = .
	*replace hlp_wmonths_`num' = 1 if !mi(coe1_p5f`num') & inrange(quarter, 1, 4)
...
*<_contract_>
	* Contract is 3J in extended, 3I in basic questionnaire - 2005 is all extended
	gen byte contract = .
	replace contract = coe1_p3j
...
*<_union_>
	* Only in extended questionnaire - 2005 is all extended
	gen byte union = coe1_p3i
	recode union (2 = 0) (9 = .)
...
*<_firmsize_l_>
	...
	* Firmsize for employees is 3Q in extended questionnaire, 3L in basic - 2005 is all extended
	gen helper_fs = .
	replace helper_fs = coe1_p3q
