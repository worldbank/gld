...
<_Program name_>				[MEX_2021_ENOEN_V01_M_V01_A_GLD_ALL.do] </_Program name_>
...
<_Data collection to_>			[05/2021] </_Data collection to_>
<_Source of dataset_> 			[Mexico NSO] </_Source of dataset_>
<_Sample size (HH)_> 			[157,660] </_Sample size (HH)_>
<_Sample size (IND)_> 			[609,843] </_Sample size (IND)_>
...
<_OCCUP National_>				[Sinco 2011 Q1, Q2, SINCO 2019 Q3, Q4] </_OCCUP National_>
<_ISIC Version_>				[Rev.4] </_ISIC Version_>
<_INDUS National_>				[SCIAN 2007 Q1, Q2, SCIAN 2018 Q3, Q4] </_INDUS National_>
-----------------------------------------------------------------------
<_Version Control_>
...
/*
* There is a change in questionnaire and classifiers. Please see INEGI website
...
local exclude "cd_a ent con v_sel n_hog h_mud n_ren loc t_loc t_loc_tri t_loc_men fac fac_tri fac_men mun est est_d ageb ur n_ent r_def upm n_pro_viv per tipo ca mes_cal quarter est_d_tri est_d_men"

*** COE1 ***
clear
forvalues quarter = 1/4 {
	
	quietly: append using "`path_in_stata'/ENOEN_COE1T`quarter'21.dta"
	cap gen quarter = `quarter'
	replace quarter = `quarter' if mi(quarter)
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOEN_COE2T`quarter'21.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOEN_SDEMT`quarter'21.dta"
	cap gen quarter = `quarter'
	replace quarter = `quarter' if mi(quarter)
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOEN_HOGT`quarter'21.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOEN_VIVT`quarter'21.dta"
...
merge 1:1 cd_a ent con v_sel tipo mes_cal ca n_hog h_mud n_ren using "`coe1'", assert(match master) nogen
merge 1:1 cd_a ent con v_sel tipo mes_cal ca n_hog h_mud n_ren using "`coe2'", assert(match master) nogen

* Add household level, keeping only the households that match the HHs with individual data
* Should not lose any, thus check the number of rows is the same before and after
count
local num_rows = `r(N)'

merge m:1 cd_a ent con v_sel tipo mes_cal ca             using "`viv'", assert(match using) keep(match) nogen
merge m:1 cd_a ent con v_sel tipo mes_cal ca n_hog h_mud using "`hog'", assert(match using) keep(match) nogen

count
assert `r(N)' == `num_rows'

* Merge in the SINCO to ISCO codes - first and second job (Q1 and Q2) - SINCO 11 USED
tostring coe1_p3, gen(sinco) format("%04.0f")
merge m:1 sinco using "`path_in_stata'/SINCO_11_ISCO_08.dta", keep(master match) nogen
rename isco isco_1_first_half
replace isco_1_first_half = "" if quarter > 2
drop sinco match

tostring coe2_p7a, gen(sinco) format("%04.0f")
merge m:1 sinco using "`path_in_stata'/SINCO_11_ISCO_08.dta", keep(master match) nogen
rename isco isco_2_first_half
replace isco_2_first_half = "" if quarter > 2
drop sinco match

* Merge in the SINCO to ISCO codes - first and second job (Q3 and Q4) - SINCO 19 USED
tostring coe1_p3, gen(sinco) format("%04.0f")
merge m:1 sinco using "`path_in_stata'/SINCO_19_ISCO_08.dta", keep(master match) nogen
rename isco isco_1_second_half
replace isco_1_second_half = "" if quarter < 3
drop sinco match

tostring coe2_p7a, gen(sinco) format("%04.0f")
merge m:1 sinco using "`path_in_stata'/SINCO_19_ISCO_08.dta", keep(master match) nogen
rename isco isco_2_second_half
replace isco_2_second_half = "" if quarter <3 
drop sinco match

* Unite the halves
gen isco_1     = isco_1_first_half  if inrange(quarter,1,2)
replace isco_1 = isco_1_second_half if inrange(quarter,3,4)

gen isco_2     = isco_2_first_half  if inrange(quarter,1,2)
replace isco_2 = isco_2_second_half if inrange(quarter,3,4)

*Merge in SCIAN codes - first and second job - (Q 1&2 - SCIAN 07)
gen scian = coe1_p4a
merge m:1 scian using "`path_in_stata'/SCIAN_07_ISIC_4.dta", keep(master match) nogen
rename isic isic_1_fh
replace isic_1_fh = "" if quarter > 2
drop scian

gen scian = coe2_p7c
merge m:1 scian using "`path_in_stata'/SCIAN_07_ISIC_4.dta", keep(master match) nogen
rename isic isic_2_fh
replace isic_2_fh = "" if quarter > 2
drop scian

*Merge in SCIAN codes - first and second job - (Q 3&4 - SCIAN 18)
gen scian = coe1_p4a
merge m:1 scian using "`path_in_stata'/SCIAN_18_ISIC_4.dta", keep(master match) nogen
rename isic isic_1_sh
replace isic_1_sh = "" if quarter < 3
drop scian

gen scian = coe2_p7c
merge m:1 scian using "`path_in_stata'/SCIAN_18_ISIC_4.dta", keep(master match) nogen
rename isic isic_2_sh
replace isic_2_sh = "" if quarter < 3
drop scian

* Unite the halves
gen isic_1     = isic_1_fh if inrange(quarter,1,2)
replace isic_1 = isic_1_sh if inrange(quarter,3,4)

gen isic_2     = isic_2_fh  if inrange(quarter,1,2)
replace isic_2 = isic_2_sh if inrange(quarter,3,4)

* Save the file
save "`path_in_stata'/ENOEN_2021.dta", replace
*/

* Call the file directly (quicker)
* For the first time, use the above outcommented code
use "`path_in_stata'/ENOEN_2021.dta", clear	
...
/* <_hhid_note>
	...
	For ENOE-N years, we need the additional tipo (standard from now on) and mes_cal as well as
	ca variables. According to CONEVAL in their ITPL calcualtions (*) ca is only necessary until
	2nd quarter 2021. We keep it nonetheless for consistency
		
	(*) - See codes in https://www.coneval.org.mx/Medicion/Paginas/ITLP-IS_pobreza_laboral.aspx

</_hhid_note> */

	tostring (ent v_sel mes_cal n_ren), gen(ent_str v_sel_str mes_cal_str n_ren_str) format("%02.0f")
	tostring con, gen(con_str) format("%05.0f")
	tostring (n_hog h_mud tipo ca), gen(n_hog_str h_mud_str tipo_str ca_str) format("%01.0f")

	egen hhid=concat(ent_str con_str v_sel_str n_hog_str h_mud_str tipo_str)
...
/* <_weight_note>
	...
	Weight is fac_tri: at quarter level, need to annualise.
	Do this by obs numbers in each quarter

</_weight_note> */
	
	gen help_1 = 1
	egen help_2 = total(help_1)
	bys quarter : egen help_3 = total(help_1)
	gen help_4 = help_3/help_2
	gen weight = fac_tri*help_4
	drop help_*
...
*<_strata_>
	* Data has estrato for trimester & month.
	* Monthly has missing values, go for quarter
...
/* <_type_of_interview_note>

	Since ENOE-N, there are two kinds of interviews: face to face and by
	telephone. The latter can occur more than once in a quarter. Need it 
	to differentiate.
	
</_type_of_interview_note> */
	gen type_of_interview = tipo
	label var type_of_interview "Type of interview (in person or telephone)"
	la de lbltype 1 "Face to face" 2 "Telephone"
 	label values type_of_interview lbltype
*</_type_of_interview_>
...
*<_urban_>
	gen byte urban=.
 	replace urban=1 if inrange(t_loc_tri,1,3)
 	replace urban=0 if t_loc_tri==4
...
*<_relationharm_>
	...
	replace relationharm = 5 if inrange(sdem_par_c, 403, 424)
...
*<_migrated_binary_>
	...
	* Migrated if either in different country or state (20a) or a different municipality (20b)
	replace migrated_binary = 1 if inrange(sdem_cs_p20a_1,2,4) | sdem_cs_p20b_1 == 2
	label de lblmigrated_binary 0 "No" 1 "Yes", replace
...
*<_migrated_from_cat_>
	...
	replace migrated_from_cat = 3 if sdem_cs_p20a_1 == 1 & sdem_cs_p20b_1 == 2
	replace migrated_from_cat = 4 if sdem_cs_p20a_1 == 2
	replace migrated_from_cat = 5 if inrange(sdem_cs_p20a_1,3,4)
...
*<_migrated_from_code_>
	* We want to code this as subnatid1, use that info to merge it in 
	gen mig_code_helper = sdem_cs_p20a_c if sdem_cs_p20a_c < 100 & migrated_binary == 1
	preserve
			
			keep subnatid1
			rename subnatid1 long_name
			gen counter = 1
			collapse (rawsum) counter, by(long_name)
			drop counter
			gen mig_code_helper = substr(long_name,1,2)
			destring mig_code_helper, replace
			tempfile mig_helper
			save "`mig_helper'"
			
	restore
	
	merge m:1 mig_code_helper using "`mig_helper'", assert(match master) nogen
	
	gen migrated_from_code = long_name
	drop long_name mig_code_helper
...
*<_migrated_from_country_>
	...
	replace migrated_from_country = "ZAF" if sdem_cs_p20a_c == 154
	replace migrated_from_country = "ARG" if sdem_cs_p20a_c == 204
	replace migrated_from_country = "BLZ" if sdem_cs_p20a_c == 208
	replace migrated_from_country = "BRA" if sdem_cs_p20a_c == 211
	replace migrated_from_country = "CAN" if sdem_cs_p20a_c == 213
	replace migrated_from_country = "COL" if sdem_cs_p20a_c == 214
	replace migrated_from_country = "CRI" if sdem_cs_p20a_c == 215
	replace migrated_from_country = "CUB" if sdem_cs_p20a_c == 216
	replace migrated_from_country = "CHL" if sdem_cs_p20a_c == 217
	replace migrated_from_country = "ECU" if sdem_cs_p20a_c == 219
	replace migrated_from_country = "SLV" if sdem_cs_p20a_c == 220
	replace migrated_from_country = "USA" if sdem_cs_p20a_c == 221
	replace migrated_from_country = "GTM" if sdem_cs_p20a_c == 225
	replace migrated_from_country = "HTI" if sdem_cs_p20a_c == 228
	replace migrated_from_country = "HND" if sdem_cs_p20a_c == 229
	replace migrated_from_country = "NIC" if sdem_cs_p20a_c == 234
	replace migrated_from_country = "PAN" if sdem_cs_p20a_c == 235
	replace migrated_from_country = "PRY" if sdem_cs_p20a_c == 236
	replace migrated_from_country = "PRI" if sdem_cs_p20a_c == 238
	replace migrated_from_country = "DOM" if sdem_cs_p20a_c == 239
	replace migrated_from_country = "URY" if sdem_cs_p20a_c == 247
	replace migrated_from_country = "VEN" if sdem_cs_p20a_c == 250
	replace migrated_from_country = "IND" if sdem_cs_p20a_c == 325
	replace migrated_from_country = "DEU" if sdem_cs_p20a_c == 402
	replace migrated_from_country = "ESP" if sdem_cs_p20a_c == 415
	replace migrated_from_country = "FRA" if sdem_cs_p20a_c == 419
	replace migrated_from_country = "ITA" if sdem_cs_p20a_c == 425
	replace migrated_from_country = "GBR" if sdem_cs_p20a_c == 439
	replace migrated_from_country = "AUS" if sdem_cs_p20a_c == 501
...
*<_migrated_reason_>
	gen migrated_reason = sdem_cs_p20c_1
	recode migrated_reason (3 4 = 1) (5 = 2) (1 2 = 3) (6 7 9 = 4) (8 10 11 = 5)
...
*<_whours_>
	* Var differs if extended or basic questionnaire
...
*<_wmonths_>
* Var changes if extended or basic questionnaire, unify 
...
*<_contract_>
	* Contract is 3J in extended, 3I in basic questionnaire 
	gen byte contract = .
	replace contract = coe1_p3i if inrange(quarter,2,4)
	replace contract = coe1_p3j if quarter == 1
...
*<_union_>
	* Only in extended questionnaire
	gen byte union = coe1_p3i if quarter == 1
	recode union (2 = 0) (9 = .)
...
*<_firmsize_l_>
	...
	* Firmsize for employees is 3Q in extended questionnaire, 3L in basic 
	gen helper_fs = .
	replace helper_fs = coe1_p3l if inrange(quarter,2,4)
	replace helper_fs = coe1_p3q if quarter == 1
	
	gen byte firmsize_l = .
	replace firmsize_l =   1 if helper_fs == 1
	replace firmsize_l =   2 if helper_fs == 2
	replace firmsize_l =   6 if helper_fs == 3
	replace firmsize_l =  11 if helper_fs == 4
	replace firmsize_l =  16 if helper_fs == 5
	replace firmsize_l =  21 if helper_fs == 6
	replace firmsize_l =  31 if helper_fs == 7
	replace firmsize_l =  51 if helper_fs == 8
	replace firmsize_l = 101 if helper_fs == 9
	replace firmsize_l = 251 if helper_fs == 10
	replace firmsize_l = 501 if helper_fs == 11
...
*<_firmsize_u_>
	gen byte firmsize_u = .
	replace firmsize_u =   1 if helper_fs == 1
	replace firmsize_u =   5 if helper_fs == 2
	replace firmsize_u =  10 if helper_fs == 3
	replace firmsize_u =  15 if helper_fs == 4
	replace firmsize_u =  20 if helper_fs == 5
	replace firmsize_u =  30 if helper_fs == 6
	replace firmsize_u =  50 if helper_fs == 7
	replace firmsize_u = 100 if helper_fs == 8
	replace firmsize_u = 250 if helper_fs == 9
	replace firmsize_u = 500 if helper_fs ==10
	replace firmsize_u =   . if helper_fs ==11
...
/* <_firmsize_l_2_note>
	...
	Firm size of second job only available in first quarter (expanded
	questionnaire) and not for employers

</_firmsize_l_2_note> */
	gen byte firmsize_l_2 = .
	replace firmsize_l_2=1 if coe2_p7e==01
	replace firmsize_l_2=2 if coe2_p7e==02
	replace firmsize_l_2=6 if coe2_p7e==03
	replace firmsize_l_2=11 if coe2_p7e==04
	replace firmsize_l_2=16 if coe2_p7e==05
	replace firmsize_l_2=21 if coe2_p7e==06
	replace firmsize_l_2=31 if coe2_p7e==07
	replace firmsize_l_2=51 if coe2_p7e==08
	replace firmsize_l_2 = 101 if coe2_p7e==09
...
*<_firmsize_u_2_>
	...
	replace firmsize_u_2=1 if coe2_p7e==01
	replace firmsize_u_2=5 if coe2_p7e==02
	replace firmsize_u_2=10 if coe2_p7e==03
	replace firmsize_u_2=15 if coe2_p7e==04
	replace firmsize_u_2=20 if coe2_p7e==05
	replace firmsize_u_2=30 if coe2_p7e==06
	replace firmsize_u_2=50 if coe2_p7e==07
	replace firmsize_u_2=100 if coe2_p7e==08
	replace firmsize_u_2 = 250 if coe2_p7e==09
