...
<_Program name_>				[MEX_2023_ENOE_V01_M_V01_A_GLD_ALL.do] </_Program name_>
<_Application_>					[STATA] <_Application_>
<_Author(s)_>					[The World Bank Jobs Group] </_Author(s)_>
<_Date created_>				2024-10-29 </_Date created_>
...
* that we use for merging. We create a local with the ones we exclude 
local exclude "cd_a ent con v_sel n_hog h_mud n_ren loc t_loc t_loc_tri t_loc_men fac fac_tri fac_men mun est est_d ageb ur n_ent r_def upm n_pro_viv per tipo mes_cal quarter est_d_tri est_d_men"

*** COE1 ***
clear
forvalues quarter = 1/4 {
	
	quietly: append using "`path_in_stata'/ENOE_COE1T`quarter'23.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOE_COE2T`quarter'23.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOE_SDEMT`quarter'23.dta"
	cap gen quarter = `quarter'
	replace quarter = `quarter' if mi(quarter)
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOE_HOGT`quarter'23.dta"
...
forvalues quarter = 1/4 {
	...
	quietly: append using "`path_in_stata'/ENOE_VIVT`quarter'23.dta"
...
merge 1:1 cd_a ent con v_sel tipo mes_cal n_hog h_mud n_ren using "`coe1'", assert(match master) nogen
merge 1:1 cd_a ent con v_sel tipo mes_cal n_hog h_mud n_ren using "`coe2'", assert(match master) nogen
...
merge m:1 cd_a ent con v_sel tipo mes_cal             using "`viv'", assert(match using) keep(match) nogen
merge m:1 cd_a ent con v_sel tipo mes_cal n_hog h_mud using "`hog'", assert(match using) keep(match) nogen

count
assert `r(N)' == `num_rows'

* Merge in the SINCO to ISCO codes - SINCO 19 USED
...
/* <_hhid_note>
	...
	tostring (n_hog h_mud tipo), gen(n_hog_str h_mud_str tipo_str) format("%01.0f")
...
*selected main cities from states
	...
	* Unite urban complements
	recode subnatid2 82/86=81
	label de lblsubnatid2 1 "1 - Mexico" 2 "2 - Guadalajara" 3 "3 - Monterrey" 4 "4 - Puebla" 5 "5 - Leon" 7 "6 - San Luis Potosi" 8 "7 - Merida" 9 "8 - Chihuahua" 10 " 9 - Tampico" 12 "10 - Veracruz" 13 "11 - Acapulco" 14 "12 - Aguacalientes" 15 "13 - Morelia" 16 "14 - Toluca" 17 "15 - Saltillo" 18 "16 - Villahermosa" 19 "17 - Tuxtla Gutierrez" 21 "18 - Tijuana" 24 "19 - Culiacan" 25 "20 - Hermosillo" 26 "21 - Durango" 27 "22 - Tepic" 28 "23 - Campeche" 29 "24 - Cuernavaca" 31 "25 - Oaxaca" 32 "26 - Zacatecas " 33 "27 - Colima" 36 "28 - Queretaro" 39 "29 - Tlaxcala" 40 "30 - La Paz " 41 "31 - Cancun" 43 "32 - Pachuca" 42 "33 - Ciudad del Carmen" 44 "34 - Mexicali" 46 "35 - Reynosa" 52 "36 Tapachula" 81 "99 - Complemento Urbano Rural", replace
...
*<_migrated_from_country_>
	...
	replace migrated_from_country = "AGO" if sdem_cs_p20a_c == 101
	replace migrated_from_country = "GIN" if sdem_cs_p20a_c == 123
	replace migrated_from_country = "ARG" if sdem_cs_p20a_c == 204
	replace migrated_from_country = "BLZ" if sdem_cs_p20a_c == 208
	replace migrated_from_country = "BOL" if sdem_cs_p20a_c == 210
	...
	replace migrated_from_country = "PER" if sdem_cs_p20a_c == 237
	replace migrated_from_country = "PRI" if sdem_cs_p20a_c == 238
	replace migrated_from_country = "DOM" if sdem_cs_p20a_c == 239
	replace migrated_from_country = "SUR" if sdem_cs_p20a_c == 244
	replace migrated_from_country = "URY" if sdem_cs_p20a_c == 247
	replace migrated_from_country = "VEN" if sdem_cs_p20a_c == 250
	replace migrated_from_country = "KOR" if sdem_cs_p20a_c == 313
	replace migrated_from_country = "CHN" if sdem_cs_p20a_c == 315
	replace migrated_from_country = "JPN" if sdem_cs_p20a_c == 330
	replace migrated_from_country = "SGP" if sdem_cs_p20a_c == 346
	replace migrated_from_country = "ESP" if sdem_cs_p20a_c == 415
	replace migrated_from_country = "FIN" if sdem_cs_p20a_c == 418
	replace migrated_from_country = "FRA" if sdem_cs_p20a_c == 419
	replace migrated_from_country = "ITA" if sdem_cs_p20a_c == 425
	replace migrated_from_country = "POL" if sdem_cs_p20a_c == 437
	replace migrated_from_country = "GBR" if sdem_cs_p20a_c == 439
	replace migrated_from_country = "CZE" if sdem_cs_p20a_c == 440
	replace migrated_from_country = "RUS" if sdem_cs_p20a_c == 442
	replace migrated_from_country = "CHE" if sdem_cs_p20a_c == 446
