...
*<_weight_q_>
	gen weight_q = poids_1_2017  if wav == 1
	replace weight_q = poids_3_2017 if wav==3
	replace weight_q = poids_t4_2017 if wav==4
	replace weight = . if missing(weight)
...
/* <_subnatid1_note>
	...
	*Q4
	...
	replace subnatid1 = "7 - Dakar"         if inrange(dr_int, 1, 23)   & missing(subnatid1)
	replace subnatid1 = "8 - Ziguinchor"    if inrange(dr_int, 24, 38)  & missing(subnatid1)
	replace subnatid1 = "9 - Diourbel"      if inrange(dr_int, 39, 55)  & missing(subnatid1)
	replace subnatid1 = "10 - Saint-Louis"  if inrange(dr_int, 56, 73)  & missing(subnatid1)
	replace subnatid1 = "11 - Tambacounda"  if inrange(dr_int, 74, 88)  & missing(subnatid1)
	replace subnatid1 = "12 - Kaolack"      if inrange(dr_int, 89, 105) & missing(subnatid1)
	replace subnatid1 = "13 - Thies"        if inrange(dr_int, 106, 125)& missing(subnatid1)
	replace subnatid1 = "14 - Louga"        if inrange(dr_int, 126, 141)& missing(subnatid1)
	replace subnatid1 = "1 - Fatick"        if inrange(dr_int, 142, 156)& missing(subnatid1)
	replace subnatid1 = "2 - Kolda"         if inrange(dr_int, 157, 172)& missing(subnatid1)
	replace subnatid1 = "3 - Matam"         if inrange(dr_int, 173, 187)& missing(subnatid1)
	replace subnatid1 = "4 - Kaffrine"      if inrange(dr_int, 188, 201)& missing(subnatid1)
	replace subnatid1 = "5 - Kedougou"      if inrange(dr_int, 202, 212)& missing(subnatid1)
	replace subnatid1 = "6 - Sedhiou"       if inrange(dr_int, 213, 225)& missing(subnatid1)
...
*<_relationharm_>
	...
	*If household has no head or more than one head:
	...
	unique hhid if number_heads == 0
		Number of unique values of hhid is  122
		Number of records is  357
...
foreach v of local ed_var {
	cap confirm numeric variable `v'
	if _rc == 0 { // is indeed numeric
		replace `v'=. if ( age < educy & !missing(age) )
	}
	else { // is not
		replace `v'= "" if ( age < educy & !missing(age) )
	}
...
*<_wage_no_compen_>
	*exact salary amount no available on the raw data
...
*<_wmonths_>
	gen wmonths = d29a0 if lstatus == 1
