...
<_Program name_>				ALB_2011_LFS_V01_M_V01_A_GLD </_Program name_>
...
<_Study ID_>					ALB_2011_LFS_V01_M </_Study ID_>
<_Data collection from_>			07/2011 </_Data collection from_>
<_Data collection to_>				09/2011 </_Data collection to_>
<_Source of dataset_> 				INSTAT </_Source of dataset_>
<_Sample size (HH)_> 				5037 </_Sample size (HH)_>
<_Sample size (IND)_> 				13598 </_Sample size (IND)_>
...
use "`path_in_stata'/lfs2011_english.dta", clear
...
/* <_hhid_note>
	...
	tostring A5, gen(psu_str) format(%03.0f)
	tostring A6, gen(hh_str) format(%02.0f)
...
/* <_relationharm_note>
	...
	*Exception
	There are some households (4) where there are more than 1 head. Put the oldest person as head an recode teh rest as 5 "Other relatives"
                                                                           
</_relationharm_note> */
	gen heads = 1 if hhlink == 1
	bys hhid : egen total_heads = total(heads)
	*unique hhid if total_heads == 0  //288 hh
	
	*spouse
		gen new_head = 1 if total_heads == 0 & hhlink == 2 //262 hh
		bys hhid : egen hh_new_head = count(new_head)
	*oldest member
		bysort hhid (age): gen oldest = (_n == _N) if total_heads == 0 & hh_new_head == 0 //26 hh
	...
	*Exception
	...
	bys hhid: egen totalhh = total(D_head)
	bysort hhid (age): gen oldest_exception = (_n == _N) if inrange(totalhh,2,99)
...
*<_migrated_years_>
	...
	gen years_bornoutside = 2011 - yearesid if !inlist(yearesid,.,0)
...
/* <_migrated_from_country_note>
	...
	* there is one exception, one person said he was abroad but when asked about the country he answered albania. Code migrated_from_cat == 4
...
/* <_migrated_from_country_note>
	...
	* There are people from albania who were abroad, however the questionnaire does not ask about the country they were in: tab yearesid if !inlist(yearesid,.,0) & Q16 == 1:  2 people. Put them as missing
	* The raw data has no information about the country if the place of birth is abroad, code those cases as "Other World"
                                                                           
</_migrated_from_country_note> */
	gen migrated_from_country = ""
	replace migrated_from_country = "GRC" if (Q20 == 5 | Q16_2 == 5) & migrated_from_cat == 5
	replace migrated_from_country = "ITA" if Q20 == 9 & migrated_from_cat == 5
	replace migrated_from_country = "Other World" if Q16 == 2 & migrated_from_cat == 5
...
/* <_educat7_note>
	...
	After harmonization there are some inconsistencies
		tab Q124 Q128 if educat7 == . // 76 obs
...
/* <_lstatus_note>
	...
	*** employed ***
	...
	2) maternity leave (11 to Q23) OR illness, injury or temporary disability (12 in Q23) OR expect to return to work(1 or 2 Q25) OR education / training directly related to the actual work (1 2 or 3 in Q24)
...
*<_industry_orig_>
	gen industry_orig = string(NACE_REV1)
...
/* <_industrycat_isic_note>
	...
	gen industrycat_isic = string(NACE_REV1 * 100, "%04.0f")
...
*<_industrycat10_>
	gen byte industrycat10 = NACE_REV1
...
/* <_occup_isco_note>
	...
	*Increase hierarchy
	...
	replace occup_isco = "5100" if occup_isco == "5190"
	replace occup_isco = "7300" if occup_isco == "7330"
	replace occup_isco = "6000" if occup_isco == "6420"
	replace occup_isco = "6200" if occup_isco == "6250"
	replace occup_isco = "6100" if occup_isco == "6150"
	replace occup_isco = "5200" if occup_isco == "5180"
	...
	replace occup_isco = "9000" if occup_isco == "9990"
...
*<_t_hours_total_>
	gen t_hours_total = Q84
	replace t_hours_total = . if inrange(t_hours_total,120,999)
...
/* <_empstat_year_note>
	...
	Present in the questionnaire but no in the data
	
</_empstat_year_note> */
	gen byte empstat_year = Q167
