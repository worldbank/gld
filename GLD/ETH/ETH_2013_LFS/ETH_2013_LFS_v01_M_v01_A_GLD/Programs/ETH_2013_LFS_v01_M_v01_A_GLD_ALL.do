...
/* -----------------------------------------------------------------------
<_Program name_>				ETH_2013_LFS_v01_M_v01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata MP 16.1 <_Application_>
<_Author(s)_>					Wolrd Bank Job's Group </_Author(s)_>
<_Date created_>				2022-10-18 </_Date created_>
-------------------------------------------------------------------------
...
<_Study ID_>					ETH_2013_LFS_v01_M </_Study ID_>
<_Data collection from (M/Y)_>	[March/2013] </_Data collection from (M/Y)_>
<_Data collection to (M/Y)_>	[June/2013] </_Data collection to (M/Y)_>
...
<_Sample size (HH)_> 			58,321 </_Sample size (HH)_>
<_Sample size (IND)_> 			240,659 </_Sample size (IND)_>
...
<_Geographic coverage_> 		Both urban and rural parts of the country, 
								except the non-sedentary areas of the six 
								zones of Somali region. </_Geographic coverage_>
...
*<_hhid_>
	foreach var of varlist reg01-ID108{
		tostring `var', gen(str_`var') format(%03.0f)
	}
	egen hhid=concat(str_reg01 str_ID102 str_ID103 str_ID104 str_ID105 str_ID106 str_ID107 str_ID108)
...
*<_psu_>
	*egen psu=group(reg01 ID102 ID103 ID104 ID105 ID106 ID107)	
...
*<_relationharm_>
	...
	recode relationharm (0=1) (1=2) (2/4=3) (5=4) (6 8=5) (7 9=6)
...
The disability model has the following 14 options:
1  Blindness/Both eyes
2  Blindness/one eye
3  Deaf/two ears
4  Deaf/one ear
5  Inability to speak
6  Inaility to listening/speaking
7  Inaility to listening/seeing
...
i.e. without knowing whether the legs or the hands are paralized, we cannot know
...
*<_migrated_binary_>
	gen migrated_binary=cond(LF206==98, 0, 1)
...
*<_migrated_years_>
   gen migrated_years=LF206 if inrange(LF206, 1, 97)
   replace migrated_years=0.5 if LF206==0
...
*<_migrated_reason_>
	...
	recode migrated_reason (1 5/6=3) (3/4 8=1) (7=4) (9/13=5) (99=.)
...
*<_school_>
	gen school=.
	replace school=1 if LF213==1
	replace school=0 if LF213!=1
...
*<_educy_>
	drop educy 
	gen byte educy=LF214 if inrange(LF214,0,12)
	replace educy=12 if LF214==13
	replace educy=13 if LF214==14|LF214==15
	replace educy=14 if LF214==16
	replace educy=15 if LF214==17|LF214==18
	replace educy=16 if LF214==19
	replace educy=19 if LF214==20
	replace educy=9 if LF214==21
	replace educy=10 if LF214==22
	replace educy=11 if inlist(LF214,23,25,28)
	replace educy=12 if inlist(LF214,24,26,27,29)
	replace educy=13 if inlist(LF214,30,31)
	replace educy=14 if LF214==32
	replace educy=15 if LF214==33
	replace educy=16 if LF214==34
	replace educy=19 if LF214==35
	replace educy=. if inrange(LF214,93,99)
...
*<_educat_isced_>
	gen educat_isced=.
	replace educat_isced=020 if LF214==0
	replace educat_isced=100 if inrange(LF214,1,7)
	replace educat_isced=242 if LF214==8
	replace educat_isced=244 if inrange(LF214,9,11)|inrange(LF214,21,23) 
	replace educat_isced=344 if LF214==12|LF214==13|LF214==24
	replace educat_isced=254 if LF214==14|214==25|LF214==28
	replace educat_isced=352 if LF214==27|LF214==29
	replace educat_isced=353 if LF214==30|LF214==17
	replace educat_isced=660 if LF214==19|LF214==34
	replace educat_isced=760 if LF214==20|LF214==35
...
*<_vocational_field_orig_>
	gen vocational_field_orig=LF215
	decode LF215, gen(training_field)
...
*<_lstatus_>
	drop lstatus
	gen byte lstatus=.
	replace lstatus=1 if LF301==1 | LF306<4
	replace lstatus=2 if LF306==4 & LF401==1
	replace lstatus=3 if LF306==4 & LF401==2
...
*<_underemployment_>
	...
	replace underemployment=1 if LF322==1
	replace underemployment=0 if LF322==2
...
*<_nlfreason_>
	...
	recode nlfreason (0=2) (2=1) (7 8=3) (3=4) (1 4/6 9=5)
...
*<_empstat_>
	drop empstat 
	gen byte empstat=LF311
	recode empstat (1/7=1) (8=4) (9 11=2) (10=3) (12=5)
...
*<_ocusec_>
	drop ocusec
	gen byte ocusec=LF311
	recode ocusec (1/2=1) (3/5=2) (6/12=4)
	replace ocusec=. if lstatus!=1
	label var ocusec "Sector of activity primary job 7 day recall"
	la de lblocusec 1 "Public Sector, Central Government, Army" 2 "Private, NGO" 3 "State owned" 4 "Public or State-owned, but cannot distinguish", modify
	label values ocusec lblocusec
*</_ocusec_>


*<_industry_orig_>
	gen industry_orig=LF310
...
*<_industrycat_isic_>
	gen industrycat_isic=LF310
	replace industrycat_isic=0990 if LF310==0999
	replace industrycat_isic=1070 if LF310==1076|LF310==1077
	replace industrycat_isic=1100 if LF310==1105|LF310==1106|LF310==1107
	replace industrycat_isic=9410 if LF310==9413
...
*<_industrycat10_>
	gen long industrycat10=floor(LF310/100)
...
*<_occup_isco_>	
	drop occup 
	gen occup=LF308
	tostring occup, format(%04.0f) replace
	gen occup_isco=substr(occup,1,2)
	destring occup_isco, replace
	replace occup_isco=occup_isco*100
	tostring occup_isco, format(%04.0f) replace
	replace occup_isco="" if occup_isco=="3600"|occup_isco=="3700"|occup_isco=="9900"
...
*<_occup_>
	gen byte occup=floor(LF308/1000)
	replace occup=99 if LF308==9998
...
*<_wage_no_compen_>
	gen double wage_no_compen=LF315
...
*<_unitwage_>
	...
	la de lblunitwage 1 "Daily" 2 "Weekly" 3 "Every two weeks" 4 "Bimonthly"  5 "Monthly" 6 "Trimester" 7 "Biannual" 8 "Annually" 9 "Hourly" 10 "Other", modify
...
*<_contract_>
	...
	la de lblcontract 0 "Without contract" 1 "With contract", modify
...
*<_empstat_year_>
	...
	recode empstat_year (1/7=1) (8=4) (9 11=2) (10=3) (12=5)
...
*<_ocusec_year_>
	...
	recode ocusec_year (1/2=1) (3/5=2) (6/12=4)
	replace ocusec_year=. if lstatus_year!=1
	label var ocusec_year "Sector of activity primary job 12 day recall"
	la de lblocusec_year 1 "Public Sector, Central Government, Army" 2 "Private, NGO" 3 "State owned" 4 "Public or State-owned, but cannot distinguish", modify
...
*<_industrycat10_year_>
	gen byte industrycat10_year=LF505
	recode industrycat10_year (5=4) (6=5) (7 9=6) (8 10=7) (11/14=8) (15=9) (16/21=10)
...
*<_occup_skill_year_>
	gen occup_skill_year=LF504
	recode occup_skill_year (1/3=3) (4/8=2) (9=1) (10=.)
...
*<_njobs_>
	drop njobs
	gen njobs=.
	replace njobs=1 if lstatus==1 & LF303==98
	replace njobs=2 if lstatus==1 & inrange(LF303,0,97)
