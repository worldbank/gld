...
<_Program name_>				MNE_2011_LFS_V01_M_V01_A_GLD_ALL.do </_Program name_>
...
use "`path_in_stata'/ars12kon.dta", clear
...
/* <_hhid_note>
	...
	gen rbrars1_str = string(rbrars1, "%03.0f")
	egen hhid = concat(rbrars1_str opstina popkrug stan)
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>	
	destring p1, generate(p1_str)
	format p1_str %03.0f
	egen  pid = concat(hhid p1_str)
	isid hhid pid
...
*<_subnatid1_>
	gen subnatid1= ""
	la de lblsubnatid1 1 "Central" 2 "South" 3 "North"
	label var subnatid1 "Subnational ID at First Administrative Level"
*</_subnatid1_>


*<_subnatid2_>
	gen subnatid2= opstina + "-" //the name is unknown
...
*<_age_>
	gen age=.
	gen rgodina=real(godina)
	replace age = rgodina - god
	label var age "Individual age"
*</_age_>


*<_male_>
	gen byte male=real(pol)
	recode male (2 = 0) (0 = .)
...
*<_relationharm_>
	gen relationharm = real(odnos)
	replace relationharm=5 if  relationharm==4 | relationharm==5
	replace relationharm=6 if  relationharm==6 | relationharm==0
...
*<_relationcs_>
	gen relationcs = ""
	replace relationcs = "Household reference person" if odnos == "1"
	replace relationcs = "Spouse (or cohabiting partner) of reference person" if odnos == "2"
	replace relationcs = "Household reference person's (or his partner's) children" if odnos == "3"
	replace relationcs = "Household reference person's (or his partner's) parents" if odnos == "4"
	replace relationcs = "Household reference person's other relatives" if odnos == "5"
	replace relationcs = "Other" if odnos == "6"
...
*<_marital_>
	gen marital=. 
	replace marital=1 if brak=="2" 
	replace marital=4 if brak=="4" 
	replace marital=5 if brak=="3" 
	replace marital=2 if brak=="1" 
...
*<_educy_>
	destring skola, replace
	gen byte educy=.
	replace educy = 0  if skola == 1   // No school
	replace educy = 2  if skola == 2   // 1–4 grades elementary school
	replace educy = 6  if skola == 3   // 5–7 grades elementary school
	replace educy = 8  if skola == 4   // Elementary school, finished
	replace educy = 10 if skola == 5   // Secondary vocational 1–3 yrs
	replace educy = 12 if skola == 6   // Secondary vocational 4–5 yrs or high school
	replace educy = 14 if skola == 7   // Junior college (undergraduate)
	replace educy = 16 if skola == 8   // College or Academy (graduate)
	replace educy = 18 if skola == 9   // Postgraduate, master's degree
	replace educy = 21 if skola == 10  // PhD degree
...
*<_educat7_>
	gen byte educat7=.
	replace educat7 = 1 if skola == 1                       // No education
	replace educat7 = 2 if inlist(skola, 2, 3)              // Primary incomplete (1–7 grades)
	replace educat7 = 3 if skola == 4                       // Primary complete (elementary finished)
	replace educat7 = 4 if skola == 5                       // Secondary incomplete (short vocational)
	replace educat7 = 5 if skola == 6                       // Secondary complete (4–5 years / high school)
	replace educat7 = 6 if skola == 7                       // Higher than secondary but not university
	replace educat7 = 7 if inlist(skola, 8, 9, 10)           // University incomplete or complete
...
*<_educat_orig_>
	gen educat_orig = ""
	replace educat_orig = "No school" if skola == 1
	replace educat_orig = "1–4 grades elementary school" if skola == 2
	replace educat_orig = "5–7 grades elementary school" if skola == 3
	replace educat_orig = "Elementary school, finished" if skola == 4
	replace educat_orig = "Secondary vocational school lasting 1–3 years" if skola == 5
	replace educat_orig = "Secondary vocational school lasting 4–5 years or high school" if skola == 6
	replace educat_orig = "Junior college (undergraduate)" if skola == 7
	replace educat_orig = "College or Academy (graduate)" if skola == 8
	replace educat_orig = "Postgraduate, master's degree" if skola == 9
	replace educat_orig = "PhD degree" if skola == 10
...
*<_lstatus_>
	...
	* Employed
	replace lstatus=1 if inlist(a3,1,2,3,4,5) 
	
	* Unemployed:	
		// In the last four weeks they were active in seeking a job and in case they were offered a job they were able to start within the following 2 weeks.
		//	In the last four weeks they were not active in seeking a job because they found a job which they were supposed to start within 3 months upon expiration of the reference week. 
	
	replace lstatus=2 if b8==1 & b12 == 1 & missing(lstatus) 
	replace lstatus=2 if b14 == 2 //future starter
	
	replace lstatus=3 if lstatus == . & age>=minlaborage & !missing(age)
...
*<_potential_lf_>
	gen byte potential_lf = b8==1 | b12 == 1
...
*<_nlfreason_>
	gen byte nlfreason=b14 if lstatus==3
	recode nlfreason (1 3 8 9 10 11 = 5) (5 = 2) (6 = 1) (7 = 3) (0 = . )
...
*<_unempldur_l_>
	gen byte unempldur_l= b9g*12 + b9m + (b9d >= 15) if lstatus == 2
...
{
*<_empstat_>
	gen byte empstat=a3 if lstatus==1
	recode empstat (1 2 = 3) (3 = 4) (4 = 1) (5 = 2)
...
*<_ocusec_>
	gen byte ocusec=real(a7) if lstatus==1
	recode ocusec (3 = 2) (5 = 3) (6 = 4) 
...
*<_industry_orig_>
	gen industry_orig = a6 if lstatus == 1
	label var industry_orig "Original survey industry code, main job 7 day recall"
*</_industry_orig_>


*<_industrycat_isic_>
	*keep just the two first digits
	...
	replace industrycat_isic = "" if lstatus != 1 |industrycat_isic == "0000"
		* Check that no errors --> using our universe check functio , count should be 0 (no obs wrong)
...
*<_occup_orig_>
	gen occup_orig = a5 if lstatus == 1
...
*<_occup_isco_>
	...
	replace occup_isco = "1110" if occup_isco == "1111"
	replace occup_isco = "2200" if occup_isco == "2243"
	replace occup_isco = "5220" if occup_isco == "5223"
	replace occup_isco = "9000" if occup_isco == "9992"
	replace occup_isco = "9000" if occup_isco == "9993"
...
*<_occup_>
	gen byte occup=int(real(a5)/1000) if   lstatus == 1
	recode occup 0=.
	replace occup=10 if a5=="0110"
...
*<_wage_no_compen_>
	gen double wage_no_compen=v92 if lstatus == 1
...
*<_whours_>
	gen whours= real(a15a) if lstatus==1
	replace whours = a15 if inlist(whours,0,.) & lstatus == 1
...
*<_firmsize_l_>
	gen byte firmsize_l=real(a8) if lstatus == 1
	label var firmsize_l "Firm size (lower bracket) primary job 7 day recall"
*</_firmsize_l_>


*<_firmsize_u_>
	gen byte firmsize_u=firmsize_l
...
{
*<_empstat_2_>
	gen byte empstat_2=real(a25)
	recode empstat_2 (1=3) (2=4) (3=1) (4=2) (0=.)
...
*<_industry_orig_2_>
	gen industry_orig_2= a26 if !missing(empstat_2)
	label var industry_orig_2 "Original survey industry code, secondary job 7 day recall"
*</_industry_orig_2_>

*<_industrycat_isic_2_>
	gen industrycat_isic_2 = substr(a26, 1, 2) + "00" if !missing(industry_orig_2)
...
*<_whours_2_>
	gen whours_2 = real(a24s) if !missing(empstat_2)
