...
use "`path_in_stata'/ARS2010.dta", clear
...
/* <_hhid_note>
	...
	gen str2 Nedelja_str = string(Nedelja, "%02.0f")
	egen hhid = concat(Nedelja_str  Opstina  PopKrug  Stan  Dom)
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>
	egen  pid = concat(hhid q1)
	duplicates tag pid, gen(tag)
	tab tag // just 2 obs
	duplicates drop pid, force //drop 1 obs
	isid pid Kvartal
...
*<_weight_>
	gen double weight= . //not in raw data
...
*<_subnatid2_>
	gen byte subnatid2=.
	la de lblsubnatid2 1 "Andrijevica" 2 "Bar" 3 "Berane" 4 "Bijelo Polje" 5 "Budva" 6 "Cetinje" 7 "Danilovgrad" 8"Herceg Novi" 9 "Kolasin" 10"Kotor" 11 "Mojkovac" 12 "Niksic" 13 "Plav" 14 "Pljevlja" 15 "Pluzine" 16 "Podgorica" 17 "Rozaje" 18 "Savnik" 19 "Tivat" 20"Ulcinij" 21 "Zabljak"
...
*<_relationharm_>
	...
	*(4) oldest member if more than one head 
	bys hhid relationharm (age) : gen oldest_head = 1 if (_n == _N) & relationharm == 1
	gen head = 1 if relationharm == 1
	bys hhid : egen number_heads = total(head)
	
	bys hhid: replace relationharm = 5 if relationharm == 1 & inrange(number_heads,2,99) & oldest_head != 1
...
*<_relationcs_>
	...
	replace relationcs = "Household reference person" if q8 == 1
	replace relationcs = "Spouse (or cohabiting partner) of reference person" if q8 == 2
	replace relationcs = "Household reference person's (or his partner's) children" if q8 == 3
	replace relationcs = "Household reference person's (or his partner's) parents" if q8 == 4
	replace relationcs = "Household reference person's other relatives" if q8 == 5
	replace relationcs = "Other" if q8 == 6
...
*<_marital_>
	destring q18, replace
	gen byte marital=q18
	recode marital 1=2 2=1 3=5 4=4 9=2 0=.
...
*<_migrated_binary_>
	gen q29_int = real(q29)
	gen migrated_binary = inlist(q29_int,2,3)
...
*<_migrated_from_cat_>
	...
	replace migrated_from_cat = 6 if q29_int == 2
	replace migrated_from_cat = 6 if q29_int == 3
...
*<_migrated_from_country_>
	...
	replace migrated_from_country = "AUT" if q31 == "AT"
	replace migrated_from_country = "BIH" if q31 == "BA"
	replace migrated_from_country = "CHE" if q31 == "CH"
...
*<_educat7_>
	...
	replace educat7 = 6 if inlist(q19, 8, 9)  
	replace educat7 = 7 if inlist(q19, 10, 11, 12, 13)
...
*<_lstatus_>
	gen byte lstatus=.
	destring a1_1 a1_2 a1_3 a1_4 a1_5 a1_6 a1_8 a1_13 a1_14 b7 b8 b9 b13 b17 m* s*,replace
	...
	* Unemployed defined as looking in the past 4 months and available to work in the next 2 weeks
	...
	replace lstatus=2 if b8 == 1 //future starter
...
*<_potential_lf_>
	gen byte potential_lf = b7==1 | b17 == 1
...
*<_nlfreason_>
	...
	recode nlfreason 5=1 4=2 3=2 2=4 6=3 7/8=5 1=5 9=.
...
*<_empstat_>
	...
	recode empstat 1=3 2=4 3=1 4=2 9=5
...
*<_ocusec_>
	...
	recode ocusec 1=1 2=3 3=1 4/5=2 6=. 9=.
...
*<_industry_orig_>
	gen industry_orig = string(m2, "%05.0f") if lstatus == 1
...
*<_whours_>
	...
	replace whours = m18a if inlist(whours,0,.) & lstatus == 1
...
*<_firmsize_l_>
	...
	recode firmsize_l (12 = 20) (13 = 50) (14 = .) (15 = 11)
...
*<_firmsize_u_>
	...
	recode firmsize_u (11 = 19) (12 = 49) (13 = .) (14 = 11) (15 = .)
...
*<_industrycat_isic_2_>
	gen industrycat_isic_2 = substr(string(s3, "%05.0f"), 1, 2) + "00" if !missing(industry_orig_2)
...
*<_whours_2_>
	gen whours_2 = s4 if !missing(empstat_2)
