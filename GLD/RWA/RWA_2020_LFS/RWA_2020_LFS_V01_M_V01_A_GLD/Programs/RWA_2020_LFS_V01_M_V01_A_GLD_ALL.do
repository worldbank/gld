...
<_Program name_>				RWA_2020_LFS_v01_M_v01_A_GLD_ALL </_Program name_>
...
<_Data collection from_>		02/2020 </_Data collection from_>
<_Data collection to_>			11/2020  </_Data collection to_>
<_Source of dataset_> 			National Institute of Statistics Rwanda </_Source of dataset_>
<_Sample size (HH)_> 			16,718 </_Sample size (HH)_>
<_Sample size (IND)_> 			70,172 </_Sample size (IND)_>
<_Sampling method_> 			The survey follows a two-stage stratified design based on the sampling frame of the Population and Housing Census of 2012. Around 16 households are sampled from a total of 288 enumeration areas with probability proportional to size. The sample is split into three rotation groups. Unlike the previous years where sampling replacement is on a round-to-round basis, beginning 2020, the survey design follows a 1-1-1 rotation such that a household is interviewed every other round three times. 

Adjustments have been made in response to covid-19. In the May and November 2020 rounds, interviews were done through phone. Also, an abridged version was used in the May 2020 round prompting the NSO to present in the annual report only indicators available in that abridged version. Only labor status and the geographic characteristics were collected in that round. 
...
use "`path_in_stata'/Labour_force_survey_2020_pub.dta", clear
/*%%=============================================================================================
	2: Survey & ID
==============================================================================================%%*/
...
*<_hhid_>
	* Note that in the May 2020 round, HHID is missing, but it can be constructed based on available information
	* HHID is constructed by concatenating PSU_NO and QH_NO
	gen PSU_str = string(PSU_NO, "%04.0f")
	gen QH_str = string(QH_NO, "%02.0f" )
	
	gen hhid = PSU_str + QH_str
	
	* Check if same with HHID
	tostring HHID, gen(HHID_str)
	assert HHID_str == hhid if LFS_round != 2
...
*<_pid_>
	gen serial = string(PID, "%02.0f")
	egen pid = concat(hhid serial)
...
/* <_subnatid1_note>
	...
	gen subnatid1 = "4 - Northern Province" if prov == 1
	replace subnatid1 = "2 - Southern Province" if prov == 2
	replace subnatid1 = "5 - Eastern Province" if prov == 3
	replace subnatid1 = "3 - Western Province" if prov == 4
	replace subnatid1 = "1 - Kigali City" if prov == 5
...
*<_migrated_mod_age_>
	...
	replace migrated_mod_age = . if wave == 2
...
*<_migrated_ref_time_>
	...
	replace migrated_ref_time = . if wave == 2
...
*<_migrated_binary_>
	...
	replace migrated_binary = . if wave == 2
...
*<_potential_lf_>
	...
	* P4 - Not looking for work, have not found a job, wants a job and available to work (potential LF)
	...
	replace potential_lf = . if wave == 2
...
* Note that the GLD's definition of underemployment = time-related underemployment only
	...
	* #2: People who did not look for work, would want to take more hours and would take the work if it becomes available
	...
	replace underemployment = . if wave == 2
...
1. There are more responses here. 
	E.g., in Feb 2020 round, 4960 responses but only 397 for unavailability
...
*<_occup_isco_2_>
	...
	replace occup_isco_2 = "3200" if occup_isco_2 == "3223"
