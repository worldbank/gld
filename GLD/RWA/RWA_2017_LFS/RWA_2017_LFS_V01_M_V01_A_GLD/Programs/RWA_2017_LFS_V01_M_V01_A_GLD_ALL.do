...
<_Program name_>				RWA_2017_LFS_v01_M_v01_A_GLD_ALL </_Program name_>
<_Application_>					Stata 17 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2022-11-28 </_Date created_>
...
<_Data collection from_>		02/2017 (Feb round); 08/2017 (Aug round) </_Data collection from_>
<_Data collection to_>			04/2017 (Feb round); 10/2017 (Aug round) </_Data collection to_>
<_Source of dataset_> 			National Institute of Statistics Rwanda </_Source of dataset_>
<_Sample size (HH)_> 			17,938 </_Sample size (HH)_>
<_Sample size (IND)_> 			77,761 </_Sample size (IND)_>
<_Sampling method_> 			The survey follows a two-stage stratified design based on information from the Population and Housing Census of 2012. Around 16 households are selected on each of the 203 urban and 381 rural enumeration areas (EAs). These EAs were selected from each of the 30 districts by urban/rural with probability proportional to size. The sample is split into four rotation groups. One rotation group is replaced with a new sample on a round-to-round basis such that 75% of the sample in the previous round are also interviewed in the next round. As the Rwanda NIS rolls out the LFS, field work only occurred in certain months of the year: the Feb round contains data from Feb to April, while the August round has data from August to October. The full year data collection does not happen until 2019. 
...
use "`path_in_stata'/rwa-nisr-lfs-2017-data.dta", clear
/*%%=============================================================================================
	2: Survey & ID
==============================================================================================%%*/
...
/* <_hhid_note>
	...
	gen psu_str = string(PSU_NO, "%03.0f")
	tostring pid, gen(pid_str)
	gen pid_last4 = substr(pid_str, -4, 4)
	gen hhid_2d = substr(pid_last4, 1, 2)
	gen pid_2d = substr(pid_last4, 3, 2)
	egen hhid = concat(prov_str district_str psu_str hhid_2d)
...
*<_subnatid2_>
	sdecode district, gen(districtname)
	gen subnatid2 = district_str + " - " + districtname
...
/* <_subnatidsurvey_note>
	...
	tostring ur, gen(ur_str)
	gen str subnatidsurvey = subnatid1 + ur_str
...
*<_migrated_reason_>
	...
	recode migrated_reason (1 2 4 5 6 = 1) (3 = 2) (7 8 9 10 11 12 = 3) (13 14 =5)
...
*<_vocational_field_orig_>
	gen str vocational_field_orig = string(B10, "%2.0f")
	label var vocational_field_orig "Original field of training information"
*</_vocational_field_orig_>

*<_vocational_financed_>
	gen vocational_financed = B12
	recode vocational_financed (1 = 2) (2 = 1) (3 = 4) (4 5 6 7 8 = 5)
...
*<_lstatus_>
  ...
  ***************************
	gen didwhatittakes = (C21A != 8 & !missing(C21A))
...
* Note that the GLD's definition of underemployment = time-related underemployment only
	...
	* #1: People who looked for work and would take the work if it becomes available
	replace underemployment = 1 if (C16 == 1 & C18 == 1) 
	
	* #2: People who did not look for work, would want to take more hours and would take the work if it becomes available
	replace underemployment = 1 if (C16 == 2 & C17 == 1 & C18 == 1)
...
1. There are more responses here. 
	E.g., in Feb 2017 round, 4960 responses but only 397 for unavailability
...
*<_industry_orig_>
	gen industry_orig = string(isic2d, "%02.0f")
...
*<_occup_isco_>
	...
	replace occup_isco = "" if occup_isco == "00" | lstatus !=1
	label var occup_isco "ISCO code of primary job 7 day recall"
*</_occup_isco_>


*<_occup_>
	gen byte occup = indd01
...
*<_wage_no_compen_>
  ...
  * Note here that earnings is a total for all economic activities
	...
	* W1: For wage workers who reported amounts in cash
	gen totcash = D12A
	* Note here there are 15 non-paid employees reporting wages
...
. tab D13 D16, mi
  ...
  How long did |                     What period did it cover?
     it cover? |     Month  Two weeks   One week    One day  Other(spe          . |     Total
---------------+------------------------------------------------------------------+----------
         Month |       199          2          4         37         34      5,099 |     5,375 
     Two weeks |         0          1          1          0          0        264 |       266 
      One week |         1          0         19         14          1        807 |       842 
       One day |         6          4         32        248          0      6,846 |     7,136 
Other(specify) |         0          0          0          0          0          5 |         5 
             . |         0          0          0          0          0     64,137 |    64,137 
---------------+------------------------------------------------------------------+----------
         Total |       206          7         56        299         35     77,158 |    77,761 
  ...
  * Net business income
  ***************************]
  * B1: For those who reported net MONTHLY business income
  sum D18A1, detail
  * Here we can see that 25% reported value = 0 (perhaps net losses are recoded to zero since zero is the minimum)
  tab D12 D18A
  * Wage and self-employment income are mutually exclusive! 
  gen net_income = D18A1
  
  * B2: For those who reported net business income as a range of values
  	replace net_income = 13333 if D18B == 1
	replace net_income = 24000 if D18B == 2
	replace net_income = 37500 if D18B == 3
	replace net_income = 66667 if D18B == 4
...
*<_firmsize_l_>
	...
	* D22A gives exact number if less than 10 employees in firm while D22B gives a range of values, to simplify, treat D22A as a range of 1 - 10;  D22A = 0 indicates that employee count > 10, so use D22B
	gen byte firmsize_l = .
	replace firmsize_l = 1 if inrange(D22A, 1, 9)
	replace firmsize_l = 10 if D22B == 1
	replace firmsize_l = 31 if D22B == 2
	replace firmsize_l = 51 if D22B == 3
	replace firmsize_l = 101 if D22B == 4
...
*<_firmsize_u_>
	gen byte firmsize_u= .
	replace firmsize_u = 9 if inrange(D22A, 1, 9)
	replace firmsize_u = 30 if D22B == 1
	replace firmsize_u = 50 if D22B == 2
	replace firmsize_u = 100 if D22B == 3
	replace firmsize_u = . if D22B == 4
...
*<_ocusec_2_>
	gen byte ocusec_2 = .
	replace ocusec_2 = 1 if E03 == 1
	replace ocusec_2 = 2 if inlist(E03, 3, 4, 5, 6, 7)
	replace ocusec_2 = 4 if E03 == 2
...
*<_industry_orig_2_>
	gen industry_orig_2 = string(isic2e, "%02.0f")
...
*<_industrycat10_2_>
	gen byte industrycat10_2 = isic2e
...
*<_occup_orig_2_>
	gen occup_orig_2 = string(isco2e, "%02.0f")
...
*<_occup_isco_2_>
	gen occup_isco_2 = occup_orig_2 + "00"
...
*<_firmsize_l_2_>
	...
	replace firmsize_l_2 = 1 if inrange(E09A, 1, 9)
	replace firmsize_l_2 = 10 if E09B == 1
	replace firmsize_l_2 = 31 if E09B == 2
	replace firmsize_l_2 = 51 if E09B == 3
	replace firmsize_l_2 = 101 if E09B == 4
...
*<_firmsize_u_2_>
	...
	replace firmsize_u_2 = 9 if inrange(E09A, 1, 9)
	replace firmsize_u_2 = 30 if E09B == 1
	replace firmsize_u_2 = 50 if E09B == 2
	replace firmsize_u_2 = 100 if E09B == 3
	replace firmsize_u_2 = . if E09B == 4
