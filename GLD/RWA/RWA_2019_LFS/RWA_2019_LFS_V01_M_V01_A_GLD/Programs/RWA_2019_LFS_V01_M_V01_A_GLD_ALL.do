...
<_Program name_>				RWA_2019_LFS_v01_M_v01_A_GLD_ALL </_Program name_>
...
use "`path_in_stata'/Labour force survey_2019.dta", clear
/*%%=============================================================================================
	2: Survey & ID
==============================================================================================%%*/
...
*<_hhid_>
	gen pid_round = string(pkey, "%010.0f")

	tostring province, gen(prov_str)
	tostring code_dis, gen(district_str)
	gen hhid = substr(pid_round, 1, 6)
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>
	gen pid = substr(pid_round, 1, 8)
	label var pid "Individual ID"
*</_pid_>


*<_weight_>
	* Note here that weights used is for annual estimates. 
	label var weight "Survey sampling weight"
*</_weight_>


*<_psu_>
	gen pkey_round = substr(pid_round, -2, 2)
	gen pkey_4d = substr(pid_round, 1, 4)
	
	bys pkey_round: distinct pkey_4d
	* We get 288 each consistent with the report!
	
	gen psu = substr(pid_round, 1, 4)
...
*<_potential_lf_>
	...
	* P4 - Not looking for work, have not found a job, wants a job and available to work (potential LF)
	...
	gen w_plf = inrange(PLF, 1, 2)
	tab w_plf potential_lf if age>=16
	* 17 cases tagged as not PLF by survey but PLF based on GLD: these people did not make an effort in job search but available to work.
...
* Note that the GLD's definition of underemployment = time-related underemployment only
	...
	replace underemployment = 0 if lstatus == 1 & C14 != 1
	...
	* Special case: missing response for wanting additional hours but available for more hours
	replace underemployment = 1 if (C16 == 2 & missing(C17) & C18 == 1)
...
1. There are more responses here. 
	E.g., in Feb 2019 round, 4960 responses but only 397 for unavailability
...
*<_occup_orig_>
	...
	replace occup_orig = "" if occup_orig == "0002"
