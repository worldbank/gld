...
/* -----------------------------------------------------------------------
<_Program name_> 			LKA_2013_LFS_V01_M_V03_A_GLD_ALL.do </_Program name_>
...
<_Study ID_>					LKA_2013_LFS_v01_M </_Study ID_>
<_Data collection from (M/Y)_>	[Jan/2013] </_Data collection from (M/Y)_>
<_Data collection to (M/Y)_>	[Dec/2013] </_Data collection to (M/Y)_>
...
* Date: [2023-10-19] File: [LKA_2013_LFS_v01_M_v02_A_GLD_ALL] - [Typo correction of ISCED]
...
*<_educat_isced_>
	gen educat_isced=.
	replace educat_isced=100 if inrange(p10,0,5)
	replace educat_isced=244 if inrange(p10,6,9)
	replace educat_isced=343 if p10==10|p10==11
	replace educat_isced=344 if p10==12|p10==13
	replace educat_isced=660 if p10==18
	replace educat_isced=760 if p10==19
...
q45a1-3 are for monthly salary earners
...
. list pid industrycat10 occup empstat if daily==1&employer==1

       +----------------------------------------------------------------------------------------+
       |                 pid                  industrycat10               occup         empstat |
       |----------------------------------------------------------------------------------------|
 5025. | 0102610090901109-01   Transport and Comnunications   Machine operators   Self-employed |
 5026. | 0102610090901109-02                  Manufacturing       Craft workers   Self-employed |
 5030. | 0102610091001110-01                  Manufacturing       Craft workers   Self-employed |
18912. | 0302720120801028-01                  Manufacturing       Craft workers   Self-employed |
       +----------------------------------------------------------------------------------------+
...
*<_wage_no_compen_note_>*/
	...
	egen monthly=rownonmiss(q45a1-q45a3)
	egen daily=rownonmiss(q45b1-q45b4)
	gen employer=1 if !mi(q45c1)&q45c1>0
...
*<_unitwage_>
	...
	replace unitwage=5 if employer==1&daily==1
...
21 observations were left as missing as they are both monthly salary earners and 
daily wage earners.
*<_wage_no_compen_2_note_>*/
