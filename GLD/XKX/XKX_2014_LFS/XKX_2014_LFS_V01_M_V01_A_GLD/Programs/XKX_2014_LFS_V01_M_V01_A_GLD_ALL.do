...
<_Program name_>				XKX_2014_LFS_V01_M_V01_A_GLD_ALL </_Program name_>
...
<_Sample size (HH)_> 			15,265 </_Sample size (HH)_>
<_Sample size (IND)_> 			87,617 </_Sample size (IND)_>
...
use "`path_in_stata'/Kosovo_LFS_2014_13.05.2015.dta", clear
...
**** A household is not repeated in the following quarters. ****
	...
	gen hhid = HHID + "Q" + string(KUARTALI)
...
*<_weight_q_>
	gen weight_q = Adj_quarter_wt_first
...
/* <_subnatid1_note>
	...
	decode RAJONI, gen(region_str)
	replace region_str = proper(lower(region_str))
	egen subnatid1 =concat(RAJONI region_str), punct(" - ")
...
*<_subnatid2_>
	...
	*subnatid2 is unique, hence, subnatid1 does not have the subnatid2 of other subnatid1
	*tab subnatid2 subnatid1
	replace subnatid2 = "" if subnatid2 == "3" & inlist(RAJONI,1,5)
	replace subnatid2 = "" if subnatid2 == "1" & inlist(RAJONI,4)
...
*<_relationharm_>
	...
	*If household has no head or more than one head:
	...
	--------------+--------------------------------------------
	Valid   0     |       2129       2.57       2.57       2.57
			1     |      80719      97.37      97.37      99.94
			2     |         44       0.05       0.05      99.99
			3     |          6       0.01       0.01     100.00
			Total |      82898     100.00     100.00           
	-----------------------------------------------------------


	unique hhid if number_heads == 0
		Number of unique values of hhid is  391
		Number of records is  2129

	There are 391 households (2129 individuals) with no household head
	
	unique hhid if inrange(number_heads,2,999)
		Number of unique values of hhid is  9
		Number of records is  50
		
	There are 9 households (5 individuals) with more than one head
...
*<_migrated_from_cat_>
	...
	replace migrated_from_cat = 1 if string(PPF_P20) == subnatid3
	replace migrated_from_cat = 2 if string(PPF_P19) == subnatid2 & missing(migrated_from_cat)
	
	
*We need to know which region the municipality belongs to.
preserve
	* Keep only distinct cities and their regions
	keep subnatid2 subnatid1
	destring subnatid2 subnatid1, replace
	duplicates drop
	rename (subnatid2 subnatid1) (PPF_P19 region_prev)
	drop if missing(PPF_P19)
	* Save as lookup table
	tempfile regions_cities
	save `regions_cities'
restore

	merge m:1 PPF_P19 using `regions_cities', keepusing(region_prev) nogenerate keep(1 3)
	assert !missing(region_prev) if !missing(PPF_P19)

	
	replace migrated_from_cat = 3 if region_prev == subnatid1 & missing(migrated_from_cat) & !missing(region_prev) & PPF_P18 == "KS"
	replace migrated_from_cat = 4 if region_prev != subnatid1 & missing(migrated_from_cat) & !missing(region_prev) & PPF_P18 == "KS"
...
*<_migrated_from_code_>
	gen migrated_from_code = string(PPF_P20) if migrated_from_cat == 2
	replace migrated_from_code = string(PPF_P19) if migrated_from_cat == 3
	replace migrated_from_code = region_prev if migrated_from_cat == 4
...
*<_migrated_from_country_>
	...
	replace migrated_from_country = "CZE" if PPF_P18 == "CE"   // Czechia
	replace migrated_from_country = "DEU" if PPF_P18 == "DE"   // Germany
	replace migrated_from_country = "DNK" if PPF_P18 == "DK"   // Denmark
	replace migrated_from_country = "HRV" if PPF_P18 == "HR"   // Croatia
	replace migrated_from_country = "HUN" if PPF_P18 == "HU"   // Hungary
	replace migrated_from_country = "ITA" if PPF_P18 == "IT"   // Italy
	replace migrated_from_country = "KOR" if PPF_P18 == "KR"   // Korea, Republic of
	replace migrated_from_country = "MNE" if PPF_P18 == "ME"   // Montenegro
	replace migrated_from_country = "SRB" if PPF_P18 == "RS"   // Serbia
	replace migrated_from_country = "SVN" if PPF_P18 == "SI"   // Slovenia
	replace migrated_from_country = "SWZ" if PPF_P18 == "SZ"   // Eswatini (Swaziland)
	replace migrated_from_country = "ZWE" if PPF_P18 == "ZW"   // Zimbabwe
...
*They use NACE 2 which is in line with ISIC rev 4 at 2 digits level
	gen industrycat_isic = cond(missing(P40_B_CODE), "", string(P40_B_CODE, "%02.0f") + "00")
...
*<_occup_isco_>
	gen occup_isco = cond(missing(P42_A_CODE), "", string(P42_A_CODE, "%03.0f") + "0")
	...
	replace occup_isco = "4000" if occup_isco == "0410"
	replace occup_isco = "9000" if occup_isco == "0920"
	replace occup_isco = "2000" if occup_isco == "2090"
	replace occup_isco = "8000" if occup_isco == "8520"
...
*<_industrycat_isic_2_>
	*They use NACE 2 which is in line with ISIC rev 4 at 2 digits level
	gen industrycat_isic_2 = cond(missing(P56_B_CODE), "", string(P56_B_CODE, "%02.0f") + "00")
...
*<_industrycat_isic_year_>
	gen industrycat_isic_year = cond(missing(P101_B_CODE), "", string(P101_B_CODE, "%02.0f") + "00")
