...
<_Program name_>				XKX_2005_LFS_V01_M_V01_A_GLD_ALL </_Program name_>
...
<_Data collection from_>		09/2005 </_Data collection from_>
<_Data collection to_>			10/2005 </_Data collection to_>
<_Source of dataset_> 			Kosovo Agency of Statistics </_Source of dataset_>
<_Sample size (HH)_> 			3,200 </_Sample size (HH)_>
<_Sample size (IND)_> 			19,590 </_Sample size (IND)_>
...
<_INDUS National_>				NACE rev 1 </_INDUS National_>
-----------------------------------------------------------------------
<_Version Control_>
...
use "`path_in_stata'/LFS_2005.dta", clear
...
*<_int_month_>
	gen  int_month = 9
	replace int_month = 10 if inlist(b1a, 16710, 16711)
...
**** A household is not repeated in the following quarters. ****
	...
	* In the data, variable ea is the same as the combination of municipality, town, enum area and housing unit
	...
	gen hhid = dbo_identifikimeksh_ekshtid
...
*<_relationharm_>
	...
	* Create runner (oldest, male, HH head)
	gen neg_age = -age
	bys hhid (relationharm q5 age): gen hhrunner = _n
...
*<_relationcs_>
	...
	replace relationcs = "Head of the household" if q1 == 1
	replace relationcs = "Spouse of hh (or cohabiting partner)" if q1 == 2
	replace relationcs = "Child of head of household (or of his/her spouse or cohabiting partner)" if q1 == 3
	replace relationcs = "Father/mother of head of household (or of his/her spouse or cohabiting partner)" if q1 == 4
	replace relationcs = "Other relative of hh (son/daughter-in-law, grandchild, sister/brother, brother/sister-in-law, niece/nephew, grandfather/mother, etc)" if q1 == 5
	replace relationcs = "Other (not relative of hh)" if q1 == 6
...
*<_lstatus_>
	...
	**** Employed ****
	******************
	* worked in the past 7 days; includes own consumption (since ICLS 13)
	* p13 categories 5 and 6 capture own consumption workers for at least 1 hour

	replace lstatus = 1 if inrange(p13, 1, 4)
	
	* absent from work for less than three months or presumed attachment (e.g., maternity leave or strike)
	replace lstatus = 1 if (p14 == 1 & p16 == 1) | (p14 == 1 & inrange(p15, 5, 10))
	
	* absent from work for more than three months and will continue to receive wages (this is our indication of job attachment)
	replace lstatus = 1 if p14 == 1 & p16 == 2 & p17 == 1
	
	******************
	*** Unemployed ***
	******************
	* Traditional unemployed: looking and available to start
	replace lstatus = 2 if (p56 == 1 & p63 == 1)
	
	*Future starters: Have found a job which will start in no more than 3 months & available
	replace lstatus = 2 if p55 == 1 & p63 == 1 
...
*<_potential_lf_>
	...
	* Job seekers who are unavailable
	replace potential_lf = 1 if p56 == 1 & p63 == 2 & lstatus == 3
	
	* Available non-jobseekers/discouraged workers
	replace potential_lf = 1 if p56 == 2 & p63 == 1 & lstatus == 3
...
*<_underemployment_>
	...
	* want to work more hours (leave it up to data user to impose restrictions on hours)
	replace underemployment = 1 if inrange(p35, 2, 5)
...
*<_nlfreason_>
	gen byte nlfreason = p57
	recode nlfreason (1 = 4) (3 = 1) (4 = 3) (5 6 7 = 5) (0 = .)
...
*<_unempldur_l_>
    ...
    replace unempldur_l = 0   if inlist(p60, 1, 2)   // Not yet started OR <1 month
    replace unempldur_l = 1   if p60 == 3            // 1–2 months
    replace unempldur_l = 3   if p60 == 4            // 3–5 months
    replace unempldur_l = 6   if p60 == 5            // 6–11 months
    replace unempldur_l = 12  if p60 == 6            // 1–2 years
    replace unempldur_l = 24  if p60 == 7            // 2+ years
...
*<_unempldur_u_>
    ...
    replace unempldur_u = 0    if p60 == 1           // Not yet started
    replace unempldur_u = 1    if p60 == 2           // <1 month
    replace unempldur_u = 2    if p60 == 3           // 1–2 months
    replace unempldur_u = 5    if p60 == 4           // 3–5 months
    replace unempldur_u = 11   if p60 == 5           // 6–11 months
    replace unempldur_u = 23   if p60 == 6           // 1–2 years
    replace unempldur_u = .    if p60 == 7           // 2+ years (open-ended)
...
{
*<_empstat_>
	gen byte empstat = p20 if lstatus == 1
...
*<_industry_orig_>
	gen industry_orig = string(p18bk, "%02.0f") if lstatus == 1
...
*<_occup_orig_>
	gen occup_orig = string(p24ak, "%02.0f") if lstatus == 1 & p24ak != 0
...
*<_whours_>
	gen whours = p31 
	replace whours = p30 if missing(whours) | whours == 0  // usually work
...
*<_firmsize_l_>
    gen firmsize_l = .
    replace firmsize_l = 1           if p19a == 1   // Exact number if 1–10 reported
    replace firmsize_l = 11             if p19a == 2   // 11–19
    replace firmsize_l = 20             if p19a == 3   // 20–49
    replace firmsize_l = 50             if p19a == 4   // 50+
    replace firmsize_l = 1              if p19a == 5   // Don't know but <11 → assume min 1
    replace firmsize_l = 11             if p19a == 6   // Don't know but >10 → assume min 11
    replace firmsize_l = 500            if p19a == 7   // 500+
...
*<_firmsize_u_>
    gen firmsize_u = .
    replace firmsize_u = 10           if p19a == 1   // Exact number if 1–10 reported
    replace firmsize_u = 19             if p19a == 2   // 11–19
    replace firmsize_u = 49             if p19a == 3   // 20–49
    replace firmsize_u = .              if p19a == 4   // 50+ open-ended
    replace firmsize_u = 10             if p19a == 5   // Don't know but <11 → assume max 10
    replace firmsize_u = .              if p19a == 6   // Don't know but >10 → no upper bound
    replace firmsize_u = .              if p19a == 7   // 500+ open-ended
...
{
*<_empstat_2_>
	gen byte empstat_2 = p46 if lstatus == 1
...
*<_industry_orig_2_>
	gen industry_orig_2 = string(p45k, "%02.0f") if lstatus == 1
...
*<_whours_2_>
	gen whours_2 = p47 if !missing(empstat_2) & lstatus == 1 & p47 != 0
...
*<_lstatus_year_>
	...
	*** employed ***
	replace lstatus_year = 1 if p75 == 1
	
	*** unemployed ***
	replace lstatus_year = 2 if p75 == 2
	
	*** NLF ***
	replace lstatus_year = 3 if inlist(p75,3,4,5,6,7, 8)
...
*<_empstat_year_>
	gen byte empstat_year = p76 if lstatus_year == 1
...
*<_industry_orig_year_>
	gen industry_orig_year = string(p77k, "%02.0f") if lstatus_year == 1
...
*<_industrycat10_year_>
	...
	replace industrycat10_year = 1 if isic_1d == 0
	replace industrycat10_year = 2 if inrange(isic_2d, 10, 14)
	replace industrycat10_year = 3 if inrange(isic_2d, 15, 37)
	replace industrycat10_year = 4 if inrange(isic_2d, 40, 41)
	replace industrycat10_year = 5 if isic_2d == 45
	replace industrycat10_year = 6 if isic_1d == 5
	replace industrycat10_year = 7 if inrange(isic_2d, 60, 64)
	replace industrycat10_year = 8 if inrange(isic_2d, 65, 74)
	replace industrycat10_year = 9 if isic_2d == 75
	replace industrycat10_year = 10 if inrange(isic_2d, 80, 99)
...
*<_occup_orig_year_>
* Occupation texts only provided
	gen occup_orig_year = .
