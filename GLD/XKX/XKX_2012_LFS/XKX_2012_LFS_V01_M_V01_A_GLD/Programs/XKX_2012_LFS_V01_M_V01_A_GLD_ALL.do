...
<_Program name_>				XKX_2012_LFS_V01_M_V01_A_GLD_ALL </_Program name_>
...
<_Sample size (HH)_> 			9,682 </_Sample size (HH)_>
<_Sample size (IND)_> 			56,762 </_Sample size (IND)_>
...
use "`path_in_stata'/Kosovo_LFS_2012_final_no_names_aug14.dta", clear
...
*<_int_year_>
	gen int_year = 2012
	replace int_year = 2013 if B1_A_VITI_1 == 2013
...
*<_relationharm_>
	...
	*If household has no head or more than one head:
	...
	--------------+--------------------------------------------
	Valid   0     |       1191       2.10       2.10       2.10
			1     |      55520      97.81      97.81      99.91
			2     |         51       0.09       0.09     100.00
			Total |      56762     100.00     100.00           
	-----------------------------------------------------------

	unique hhid if number_heads == 0
		Number of unique values of hhid is  245
		Number of records is  1191

	There are 245 households (1191 individuals) with no household head
	
	unique hhid if inrange(number_heads,2,999)
		Number of unique values of hhid is  10
		Number of records is  51
		
	There are 10 households (51 individuals) with more than one head
	...
	*(4) If there is more than one head, keep the oldest
	...
	bys hhid: replace relationharm = 1 if relationharm == 1 & number_heads == 2 & oldest_head == 1
	bys hhid: replace relationharm = 5 if number_heads == 2 & oldest_head != 1
...
*<_migrated_from_country_>
	...
	replace migrated_from_country = "ALB" if PPF_P18 == "AL"
	...
	replace migrated_from_country = "HRV" if PPF_P18 == "HR"
	replace migrated_from_country = "ITA" if PPF_P18 == "IT"
	replace migrated_from_country = "MNE" if PPF_P18 == "ME"
	replace migrated_from_country = "SRB" if PPF_P18 == "RS"
	replace migrated_from_country = "SVN" if PPF_P18 == "SI"
	replace migrated_from_country = "SWZ" if PPF_P18 == "SZ" // Esuatini (Swazilandia)
