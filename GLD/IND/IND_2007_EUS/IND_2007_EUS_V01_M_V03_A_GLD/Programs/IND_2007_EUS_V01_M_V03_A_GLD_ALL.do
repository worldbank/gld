...
<_Program name_> 			IND_2007_EUS_V01_M_V03_A_GLD_ALL.do </_Program name_>
...
use "`path_in_stata'/Block-5-members-time-disposition-records.dta", clear
...
* Merge with block 4
merge m:1 key_memb using "`path_in_stata'/Block-4-demographic-usual-activity-members-records.dta" , keep(master match) nogen

* Merge with block 1 - 3
gen key_Hhold = key_hhold

* Start with Block 1
merge m:1 key_Hhold using "`path_in_stata'/Block-1-sample-household-identification-records.dta" , keep(master match) nogen

* Block 3dot1 - this data is not relevant for GLD
merge 1:1 key_memb using "`path_in_stata'/Block-3dot1-out-migrants-records.dta" , keep(master match) nogen

* Block 3 only
gen Key_hhold = key_hhold
merge m:1 Key_hhold using "`path_in_stata'/Block-3-household-characteristics-ecords.dta" , keep(master match) nogen

* Merge with block 6
merge 1:1 key_memb using "`path_in_stata'/Block-6-members-migration-records.dta" , keep(master match) nogen
