...
<_Author(s)_>					Mario Gronert </_Author(s)_> 
<_Date created_>				2021-03-23 </_Date created_> 
<_Date modified>				2021-03-23 </_Date modified_>
...
local path_in "C:\Users\wb529026\OneDrive - WBG\Documents\Country Work\IND\IND_1999_NSS55-SCH10\IND_1999_NSS55-SCH10_v01_M\Data\Stata"
local path_output "C:\Users\wb529026\OneDrive - WBG\Documents\Country Work\IND\IND_1999_NSS55-SCH10\IND_1999_NSS55-SCH10_v01_M_v01_A_GLD\Data\Harmonized"

*----------1.3: Database assembly------------------------------*

* Start with Block 5.3 as this has several lines per individual
use "`path_in'\Block53-sch10-Persons-daily-activity-time-disposition-Records.dta", clear

* Switch serial numbering of activities to integer
destring B53_q3, replace
tab B53_q3,m

keep B53_q3 B53_q4 B53_q5 B53_q14 B53_q15 B53_q16 B53_q17 B53_q18 B53_q19 B53_q20 B53_q21  B53_q22 Key_Hhold Key_PRSN Hhold_Slno Prsn_Slno_B53_q1 State
reshape wide B53_q4 B53_q5 B53_q14 B53_q15 B53_q16 B53_q17 B53_q18, i(Key_PRSN) j(B53_q3)
...
count if missing(B53_q44) & !missing(B53_q45)

* Correct 2nd / 3rd activity
gen condition = missing(B53_q42) & !missing(B53_q43)
replace B53_q42 =  B53_q43 if condition == 1
replace B53_q52 =  B53_q53 if condition == 1 
replace B53_q142 = B53_q143 if condition == 1 
replace B53_q152 = B53_q153 if condition == 1 
replace B53_q162 = B53_q163 if condition == 1 
replace B53_q172 = B53_q173 if condition == 1
replace B53_q182 = B53_q183 if condition == 1

replace B53_q43 = "" if condition == 1
replace B53_q53 = "" if condition == 1
replace B53_q143 = . if condition == 1
replace B53_q153 = . if condition == 1
replace B53_q163 = . if condition == 1
replace B53_q173 = . if condition == 1
replace B53_q173 = . if condition == 1
replace B53_q183 = "" if condition == 1

* The other cases are not relevant for usual, secondary activity, so no need to reassign, all other cases can be summarised as "other jobs"
drop condition

* Check: if first activity has most time spent on but neither activity nor industry are listed
egen week_max_work = rowmax(B53_q141 B53_q142 B53_q143 B53_q144 B53_q145)
count if (missing(B53_q41) & missing(B53_q51)) & (B53_q141 == week_max_work)
...
gen str14 Key_prsn = substr( Key_Prsn_slno,3,.)
isid Key_prsn

tempfile weekly_unemp
save `weekly_unemp'

* Next is Block 5.2 which lists secondary activties. 17,670 individuals have a secondary
* activity, 1558 of them have two.
use "`path_in'\Block52-sch10-Persons-subsidiary-activity-Records.dta", clear
destring B52_q17, replace
keep Prsn_Slno_B52_q1 B52_q2 B52_q3 B52_q5 B52_q6 B52_q7 B52_q8 B52_q9 B52_q10 B52_q11 B52_q12 B52_q13 B52_q14 B52_q15 B52_q16 Key_Hhold Key_prsn B52_q17
...
rename Key_prsn Key_PRSN

tempfile subsidiary_act
save `subsidiary_act'

* Start process with individual block 4
use "`path_in'\Block4-sch10-persons-demographic-migration-records.dta", clear
* Block 4 has key_prsn, which is distinct but str14. Block 53 had Key_PRSN which is str16 by adding
* sub round and sub sample at the start. That is unique. But the merging gives 2314 cases of non
* match, exactly 1157 on either. These should match but I cannot find the logic of the mistake that
* caused them to diverge.
* Given that other blocks use the str14 version and those merge, drop the 1157 cases we cannot match.
* It is a pity but I don't know a way around

* Note that all these cases are from the same state - Kakshadweep (https://en.wikipedia.org/wiki/Lakshadweep). The full sample size is 2769, so we lose about 40% of the sample for this state for the weekly status information.
...
egen Key_PRSN = concat(sub_round Sub_sample key_prsn)

* Merge block 4 with 7 day time spent info
merge 1:1 Key_PRSN using `weekly_act', keep(master match) nogen

* Merge with Block 5.1 - Note var in 51 is Key_prsn. These little upper case lower case differences...
gen Key_prsn = key_prsn
merge 1:1 Key_prsn using "`path_in'\Block51-sch10-Persons-usual-principal-activity-Records.dta", assert(match) nogen

* Merge with Block 5.2 (a subset of 5.1, so match, master)
* As previously, we have some issues with matching unique certain individuals from a few FSUs
* in Lakshadweep. In this case we lose info from 52 individuals from there.
merge 1:1 Key_PRSN using "`subsidiary_act'", keep(master match) nogen

* Merge with Block 6
merge 1:1 Key_prsn using "`weekly_unemp'", assert(master match) nogen

* Merge with Block 7.1
gen Key_prsn_slno = Key_PRSN
merge 1:1 Key_prsn_slno using "`path_in'\Block71-sch10-Persons-availability-for-work-Records.dta", assert(master match) nogen

* Merge with Block 7.2
replace Key_Prsn_slno = Key_PRSN
merge 1:1 Key_Prsn_slno using "`path_in'\Block72-sch10-Persons-change-of-work-Records.dta", assert(master match) nogen

* Merge with Block 3 (HH info)
* As previosuly, there is some mix upin Lakshadweep. HH data on 96 HHs is lost.
drop Key_hhold
egen Key_hhold = concat(sub_round Sub_sample key_Hhold)
merge m:1 Key_hhold using "`path_in'\Block3-sch10--Household-Characteristics-records.dta", keep(master match) nogen
...
/* <_hhid_note>
	...
	egen str9 hhid = concat(fsu_no seg_no Stg2_stratm Hhold_Slno)
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>
	egen  str11 pid = concat(hhid Prsn_Slno_B4_q1) 
...
/* <_weight_note>
	...
	gen weight = .
	replace weight = Wgt if ss_replicate == 1
	replace weight = Wgt/2 if ss_replicate > 1
...
/* <_relationharm_note>
	...
	2198 observations (433 HHs) have either no head or more than one.Force it by making 
	...
	destring Prsn_Slno_B4_q1, gen(pno)
	...
	* 22 cases left, 3 HHs, change manually.
	replace relation = 1 if pid == "2114610202001"
	replace relation = . if pid == "2323410206011"
	replace relation = . if pid == "2836510222010"
...
*<_educat7_>
	...
	gen byte educat7 = .
	replace educat7=1 if inrange(genedulev,1,5) // No educ
...
*<_educat_isced_>
	...
	replace educat_isced = 3 if inrange(genedulev,8,9)
	replace educat_isced = 6 if genedulev > 9
...
*<_vocational_>
	gen vocational = 1 if B4_q8 != "1"
	replace vocational = 0 if B4_q8 == "1"
...
*<_unempldur_l_>
	gen unempldur_l=.
	replace unempldur_l=0 if inlist(B6_q4,"1","2","3")
...
*<_unempldur_u_>
	gen unempldur_u=.
	replace unempldur_u=1 if inlist(B6_q4,"1","2","3")
	...
	replace unempldur_u=. if B6_q4=="8" | missing(B6_q4)
...
*<_occup_>
	...
	merge m:1 code_68 using "`path_in'\IND_1999_NSS55-SCH10_NCO_68_to_2004_conversion.dta", nogen assert(match master)
...
*<_wage_no_compen_>
	gen double wage_no_compen = B53_q151
...
*<_wage_no_compen_2_>
	gen double wage_no_compen_2 = B53_q152
...
*<_t_hours_others_>
	...
	gen fifth_h = B53_q145*8 if inlist(B53_q45,"11", "12", "21", "31", "41", "51") | inlist(B53_q45, "61", "62", "71", "72")
	egen t_hours_others = rowtotal(third_h fourth_h fifth_h), missing
	label var t_hours_others "Annualized hours worked in all but primary and secondary jobs 7 day recall"
	drop third_h fourth_h fifth_h 
*</_t_hours_others_>


*<_t_wage_nocompen_others_>
	gen third_h = B53_q153*8 if inlist(B53_q43,"11", "12", "21", "31", "41", "51") | inlist(B53_q43, "61", "62", "71", "72")
	gen fourth_h = B53_q154*8 if inlist(B53_q44,"11", "12", "21", "31", "41", "51") | inlist(B53_q44, "61", "62", "71", "72")
	gen fifth_h = B53_q155*8 if inlist(B53_q45,"11", "12", "21", "31", "41", "51") | inlist(B53_q45, "61", "62", "71", "72")
	egen t_wage_nocompen_others = rowtotal(third_h fourth_h B53_q143 fifth_h), missing 
...
*<_industrycat10_year_>
	gen red_indus =substr(B51_q5,1,2)
	destring red_indus, replace 
	
	gen red_indus_s =substr(B52_q51,1,2)
...
*<_occup_year_>
	...
	drop code_68 code_68_s code_04
*</_occup_year_>
...
/* <_wmonths_year_note>
	...
	gen wmonths_year = .
	replace wmonths_year = 12 - B71_q7
	replace wmonths_year = 12 if missing(B71_q7) & B71_q6 == "1"
...
*<_industry_orig_2_year_>
	gen industry_orig_2_year = B52_q51
...
*<_industrycat10_2_year_>
	gen red_indus =substr(B52_q51,1,2)
	destring red_indus, replace 
...
*<_occup_2_year_>
	...
	merge m:1 code_68 using "`path_in'\IND_1999_NSS55-SCH10_NCO_68_to_2004_conversion.dta", nogen keep(match master)
	destring code_04, replace
...
*<_njobs_>
	...
	replace njobs = 3 if !missing(B52_q32)
...
save "`path_output'\IND_1999_NSS55-SCH10_V01_M_V01_A_GLD.dta", replace
