...
rename *_hhv1 *
rename *, lower
egen hh_key = concat(b1q1 b1q5 b1q13 b1q14 b1q15 visit qtr)
...
* Merge them, keep only urban
merge m:1 hh_key using `hh_file', assert(match) nogen
keep if b1q3 == "2"
...
* RV Block
use "`path_in_stata'/hhrv.dta", clear
rename *_hhrv *
rename *, lower
egen hh_key = concat(b1q1 b1q5 b1q13 b1q14 b1q15 visit qtr)
...
use "`path_in_stata'/perrv.dta", clear
rename *_perrv *
* One odd case (pervv not perrv)
...
* Set of variables are string/numeric mismatched; set to V1
destring b4q10, replace
destring b6q4_3pt2, replace
destring b6q5_3pt2, replace
destring b6q4_act2_3pt6, replace

rename b6q5_act2_3pt5 otro
gen b6q5_act2_3pt5 = string(otro,"%02.0f")
drop otro

append using `v1_file'
...
/* <_hhid_note>
	...
	egen hhid = concat(b1q1 b1q5 b1q13 b1q14 b1q15)
...
/* <_pid_note>
	...
	destring b4q1, replace
	gen indiv_id = string(b4q1,"%02.0f")
...
/* <_weight_note>
	...
	* w/o weight. Cannot really use their info.
	...
	gen weight = .
	replace weight = (mult/no_qtr)/100 if nss == nsc
	replace weight = (mult/no_qtr)/200 if nss != nsc
...
*<_subnatid2_>
	...
	gen hlp_dist = string(distcode, "%02.0f")
...
*<_male_>
	...
	* Need to reach out to GMD team, update dictionary
	gen male = .
	destring b4q5, replace
	replace male = 1 if b4q5 == 1
	replace male = 0 if b4q5 == 2
...
*<_relationharm_>
	...
	destring b4q4, replace
	bys hhid visit: gen one=1 if b4q4 == 1
	bys hhid visit: egen temp=count(one)
	tab temp
	*assert `r(r)' == 1
	drop temp one

	gen relationharm = b4q4
...
*<_marital_>
	* Var marital exists in raw but with name "b4q7"
	gen marital = b4q7
...
*<_school_>
	* Var current_attendance exists in raw but with name "b4q11"
	gen current_attendance = b4q11
...
*<_literacy_>
	* Var general_ed exists in raw but with a name "b4q8"
	gen general_ed = b4q8
...
*<_vocational_>
	* Var any_voc_training exists in raw but with name "b4q12"
	gen any_voc_training = b4q12
...
*<_vocational_length_l_>
	* Var duration_train exists in raw but with name "b4pt1q5"
	gen duration_train = b4pt1q5
...
*<_vocational_field_orig_>
	* Var field_training exists in raw but with name "b4pt1q4"
	gen field_training = b4pt1q4
...
{
*<_lstatus_>
	* Var cws exists in raw but with name "b6q5"
	gen cws = b6q5
...
*<_occup_isco_>
	gen occup_isco = b6q7 + "0" if lstatus == 1
...
/* <_wage_no_compen_note>
	...
	* La var of b6q9_3pt2 is wrong should be:
	la var b6q9_3pt2 "wage earning for activity 1 on 6 th day"

	egen help_1 = rowtotal(b6q9_3pt1 b6q9_3pt2 b6q9_3pt3 b6q9_3pt4 b6q9_3pt5 b6q9_3pt6 b6q9_3pt7)

	* Var earnings_salaried exists in raw but with name "b6q9"
	* Var earnings_selfemp exists in raw but with name "b6q10"
	gen earnings_salaried = b6q9
	gen earnings_selfemp = b6q10
...
*<_whours_>
	egen help_1 = rowtotal(b6q6_3pt1 b6q6_3pt2 b6q6_3pt3 b6q6_3pt4 b6q6_3pt5 b6q6_3pt6 b6q6_3pt7)
...
/* <_lstatus_year_note>
	...
	* Var p_status_code exists in raw but with name "b5pt1q3"
	gen p_status_code = b5pt1q3
	gen primary_help = p_status_code
	destring primary_help, replace
	recode primary_help  11/72=1 81 82=2 91/98=3 99=.
	* Var s_status_code exists in raw but with name "b5pt2q3"
	gen s_status_code = b5pt2q3
...
*<_unempldur_l_year_>
	* Var unem_dur exists in raw but with name "b5pt3q9"
	gen unem_dur = b5pt3q9
...
*<_ocusec_year_>
	* Var p_ent_type_code exists in raw but with name "b5pt1q9"
	* Var s_ent_type_code exists in raw but with name "b5pt2q8"
	gen p_ent_type_code = b5pt1q9
	gen s_ent_type_code = b5pt2q8
...
*<_industry_orig_year_>
	* Var p_industry_code exists in raw but with name "b5pt1q5"
	gen p_industry_code = b5pt1q5
	gen help_1 = p_industry_code
	* Var s_industry_nic_code exists in raw but with name "b5pt2q5"
	gen s_industry_nic_code = b5pt2q5
...
*<_occup_orig_year_>
	* Var p_occup_code exists in raw but with name "b5pt1q6"
	gen p_occup_code = b5pt1q6
	gen help_1 = p_occup_code
	* Var s_occupation_nco_code exists in raw but with name "b5pt2q6"
	gen s_occupation_nco_code = b5pt2q6
...
*<_contract_year_>
	* Var p_type_contract exists in raw but with name "b5pt1q11"
	gen p_type_contract = b5pt1q11
	destring p_type_contract, replace
	gen help_1 = p_type_contract
	* Var s_type_contract exists in raw but with name "b5pt2q10"
	gen s_type_contract = b5pt2q10
...
*<_socialsec_year_>
	* Var p_social_sec exists in raw but with name "b5pt1q13"
	gen p_social_sec = b5pt1q13
	destring p_social_sec, replace
	gen help_1 = p_social_sec
	* Var s_social_sec exists in raw but with name "b5pt2q12"
	gen s_social_sec = b5pt2q12
...
*<_firmsize_l_year_>
	* Var p_no_workrs_ent exists in raw but with name "b5pt1q10"
	gen p_no_workrs_ent = b5pt1q10
	destring p_no_workrs_ent, replace
	gen help_1 = p_no_workrs_ent
	* Var s_no_workrs_ent exists in raw but with name "b5pt2q9"
	gen s_no_workrs_ent = b5pt2q9
