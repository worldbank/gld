...
** DIRECTORY
	local path "Z:\__I2D2\India\2004\NSS_SCH10"
...
** HOUSEHOLD IDENTIFICATION NUMBER
	*ADD
	tostring hid, gen(idh) format(%9.0f)
...
** YEARS OF EDUCATION COMPLETED
	...
	/* higher secondary */
	replace educy=12 if genedulev==10
	/* diploma , certificate - 2 years */
	replace educy=14 if genedulev==11
	/* graduate  4 years */
	replace educy=16 if genedulev==12
	/* postgraduate*/
	replace educy=18 if genedulev==13
	...
	* Higher secondary
	replace educy = educy - (2 - (age - 16)) if genedulev ==10 & inrange(age,16,18)
...
** EDUCATIONAL LEVEL 1
	gen edulevel1=.
	replace edulevel1 = 1 if inrange(genedulev,1,4) // No educ or no formal educ
	replace edulevel1 = 2 if genedulev == 6 & educy < 5 // Primary incomplete
	replace edulevel1 = 3 if genedulev == 6 & educy >= 5  // Primary complete
	replace edulevel1 = 4 if inrange(genedulev,7,10) & educy < 12  // Secondary incomplete
	replace edulevel1 = 5 if genedulev == 10 & educy >= 12  // Secondary complete
	replace edulevel1=7 if genedulev==11 | genedulev==12 | genedulev==13
...
* SECTOR OF ACTIVITY: PUBLIC - PRIVATE
	gen ocusec=.
	replace ocusec=1 if enterprisetype==5 
	replace ocusec=2 if  enterprisetype>=1 & enterprisetype<=4 | enterprisetype == 7
	replace ocusec=2 if  enterprisetype==8 
	replace ocusec=4 if enterprisetype==6
...
* INDUSTRY CLASSIFICATION
	...
	replace industry=10 if inrange(Inddiv_1,80,99)
...
*SURVEY SPECIFIC INDUSTRY CLASSIFICATION
	...
	label var industry_orig "Original Industry Codes - NIC 1998 2 Digit"
...
** OCCUPATION CLASSIFICATION
	...
	rename occupweek occupation
	destring occupation, gen(occupweek) force
	...
	* Add in cases that had an "X" as other
	gen helper_occup  = regexm(occupation,"X")
	replace occup=99 if helper_occup == 1
	...
	drop helper_occup occupweek
	rename occupation occupweek
...
* SURVEY SPECIFIC OCCUPATION CLASSIFICATION
	...
	destring occupweek, gen(princocc_CODE3) force
	clonevar occup_3digit = princocc_CODE3
...
* EMPLOYMENT STATUS - SECOND JOB
	...
	rename usualsubsocc occupation
	destring occupation, gen(occupweek_2) force
	
	gen occup_2=.
	replace occup_2=1 if (occupweek_2>=100 & occupweek_2<200) |(occupweek_2>=1 & occupweek_2<20)
	replace occup_2=2 if (occupweek_2>=200 & occupweek_2<300) |(occupweek_2>=20 & occupweek_2<30)
	replace occup_2=3 if (occupweek_2>=300 & occupweek_2<400) |(occupweek_2>=30 & occupweek_2<40)
	replace occup_2=4 if (occupweek_2>=400 & occupweek_2<500) |(occupweek_2>=40 & occupweek_2<50)
	replace occup_2=5 if (occupweek_2>=500 & occupweek_2<600) |(occupweek_2>=50 & occupweek_2<60)
	replace occup_2=6 if (occupweek_2>=600 & occupweek_2<700) |(occupweek_2>=60 & occupweek_2<70)
	replace occup_2=7 if (occupweek_2>=700 & occupweek_2<800) |(occupweek_2>=70 & occupweek_2<80)
	replace occup_2=8 if (occupweek_2>=800 & occupweek_2<900)|(occupweek_2>=80 & occupweek_2<90)
	replace occup_2=9 if (occupweek_2>=900 & occupweek_2<1000) |(occupweek_2>=90 & occupweek_2<100)
	* Add in cases that had an "X" as other
	gen helper_occup  = regexm(occupation,"X")
	replace occup_2=99 if helper_occup == 1
	replace occup_2=. if lstatus!=1
	label values occup_2 occup
	
	drop helper_occup occupweek_2
	rename occupation usualsubsocc
...
** SOCIAL SECURITY
	gen socialsec=. 
	replace socialsec = 1 if inrange(socsecbenefits,1,7)
	replace socialsec = 0 if socsecbenefits == 8
...
** UNION MEMBERSHIP
	gen union=. 
	replace union = 1 if memberunion== "1"
	replace union = 0 if inlist(memberunion, "0", "2", "3", "4", "5", "X")
	replace union = 0 if inrange(unionactivity,2,9)
...
* DELETE MISSING VARIABLES	local keep ""
	...
	save "`path'\Processed\IND_2004_I2D2_NSS61_SCH10_v01_M_v02_A.dta", replace
