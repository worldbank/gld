...
** YEAR	2004
** SURVEY NAME	NATIONAL SAMPLE SURVEY 60TH ROUND SCHEDULE 10
** SURVEY AGENCY	GOVERNMENT OF INDIA NATIONAL SAMPLE SURVEY ORGANISATION
** SURVEY SOURCE	
** UNIT OF ANALYSIS	Individuals
** INPUT DATABASES Y:\__I2D2\India\2004_NSS60\NSS_SCH10\Original\Data
** RESPONSIBLE	Angelo Santos
** Created	6/29/2021
** Modified NA
** NUMBER OF HOUSEHOLDS	59,159
** NUMBER OF INDIVIDUALS 303,828
** EXPANDED POPULATION	
**                                                                                                  **
******************************************************************************************************
*****************************************************************************************************/
...
** DIRECTORY
	global path "Y:\__I2D2\India\2004_NSS60\NSS_SCH10\Original\Data"
	
	global path_out
"Y:\__I2D2\India\2004_NSS60\NSS_SCH10\Processed"
...
use "$path\Block-5-Members-time-disposition-records", clear
...
* Merge with block 4
gen Key_memb = PID
merge m:1 Key_memb using "$path/Block-4-Demographic-usual- activity-members-records.dta", keep(using match) nogen

* Merge with blocks 1 and 3
merge m:1 Key_hhold using "$path/Block-1-sample-household-identification-records.dta", keep(master match) nogen
merge m:1 Key_hhold using "$path/Block-3-Household-characteristics-records.dta", keep(master match) nogen

* Merge with block 6
gen key_membr = Key_memb
merge 1:1 key_membr using "$path/Block-6-unemployed-persons-records.dta", keep(master match) nogen
drop key_membr PID

* Merge with block 7
preserve
	tempfile block7
	use "$path/Block-7-members-vocational-training-records.dta", clear
	keep key_hhold B7_c1 B7_c4 - B7_c8 B7_c10 -  B7_c15
	
	* Sort such that activity with the longest duration comes first
...
** YEAR OF INTERVIEW
	* All households interviewed in 2004
	gen intv_year=2004
...
** CURRENTLY AT SCHOOL
	* This is not asked in the 2004 NSS 60 survey
	gen atschool=.
...
** EDUCATIONAL LEVEL 1
	...
	gen byte edulevel1 = .
	replace edulevel1= 1 if genedulev<=2 
	replace edulevel1 = 2 if genedulev == 3 & educy < 5 // Primary incomplete
	replace edulevel1 = 3 if genedulev == 4 & educy >= 5  // Primary complete
	replace edulevel1 = 4 if (genedulev == 5 | genedulev == 6) & educy < 12  // Secondary incomplete
	replace edulevel1 = 5 if genedulev == 7 & educy >= 12  // Secondary complete
	replace edulevel1 = 6 if genedulev == 8| genedulev ==11 
	replace edulevel1= 7 if genedulev==10 | genedulev==11
...
** EMPLOYMENT STATUS - SECOND JOB
	gen has_job_primary = inlist(B5_c41,"11", "12", "21", "31", "41", "51") | inlist(B5_c41, "61", "62", "71", "72")
...
** OCCUPATION CLASSIFICATION
	...
	merge m:1 code_68 using "$path/IND_2004_NSS60-SCH10_NCO_68_to_2004_conversion.dta"
...
** SURVEY SPECIFIC OCCUPATION CLASSIFICATION
	destring B5_c20, gen(occup_orig) force
...
** DELETE MISSING VARIABLES
	...
	save "$path_out\IND_2004_I2D2_NSS60_SCH10.dta", replace
	save "Y:\__CURRENT\IND_2004_I2D2_NSS60_SCH10.dta",replace
	log close
