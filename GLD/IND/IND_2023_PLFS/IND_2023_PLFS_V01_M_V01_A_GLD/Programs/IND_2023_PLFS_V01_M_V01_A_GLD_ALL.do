...
<_Program name_>				IND_2023_PLFS_V01_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 18 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2025-09-17 </_Date created_>
...
<_Study ID_>					DDI-IND-CSO-PLFS-2022-23 </_Study ID_>
<_Data collection from_>		07/2023 </_Data collection from_>
<_Data collection to_>			06/2024 </_Data collection to_>
<_Source of dataset_> 			https://microdata.gov.in/NADA/index.php/catalog/213 </_Source of dataset_>
<_Sample size (HH)_> 			101920 </_Sample size (HH)_>
<_Sample size (IND)_> 			418159 </_Sample size (IND)_>
...
use "`path_in_stata'/hhv1.dta", clear
*use "C:\Users\wb637898\Downloads\hhv1.dta", clear
egen str9 hh_key = concat(b1q1_hhv1 b1q13_hhv1 b1q14_hhv1 b1q15_hhv1 qtr_hhv1 visit_hhv1)

tempfile hh_file
save `hh_file'

use "`path_in_stata'/perv1.dta", clear
*use "C:\Users\wb637898\Downloads\perv1.dta", clear

egen str9 hh_key = concat(b1q1_perv1 b1q13_perv1 b1q14_perv1 b1q15_perv1 qtr_perv1 visit_perv1)

merge m:1 hh_key using `hh_file', assert(match)
drop _merge hh_key
...
/*
Some variables have capital letters, in order to facilite the code we run this:
*/
...
*<_int_year_>
	destring b1q9_hhv1, gen(monthnum)
	gen int_year= .
	replace int_year = 2023 if inrange(monthnum,7,12)
	replace int_year = 2024 if inrange(monthnum,1,6)
...
/* <_hhid_note>
	...
	egen hhid = concat(b1q1_hhv1 b1q13_hhv1 b1q14_hhv1 b1q15_hhv1)
...
/* <_pid_note>
	...
	destring b4q1_perv1, replace
	gen indiv_id = string(b4q1_perv1,"%02.0f")
	egen  pid = concat(hhid indiv_id)
	label var pid "Individual ID"
	drop indiv_id
	

	isid pid visit_perv1 qtr_perv1
*</_pid_>
...
/* <_weight_note>
	...
	* Additionally, there are two households with (4 and 5 = ) 9 people
	* w/o weight. Cannot really use their info.

</_weight_note> */
	gen weight = .
	replace weight = (mult_hhv1/no_qtr_hhv1)/100 if nss_hhv1 == nsc_hhv1
	replace weight = (mult_hhv1/no_qtr_hhv1)/200 if nss_hhv1 != nsc_hhv1
...
*<_visit_no_>
	destring visit_perv1, ignore("V") gen(visit_no)
	label var visit_perv1 "Visit number in panel"
*</_visit_no_>
...
*<_subnatid2_>
	destring state_hhv1, replace
	gen hlp_state = string(state_hhv1, "%02.0f")
	destring distcode_hhv1, replace
	gen hlp_dist = string(distcode_hhv1, "%02.0f")
...
*<_age_>
	ren b4q6_perv1 original_age
	tostring original_age, replace
...
*<_male_>
	...
	* Need to reach out to GMD team, update dictionary
	gen male = .
	destring b4q5_perv1, replace
	replace male = 1 if b4q5_perv1 == 1
	replace male = 0 if b4q5_perv1 == 2
...
*<_relationharm_>
	...
	destring b4q4_perv1, replace
	bys hhid visit_perv1: gen one=1 if b4q4_perv1 == 1
	bys hhid visit_perv1: egen temp=count(one)
	tab temp
	*assert `r(r)' == 1
	drop temp one

	gen relationharm = b4q4_perv1
...
*<_relationcs_>
	gen relationcs = b4q4_perv1
	label var relationcs "Relationship to the head of household - Country original"
*</_relationcs_>


*<_marital_>
	* Var marital exists in raw but with name "b4q7_perv1"
	gen marital = b4q7_perv1
...
*<_migrated_from_country_>
	...
	*label de lblmigrated_from_country
	*label values migrated_from_country lblmigrated_from_country
...
*<_school_>
	* Var current_attendance exists in raw but with name "b4q11_perv1"
	gen current_attendance = b4q11_perv1
...
*<_literacy_>
	* Var general_ed exists in raw but with a name "b4q8_perv1"
	gen general_ed = b4q8_perv1
	destring general_ed, replace
	gen byte literacy = .
...
*<_vocational_>
	* Var any_voc_training exists in raw but with name "b4q12_perv1"
	gen any_voc_training = b4q12_perv1
	destring any_voc_training, replace
	gen vocational = .
...
*<_vocational_length_l_>
	* Var duration_train exists in raw but with name "b4pt1q5_perv1"
	gen duration_train = b4pt1q5_perv1
	destring duration_train, replace
...
*<_vocational_field_orig_>
	* Var field_training exists in raw but with name "b4pt1q4_perv1"
	gen field_training = b4pt1q4_perv1
	destring field_training, replace
	sdecode field_training, gen(vocational_field_orig)
	label var vocational_field_orig "Field of training - As in original data" //Names of each field of training available in section Block 4.1 in the 2022-23 Schedule (first visit).
	
*</_vocational_field_orig_>

*<_vocational_financed_>
	gen vocational_financed = .
	gen fund_train = b4pt1q7_perv1
	destring fund_train, replace
...
{
*<_lstatus_>
	* Var cws exists in raw but with name "b6q5_perv1"
	gen cws = b6q5_perv1
	destring cws, replace
...
* JS - Clarification on code 82 - PLFS does include 82 in their unemployment rate estimations, and I believe that's why we also include it so that the numbers match. 
...
*<_occup_isco_>
	gen occup_isco = b6q7_perv1 + "0" if lstatus == 1
...
/* <_wage_no_compen_note>
	...
	* La var of b6q9_3pt2_perv1 is wrong should be:
	la var b6q9_3pt2_perv1 "wage earning for activity 1 on 6 th day"

	egen help_1 = rowtotal(b6q9_3pt1_perv1 b6q9_3pt2_perv1 b6q9_3pt3_perv1 b6q9_3pt4_perv1 b6q9_3pt5_perv1 b6q9_3pt6_perv1 b6q9_3pt7_perv1)

	* Var earnings_salaried exists in raw but with name "b6q9_perv1"
	* Var earnings_selfemp exists in raw but with name "b6q10_perv1"
	gen earnings_salaried = b6q9_perv1
	gen earnings_selfemp = b6q10_perv1
	...
	* There are 25,530 people who have a salary of 0 or negative (17 negative,
	* rest is 0). Of those 23,746 (97%) are helpers in HH enterprise, that is, 
	* they are not really getting a salary (in the classic sense). Set to missing
...
*<_whours_>
	egen help_1 = rowtotal(b6q6_3pt1_perv1 b6q6_3pt2_perv1 b6q6_3pt3_perv1 b6q6_3pt4_perv1 b6q6_3pt5_perv1 b6q6_3pt6_perv1 b6q6_3pt7_perv1)
...
/* <_lstatus_year_note>
	...
	* Var p_status_code exists in raw but with name "b5pt1q3_perv1"
	gen p_status_code = b5pt1q3_perv1
	gen primary_help = p_status_code
	destring primary_help, replace
	recode primary_help  11/72=1 81 82=2 91/98=3 99=.
	* Var s_status_code exists in raw but with name "b5pt2q3_perv1"
	gen s_status_code = b5pt2q3_perv1
	gen secondary_help = s_status_code
	destring secondary_help, replace
...
*<_nlfreason_year_>
	...
	destring nlfreason_year, replace
...
*<_unempldur_l_year_>
	* Var unem_dur exists in raw but with name "b5pt3q9_perv1"
	gen unem_dur = b5pt3q9_perv1
	destring unem_dur, replace
...
*<_ocusec_year_>
	* Var p_ent_type_code exists in raw but with name "b5pt1q9_perv1"
	* Var s_ent_type_code exists in raw but with name "b5pt2q8_perv1"
	gen p_ent_type_code = b5pt1q9_perv1
	gen s_ent_type_code = b5pt2q8_perv1
	destring p_ent_type_code s_ent_type_code, replace
...
*<_industry_orig_year_>
	* Var p_industry_code exists in raw but with name "b5pt1q5_perv1"
	gen p_industry_code = b5pt1q5_perv1
	gen help_1 = p_industry_code
	* Var s_industry_nic_code exists in raw but with name "b5pt2q5_perv1"
	gen s_industry_nic_code = b5pt2q5_perv1
...
*<_occup_orig_year_>
	* Var p_occup_code exists in raw but with name "b5pt1q6_perv1"
	gen p_occup_code = b5pt1q6_perv1
	gen help_1 = p_occup_code
	* Var s_occupation_nco_code exists in raw but with name "b5pt2q6_perv1"
	gen s_occupation_nco_code = b5pt2q6_perv1
...
*</_occup_isco_year_>
	...
	replace occup_isco_year = "" if occup_isco_year == "0"
...
*<_contract_year_>
	* Var p_type_contract exists in raw but with name "b5pt1q11_perv1"
	gen p_type_contract = b5pt1q11_perv1
	destring p_type_contract, replace
	gen help_1 = p_type_contract
	* Var s_type_contract exists in raw but with name "b5pt2q10_perv1"
	gen s_type_contract = b5pt2q10_perv1
	destring s_type_contract, replace
...
*<_socialsec_year_>
	* Var p_social_sec exists in raw but with name "b5pt1q13_perv1"
	gen p_social_sec = b5pt1q13_perv1
	destring p_social_sec, replace
	gen help_1 = p_social_sec
	* Var s_social_sec exists in raw but with name "b5pt2q12_perv1"
	gen s_social_sec = b5pt2q12_perv1
	destring s_social_sec, replace
...
*<_firmsize_l_year_>
	* Var p_no_workrs_ent exists in raw but with name "b5pt1q10_perv1"
	gen p_no_workrs_ent = b5pt1q10_perv1
	destring p_no_workrs_ent, replace
	gen help_1 = p_no_workrs_ent
	* Var s_no_workrs_ent exists in raw but with name "b5pt2q9_perv1"
	gen s_no_workrs_ent = b5pt2q9_perv1
	destring s_no_workrs_ent, replace
...
*<_empstat_2_year_>
	...
	destring empstat_2_year, replace
...
*<_occup_isco_2_year_>
	...
	replace occup_isco_2_year = "" if occup_isco_2_year == "0" 
