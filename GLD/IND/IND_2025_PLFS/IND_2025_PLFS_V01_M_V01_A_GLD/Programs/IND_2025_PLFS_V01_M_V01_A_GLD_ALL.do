...
<_Program name_>				IND_2025_PLFS_V01_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 18 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2026-06-05 </_Date created_>
...
<_Study ID_>					DDI-IND-MOSPI-PLFS-2025 </_Study ID_>
...
<_Sampling method_> 			See IND_2025_PLFS_V01_M/Doc/Technical materials. </_Sampling method_>
...
* Date: 2026-06-05 - Initial 2025 harmonization do-file drafted from 2023 PLFS template.
...
local server  "C:/Users/`c(username)'/WBG/GLD - Focal Point/Countries"
local country	"IND"
local year		"2025"
local survey	"PLFS"
local vermast	"V01"
...
use "`path_in_stata'/IND_2025_PLFS_raw_CPERV1.dta", clear
merge m:1 hhid_svy using "`path_in_stata'/IND_2025_PLFS_raw_CHHV1.dta", assert(3) nogen
rename *, lower
...
*<_int_year_>
	gen int_year = substr(surveydate,-4,.)
...
/* <_hhid_note>
	...
	Uses the unified household ID logic from the 2025 CPERV1 reader:
	sector + FSU + second-stage stratum + household number.

</_hhid_note> */
	gen hhid = string(sector, "%01.0f") + string(fsu, "%05.0f") + string(sss, "%01.0f") + string(hhno, "%02.0f")
...
/* <_pid_note>
	...
	Uses the unified person ID logic from the 2025 CPERV1 reader:
	hhid_uni + person serial number.
...
*<_weight_>
	gen weight = weight_svy
	capture replace weight = mult/100 if missing(weight) & !missing(mult)
...
*<_subnatid2_>
	...
	merge m:1 district_code using "`district_codes'", assert(2 3) keep(3) nogen
...
*<_relationcs_>
	gen relationcs = rel_hh
	label var relationcs "Relationship to the head of household - Country original"
*</_relationcs_>

*<_marital_>
	rename marital marital_orig
	gen marital = .
...
*<_literacy_>
	gen byte literacy = .
	replace literacy = 0 if general_edu_level == 1
	replace literacy = 1 if general_edu_level != 1 & !missing(general_edu_level)
...
*<_educy_>
	gen byte educy = edu_grade
	label var educy "Number of years in formal education"
*</_educy_>

*<_educat7_>
	gen byte educat7 = .
	replace educat7 = 1 if inrange(general_edu_level,1,4)
	replace educat7 = 2 if general_edu_level == 5
	replace educat7 = 3 if general_edu_level == 6
	replace educat7 = 4 if general_edu_level == 7
	replace educat7 = 5 if inrange(general_edu_level,8,10)
	replace educat7 = 6 if general_edu_level == 11
	replace educat7 = 7 if inrange(general_edu_level,12,13)
...
*<_educat_orig_>
	gen educat_orig = general_edu_level
	label var educat_orig "Original survey education code"
*</_educat_orig_>

*<_educat_isced_>
	gen educat_isced = .
	replace educat_isced = 100 if general_edu_level == 6
	replace educat_isced = 244 if inlist(general_edu_level,7,8)
	replace educat_isced = 344 if general_edu_level == 10
	replace educat_isced = 353 if general_edu_level == 11
	replace educat_isced = 660 if general_edu_level == 12
	replace educat_isced = 760 if general_edu_level == 13
...
*<_vocational_field_orig_>
	tostring training_field, gen(vocational_field_orig)
	replace vocational_field_orig = "" if missing(training_field)
...
*<_industry_orig_>
	gen industry_orig = cws_industry
...
*<_occup_orig_>
	gen occup_orig = string(cws_occupation, "%03.0f")
	replace occup_orig = "" if missing(cws_occupation) | lstatus != 1
	label var occup_orig "Original occupation record primary job 7 day recall"
*</_occup_orig_>

*<_occup_isco_>
	gen occup_isco = string(cws_occupation, "%03.0f") + "0" if lstatus == 1 & !missing(cws_occupation)
...
/* <_wage_no_compen_note>
	...
	egen help_1 = rowtotal(wage_act1_day1 wage_act1_day2 wage_act1_day3 wage_act1_day4 wage_act1_day5 wage_act1_day6 wage_act1_day7)
	gen double wage_no_compen = .
	replace wage_no_compen = help_1 if inlist(cws,41,42,51)
	replace wage_no_compen = earnings_sal if inlist(cws,31,71,72)
...
*<_nlfreason_year_>
	* On the basis of the codes in Field Isntructions, not questionnaire
	* Applies only to those who have worked in their lives, not in past 365
	* Rest apply codes
	gen nlfreason_year = .
...
*<_industry_orig_year_>
	gen industry_orig_year = prn_industry
	replace industry_orig_year = sub_industry if lstatus_year == 1 & industry_orig_year == ""
...
*<_occup_orig_year_>
	gen occup_orig_year = string(prn_occupation, "%03.0f")
	replace occup_orig_year = "" if missing(prn_occupation)
	replace occup_orig_year = string(sub_occupation, "%03.0f") if lstatus_year == 1 & occup_orig_year == "" & !missing(sub_occupation)
	replace occup_orig_year = "" if missing(prn_occupation) & missing(sub_occupation)
...
*<_industry_orig_2_year_>
	gen industry_orig_2_year = sub_industry if seconds == 1
...
*<_occup_orig_2_year_>
	gen occup_orig_2_year = string(sub_occupation, "%03.0f") if seconds == 1
	replace occup_orig_2_year = "" if missing(empstat_2_year) | missing(sub_occupation)
