...
<_Program name_>				MNG_2024_LFS_V01_M_V01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 17 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2026-03-30 </_Date created_>
...
<_Sample size (HH)_> 			6,584 unique households in the harmonized file </_Sample size (HH)_>
<_Sample size (IND)_> 			49,654 person-quarter records in the harmonized file </_Sample size (IND)_>
...
use "`path_in_stata'/lfs2024_eng.dta", clear
rename *, upper

* Restore 2022-compatible raw names after importing the SPSS file.
capture rename WEIGHTY_CAL_Y WEIGHT_YEARS
...
The Mongolia LFS is a within-year rotating panel, so the same person can
appear in multiple quarters by design. We keep pid as the household-year ID
plus the within-household line number, then verify uniqueness jointly with
QUARTER. Ineligible roster members are dropped before the uniqueness check.

</_pid_note> */
	drop if A01C == 2
	gen ind_str = string(P01, "%02.0f")
	gen pid = hhid + ind_str
	duplicates tag pid QUARTER, gen(tag_pid_wave)
	drop if tag_pid_wave >= 1 & missing(P01)
	duplicates drop pid QUARTER, force
	isid pid QUARTER
	drop tag_pid_wave
...
*<_subnatid1_>
	sdecode AXCAIMAGCODE, gen(aimag_name)
	gen byte region = .
	replace region = 1 if aimag_name == "Ulaanbaatar"
	replace region = 2 if inlist(aimag_name, "Tuv", "Govisumber", "Selenge", "Dornogovi", "Darkhan-Uul", "Umnugovi", "Dundgovi")
	replace region = 3 if inlist(aimag_name, "Dornod", "Khentii", "Sukhbaatar")
	replace region = 4 if inlist(aimag_name, "Bayan-Ulgii", "Govi-Altai", "Uvs", "Khovd", "Zavkhan")
	replace region = 5 if inlist(aimag_name, "Arkhangai", "Bayankhongor", "Bulgan", "Orkhon", "Uvurkhangai", "Khuvsgul")
	la de lblreg 1 "Ulaanbaatar" 2 "Central" 3 "East" 4 "West" 5 "Highlands"
	label values region lblreg

	sdecode region, gen(region_str)
	gen subnatid1 = string(region) + " - " + region_str
	label var subnatid1 "Subnational ID at First administrative Level"
*</_subnatid1_>


*<_subnatid2_>
	tostring AXCAIMAGCODE, gen(divnum) format(%02.0f)
	gen subnatid2 = divnum + " - " + aimag_name
...
The 2024 LFS raw file identifies the province or capital city directly through
...
*<_marital_>
	gen byte marital = .
	replace marital = 2 if A23 == 1
	replace marital = 1 if A23 == 2
	replace marital = 3 if A23 == 3
	replace marital = 4 if inlist(A23, 4, 5)
	replace marital = 5 if A23 == 6
...
*<_eye_dsablty_>
	gen eye_dsablty = A06
	replace eye_dsablty = . if inlist(A06, 98, 99)
...
*<_hear_dsablty_>
	...
	replace hear_dsablty = . if inlist(A07, 98, 99)
...
*<_walk_dsablty_>
	...
	replace walk_dsablty = . if inlist(A08, 98, 99)
	label define dsablty 1 "No – no difficulty" 2 "Yes – some difficulty" 3 "Yes – a lot of difficulty" 4 "Cannot do at all", replace
...
*<_conc_dsord_>
	gen conc_dsord = A09
	replace conc_dsord = . if inlist(A09, 98, 99)
	label define dsablty 1 "No – no difficulty" 2 "Yes – some difficulty" 3 "Yes – a lot of difficulty" 4 "Cannot do at all", replace
...
*<_slfcre_dsablty_>
	...
	replace slfcre_dsablty = . if inlist(A10, 98, 99)
	label define dsablty 1 "No – no difficulty" 2 "Yes – some difficulty" 3 "Yes – a lot of difficulty" 4 "Cannot do at all", replace
...
*<_comm_dsablty_>
	...
	replace comm_dsablty = . if inlist(A11, 98, 99)
	label define dsablty 1 "No – no difficulty" 2 "Yes – some difficulty" 3 "Yes – a lot of difficulty" 4 "Cannot do at all", replace
...
*<_migrated_from_cat_>
	...
	* Need to create a helper variable that identifies the subnatid1 of the province of previous residence
	gen  subnatid1_pr = ""
	replace subnatid1_pr = "1 - Ulaanbaatar" if A19B == 1
	replace subnatid1_pr = "2 - Central" if inrange(A19B, 41, 48)
	replace subnatid1_pr = "3 - East" if inrange(A19B, 21, 23)
	replace subnatid1_pr = "4 - West" if inrange(A19B, 81, 85)
	replace subnatid1_pr = "5 -  Highlands" if inrange(A19B, 61, 67)
	
	replace migrated_from_cat = 3 if subnatid1_pr == subnatid1 & !missing(A19B)
	replace migrated_from_cat = 4 if subnatid1_pr != subnatid1 & !missing(A19B)
	replace migrated_from_cat = 5 if A19B == 99
...
*<_migrated_reason_>
	...
	recode migrated_reason (1 2 3= 3) (4 5= 2) (6 7 = 1) (9 = 4) (8 10 11 12 13= 5) (99 = 5)
...
- ran or helped in a non-agricultural business in the last 7 days
- was temporarily absent from a job or business but still had job attachment
- did farming, livestock, fishing, or logging activity that the questionnaire
  keeps inside the employed branch
- was hired by someone else to do agricultural work

Unemployed:
- not employed
- looked for paid work or tried to start a business in the last 30 days
- available to start work

Non-labour force:
- everyone else age 15 or older
	...
	* E1 - Worked for pay in the last 7 days.
	replace lstatus = 1 if B04 == 1

	* E2 - Operated or supported a non-agricultural business in the last 7 days.
	replace lstatus = 1 if missing(lstatus) & D01A == 5

	* E3 - Was temporarily absent but still reported job attachment through C06.
	replace lstatus = 1 if missing(lstatus) & C06 == 1

	* E4 - Did market-oriented agricultural, livestock, or fishery activity.
	replace lstatus = 1 if missing(lstatus) & inlist(D02, 1, 2)
	replace lstatus = 1 if missing(lstatus) & inlist(D03, 1)
	replace lstatus = 1 if missing(lstatus) & inlist(D04, 1)
	replace lstatus = 1 if missing(lstatus) & inlist(D07, 1)
	replace lstatus = 1 if missing(lstatus) & inlist(D07A, 1)

	* E5 - Was hired by someone else to do agricultural work.
	replace lstatus = 1 if missing(lstatus) & D08 == 1

	************************
	* Define those unemployed
	************************
	gen byte look = (H01A == 1 | H01B == 1) if !missing(H01A) | !missing(H01B)
	replace lstatus = 2 if missing(lstatus) & look == 1 & H10 == 1

	************************
	* Define those out of the labour force
	************************
...
*<_potential_lf_>
	...
	replace potential_lf = 1 if lstatus == 3 & age >= minlaborage & ((look == 1 & H10 != 1) | (look != 1 & H10 == 1))
	replace potential_lf = 0 if lstatus == 3 & age >= minlaborage & missing(potential_lf)
...
*<_nlfreason_>
	...
	drop nlookreason
	replace nlfreason = 5 if inlist(nlfreason, 99, 9)
...
The 2024 questionnaire no longer uses the earlier paid-vs-unpaid employee
question from prior rounds. Instead, employee and apprentice cases report a
type of payment in E18, where codes 1 to 7 are all forms of remuneration and
code 8 is "none of above (including unpaid)". GLD therefore treats E04 = 3 or
4 with E18 in 1/7 as paid employees, keeps E18 = 8 as non-paid employees, and
still codes E04 = 5 as non-paid family work.

</_empstat_note> */
	gen byte empstat = .
	replace empstat = 4 if inlist(E04, 1, 2)
	replace empstat = 1 if inlist(E04, 3, 4) & inrange(E18, 1, 7)
	replace empstat = 2 if inlist(E04, 3, 4) & E18 == 8
	replace empstat = 2 if E04 == 5
	replace empstat = 3 if empstat == 4 & inlist(E07, 1, 2)
...
*<_ocusec_>
	gen byte ocusec=E24
	recode ocusec (11 20/30 71 72 76/92 = 2) (69 75 = 1) (10 = 4) (13 14 68 = 3) (93 98 = .) 
...
The raw 2024 code list is almost entirely ISIC-4 compliant but still contains
a small number of non-standard detailed values such as 1395 and 8289. To avoid
mixing exact 4-digit and fallback codes, GLD keeps the first two ISIC digits
and appends 00 for all nonmissing main-job industry codes.

</_industrycat_isic_note> */
	gen str4 industrycat_isic = ""
	gen str8 industry_orig_str = subinstr(strtrim(string(industry_orig, "%9.1f")), ".", "", .)
	replace industry_orig_str = "0" + industry_orig_str if !missing(industry_orig) & industry_orig < 1000
	replace industrycat_isic = substr(industry_orig_str, 1, 2) + "00" if !missing(industry_orig) & length(industry_orig_str) >= 2
	replace industrycat_isic = "0100" if missing(industrycat_isic) & inlist(D01B, 1, 2)
	replace industrycat_isic = "0300" if missing(industrycat_isic) & D01B == 3
	replace industrycat_isic = "0200" if missing(industrycat_isic) & D01B == 4
	replace industrycat_isic = "0100" if missing(industrycat_isic) & inlist(D01A, 1, 2) & (B05 == 1 | B06 == 1)
	replace industrycat_isic = "0300" if missing(industrycat_isic) & D01A == 3 & (B05 == 1 | B06 == 1)
	replace industrycat_isic = "0200" if missing(industrycat_isic) & D01A == 4 & (B05 == 1 | B06 == 1)
	replace industrycat_isic = "" if lstatus != 1
	drop industry_orig_str
...
The questionnaire identifies occupation as ISCO-08, but the raw 2024 code list
contains several non-standard detailed values such as 0111, 0314, and 9319.
...
family level plus 00 for all nonmissing main-job occupations.
	...
	gen str4 occup_isco = ""
	gen str8 occup_orig_str = subinstr(strtrim(string(occup_orig, "%9.2f")), ".", "", .)
	replace occup_isco = substr(occup_orig_str, 1, 2) + "00" if !missing(occup_orig) & length(occup_orig_str) >= 2
...
*<_occup_>
	gen byte occup = .
	gen str8 occup_orig_str = subinstr(strtrim(string(occup_orig, "%9.2f")), ".", "", .)
	replace occup = real(substr(occup_orig_str, 1, 1)) if !missing(occup_orig) & length(occup_orig_str) >= 1
	replace occup = 10 if occup_orig == 0
	replace occup = 99 if occup_orig >= 9630 & real(substr(occup_orig_str, 1, 1)) == 9
...
The questionnaire asks for the most recent cash payment amount, but the raw
file stores that amount in thousands of tugriks. GLD converts positive values
to tugriks by multiplying by 1,000 and leaves zero values missing because they
correspond to non-cash or unpaid cases that should not be treated as a cash wage.
