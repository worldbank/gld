...
<_Program name_>				MNG_2022_LFS_V02_M_V01_A_GLD_ALL.do </_Program name_>
...
<_Sample size (HH)_> 			6,957 unique households in the harmonized file </_Sample size (HH)_>
<_Sample size (IND)_> 			52,195 person-quarter records in the harmonized file </_Sample size (IND)_>
...
- Added a raw-folder conversion program in V02_M/Programs and updated harmonization to load `LFS_2022_eng.dta` from V02_M/Data/Stata.
- Compared V01_M and V02_M raw files: observation count is unchanged at 52,199, while the variable count falls from 218 to 207.
- V02_M standardizes geography and weight fields from `AXCLOCATION`, `WEIGHT_Q`, and `WEIGHT_YEARS` to `AXCAIMAGCODE`, `AXCSOUMCODE`, `LOCATION`, `WEIGHT_CAL_Q`, and `WEIGHT_CAL_Y`, and drops the repeated visit-date fields (`V2_*`, `V3_*`, `V4_*`).
- The V02_M delivery is closer to the 2023 standardized structure, so the setup block only needs the same alias step used in 2023 to restore `AXCLOCATION`, `WEIGHT_Q`, and `WEIGHT_YEARS`.
...
use "`path_in_stata'/LFS_2022_eng.dta", clear
rename *, upper
...
The 2022 V02_M raw file identifies the province or capital city directly through
...
*<_empstat_>
	...
	replace empstat = 1 if inlist(E04, 3, 4) & E11 == 1
	replace empstat = 2 if inlist(E04, 3, 4) & E11 == 2
...
The raw 2022 V02_M code list is almost entirely ISIC-4 compliant but still contains
...
The questionnaire identifies occupation as ISCO-08, but the raw 2022 V02_M code list
