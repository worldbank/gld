...
<_Program name_>				TJK_2023_HBS_V01_M_V02_A_GLD_ALL.do </_Program name_>
...
use "`path_in_stata'/tjk_hbs_2023_H12_20231231_M01_roster.dta", clear
drop _merge
rename (quarter) (quarter_str)
gen quarter = real(substr(quarter_str, 2, .))

merge m:1 quarter sid using "`path_in_stata'/tjk_hbs_2023_id.dta"
keep if inlist(_merge,1,3) 
drop _merge
rename (quarter quarter_str)(quarter_str quarter)
...
/*
    ...
    -----------------------------------------
    Not matched                         1,113
        from master                       954  (_merge==1)
        from using                        159  (_merge==2)

    Matched                            67,680  (_merge==3)
    -----------------------------------------


*/

merge 1:1 quarter sid fmid using "`path_in_stata'/tjk_hbs_2023_H12_20231231_M02_migration.dta"
keep if _merge == 3
drop _merge
...
/*
    ...
    -----------------------------------------
    Not matched                             0
    Matched                            68,634  (_merge==3)
    -----------------------------------------

*/

merge 1:1 quarter sid fmid using "`path_in_stata'/tjk_hbs_2023_H12_20231231_M03_education.dta"
keep if _merge == 3
drop _merge
gen qq = quarter_str
...
merge 1:1 qq sid fmid using "`path_in_stata'/tjk_hbs_2023_20231231_m05_employment.dta"
keep if inlist(_merge,1,3) 
drop _merge
...
/* <_hhid_note>
	...
	drop if missing(hbs_uuid) //they dont have subnat, labour info, 101 cases
	rename hhid hhid_aux
	tostring hbs_uuid, gen(hhid_a) format(%12.0f)
	tostring hhsize, gen(hhsize_str) format(%02.0f)
	
	gen wave = "Q"+string(qq)
	egen hhid = concat(hhid_a hhsize_str wave)
...
*<_weight_>
	gen hhweight_aux = hhweight
	gen total_yr = _N
	bys quarter : gen total_q = _N
	gen weight = (hhweight_aux)*(total_q/total_yr)
...
/* <_subnatid1_note>
	...
	gen str subnatid1 = ""
	replace subnatid1 = "501 - Dushanbe city" if province == 501
	replace subnatid1 = "505 - Sogd oblast" if province == 505
	replace subnatid1 = "507 - Khatlon oblast" if province == 507
	replace subnatid1 = "509 - RRS" if province == 509
	replace subnatid1 = "590 - GBAO" if province == 590
	label var subnatid1 "Subnational ID at First Administrative Level"
*</_subnatid1_>


*<_subnatid2_>
	tostring district, gen(subnatid2_num)
	decode district, gen(subnatid2_name)
	egen subnatid2 = concat(subnatid2_num subnatid2_name), punct(" - ")
	replace subnatid2 = "" if subnatid2 == ". -"
...
*<_relationharm_>
	...
	bys hhid : egen count_heads = total(heads)
	
	gen relationharm_aux = relation
	recode relationharm_aux (4 = 3) (5 6 = 4) (7 8 9 10 11 = 5) (12 = 6)
	replace relationharm = relationharm_aux if missing(relationharm) & count_heads == 0
	
	drop heads count_heads
	*no head
	gen heads = 1 if relationharm == 1
	bys hhid : egen count_heads = total(heads)
	
	*spuse as head
	bys hhid: replace relationharm = 1 if relationharm == 2 & count_heads == 0
	...
	*older as head 
	...
	bys hhid: replace relationharm = 1 if oldest == 1 & count_heads == 0
	bys hhid: replace relationharm = 5 if oldest == . & count_heads == 0
...
*<_migrated_years_>
	gen migrated_years = 2023 - real(substr(mga4, 1, 4))
...
*<_migrated_from_cat_>
	...
	replace migrated_from_cat = 5 if !inlist(mga2a,1,.)
	replace migrated_from_cat = 6 if inlist(mga2a,1)
...
*<_migrated_from_country_>
	...
	replace migrated_from_country = "KAZ" if mga2a == 3
	replace migrated_from_country = "KGZ" if mga2a == 4
	replace migrated_from_country = "RUS" if mga2a == 5
	replace migrated_from_country = "TKM" if mga2a == 6
	replace migrated_from_country = "UZB" if mga2a == 7
	replace migrated_from_country = "ALB" if mga2a == 8
	replace migrated_from_country = "DZA" if mga2a == 9
	replace migrated_from_country = "DEU" if mga2a == 71
...
*<_literacy_>
	gen byte literacy = inlist(ed1,1,2) & inlist(ed2,1,2)
...
*<_educy_>
	gen byte educy = ed5
	replace educy = . if inrange(ed5,30,40)
	replace educy = 0 if ed3 == 2
...
*<_educat7_>
	gen byte educat7 = .
	replace educat7 = 1 if ed4 == 0 | ed3 == 2
	replace educat7 = 2 if ed7a == 2
	replace educat7 = 3 if (ed4 == 1 & school == 0) | (ed4 == 1 & ed7a == 2)
	replace educat7 = 4 if (ed4 == 2 & school == 0) | (inlist(ed4,1,2) & ed7a == 3) 
	replace educat7 = 5 if (inlist(ed4,3,4) & school == 0) | (inlist(ed4,2,3,4) & inlist(ed7a,4,5))
	replace educat7 = 6 if (ed4 == 5 & school == 0) | (inlist(ed4,3,4,5) & ed7a == 6) 
	replace educat7 = 7 if inlist(ed4,6,7) | inlist(ed7a,7,8)
...
*<_industry_orig_>
	decode emp_industry , gen(industry_orig)
...
*<_industrycat_isic_>
	...
	replace industrycat_isic = char(64 + emp_industry) if inrange(emp_industry, 1, 21)
	replace industrycat_isic = "" if emp_industry == 96
...
*<_industrycat10_>
	gen industrycat10 = .
	replace industrycat10 = 1 if emp_industry == 1
	replace industrycat10 = 2 if emp_industry == 2
	replace industrycat10 = 3 if emp_industry == 3
	replace industrycat10 = 4 if inrange(emp_industry,4,5)
	replace industrycat10 = 5 if emp_industry == 6
	replace industrycat10 = 6 if inrange(emp_industry,7,8)
	replace industrycat10 = 7 if inrange(emp_industry,9,10)
	replace industrycat10 = 8 if inrange(emp_industry,11,14)
	replace industrycat10 = 9 if emp_industry == 15
	replace industrycat10 = 10 if inrange(emp_industry,16,96)
...
*<_occup_orig_>
	decode emp_occupation, gen(occup_orig)
...
*<_occup_isco_>
	gen str4 occup_isco = substr(string(emp_occupation) + "0000", 1, 4)
	replace occup_isco = "" if occup_isco == ".000" 
	replace occup_isco= "0110" if occup_isco == "9010" 
	replace occup_isco= "0210" if occup_isco == "9020" 
	replace occup_isco= "0310" if occup_isco == "9030" 
