...
tempfile edu
datalibweb, coun(GEO) y(2016) t(ECARAW) sur(GEO_2016_HIS_v02_M) filen(tblshinda02.dta)
rename * edu_*
rename (edu_uid  edu_memberno) (hhid pid)
sort hhid pid
save "`edu'"

tempfile labor
datalibweb, coun(GEO) y(2016) t(ECARAW) sur(GEO_2016_HIS_v02_M) filen(tblshinda05_1.dta)
rename * lab_*
rename (lab_UID lab_MEMBERNO) (hhid pid)
sort hhid pid
save "`labor'"

tempfile income
datalibweb, coun(GEO) y(2016) t(ECARAW) sur(GEO_2016_HIS_v02_M) filen(tblincomes.dta)
rename * inc_*
rename (inc_uid ) (hhid )
sort hhid
save "`income'"
...
tempfile hh
datalibweb, coun(GEO) y(2016) t(ECARAW) sur(GEO_2016_HIS_v02_M) filen(tblshinda01.dta)
rename * hh_*
rename (hh_uid ) (hhid )
sort hhid
save "`hh'"

datalibweb, country(GEO) year(2016) type(ecapov-03) module(2 3 7 9) clear
destring hhid pid, replace
sort hhid pid
merge 1:1 hhid pid using "`edu'", assert(match) nogen
	
sort hhid pid
merge 1:1 hhid pid using "`labor'", assert(match master) nogen

sort hhid pid
merge m:1 hhid using "`hh'", assert(match) nogen

sort hhid pid
merge m:1 hhid using "`income'", assert(match) nogen

* Drop all vars that are all missing
count
local all_rows = `r(N)'
...
*<_migrated_binary_>
	gen migrated_binary = inrange(edu_yearsinthiscity, 2, 5)
...
*<_migrated_from_cat_>
	...
	replace migrated_from_cat = 5 if edu_migration == 1
	replace migrated_from_cat = 4 if edu_migration == 2
	replace migrated_from_cat = 6 if edu_migration == 3
...
*<_educat7_>
	cap drop educat7
	gen byte educat7 = .
	replace educat7 = 1 if inrange(edu_education, 1, 2)  
	replace educat7 = 3 if inrange(edu_education, 3, 3)
	replace educat7 = 4 if inrange(edu_education, 4, 4)
	replace educat7 = 5 if inrange(edu_education, 5, 7)
	replace educat7 = 6 if inrange(edu_education, 8, 8)
	replace educat7 = 6 if inrange(edu_education, 9, 11)  
...
*<_educat_orig_>
	cap drop educat_orig
	gen educat_orig = edu_education
	label var educat_orig "Original survey education code"
*</_educat_orig_>


*<_educat_isced_>
	cap drop educat_isced
	gen educat_isced = .
...
*<_lstatus_>
	gen byte lstatus = .
	replace lstatus = 1 if !mi(lab_Q25)
	gen passive = (lab_Q52 == 1)
	gen active = inrange(lab_Q48, 1, 11)
	gen found_job = (lab_Q51 == 1)
...
*<_nlfreason_>
	cap drop nlfreason
	gen byte nlfreason = .
	replace nlfreason = 1 if  lab_Q51 == 7 |lab_Q53 == 1
	replace nlfreason = 2 if  lab_Q51 == 10 
	replace nlfreason = 3 if  lab_Q51 == 8
	replace nlfreason = 4 if  lab_Q51 == 9 
...
*<_ocusec_>
	gen byte ocusec = .
	replace ocusec = 1 if lab_Q29 == 1
	replace ocusec = 2 if lab_Q29 == 2
...
/* <_wage_no_compen_note>
	...
	* compare occup) in the three month and 7 recall periods
	gen compare_ind_recalls = (lab_Q5 == lab_Q27)
...
*<_whours_>
	gen whours = .
	replace whours = 20 if lab_Q34 == 1
	replace whours = 35 if lab_Q34 == 2
	replace whours = 48 if lab_Q34 == 3
	replace whours = 60 if lab_Q34 == 4
