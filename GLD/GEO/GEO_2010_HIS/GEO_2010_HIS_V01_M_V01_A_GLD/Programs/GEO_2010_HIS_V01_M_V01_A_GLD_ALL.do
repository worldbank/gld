...
<_Study ID_>					GEO_2010_HIS_V01_M </_Study ID_>
...
tempfile edu
use "`path_in_stata'/tblshinda02_extracted.dta", clear //using the file dowloaded from GEO offical 
*datalibweb, coun(GEO) y(2010) t(ECARAW) sur(GEO_2010_HIS_v03_M) filen(tblshinda02.dta)
...
*datalibweb, coun(GEO) y(2010) t(ECARAW) sur(GEO_2010_HIS_v03_M) filen(tblshinda05_1_2010.dta)
rename (uid memberno) (hhid pid)
sort hhid pid
save "`labor'"
...
*datalibweb, coun(GEO) y(2010) t(ECARAW) sur(GEO_2010_HIS_v03_M) filen(tblincomes.dta)
rename (uid) (hhid)
...
*datalibweb, coun(GEO) y(2010) t(ECARAW) sur(GEO_2010_HIS_v03_M) filen(tblshinda01.dta)
rename (uid ) (hhid )
sort hhid
save "`roster'"

use "`path_in_stata'/ecapov-03.dta", clear
*datalibweb, country(GEO) year(2010) type(ecapov-03) module(2 3 7 9) clear
destring hhid pid, replace

* The raw data has 315K observations, ecapov has 79K (something is amiss)
merge 1:1 hhid pid using "`edu'", keep(match) nogen
merge 1:1 hhid pid using "`labor'", keep(master match) nogen
merge m:1 hhid using "`roster'", keep(match) nogen
merge m:1 hhid using "`income'", keep(match) nogen
* Keep ecapov level, matches GMD.
...
*<_educat7_>
	*educat7 already exists in raw data is missing in raw data, use Education var
	cap drop educat7
	gen byte educat7 = . 
	replace educat7 = 1 if education == 7
	*replace educat7 = 2 if education == 2
	replace educat7 = 3 if education == 1
	replace educat7 = 4 if education == 2
	replace educat7 = 5 if education == 3 | education ==  4
	replace educat7 = 7 if education == 6
...
*<_lstatus_>
	gen byte lstatus = .
	replace lstatus = 1 if !mi(v25)
	gen passive = v52 == 1
	gen active = inrange(v48, 1, 11)
	gen found_job = v51 == 1
...
*<_nlfreason_>
	cap drop nlfreason
	gen byte nlfreason = .
	replace nlfreason = 1 if  v51 == 7 |v53 == 1
	replace nlfreason = 2 if  v51 == 10 
	replace nlfreason = 3 if  v51 == 8
	replace nlfreason = 4 if  v51 == 9 
...
{
*<_empstat_>
	gen byte empstat = v30 if lstatus == 1
...
*<_ocusec_>
	gen byte ocusec = .
	replace ocusec = 1 if v29 == 1
	replace ocusec = 2 if v29 == 2
...
*<_industry_orig_>
	gen industry_orig = v27 if lstatus == 1
...
*<_industrycat_isic_>
	* First reduce orig to two digits
	gen industry_hlpr = floor(industry_orig/100) if lstatus == 1
...
*<_occup_orig_>
	gen occup_orig = v28 if lstatus == 1
...
*<_occup_>
	gen byte occup = floor(occup_orig/1000) if lstatus == 1
...
/* <_wage_no_compen_note>
	...
	* compare occup) in the three month and 7 recall periods
	gen compare_ind_recalls = (v5 == v27)
...
*<_whours_>
	gen whours = .
	replace whours = 20 if v34 == 1
	replace whours = 35 if v34 == 2
	replace whours = 48 if v34 == 3
	replace whours = 60 if v34 == 4
