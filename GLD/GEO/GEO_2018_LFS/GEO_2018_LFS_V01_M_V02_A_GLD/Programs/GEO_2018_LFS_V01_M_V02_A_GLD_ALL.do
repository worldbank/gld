...
/* -----------------------------------------------------------------------
<_Program name_>				GEO_2018_LFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata SE 18 <_Application_>
<_Author(s)_>					Wolrd Bank Job's Group </_Author(s)_>
<_Date created_>				2025-04-04 </_Date created_>
-------------------------------------------------------------------------
...
* Date: 2025-04-04 - Update data assembly, hhid, pid and weight variables
...
use "`path_in_stata'/2017-2019_LFS_Old_Standards.dta", clear
rename (UID_Random DiaryID_Random) (UID DiaryID)
keep if Year == 2018
tostring UID, replace
tempfile old_standard
save `old_standard'

use "`path_in_stata'/LFS_Demographics.dta", clear

merge 1:1 UID MemberNo using "`path_in_stata'/LFS_ECSTAT.dta",           assert(match master) nogen
merge 1:1 UID MemberNo using "`old_standard'",                           assert(match master) nogen
merge 1:1 UID MemberNo using "`path_in_stata'/LFS_Personal_Weights.dta", assert(match master) nogen
* ID should be assert(match) but has HHs that are not in Demogrpahics.
* The fact that LFS_ID has exactly 5,952 HHs per quarter while the demographics
* data varies makes me believe, this is data from the sampling frame, includes
* HHS that could not or did not want to be interviewed. Keep only matches.
merge m:1 UID          using "`path_in_stata'/LFS_ID.dta",               assert(match using) keep(match) nogen
* Family size has 6 instances (2 households) that don't match. This seems to come
* from 2 entries in the LFS_Familysize file that have UID missing (and are
* dropped when converting from SAV to DTA). This is not an issue for GLD as we
* don't use the variables, but just to inform any potential user. This should
* be assert match, but is assert match master.
merge m:1 UID          using "`path_in_stata'/LFS_Familysize.dta",       assert(match master) nogen
...
/*<_hhid_note_>
	...
	* Additionally, Diary ID is only in a file for 15+, hence we need to apply it
	* to <15 (and check it is OK)

*<_hhid_note_>*/
	gen helper_hhid_0 = DiaryID
	bys UID : egen diary_sd = sd(DiaryID)
	summ diary_sd
	preserve
	...
	bys UID : egen diary_med = median(DiaryID)
	replace helper_hhid_0 = diary_med if mi(helper_hhid_0)
	
	tostring helper_hhid_0, gen(helper_hhid_1) format(%05.0f)
	tostring QuarterNo, replace
	gen helper_hhid_2 = "0" + QuarterNo
...
*<_weight_>
	...
	* Estimated population for 2018 thus is 3,761,556.
	* Population for 2018 as on GeoStat is  3,729,600. Tracks.
*</_weight_>
...
*<_wave_>
	gen wave=.
	replace wave=1 if QuarterNo == "87"
	replace wave=2 if QuarterNo == "88"
	replace wave=3 if QuarterNo == "89"
	replace wave=4 if QuarterNo == "90"
...
*<_migrated_from_code_>
	...
	replace migrated_from_code = "11 - Tbilisi" 						  if Prev_Region == 11
	replace migrated_from_code = "12 - Abkhazia" 						  if Prev_Region == 12
	replace migrated_from_code = "15 - Adjara" 							  if Prev_Region == 15
	replace migrated_from_code = "23 - Guria" 							  if Prev_Region == 23
	replace migrated_from_code = "26 - Imereti" 					      if Prev_Region == 26
	replace migrated_from_code = "29 - Kakheti" 						  if Prev_Region == 29
	replace migrated_from_code = "32 - Mtskheta-Mtianeti" 				  if Prev_Region == 32
	replace migrated_from_code = "35 - Racha-Lechkhumi and Kvemo Svaneti" if Prev_Region == 35
	replace migrated_from_code = "38 - Samegrelo-Zemo Svaneti" 			  if Prev_Region == 38
	replace migrated_from_code = "41 - Samtskhe-Javaxeti" 				  if Prev_Region == 41
	replace migrated_from_code = "44 - Kvemo Kartli" 					  if Prev_Region == 44
	replace migrated_from_code = "47 - Shida Kartli" 					  if Prev_Region == 47
...
- 11,771 observations out of 15,050 paid observations answered B17_2 with a specific
number of their income, which gives 78.2% of paid employees values for wage. 
- 3,251 observations out of 15,050 paid observations have missing values for B17_2
whereas 28 observations answered B17_2 zero. 
...
save "Y:/GLD-Harmonization/529026_MG/Countries/GEO/GEO_2018_LFS/GEO_2018_LFS_V01_M_V02_A_GLD/Data/Harmonized/`level_2_harm'_ALL.dta", replace
