...
<_Study ID_>					GEO_2011_HIS_V01_M </_Study ID_>
...
*datalibweb, coun(GEO) y(2011) t(ECARAW) sur(GEO_2011_HIS_v02_M) filen(tblshinda02.dta)
...
*datalibweb, coun(GEO) y(2011) t(ECARAW) sur(GEO_2011_HIS_v02_M) filen(tblshinda05_1.dta)
rename (uid memberno) (hhid pid)
sort hhid pid
save "`labor'"
...
*datalibweb, coun(GEO) y(2011) t(ECARAW) sur(GEO_2011_HIS_v02_M) filen(tblincomes.dta)
rename (uid) (hhid)
...
*datalibweb, coun(GEO) y(2011) t(ECARAW) sur(GEO_2011_HIS_v02_M) filen(tblshinda01.dta)
rename (uid ) (hhid )
sort hhid
save "`roster'"

use "`path_in_stata'/ecapov-03.dta", clear
*datalibweb, country(GEO) year(2011) type(ecapov-03) module(2 3 7 9) clear
...
* Two odd from using, has hhid, nothing else (for both cases below)
...
*<_lstatus_>
	gen byte lstatus = .
	replace lstatus = 1 if !mi(_25)
	gen passive = _52 == 1
	gen active = inrange(_48, 1, 11)
	gen found_job = _51 == 1
...
*<_nlfreason_>
	cap drop nlfreason
	gen byte nlfreason = .
	replace nlfreason = 1 if  _51 == 7 |_53 == 1
	replace nlfreason = 2 if  _51 == 10 
	replace nlfreason = 3 if  _51 == 8
	replace nlfreason = 4 if  _51 == 9 
...
{
*<_empstat_>
	gen byte empstat = _30 if lstatus == 1
...
*<_ocusec_>
	gen byte ocusec = .
	replace ocusec = 1 if _29 == 1
	replace ocusec = 2 if _29 == 2
...
*<_industry_orig_>
	gen industry_orig = _27 if lstatus == 1
...
*<_occup_orig_>
	gen occup_orig = _28 if lstatus == 1
...
/* <_wage_no_compen_note>
	...
	* compare occup) in the three month and 7 recall periods
	gen compare_ind_recalls = (_5 == _27)
...
*<_whours_>
	gen whours = .
	replace whours = 20 if _34 == 1
	replace whours = 35 if _34 == 2
	replace whours = 48 if _34 == 3
	replace whours = 60 if _34 == 4
