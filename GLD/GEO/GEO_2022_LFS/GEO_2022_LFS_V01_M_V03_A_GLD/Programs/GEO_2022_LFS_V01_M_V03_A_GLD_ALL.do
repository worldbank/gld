...
/* -----------------------------------------------------------------------
<_Program name_>				GEO_2022_LFS_V01_M_V03_A_GLD_ALL.do </_Program name_>
...
use "`path_in_stata'/LFS_Demographic_ENG_2022.dta", clear
merge 1:1 UID MemberNo using "`path_in_stata'/LFS_ECSTAT_ENG_2022.dta",   assert(match master) nogen
merge 1:1 UID MemberNo using "`path_in_stata'/GEO_2022_LFS_Rotation.dta", assert(match master) keepusing(Enumaration_Area) nogen
...
*<_weight_>
	...
	* Estimated population for 2022 thus is 3,652,985.
	* Population for 2022 as on GeoStat is  3,688'600. Tracks. 
*</_weight_>
...
*<_wave_>
	gen wave=.
	replace wave=1 if QuarterNo == 103
	replace wave=2 if QuarterNo == 104
	replace wave=3 if QuarterNo == 105
	replace wave=4 if QuarterNo == 106
...
and being available to start work, there are 164 fewer unemployed observations
than in the raw "Unemployed" variable. All 164 of these cases are available to
...
"Unemployed" variable directly for 2022.
