...
/* -----------------------------------------------------------------------
<_Program name_>				GEO_2021_LFS_V01_M_V03_A_GLD_ALL.do </_Program name_>
...
* Date: 2025-03-28 - Update data assembly, hhid, pid and weight variables
...
* For the GEOSTAT shared New Standards file keep 2021 only 
use "`path_in_stata'/2020-2022_LFS_New_Standards.dta", clear
rename (UID_Random DiaryID_Random) (UID DiaryID)
keep if Year == 2021
tempfile new_standard
save `new_standard'

use "`path_in_stata'/LFS_Demographic_ENG_2021.dta", clear
merge 1:1 UID MemberNo using "`path_in_stata'/LFS_ECSTAT_ENG_2021.dta",   assert(match master) nogen
merge 1:1 UID MemberNo using "`path_in_stata'/GEO_2021_LFS_Rotation.dta", assert(match master) keepusing(Enumaration_Area) nogen
merge 1:1 UID MemberNo using "`new_standard'",                            assert(match master) nogen
...
*<_weight_>
	...
	* Estimated population for 2021 thus is 3,682,040.
	* Population for 2021 as on GeoStat is  3,728,600. Tracks. 
*</_weight_>
...
*<_psu_>
	* Enumeration Area information is only in the 15+ age file, but equal within
	* the household 
	
	bys hhid : egen ea_sd = sd(Enumaration_Area)
	preserve
		bys hhid : keep if _n == 1
		summ ea_sd, d
	restore
	
	gen hlpr_ea = Enumaration_Area
	bys hhid : egen hlpr_med_ea = median(Enumaration_Area)
	replace hlpr_ea = hlpr_med_ea if mi(hlpr_ea)
...
Regarding unemployed, the received raw file retains the derived variable
"Unemployed" but does not retain G1, the question that identifies future starters.
G1 is important because respondents who already found a job and are available to
start can be classified as unemployed under the ILO definition even though they
are skipped out of the G3 job-search methods block.
...
When unemployment is reconstructed only as having at least one G3 search method
and being available to start work, there are 167 fewer unemployed observations
than in the raw "Unemployed" variable. All 167 of these cases are available to
start work but report no G3 search method, which is consistent with future
starters being skipped out of G3. Therefore, lstatus uses the received derived
"Unemployed" variable directly for 2021.
...
*<_lstatus_note_>*/
	...
	replace lstatus=2 if Unemployed==1&lstatus!=1
...
- 10,831 observations out of 15,324 paid observations answered B24_2 with a specific
number of their income, which gives 70.68% of paid employees values for wage. 
- 4,466 observations out of 15,324 paid observations have missing values for B24_2
whereas 33 observations answered B24_2 zero. 
