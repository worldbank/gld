...
use "`path_in_stata'/ZM_2024_LFS.dta", clear
...
*<_weight_>
	gen weight = annual_final_weight
...
*<_migrated_binary_>
	...
	replace migrated_binary = 0 if j3_a == 2 & j2_c == 2
...
*<_migrated_years_>
	gen domestic_move_year = year - j3_c_year
	gen international_move_year = year - j4_a_year 
	replace international_move_year = . if j4_a_year ==9
	gen migrated_years = min(domestic_move_year, international_move_year)
...
*<_migrated_from_cat_>
	gen byte from_prov = floor(j4_c_distict / 100) if !missing(j4_c_distict)
...
*<_migrated_from_code_>
	...
	replace migrated_from_code = string(j4_c_distict) if j2_c == 1 & !missing(j4_c_distict)
	replace migrated_from_code = "" if j3_a == 1
...
*<_migrated_from_country_>
	...
	* Assign country ISO codes based on raw numeric country codes in j5_a_country
	* Cross-border migrants only: j3_a=1 indicates moved to Zambia from another country in past 12 months
	replace migrated_from_country = "ALB" if j3_a == 1 & j5_a_country == 2      // Albania
	replace migrated_from_country = "AGO" if j3_a == 1 & j5_a_country == 5      // Angola
	replace migrated_from_country = "AUS" if j3_a == 1 & j5_a_country == 9      // Australia
	replace migrated_from_country = "BGD" if j3_a == 1 & j5_a_country == 14     // Bangladesh
	replace migrated_from_country = "BDI" if j3_a == 1 & j5_a_country == 29     // Burundi
	replace migrated_from_country = "COG" if j3_a == 1 & j5_a_country == 42     // Congo (Brazzaville)
	replace migrated_from_country = "COD" if j3_a == 1 & j5_a_country == 43     // Democratic Republic of the Congo
	replace migrated_from_country = "IND" if j3_a == 1 & j5_a_country == 82    // India
	replace migrated_from_country = "KEN" if j3_a == 1 & j5_a_country == 95     // Kenya
	replace migrated_from_country = "LBN" if j3_a == 1 & j5_a_country == 102    // Lebanon
	replace migrated_from_country = "MWI" if j3_a == 1 & j5_a_country == 110    // Malawi
	replace migrated_from_country = "NAM" if j3_a == 1 & j5_a_country == 123    // Namibia
	replace migrated_from_country = "NOR" if j3_a == 1 & j5_a_country == 131    // Norway
	replace migrated_from_country = "RWA" if j3_a == 1 & j5_a_country == 145     // Rwanda
	replace migrated_from_country = "ZAF" if j3_a == 1 & j5_a_country == 159     // South Africa
	replace migrated_from_country = "TZA" if j3_a == 1 & j5_a_country == 169    // Tanzania
	replace migrated_from_country = "ZWE" if j3_a == 1 & j5_a_country == 190    // Zimbabwe
...
*<_migrated_reason_>
	gen migrated_reason = .
	* 3 = Employment: work & income motives
	replace migrated_reason = 3 if j3_a == 1 & (j6_a == 1 | j6_a == 2)
	replace migrated_reason = 3 if j2_c == 1 & (j5_c == 1 | j5_c == 2)
	* 2 = Education: school/training (internal only)
	replace migrated_reason = 2 if j2_c == 1 & j5_c == 8
	* 1 = Family: follow family, marriage
	replace migrated_reason = 1 if j3_a == 1 & (j6_a == 6 | j6_a == 7)
	replace migrated_reason = 1 if j2_c == 1 & (j5_c == 6 | j5_c == 7)
	* 4 = Forced: eviction, refugee, displacement, health shock
	replace migrated_reason = 4 if j3_a == 1 & (j6_a == 4 | j6_a == 11 | j6_a == 14 | j6_a == 15)
	replace migrated_reason = 4 if j2_c == 1 & (j5_c == 9 | j5_c == 12)
	* 5 = Other unspecified motives
	replace migrated_reason = 5 if j3_a == 1 & j6_a == 16
	* NA case: keep missing
	replace migrated_reason = . if j3_a == 1 & j6_a == 99
...
*<_occup_isco_>
	...
	replace occup_isco = "4130" if occup_isco == "4133"
