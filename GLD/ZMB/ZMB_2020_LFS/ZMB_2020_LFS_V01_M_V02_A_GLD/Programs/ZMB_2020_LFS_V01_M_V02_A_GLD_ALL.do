...
<_Program name_>				[ZMB_2020_LFS_V01_M_V02_A] </_Program name_>
...
* Date: [2026-07-24] - [ZMB_2020_LFS_V01_M_V02_A: Update to new template； add migration]
...
use "`path_in_stata'/2020LFS.dta", clear
...
*<_migrated_binary_>
	...
	* international migration(j3_a=1) OR domestic cross-district migration(j2_b=1) = migrant
	replace migrated_binary = 1 if (j3_a == 1) | (j2_b == 1)
	replace migrated_binary = . if missing(j3_a) & missing(j2_b)
...
*<_migrated_years_>
	...
	* calculate years since latest international migration
	replace migrated_years = 2020 - j4_a_year if migrated_binary == 1 & j3_a == 1 & !missing(j4_a_year)
	* calculate years since latest domestic district migration
	replace migrated_years = 2020 - j3_b_year if migrated_binary == 1 & j2_b == 1 & !missing(j3_b_year)
...
*<_migrated_from_cat_>
	...
	* judge domestic migration belong to same province or different province
	tempvar old_prov_code
	gen `old_prov_code' = floor(j4_b_distict / 100) if migrated_binary == 1 & j2_b == 1 & !missing(j4_b_distict)
...
*<_migrated_from_code_>
	...
	* store original district code for internal migrants
	replace migrated_from_code = string(j4_b_distict, "%04.0f") if migrated_binary == 1 & j2_b == 1 & !missing(j4_b_distict)
...
*<_migrated_from_country_>
	...
	* map numeric country code j5_a_country to ISO3 standard code with English comment
	...
	replace migrated_from_country = "VIR" if j5_a_country == 182       // United States Virgin Islands
...
*<_migrated_reason_>
	...
	*==================== International migration reason (j6_a for inbound foreign migrants) ====================
	if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) {
		replace migrated_reason = 3 if j6_a == 1      // To work (employment purpose)
		...
		replace migrated_reason = 4 if j6_a == 15     // To be an Asylum seeker (forced migration)
	}

	*==================== Domestic inter-district migration reason (j5_b for internal movers) ====================
	if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) {
		replace migrated_reason = 3 if j5_b == 1      // To work
		replace migrated_reason = 3 if j5_b == 2      // Other income reasons
		replace migrated_reason = 4 if j5_b == 3      // Land related problems
		replace migrated_reason = 1 if j5_b == 4      // Follow family
		replace migrated_reason = 1 if j5_b == 5      // Marriage
		replace migrated_reason = 2 if j5_b == 6      // School/training
		replace migrated_reason = 4 if j5_b == 7      // Illness, injury
		replace migrated_reason = 1 if j5_b == 8      // Divorce/separation
		replace migrated_reason = 4 if j5_b == 9      // To escape insecurity
		replace migrated_reason = 4 if j5_b == 10     // To return home from displacement
	}

	* blank value for non-migrant respondents
