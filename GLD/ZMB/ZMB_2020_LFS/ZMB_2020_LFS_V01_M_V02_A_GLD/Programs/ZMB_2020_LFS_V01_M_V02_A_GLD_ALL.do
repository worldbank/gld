...
<_Sample size (HH)_> 				[9184] </_Sample size (HH)_>
<_Sample size (IND)_> 				[45354] </_Sample size (IND)_>
...
* Date: [2026-07-24] - [ZMB_2020_LFS_V01_M_V02_A: Updated to new template and added migration harmonization]
* Date: [2026-09-25] - [ZMB_2020_LFS_V01_M_V02_A: Correct migration harmonization based on code review; update survey metadata and sample size]
...
*<_migrated_years_>
	...
	* Domestic migration only
	replace migrated_years = 2020 - j3_b_year if migrated_binary == 1 & j2_b == 1 & j3_a != 1 & !missing(j3_b_year)

	* If both international and domestic moves are reported, retain the latest move
	replace migrated_years = min(2020 - j4_a_year, 2020 - j3_b_year) ///
...
*<_migrated_from_cat_>
	...
	* Cross-border international migration
	replace migrated_from_cat = 5 if migrated_binary == 1 & j3_a == 1

	* Domestic migration: identify previous province from previous district code
	tempvar old_prov_code
	gen `old_prov_code' = floor(j4_b_distict / 100) ///
		if migrated_binary == 1 & j2_b == 1 & !missing(j4_b_distict)

	replace migrated_from_cat = 3 if migrated_binary == 1 & j2_b == 1 ///
		& !missing(j4_b_distict) & `old_prov_code' == prov
...
*<_migrated_reason_>
	...
	* Domestic inter-district migration reason
	...
	replace migrated_reason = 1 if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) & j5_b == 4
	replace migrated_reason = 1 if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) & j5_b == 5
	replace migrated_reason = 2 if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) & j5_b == 6
	replace migrated_reason = 4 if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) & j5_b == 7
	replace migrated_reason = 1 if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) & j5_b == 8
	replace migrated_reason = 4 if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) & j5_b == 9
	replace migrated_reason = 4 if migrated_binary == 1 & j2_b == 1 & !missing(j5_b) & j5_b == 10
