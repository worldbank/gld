...
<_Program name_>				[ZMB_2014_LFS_V01_M_V01_A] </_Program name_>
...
use "`path_in_stata'/2014LFS.dta", clear
...
*<_int_year_>
	* Report says data collection was november and december 2014, but no info in data
	gen int_year = 2014
...
*<_relationharm_>
	gen relationharm = a4
	recode relationharm 4/9=5 10/11=4 12/15=5 16/17=6
...
*<_marital_>
	gen byte marital = a5
	recode marital (3 4 = 1) (1 = 2) (2 = 3) (5 6 = 4) (7 = 5)
...
*<_literacy_>
	...
	recode literacy (2 = 0)
	replace literacy = . if literacy == -9
...
*<_vocational_>
	...
	replace vocational = . if inlist(vocational, -9, 5)
	label de lblvocational 0 "No" 1 "Yes"
	label var vocational "Ever received vocational training"
*</_vocational_>

*<_vocational_type_>
	gen vocational_type = b10
	recode vocational_type (2/5 = 2) (6 = .)
...
That is not the employment concept we work with, hence the numbers may disagree.
	...
	* Unemployed are those who would be willing to accept a job and are loooking. Unfortunately, those who say yes to accepting a job or starting a business (g1) skip the question on looking. The recall periods are also
...
*<_nlfreason_>
	...
	replace nlfreason = 3 if g5 == 14
	replace nlfreason = 4 if g5 == 9
	
	* Add in those too old to work by C1 who skip G section
	replace nlfreason = 3 if c1 == 15
	
	replace nlfreason = 5 if missing(nlfreason) & lstatus == 3 & !missing(g5)
	
	* 222 wihtout answers, leave missing
	
	replace nlfreason = . if lstatus != 3
	replace nlfreason = . if nlfreason < 0
...
*<_unempldur_l_>
	* Using time trying to find job, not time w/o job
...
*<_empstat_>
	gen byte empstat=d4
	recode empstat (-9 -7 = .) (1/3=1) (4=3) (5=4) (6/7=2)
...
*<_ocusec_>
	gen byte ocusec = d13
	recode ocusec (1 2 4 = 1) (3=3) (5/8=2) (-9=.)
...
*<_industry_orig_>
	tostring d2, gen(industry_orig) force
...
*<_industrycat_isic_>
	...
	gen str industrycat_isic     = string(d2, "%04.0f")
	replace industrycat_isic = "" if industrycat_isic == "."
	replace industrycat_isic = "" if industrycat_isic == "-009"
	
	* From observation, only one error. Code 4181, closest is 4100. Maybe a bit to far. Set to missing. Just once.
	replace industrycat_isic = "" if industrycat_isic == "4181"
...
*<_occup_orig_>
	tostring d1, gen(occup_orig) force
...
*<_occup_isco_>
	gen str occup_isco = string(d1,     "%04.0f")
	replace occup_isco = "" if occup_isco == "."replace 
	replace occup_isco = "" if occup_isco == "-009"
	
	* From observation, three errors. 
	* Code 7551 has no description, set to missing
	* Code 4921 has only driver, set to 8320
	* Code 8222 has taxi driver, set to 8322
	replace occup_isco = ""     if occup_isco == "7551"
	replace occup_isco = "8322" if occup_isco == "8222"
	replace occup_isco = "8320" if occup_isco == "4921"
...
/* <_wage_no_compen_note>
	...
	Wage is asked with and without taxes. 2012 only had net but in the future questionnaires it will ask only for wage before 
	deducations, hence use that here. At least series is consistent from 2014.
	
	Then there are 800 people who have fa4a info but not fa5, bring those in - they ahve different unitwage.
	
	Then employers and self employed are asked, depending on whether agriculture or not.
	If agriculture it is fb12 net sales income in the last 12 months, otherwise it is net income
	in the last month (fb6b)
	
	Note also there was a redenomination of the Kwacha in 2013,
	slashing the unit by 1000.
	...
	* Add in business income from agriculture 
	replace wage_no_compen = fb12/12 if missing(wage_no_compen)
