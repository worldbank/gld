...
<_Program name_>				[ZMB_2021_LFS_V01_M_V02_A] </_Program name_>
...
<_Sample size (HH)_> 				[9774] </_Sample size (HH)_>
<_Sample size (IND)_> 				[44115] </_Sample size (IND)_>
...
* Date: [2026-07-24] - [ZMB_2021_LFS_V01_M_V02_A: Update to new template; add migration]
* Date: [2026-09-25] - [ZMB_2021_LFS_V01_M_V02_A: Correct migration harmonization based on code review; update sample sizes and survey metadata]
...
use "`path_in_stata'/2021LFS.dta", clear
...
/* <_hhid_note>
	...
	gen str hlpr_csa     = string(csa,     "%03.0f")
	gen str hlpr_sea     = string(sea,     "%03.0f")
	gen str hlpr_sbn     = string(sbn,     "%03.0f")
	gen str hlpr_hun     = string(hun,     "%02.0f")
	gen str hlpr_hhn     = string(hhn,     "%01.0f")
	
	* Add qsn that helps differentiate (I believe) HH in a same 
	* structure. There are ~12K with missing
	gen str hlpr_qsn     = string(qssignid,     "%02.0f")
	replace hlpr_qsn     = "99" if hlpr_qsn == "."
...
*<_pid_>
	...
	*isid pid
	* There are 1625 cases where the PID is not unique. We need timestamps and other information
	* to differentiate them. Hoping to get that when the data is released. For now, drop.
...
*<_migrated_binary_>
	...
	// International migration (j3_a=1) OR domestic inter-district migration (j2_c=1) is defined as migrant
...
*<_migrated_years_>
	...
	// Calculate years since latest cross-border migration
	replace migrated_years = 2021 - j4_a_year if migrated_binary == 1 & j3_a == 1 & !missing(j4_a_year)

	// Domestic migration only
	replace migrated_years = 2021 - j3_c_year if migrated_binary == 1 & j2_c == 1 & j3_a != 1 & !missing(j3_c_year)

	// If both international and domestic moves are reported, retain the latest move
	replace migrated_years = min(2021 - j4_a_year, 2021 - j3_c_year) ///
...
*<_migrated_from_cat_>
	...
	// Code 5 stands for cross-border international migration
	replace migrated_from_cat = 5 if migrated_binary == 1 & j3_a == 1

	// Judge domestic migration belongs to same province or different province
...
*<_migrated_from_code_>
	...
	// Store original district code for internal domestic migrants
...
*<_migrated_from_country_>
	...
	// Map numeric country code j5_a_country to standard ISO3 code
	replace migrated_from_country = "AGO" if j5_a_country == 5       // Angola
	replace migrated_from_country = "BWA" if j5_a_country == 24      // Botswana
	replace migrated_from_country = "COD" if j5_a_country == 43      // Democratic Republic of the Congo
	replace migrated_from_country = "EGY" if j5_a_country == 55      // Egypt
	replace migrated_from_country = "MWI" if j5_a_country == 110     // Malawi
	replace migrated_from_country = "MOZ" if j5_a_country == 121     // Mozambique
	replace migrated_from_country = "NAM" if j5_a_country == 123     // Namibia
	replace migrated_from_country = "NZL" if j5_a_country == 127     // New Zealand
...
*<_potential_lf_>
	* Data has in it a variable with this name
	rename potential_lf pot_lf_orig
