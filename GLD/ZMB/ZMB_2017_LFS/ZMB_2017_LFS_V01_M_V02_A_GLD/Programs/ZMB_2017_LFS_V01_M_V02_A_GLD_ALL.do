...
*<_migrated_binary_>
	...
	replace migrated_binary = 1 if j3 == 1
	replace migrated_binary = . if missing(j3)
...
*<_migrated_years_>
	gen migrated_years = .
	replace migrated_years = 2017 - j4_year if migrated_binary == 1 & !missing(j4_year)
...
*<_migrated_from_code_>
	...
	replace migrated_from_code = string(j5, "%04.0f") if migrated_binary == 1 & !missing(j5)
...
*<_migrated_from_country_>
	...
	* Map numeric birth country code (j1b) to standard ISO 3166 alpha-3 country code
	replace migrated_from_country = "AGO" if j1b == 5      // Angola
	replace migrated_from_country = "BGD" if j1b == 14    // Bangladesh
	replace migrated_from_country = "BIH" if j1b == 23    // Bosnia and Herzegovina
	replace migrated_from_country = "BDI" if j1b == 29    // Burundi
	replace migrated_from_country = "COG" if j1b == 42    // Congo (Brazzaville)
	replace migrated_from_country = "COD" if j1b == 43    // Democratic Republic of the Congo
	replace migrated_from_country = "IND" if j1b == 82    // India
	replace migrated_from_country = "KEN" if j1b == 95    // Kenya
	replace migrated_from_country = "LBN" if j1b == 102   // Lebanon
	replace migrated_from_country = "MWI" if j1b == 110   // Malawi
	replace migrated_from_country = "MOZ" if j1b == 121   // Mozambique
	replace migrated_from_country = "NAM" if j1b == 123   // Namibia
	replace migrated_from_country = "PHL" if j1b == 139   // Philippines
	replace migrated_from_country = "RWA" if j1b == 145   // Rwanda
	replace migrated_from_country = "ZAF" if j1b == 159   // South Africa
	replace migrated_from_country = "TZA" if j1b == 169   // Tanzania
	replace migrated_from_country = "TUR" if j1b == 175   // Turkey
	replace migrated_from_country = "GBR" if j1b == 180   // United Kingdom
	replace migrated_from_country = "ZMB" if j1b == 189   // Zambia
	replace migrated_from_country = "ZWE" if j1b == 190   // Zimbabwe

	* Blank field for individuals with no migration
