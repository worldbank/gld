...
<_Program name_>				[ZMB_2022_LFS_V01_M_V02_A] </_Program name_>
...
<_Sample size (HH)_> 				[10251] </_Sample size (HH)_>
<_Sample size (IND)_> 				[47060] </_Sample size (IND)_>
...
* Date: [2026-07-24] - [ZMB_2022_LFS_V01_M_V02_A: Update to new template; add migration]
* Date: [2026-09-25] - [ZMB_2022_LFS_V01_M_V02_A: Correct migration harmonization based on code review; update sample sizes and survey metadata]
...
use "`path_in_stata'/ZM_2022_LFS.dta", clear
...
/* <_hhid_note>
	...
	gen str hlpr_qssignid = string(qssignid, "%03.0f")
...
*<_pid_>
	...
	*isid pid
	* There are 19 cases of repetitions (38 observations)
	duplicates tag pid, gen(pid_dup)
	
	* A few seem genuine repeats. We can look at the PID duplicates that also
	* have the same answers for all cases.
	duplicates tag pid a2-av_income, gen(full_dup)
	
	* Create a running number by pid
	bys pid : gen helper_dup = _n
	
	* For the true duplicates, drop the second iteration 
	drop if pid_dup == 1 & full_dup == 1 & helper_dup == 2
	
	* For the rest we inspect (12 cases, 24 obs), take manual decisions 
	drop pid_dup 
	duplicates tag pid, gen(pid_dup)
	levelsof hhid if pid_dup == 1, local(hh_with_issues)
	gen hh_with_dup = .
	foreach household of local hh_with_issues {
		replace hh_with_dup = 1 if hhid == "`household'"
	}
	sort hhid pid
	*br hun - a5 if hh_with_dup == 1
	
	* Drop what look like true repeats
	drop if pid == "0303010380701970510340010101401" & helper_dup == 2
	drop if pid == "0303010380701970510340010101402" & helper_dup == 2
	
	* Replace cases where it seems the internal number skipped (e.g., 1-2-3-3-5-6)
	replace pid = "0303020420101270810050010100304" if pid == "0303020420101270810050010100303" & helper_dup == 2
	replace pid = "0303090551801550220050010100303" if pid == "0303090551801550220050010100304" & helper_dup == 1
	replace pid = "0404100680202540510110010101005" if pid == "0404100680202540510110010101004" & helper_dup == 2
	replace pid = "0505060790102151331110030101508" if pid == "0505060790102151331110030101505" & helper_dup == 2
	replace pid = "0505060790102151331110030101509" if pid == "0505060790102151331110030101507" & helper_dup == 2
	replace pid = "0505060790102151331110030101510" if pid == "0505060790102151331110030101506" & helper_dup == 2
	
	* Change what looks like two different households.
	replace hhid = "04041006802025405102000101999" if hhid == "04041006802025405102000101014" & (pid ==	"0404100680202540510200010101401" & helper_dup == 2)
	replace hhid = "04041006802025405102000101999" if hhid == "04041006802025405102000101014" & (pid ==	"0404100680202540510200010101402" & helper_dup == 1)
	replace hhid = "04041006802025405102000101999" if hhid == "04041006802025405102000101014" & (pid ==	"0404100680202540510200010101403" & helper_dup == 2)
	replace hhid = "04041006802025405102000101999" if hhid == "04041006802025405102000101014" & (pid ==	"0404100680202540510200010101404" & helper_dup == 1)
	
	replace pid = "0404100680202540510200010199901" if hhid == "04041006802025405102000101999" & pid == "0404100680202540510200010101401" 
	replace pid = "0404100680202540510200010199902" if hhid == "04041006802025405102000101999" & pid == "0404100680202540510200010101402"
	replace pid = "0404100680202540510200010199903" if hhid == "04041006802025405102000101999" & pid == "0404100680202540510200010101403"
	replace pid = "0404100680202540510200010199904" if hhid == "04041006802025405102000101999" & pid == "0404100680202540510200010101404"
...
*<_migrated_binary_>
	...
	// International migration j3_a=1 or domestic cross-district migration j2_c=1 is migrant
	replace migrated_binary = 1 if (j3_a == 1) | (j2_c == 1)
	replace migrated_binary = . if missing(j3_a) & missing(j2_c)
...
*<_migrated_years_>
	...
	// Calculate years since latest international migration
	replace migrated_years = 2022 - j4_a_year if migrated_binary == 1 & j3_a == 1 & !missing(j4_a_year)

	// Domestic migration only
	replace migrated_years = 2022 - j3_c_year if migrated_binary == 1 & j2_c == 1 & j3_a != 1 & !missing(j3_c_year)

	// If both international and domestic moves are reported, retain the latest move
	replace migrated_years = min(2022 - j4_a_year, 2022 - j3_c_year) ///
		if migrated_binary == 1 & j3_a == 1 & j2_c == 1 ///
		& !missing(j4_a_year) & !missing(j3_c_year)
...
*<_migrated_from_urban_>
	...
	label define lblmigrated_from_urban 0 "Rural" 1 "Urban"
...
*<_migrated_from_cat_>
	...
	// Code 5 = cross-border international migration
	replace migrated_from_cat = 5 if migrated_binary == 1 & j3_a == 1

	tempvar old_prov_code
	gen `old_prov_code' = floor(j4_c_distict / 100) if migrated_binary == 1 & j2_c == 1 & !missing(j4_c_distict)

	replace migrated_from_cat = 3 if migrated_binary == 1 & j2_c == 1 ///
		& !missing(j4_c_distict) & `old_prov_code' == prov

	replace migrated_from_cat = 4 if migrated_binary == 1 & j2_c == 1 ///
		& !missing(j4_c_distict) & `old_prov_code' != prov
...
*<_migrated_from_code_>
	gen str migrated_from_code = ""
	// Fill original district code for domestic migrants
	replace migrated_from_code = string(j4_c_distict, "%04.0f") if migrated_binary == 1 & j2_c == 1 & !missing(j4_c_distict)
	replace migrated_from_code = "" if migrated_binary != 1
	label var migrated_from_code "Code of migration area as subnatid level of migrated_from_cat"
*</_migrated_from_code_>

*<_migrated_from_country_>
	gen str migrated_from_country = ""
	// Map numeric country code to ISO 3 code
	replace migrated_from_country = "AGO" if j5_a_country == 5      // Angola
	replace migrated_from_country = "AUS" if j5_a_country == 9      // Australia
	replace migrated_from_country = "BDI" if j5_a_country == 29     // Burundi
	replace migrated_from_country = "COG" if j5_a_country == 42     // Congo (Brazzaville)
	replace migrated_from_country = "COD" if j5_a_country == 43     // Democratic Republic of the Congo
	replace migrated_from_country = "ETH" if j5_a_country == 60     // Ethiopia
	replace migrated_from_country = "IND" if j5_a_country == 82     // India
	replace migrated_from_country = "KEN" if j5_a_country == 95     // Kenya
	replace migrated_from_country = "MWI" if j5_a_country == 110    // Malawi
	replace migrated_from_country = "RWA" if j5_a_country == 145    // Rwanda
	replace migrated_from_country = "ZAF" if j5_a_country == 159    // South Africa
	replace migrated_from_country = "SDN" if j5_a_country == 163    // Sudan
	replace migrated_from_country = "TZA" if j5_a_country == 169    // Tanzania
	replace migrated_from_country = "ZMB" if j5_a_country == 189    // Zambia
	replace migrated_from_country = "ZWE" if j5_a_country == 190    // Zimbabwe
...
*<_migrated_reason_>
	...
	// International migration reason mapping
	...
	replace migrated_reason = 4 if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) & j6_a == 4
	...
	replace migrated_reason = 2 if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) & j6_a == 8
	replace migrated_reason = 4 if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) & j6_a == 11
	replace migrated_reason = 4 if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) & j6_a == 14
	replace migrated_reason = 4 if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) & j6_a == 15
	replace migrated_reason = 5 if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) & j6_a == 16
	replace migrated_reason = . if migrated_binary == 1 & j3_a == 1 & j6_a == 99

	// Domestic inter-district migration reason mapping
	replace migrated_reason = 3 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 1
	replace migrated_reason = 3 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 2
	replace migrated_reason = 4 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 3
	replace migrated_reason = 4 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 4
	replace migrated_reason = 4 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 5
	replace migrated_reason = 1 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 6
	replace migrated_reason = 1 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 7
	replace migrated_reason = 2 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 8
	replace migrated_reason = 4 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 9
	replace migrated_reason = 1 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 10
	replace migrated_reason = 4 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 11
	replace migrated_reason = 4 if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) & j5_c == 12
...
*<_vocational_type_>
	gen vocational_type = b12
	recode vocational_type 1=2 2=1 3/5=2
...
*<_vocational_field_orig_>
	decode b11, gen(vocational_field_orig)
...
*<_empstat_>
	gen byte empstat=d7
	recode empstat (1/3 7/8 = 1) (6 = 2) (4 = 3) (5 = 4)
...
*<_industry_orig_>
	tostring d2_isic, gen(isic_1) force
...
*<_industrycat_isic_>
	gen str industrycat_isic     = string(d2_isic, "%04.0f")
...
*<_occup_orig_>
	tostring d1_isco, gen(isco_1) force
...
*<_occup_isco_>
	gen str occup_isco = string(d1_isco,     "%04.0f")
