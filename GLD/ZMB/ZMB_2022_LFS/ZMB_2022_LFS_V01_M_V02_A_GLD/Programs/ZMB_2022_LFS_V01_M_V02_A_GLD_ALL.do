...
*<_migrated_years_>
	...
	// Calculate years since latest domestic district migration
	replace migrated_years = 2022 - j3_c_year if migrated_binary == 1 & j2_c == 1 & !missing(j3_c_year)
...
*<_migrated_from_cat_>
	...
	replace migrated_from_cat = 3 if `old_prov_code' == prov    // Same province (admin1), different district
	replace migrated_from_cat = 4 if `old_prov_code' != prov    // Different province
	drop `old_prov_code'
...
*<_migrated_reason_>
	...
	// International migration reason mapping (j6_a for cross-border migrants)
	if migrated_binary == 1 & j3_a == 1 & !missing(j6_a) {
		replace migrated_reason = 3 if j6_a == 1      // To work
		replace migrated_reason = 3 if j6_a == 2      // Other income reasons
		replace migrated_reason = 4 if j6_a == 4      // Eviction
		replace migrated_reason = 1 if j6_a == 6      // Follow family
		replace migrated_reason = 1 if j6_a == 7      // Marriage
		replace migrated_reason = 2 if j6_a == 8      // School/training
		replace migrated_reason = 4 if j6_a == 11     // To escape insecurity
		replace migrated_reason = 4 if j6_a == 14     // To be refugee
		replace migrated_reason = 4 if j6_a == 15     // To be an Asylum seeker
		replace migrated_reason = 5 if j6_a == 16     // Other
		replace migrated_reason = . if j6_a == 99     // Missing/invalid code
	}

	// Domestic inter-district migration reason mapping (j5_c)
	if migrated_binary == 1 & j2_c == 1 & !missing(j5_c) {
		replace migrated_reason = 3 if j5_c == 1      // To work
		replace migrated_reason = 3 if j5_c == 2      // Other income reasons
		replace migrated_reason = 4 if j5_c == 3      // Drought, flood or other weather conditions
		replace migrated_reason = 4 if j5_c == 4      // Eviction
		replace migrated_reason = 4 if j5_c == 5      // Land related problems
		replace migrated_reason = 1 if j5_c == 6      // Follow family
		replace migrated_reason = 1 if j5_c == 7      // Marriage
		replace migrated_reason = 2 if j5_c == 8      // School/training
		replace migrated_reason = 4 if j5_c == 9      // Illness, injury
		replace migrated_reason = 1 if j5_c == 10     // Divorce/separation
		replace migrated_reason = 4 if j5_c == 11     // To escape insecurity
		replace migrated_reason = 4 if j5_c == 12     // To return home from displacement
	}
