...
------------+-----------------------------------
   Employed |    445,444       87.61       87.61
 Unemployed |     24,654        4.85       92.46
     Non-LF |      5,723        1.13       93.58
          . |     32,639        6.42      100.00
------------+-----------------------------------
      Total |    508,460      100.00


	  Labor particiaption 92.46% (coding backup):

	gen byte lstatus=.
	replace lstatus=1 if b5_r5b==1 | b5_r7a==1 | b5_r7b==2
	replace lstatus=2 if b5_r7b==1 & [(b5_r15a==1) | (b5_r15b==1)]
	replace lstatus=3 if b5_r7b==1 & [(b5_r15a==2) & (b5_r15b==2)]
...
. tab lstatus 91.82%, m
     ...
     status |      Freq.     Percent        Cum.
------------+-----------------------------------
   Employed |    466,823       91.81       91.81
 Unemployed |         32        0.01       91.82
     Non-LF |        316        0.06       91.88
          . |     41,289        8.12      100.00
------------+-----------------------------------
      Total |    508,460      100.00
...
*<_industrycat_isic_>
	...
	tostring industrycat_isic, replace format(%03.0f)
	gen kbli2 = int(b5_r23/100)
	gen kbli4 = int(b5_r23/10)
	replace industrycat_isic = "072" if kbli2==73
	replace industrycat_isic = "492" if kbli2==494
	replace industrycat_isic = "552" if kbli4==5519
	replace industrycat_isic = "960" if inlist(kbli2, 961, 962, 969)
