...
*<_hhid_>
	*sort prov kab kec desa urb
	*egen hhid1=group(prov kab kec desa urb)
...
--------------+-----------------------------------
         Work |        231       11.02       11.02
Attend school |      1,672       79.77       90.79
 Housekeeping |         56        2.67       93.46
            6 |        137        6.54      100.00
--------------+-----------------------------------
        Total |      2,096      100.00
...
The original relationship variable p502 has 12 categories and category 10 to 12 do not have value labels so they are recoded as missing values.

<_relationharm_>*/

*<_relationharm_>
	gen byte relationharm=p502
	recode relationharm (4 6 7=5) (5=3) (8 9=6) (10/12=.)
...
non-labor force: "who do not have a job/business (p603==2)" & not seeking a job (p612=!1).
   ...
   Work at |
   least 1 |
   hour in | Activity used the most time in the past one
  the past |                    week
...
-----------+--------------------------------------------+----------
   ...
   hour in | temporarily did not work during
...
The original variable "p613" has 6 non-missing categories:
	1  Desperate
	2  Attending school
	3  Housekeeping
...
*<_empstat_>
	gen byte empstat=.
	replace empstat=1 if p608==4
	replace empstat=2 if p608==5
	replace empstat=3 if p608==3
	replace empstat=4 if p608==1|p608==2
...
The raw dataset has two wage-related variables --- p609uang, total monthly net salary
and p609brng, total monthly net salary; both refer to the wage received from the main job,
...
*<_wage_no_compen_>
	gen double wage_no_compen=p609uang
...
*<_lstatus_year_>
	...
	replace lstatus_year=1 if p614==1
	replace lstatus_year=. if p614!=1
...
save "`output'\IDN_1995_QLFS_v01_M_v01_A_GLD_ALL.dta", replace
