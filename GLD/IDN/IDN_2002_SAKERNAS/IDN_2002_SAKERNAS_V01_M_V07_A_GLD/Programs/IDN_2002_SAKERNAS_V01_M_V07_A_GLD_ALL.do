...
/* -----------------------------------------------------------------------
<_Program name_>				IDN_2002_Sakernas_v01_M_v06_A_GLD.do </_Program name_>
...
* Date: [2023-04-06] File: [IDN_2002_Sakernas_v01_M_v06_A_GLD.do] - [Updated "occup" and helper data file； set employed and unemployed observations' non-labor force reason to missing; added variable "empstat"; set "wage_no_compen" to missing for unpaid workers and zero values.]
* Date: [2026-04-20] File: [IDN_2002_Sakernas_V01_M_V07_A_GLD.do] - [Revise "subnatidsurvey"]
...
* Merge with three digit mapping
	merge m:1 kji1982 educat7 using "`path_in_stata'\kji_corresp_3d.dta", keep(match master) nogen

* For not matched cases, create two digit
	gen kji1982_2d = floor(kji1982/10) if !missing(b4cr8) & missing(option_1)

* Merge with 2 digit one by updating (same var that is missing in master will take using value)
	merge m:1 kji1982_2d educat7 using "`path_in_stata'\kji_corresp_2d.dta", keep(match master) nogen update
	tab b4cr8 kji1982_2d if missing(option_1) [iw=weight], cell nofreq
