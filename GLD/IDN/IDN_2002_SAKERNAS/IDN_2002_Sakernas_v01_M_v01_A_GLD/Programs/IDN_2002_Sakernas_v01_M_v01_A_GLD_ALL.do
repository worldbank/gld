...
<_Date created_>				2021-08-06 </_Date created_>
-------------------------------------------------------------------------
...
<_Sample size (IND)_> 			275,341 </_Sample size (IND)_>
...
* harmonized output in a single file
	...
	use "`input'\sakernas02.dta", clear
...
Note that 7 observations' or 1 households' number of household member do
not match the original household size variable "b1r10".

	bys hhid: egen hhsize=count(pid)
	gen gap=hhsize-b1r10
...
11 observations are duplicated and 6 of them observations were dropped. Only
people under 20 (attending school) or housekeeping last week were kept since
they are most possibly unique and duplicated only because they are have lots of
missing values for other labor force variables.
...
year 1995 and 2002 and East Timor is the only province missing among the 27
provinces.
...
Original code list of variable "b4ar1a" in the dataset:
0.No/never in school
1.No/not yet finish Primary School
2.Primary School
3.Public Junior High School
4.Vocational Junior High School
5.Public Senior High School
...
Note that although in 2002, variable b4r11a is not explicitly labeled as "average"
...
save "`output'\IDN_2002_QLFS_v01_M_v01_A_GLD_ALL.dta", replace
