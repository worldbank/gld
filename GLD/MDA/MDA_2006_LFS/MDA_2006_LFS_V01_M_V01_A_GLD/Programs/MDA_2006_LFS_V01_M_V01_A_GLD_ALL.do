...
<_Program name_>				MDA_2006_LFS_V01_M_V01_A_GLD_ALL</_Program name_>
...
<_Study ID_>					MDA_2006_LFS_V01_M </_Study ID_>
...
<_Sample size (HH)_> 				38,288 </_Sample size (HH)_>
<_Sample size (IND)_> 				111,510 </_Sample size (IND)_>
...
use "`path_in_stata'/lfs2006.dta",clear
...
/* <_hhid_note>
	...
	tostring centr, gen(centr_str) format("%03.0f")
	tostring loc, gen(loc_str) format("%03.0f")
	tostring luna, gen(luna_str) format("%02.0f")
	//Month of Interview: Put this explicity because each dwelling is interviewed two consecutive quarters, after that it is skipped 2 quarters and then it is enters again the survey for the next two quarters, afterwards being definitely remove from the survey.
	replace luna_str = "M" + luna_str
	egen hhid = concat(centr_str loc_str luna_str)
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>
	tostring nrp, gen(nrp_str) format("%02.0f")
	egen  pid = concat(hhid nrp_str)
	isid hhid pid
...
/* <_subnatid1_note>
	...
	gen str subnatid1 = ""
	replace subnatid1="1 - North"  if strat==10011 | strat== 10012  | strat== 11000 |strat==11100
	replace subnatid1="2 - Center"  if strat== 20011 | strat== 20012  | strat== 21000 
	replace subnatid1="3 - South" if strat==30011 | strat==30012  | strat== 31000
	replace subnatid1="4 - Cisinau" if strat== 40100 | strat== 41000
...
*<_relationharm_>
	...
	*/
	gen relationharm = grud
	recode relationharm (6 = 3) (7 = 4) (8 9 10 = 5) (11 = 6)
...
*<_relationcs_>
	gen relationcs = ""
	replace relationcs = "1.Head of the household" if grud == 1
	replace relationcs = "2.Husband / Wife" if grud == 2
	replace relationcs = "3.Son / daughter" if grud == 3
	replace relationcs = "4.Father / mother" if grud == 4
	replace relationcs = "5.Brother / sister" if grud == 5
	replace relationcs = "6.son-in-law / daughter-in-law " if grud == 6
	replace relationcs = "7.father –in-law / mother-in-law" if grud == 7
	replace relationcs = "8.grandfather / grandmother" if grud == 8
	replace relationcs = "9.grandson/granddaughter " if grud == 9
	replace relationcs = "10.other relative" if grud == 10
	replace relationcs = "11.not a relative" if grud == 11
	label var relationcs "Relationship to the head of household - Country original"
*</_relationcs_>


*<_marital_>
	gen byte marital = sciv
...
Education module is only asked to those 00 and older.
...
*<_school_>
	gen byte school = fins
	recode school (1 = 0) (2 3 4 5 = 1)
...
*<_educy_>
	/*
	1.Pre-school education or no primary education 
	2.Primary education  (forms I-IV)
	3.Gymnasium education (forms V-IX)
	4.General secondary education (forms X-XI, lyceum) 
	5.Secondary vocational education
	6.Short term higher education (colleges and former technical schools)
	7.Higher university education
	*/
	gen byte educy = .
	replace educy = 0 if nivs == 1
    replace educy = 4 if nivs == 2
	replace educy = 9 if nivs == 3
	replace educy = 11 if nivs == 4
	replace educy = 12 if nivs == 5
	replace educy = 13 if nivs == 6
	replace educy = 16 if nivs == 7 //includes master and PhD
	label var educy "Years of education"
*</_educy_>


*<_educat7_>
	gen byte educat7 = nivs
	recode educat7 (2 = 3) (3 = 4) (4 = 5) 
...
*<_educat_orig_>
	gen educat_orig = ""
	replace educat_orig = "1.Pre-school education or no primary education" if nivs == 1
	replace educat_orig = "2.Primary education (forms I-IV)" if nivs == 2
	replace educat_orig = "3.Gymnasium education (forms V-IX)" if nivs == 3
	replace educat_orig = "4.General secondary education (forms X-XI, lyceum)" if nivs == 4
	replace educat_orig = "5.Secondary vocational education" if nivs == 5
	replace educat_orig = "6.Short term higher education (colleges and former technical schools)" if nivs == 6
	replace educat_orig = "7.Higher university education" if nivs == 7
...
*<_educat_isced_>
	gen educat_isced = .
	replace educat_isced = . if nivs == 1
	replace educat_isced = 100 if nivs == 2
	replace educat_isced = 200 if nivs == 3
	replace educat_isced = 300 if inlist(nivs,4,5)
	replace educat_isced = 400 if nivs == 6
...
*<_lstatus_>
	...
	**Employed**
	replace lstatus = 1 if inrange(stap,1,5)
	...
	**Unemployed**
	gen passive = (disp == 1)
	gen active = (cautnoc == 1)
	gen starter = (moncaut == 1)
	
	replace lstatus = 2 if [(passive == 1 & active == 1) | starter == 1] & missing(lstatus)
	
	// people without job, available to work, waiting for being called back to work or who found a work place and will restart working at a date following the reference period AND available to work.
	replace lstatus = 2 if inlist(moncaut,2,3,4) & passive == 1 & missing(lstatus)
...
{
*<_empstat_>
	gen byte empstat = stap
	recode empstat (2 = 3) (3 = 4) (4 = 2) (0 = .)
	
	*Include self-employed with no market interaction and worked less than 20 hours at reference week
...
*<_ocusec_>
	gen byte ocusec = prop 
	recode ocusec (4 = 2) (5 = 4)
	replace ocusec = 2 if lstatus == 1 & agrv == 1 & missing(ocusec)
...
*<_industrycat_isic_>
	...
	replace industrycat_isic = substr(industry_orig,1,2) + "00" if length(industry_orig)==5
	replace industrycat_isic = "0" + substr(industry_orig,1,1) + "00" if length(industry_orig)==4
	
	*Include self-employed with no market interaction and worked less than 20 hours at reference week. Assume they work in the agricultural sector.
...
*<_industrycat10_>
	...
	replace industrycat10 = 1 if inlist(act,1,2) 
	
	*Include self-employed with no market interaction and worked less than 20 hours at reference week. Assume they work in the agricultural sector.
	replace industrycat10 = 1 if industrycat_isic == "A"
	
	replace industrycat10 = 2 if inlist(act,3)
	replace industrycat10 = 3 if inlist(act,4) 
	replace industrycat10 = 4 if inlist(act,5) 
	replace industrycat10 = 5 if inlist(act,6) 
	replace industrycat10 = 6 if inlist(act,7,8) 
	replace industrycat10 = 7 if inlist(act,9) 
	replace industrycat10 = 8 if inlist(act,10,11) 
	replace industrycat10 = 9 if inlist(act,12) 
	replace industrycat10 = 10 if inrange(act,13,17) 
...
*<_occup_orig_>
	gen occup_orig = string(ocup_orig)
	label var occup_orig "Original occupation record primary job 7 day recall"
*</_occup_orig_>


*<_occup_isco_>
	gen occup_isco = string(ocup_orig, "%04.0f")
	replace occup_isco = "" if lstatus != 1
	replace occup_isco = "" if occup_isco == "."
	
	*Include self-employed with no market interaction and worked less than 20 hours at reference week. Assume they work in Skilled agricultural occupations.
	...
	replace occup_isco = "4220" if occup_isco == "4224"
	...
	replace occup_isco = "5000" if occup_isco == "5310"
	...
	replace occup_isco = "5000" if occup_isco == "5410"
	replace occup_isco = "5000" if occup_isco == "5510"
	replace occup_isco = "5000" if occup_isco == "5520"
	replace occup_isco = "7230" if occup_isco == "7239"
	replace occup_isco = "7200" if occup_isco == "7250"

	replace occup_isco = "7200" if occup_isco == "7260"
	replace occup_isco = "7200" if occup_isco == "7280"
	replace occup_isco = "7300" if occup_isco == "7335"
	replace occup_isco = "7400" if occup_isco == "7439"
	replace occup_isco = "7440" if occup_isco == "7443"
	replace occup_isco = "7400" if occup_isco == "7450"
	replace occup_isco = "7400" if occup_isco == "7460"
	...
	replace occup_isco = "7000" if occup_isco == "7610"
	replace occup_isco = "8120" if occup_isco == "8125"
	replace occup_isco = "8130" if occup_isco == "8132"

	replace occup_isco = "8130" if occup_isco == "8133"
	replace occup_isco = "8130" if occup_isco == "8134"
	replace occup_isco = "8210" if occup_isco == "8215"
	replace occup_isco = "8220" if occup_isco == "8226"
	replace occup_isco = "9100" if occup_isco == "9170"
	replace occup_isco = "9330" if occup_isco == "9334"

	replace occup_isco = "9300" if occup_isco == "9350"
...
*<_whours_>
	gen whours = dure
	replace whours = . if lstatus != 1 | dure == 0
...
*<_socialsec_>
	gen byte socialsec= (contrib==1) if contrib!=.
	replace socialsec=. if contrib==4 | contrib==0
...
*<_union_>
	gen byte union= (sind==1) if sind!=.
	replace union=. if sind==0
...
*<_firmsize_l_>
	gen firmsize_l = .
	replace firmsize_l=1 if nrlucr==1
	replace firmsize_l=5 if nrlucr==2
	replace firmsize_l=10 if nrlucr==3
	replace firmsize_l=20 if nrlucr==4
	replace firmsize_l=50 if nrlucr==5
	replace firmsize_l=100 if nrlucr==6
	replace firmsize_l=200 if nrlucr==7
...
*<_firmsize_u_>
	gen firmsize_u = .
	replace firmsize_u=4 if nrlucr==1
	replace firmsize_u=9 if nrlucr==2
	replace firmsize_u=19 if nrlucr==3
	replace firmsize_u=49 if nrlucr==4
	replace firmsize_u=99 if nrlucr==5
	replace firmsize_u=199 if nrlucr==6
	replace firmsize_u=. if nrlucr==7
...
{
*<_empstat_2_>
	gen byte empstat_2 = staps
	recode empstat_2 (2 = 3) (3 = 4) (4 = 2) (0 = .)
	
	*Include self-employed with no market interaction
	replace empstat_2 = 4 if agrvs == 1 & missing(empstat_2)
...
*<_ocusec_2_>
	gen byte ocusec_2 = props
	recode ocusec_2 (4 = 2) (5 = 4) (0 = .)
...
*<_industry_orig_2_>
	gen industry_orig_2 = string(acts_orig)
...
*<_industrycat_isic_2_>
	...
	replace industrycat_isic_2 = substr(industry_orig_2,1,2) + "00" if length(industry_orig_2)==5
	replace industrycat_isic_2 = "0" + substr(industry_orig_2,1,1) + "00" if length(industry_orig_2)==4
	*Include self-employed with no market interaction. Assume they work in the agricultural sector.
	replace industrycat_isic_2 = "A" if agrvs == 1 & missing(industrycat_isic_2)
...
*<_industrycat10_2_>
	...
	replace industrycat10_2 = 1 if inlist(acts,1,2) 
	*Include self-employed with no market interaction and. Assume they work in the agricultural sector.
	replace industrycat10_2 = 1 if industrycat_isic_2 == "A"
	
	replace industrycat10_2 = 2 if inlist(acts,3)
	replace industrycat10_2 = 3 if inlist(acts,4) 
	replace industrycat10_2 = 4 if inlist(acts,5) 
	replace industrycat10_2 = 5 if inlist(acts,6) 
	replace industrycat10_2 = 6 if inlist(acts,7,8) 
	replace industrycat10_2 = 7 if inlist(acts,9) 
	replace industrycat10_2 = 8 if inlist(acts,10,11) 
	replace industrycat10_2 = 9 if inlist(acts,12) 
	replace industrycat10_2 = 10 if inrange(acts,13,17) 
...
*<_occup_orig_2_>
	gen occup_orig_2 = string(ocups_orig)
	label var occup_orig_2 "Original occupation record secondary job 7 day recall"
*</_occup_orig_2_>


*<_occup_isco_2_>
	gen occup_isco_2 = string(ocups_orig, "%04.0f")
	replace occup_isco_2 = "" if missing(empstat_2) | occup_isco_2 == "."
	
	*Include self-employed with no market interaction. Assume they work in Skilled agricultural occupations.
	replace occup_isco_2 = "6000" if agrvs == 1 & missing(occup_isco_2)

	
	replace occup_isco_2 = "2410" if occup_isco_2 == "2413"
	replace occup_isco_2 = "3140" if occup_isco_2 == "3146"
	replace occup_isco_2 = "4220" if occup_isco_2 == "4224"
	...
	replace occup_isco_2 = "5000" if occup_isco_2 == "5310"
	...
	replace occup_isco_2 = "9330" if occup_isco_2 == "9334"
	replace occup_isco_2 = "9300" if occup_isco_2 == "9350"
...
*<_occup_2_>
	gen byte occup_2 = ocups
	
	*Include self-employed with no market interaction and worked less than 20 hours at reference week. Assume they work in Skilled agricultural occupations.
	replace occup_2 = 6 if occup_isco_2 == "6000" & missing(occup_2)
...
*<_whours_2_>
	gen whours_2 = dures
	replace whours_2 = . if  missing(empstat_2) | dures == 0
