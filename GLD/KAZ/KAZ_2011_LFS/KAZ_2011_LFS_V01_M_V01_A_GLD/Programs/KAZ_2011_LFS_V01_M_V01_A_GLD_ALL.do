...
<_Program name_>				KAZ_2011_LFS_V01_M_V01_GLD_ALL.do </_Program name_>
<_Application_>					Stata 17 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2025-06-02 </_Date created_>
...
<_Sample size (HH)_> 			83,820 </_Sample size (HH)_>
<_Sample size (IND)_> 			203,821 </_Sample size (IND)_>
...
use "`path_in_stata'/sbor_baz_year2011_q1throughq4_labelled_withnewvars_s12.dta", clear
...
*<_hhid_>
*15100175: region + VOPR_02 + VOPR_04 + VOPR_09 + VOPR_091 + 
*There is already existing hhid variable. Need to rename this
	tostring region, gen(region_str)
	gen v02 = string(VOPR_02, "%02.0f")
	gen v03 = string(VOPR_03, "%02.0f")
	gen v04 = string(VOPR_04, "%03.0f")
	gen v09 = string(VOPR_09, "%04.0f")
	gen v091 = string(VOPR_091, "%06.0f")

	gen hhid = region_str + v02 + v04 + v09 + v091
	label var hhid "Household ID"
*</_hhid_>


*<_pid_>
* There is variable "pid" added in the dataset. Use this!
* Odd that pid is not given on Q4
	ren pid pid_given
	
	bysort hhid KVARTAL (q2b q2 age): gen runner = _n
	
	* Problem is when there are gaps in appearances, it will mess up the order and the same person is not tagged correctly
	sort hhid q2b q2 age KVARTAL
	
	* Identify potential issues
	*bys hhid KVARTAL: gen tot_mem_wave = _N
	*bys hhid: egen max_mem = max(tot_mem_wave)
	*bys hhid: egen min_mem = min(tot_mem_wave)
	
	replace runner = q2b
	*bys hhid KVARTAL q2b (q2 age): gen runner_2 = _n
	bys hhid KVARTAL runner  (q2b q2 age): gen runner_2 = _n
	
	gen runner_str = string(runner, "%02.0f")
	gen runner_2_str = string(runner_2, "%02.0f")
	gen  pid = hhid + runner_str + runner_2_str 
	isid pid KVARTAL
...
*<_hsize_>
* There is a harmonized vble called "hhsize" which is derived by taking the number of IDs in the HH for a given wave. This is a flawed approach bec members under 15 are not interviewed thus will not be counted following this approach.
* But instead, I will take the hsize variable in the dataset bec this will info no longer be available post-harmonization. 
	egen hsize = rowtotal(VOPR_11 VOPR_12 VOPR_13 VOPR_14 VOPR_15 VOPR_16)
* In the questionnaire it says this is only asked for the primary respondent. Thus, we fill in for the others
	bys hhid KVARTAL: egen maxhsize = max(hsize)
	replace hsize = maxhsize if hsize == 0
	
	drop maxhsize
	
	bys hhid KVARTAL: gen idcount = _N
	*assert idcount == hhsize
	drop idcount
	* Shows same !
...
*<_lstatus_>
	...
	* Worked at least one hr per week
	replace lstatus = 1 if q6 == 1
	
	* Work at home for income at least one hr per week
	replace lstatus = 1 if q7 == 1
	
	* Agriculutral production for at least one hour
	* Should be at least partly for consumption/exchange
	replace lstatus = 1 if q8 == 1
	
	* Work at friend or relative's farm for income
	replace lstatus = 1 if q11 == 1
	
	* Perform work related to sale of goods or services 
	replace lstatus = 1 if q12 == 1
	
	* Temporarily absent from work
	replace lstatus = 1 if q14_zero == 0
	
	* Odd that: none worked in the past week also worked at least 1 hour in the past week.
	tab q6 q14_zero
	* Error in lstatus_given (that came w the raw data): one individual tagged as employed despite not working/employed. Also not interviewed for labor questions
	* It seems that q6 alone determines employed
	...
	* Unemployed: looking for a job in last 4 weeks and available to start in two weeks (this is asked regardless of employment status, even among employed, so we need to apply this only for those not yet tagged)
	replace lstatus = 2 if q44 == 1 & q46 == 1 & missing(lstatus)
...
*<_potential_lf_>
	...
	replace potential_lf = 1 if (q44 == 1 & q46 == 0) | (q44 == 0 & q46 == 1)
...
*<_underemployment_>
	...
	replace underemployment = 1 if a2q3 == 1 & lstatus == 1
...
*<_nlfreason_>
	gen byte nlfreason = .
	replace nlfreason = 1 if q33 == 9
	replace nlfreason = 2 if q33 == 6
	replace nlfreason = 3 if q33 == 7
	replace nlfreason = 4 if q33 == 12
...
*<_unempldur_l_>
	...
	replace unempldur_l = 0 if q36 == 1
	replace unempldur_l = 1 if q36 == 2
	replace unempldur_l = 3 if q36 == 3
	replace unempldur_l = 6 if q36 == 4
	replace unempldur_l = 12 if q36 == 5
	replace unempldur_l = 36 if q36 == 6
...
*<_unempldur_u_>
	...
	replace unempldur_u = 1 if q36 == 1
	replace unempldur_u = 3 if q36 == 2
	replace unempldur_u = 6 if q36 == 3
	replace unempldur_u = 12 if q36 == 4
	replace unempldur_u = 36 if q36 == 5
	replace unempldur_u = . if q36 == 6
...
*<_empstat_>
	...
	recode empstat (1/4=1) (5=3) (6 8 9 = 4) (7=2) 
...
* No detailed industry codes here
	gen industrycat_isic = string(industry_orig, "%02.0f")
	replace industrycat_isic = industrycat_isic + "00"
	replace industrycat_isic = "" if missing(q17_code)
...
*<_firmsize_l_>
	gen firmsize_l = .
	replace firmsize_l = 1 if q16 == 1
	replace firmsize_l = 6 if q16 == 2
	replace firmsize_l = 11 if q16 == 3
	replace firmsize_l = 21 if q16 == 4
	replace firmsize_l = 51 if q16 == 5
	replace firmsize_l = 251 if q16 == 6
	replace firmsize_l = 501 if q16 == 7 
...
*<_firmsize_u_>
	gen firmsize_u = .
	replace firmsize_u = 5 if q16 == 1
	replace firmsize_u = 10 if q16 == 2
	replace firmsize_u = 20 if q16 == 3
	replace firmsize_u = 50 if q16 == 4
	replace firmsize_u = 250 if q16 == 5
	replace firmsize_u = 500 if q16 == 6
	replace firmsize_u = . if q16 == 7
...
*<_industry_orig_2_>
	gen industry_orig_2 = q29a_code
	label var industry_orig_2 "Original survey industry code, secondary job 7 day recall"
*</_industry_orig_2_>


*<_industrycat_isic_2_>
	tostring q29a_code, gen(indcode)
	
	gen industrycat_isic_2 = substr(indcode, 1, 2)
	...
	replace industrycat_isic_2 = "" if inlist(industrycat_isic_2, "00", "000", ".000", "0000") | lstatus != 1
...
*<_firmsize_l_2_>
	gen firmsize_l_2 = .
	replace firmsize_l_2 = 1 if aq15 == 1
	replace firmsize_l_2 = 6 if aq15 == 2
	replace firmsize_l_2 = 11 if aq15 == 3
	replace firmsize_l_2 = 21 if aq15 == 4
	replace firmsize_l_2 = 51 if aq15 == 5
	replace firmsize_l_2 = 251 if aq15 == 6
	replace firmsize_l_2 = 501 if aq15 == 7 
...
*<_firmsize_u_2_>
	gen firmsize_u_2 = .
	replace firmsize_u_2 = 5 if aq15 == 1
	replace firmsize_u_2 = 10 if aq15 == 2
	replace firmsize_u_2 = 20 if aq15 == 3
	replace firmsize_u_2 = 50 if aq15 == 4
	replace firmsize_u_2 = 250 if aq15 == 5
	replace firmsize_u_2 = 500 if aq15 == 6
	replace firmsize_u_2 = . if aq15 == 7
	replace firmsize_u = . if lstatus!=1
