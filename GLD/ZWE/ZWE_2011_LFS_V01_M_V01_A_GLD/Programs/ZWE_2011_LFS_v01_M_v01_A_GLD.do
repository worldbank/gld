...
<_Program name_>				ZWE_2011_LFS_v01_M_v01_A_GLD_ALL.do </_Program name_>
<_Application_>					Stata 17 <_Application_>
<_Author(s)_>					World Bank Jobs Group (gld@worldbank.org) </_Author(s)_>
<_Date created_>				2022-02-08 </_Date created_>
...
exception of Harare and Bulawayo, each of the other eight provinces was stratified into four
strata according to land use: Communal Lands, Large Scale Commercial Farming Areas
(LSCFA), Urban and Semi-Urban Areas, and Small Scale Commercial Farming Areas (SSCFA)
and Resettlement Areas. Only one urban stratum was formed each in Harare and Bulawayo. This
gave a total of 34 strata.

The survey used a two-stage sample design with EAs as the first and households as the second stage
sampling units. In total 400 EAs were selected with probability proportional to size (PPS), the measure
of size being the number of households enumerated in the 2002 Population Census. The selection of
the EAs was a systematic, one-stage operation, carried out independently for each of the 34 strata.

A complete listing of the households in the selected EAs was carried out and the list of
households used as the frame for the second-stage systematic random selection of households.

The 2011 LFCLS had a sample size of 10 014 households. Each enumerator was assigned two EAs
within the 21 days of data collection. Enumerators assigned urban EAs covered 21 households per EA
whilst those assigned rural EAs covered 27 households per EA. No substitutions of non-responding
households were done in this survey. If a dwelling unit was found being occupied by a different
household then that household was interviewed in place of the listed one. If a dwelling unit had been
destroyed and the household was still staying or living within the same EA, the household was
followed and interviewed.

</_Sampling method_>
<_Geographic coverage_> 			Province - Admin 1  </_Geographic coverage_>
<_Currency_> 					USD </_Currency_>
...
* Date: [2023-04-15] - [added principal and second labor variables]
...
use "`path_in_stata'\lfs_2011.dta"
rename *, lower
rename male male_1
...
*<_pid_>
	gen help_1 = string(person_number, "%02.0f")
	gen pid = hhid + help_1
...
*<_psu_>
	* It is supposed to be EA, but EA itself has 55 distinct values.
	* Cannot recreate it directly
	gen psu = .
...
*<_literacy_>
 /*<_literacy_note>
 * ZimStat Report uses above 15 and at least Grade 3 as approximation to literacy
	* Use that logic here.
 </_literacy_note>*/
	gen byte literacy = .
	replace literacy  = 1 if age >= 15 & inrange(q13,3,23)
	replace literacy  = 0 if age >= 15 & inrange(q13,0,2)
...
*<_educat7_>
/* <_educat7_note>
	* Treat ECD (pre-primary) as no education
	* Primary is 7 years, codes are 1-7
	* Secondary is lower forms (codes 11, 12)
	*			   senior secondary (codes 13, 14)
...
*no info on incomplete education then only possible to this level
	gen byte educat4 = .
	replace educat4 = 1 if q11==1 | q13==88 | q13==0
	replace educat4 = 2 if inrange(q13,1,7)
	replace educat4 = 3 if inrange(q13,8,11)
	replace educat4 = 4 if inrange(q13,12,23)
...
*<_underemployment_>
	...
	replace underemployment=1 if lstatus==1 & q43==1
	replace underemployment=1 if q43!=1
...
*<_empstat_>
	gen byte empstat=.
	replace empstat=1 if inrange(q23,1,2)
	replace empstat=2 if q23==6
	replace empstat=3 if q23==3
	replace empstat=4 if inrange(q23,4,5)
...
*<_industrycat_isic_>
	gen q30_helper=q30
	recode q30_helper 9999=.
	gen industrycat_isic= string(q30_helper,"%04.0f")
...
*<_occup_orig_>
	gen str4 occup_orig = string(q28)
	replace occup_orig="" if q28==.
...
*<_wage_no_compen_>
*q53 which gives the income information is divided in intervals.
...
*<_whours_>
*q42 records working hours in all jobs, there is no questions that identify other jobs. discuss if we keep this code
	gen whours = q42
	replace whours=. if q36==1
	replace whours=. if q42==999
...
*<_contract_>
	gen byte contract = q27
	recode contract 2=0 3/9=.
...
*<_healthins_>
	gen byte healthins = q59
	recode healthins 2=0 3/9=.
...
{
*<_empstat_2_>
	gen byte empstat_2 = q37
	recode empstat_2 1/2=1 4/5=4 6=2 9=.
...
*<_ocusec_2_>
	gen byte ocusec_2 = q40
	recode ocusec_2 1=2 5=2 7=2 2/4=1 8/9=.
...
*<_industrycat_isic_2_>
	gen q39_helper=q39
	recode q39_helper 9999=.
	gen industrycat_isic_2= string(q39_helper,"%04.0f")
...
*<_occup_orig_2_>
	gen str4 occup_orig_2 = string(q38)
	replace occup_orig_2="" if q38==.
...
*<_occup_2_>
	gen byte occup_2 = .
	replace occup_2=1 if inrange(occup_isco_2,"1111","1430")
	replace occup_2=2 if inrange(occup_isco_2,"2120","2656")
	replace occup_2=3 if inrange(occup_isco_2,"3100","3522")
	replace occup_2=4 if inrange(occup_isco_2,"4110","4419")
	replace occup_2=5 if inrange(occup_isco_2,"5111","5419")
	replace occup_2=6 if inrange(occup_isco_2,"6111","6340")
	replace occup_2=7 if inrange(occup_isco_2,"7111","7549")
	replace occup_2=8 if inrange(occup_isco_2,"8111","8344")
	replace occup_2=9 if inrange(occup_isco_2,"9111","9624")
