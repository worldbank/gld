...
<_Program name_>				ZWE_2021_QLFS_V01_M_V03_A_GLD </_Program name_>
...
<_Study ID_>					ZWE_2021_QLFS_v01_M </_Study ID_>
...
* Date: 2024-08-22 - correct main, second job of 7 day, noticed issues in variable names, label not as in questionnaire
...
use "`path_in_stata'/zwe-qlfs-q3-2021.dta", clear

gen quarter=3

append using "`path_in_stata'/zwe-qlfs-q4-2021.dta", force
...
*<_year_>
* Survey has 7 interviews each on 26th and 27th of December 2018. Technically to be recorded as 2018 survey.
* However, naming by Zimstat (and everyone else) is the 2021 LFS, to not create confusion write as 2021.
	gen int year =2021
...
*<_weight_>
	...
	gen pop=1
	bys quarter: egen rel_w = total(pop)
	gen new_weight = hhweight*rel_w
	count
	replace new_weight = new_weight/`r(N)'
...
*<_relationharm_>
	...
	recode relationharm  (6 = 4) (4 5 7 8 9 10 11 12 13= 5)  (14 96 = 6)  (98 = .)
...
*<_slfcre_dsablty_>
	gen slfcre_dsablty  = d5
	recode slfcre_dsablty (9 = .)
	label define dsablty 1 "No – no difficulty" 2 "Yes – some difficulty" 3 "Yes – a lot of difficulty" 4 "Cannot do at all", replace
...
*<_migrated_from_cat_>
	...
	replace	migrated_from_cat = 3 if province == m4 & migrated_binary == 1
	replace migrated_from_cat = 4 if province != m4 & migrated_binary == 1
	replace	migrated_from_cat = 5 if m4 == 10
...
*<_migrated_from_code_>
	...
	tostring m4, gen(m4_str)
	replace migrated_from_code = m4_str if migrated_from_cat == 4
	replace	 migrated_from_code="1 - Manicaland" if migrated_from_code == "1"
	replace	 migrated_from_code="2 - Mashonaland Central" if migrated_from_code == "2"
	replace	 migrated_from_code="3 - Mashonaland East" if migrated_from_code == "3"
	replace	 migrated_from_code="4 - Mashonaland West" if migrated_from_code == "4"
	replace	 migrated_from_code="5 - Matabeleland North" if migrated_from_code == "5"
	replace	 migrated_from_code="6 - Matabeleland South" if migrated_from_code == "6"
...
*<_migrated_reason_>
	gen migrated_reason = m5
	 recode migrated_reason (6 8 10 11 = 1) (7 = 2) (1 2 3 4 5 = 3) (9 = 4) (96 = 5)
...
*<_educy_>
	...
	* Correct for education if a person age is not at least 2 more than years in education, set to missing.
	...
	replace educy = . if educy_corrector == 1
	drop educy_corrector
...
*<_educat_isced_>
	gen educat_isced = .
	replace	educat_isced = 20  if ed3a == 0
	replace	educat_isced = 100 if ed3a == 1 | ed3a == 2
	replace	educat_isced = 244 if ed3a == 3
	replace	educat_isced = 344 if ed3a == 4
	replace	educat_isced = 453 if ed3a == 5
...
*<_lstatus_>
	gen byte lstatus = .
	replace lstatus = 1 if !missing(mj1)
	replace lstatus = 2 if (js1 == 1 | js2 == 1) & missing(mj1)
	replace lstatus = 3 if js2!= 1 & missing(mj1) & !missing(js2)
...
*<_potential_lf_>
	...
	replace potential_lf = 1 if js8 == 1 & lstatus == 3
...
*<_underemployment_>
	...
	replace underemployment = 1 if wt15 == 1 & wt16 == 1
...
*<_nlfreason_>
	gen byte nlfreason=.
	replace nlfreason = 1 if js13 == 3
	replace nlfreason = 2 if js13 == 4
	replace nlfreason = 3 if js13 == 6
	replace nlfreason = 4 if js13 == 7
	replace nlfreason = 5 if inlist(js13,1,2,5,8)

	replace nlfreason = 1 if js14 == 1
	replace nlfreason = 2 if js14 == 2
	replace nlfreason = 3 if js14 == 4
	replace nlfreason = 4 if js14 == 5
	replace nlfreason = 5 if inlist(js14,3,6,7,96)
...
*<_unempldur_l_>
	...
	replace unempldur_l = 0  if js6 == 1
	replace unempldur_l = 1  if js6 == 2
	replace unempldur_l = 3  if js6 == 3
	replace unempldur_l = 6  if js6 == 4
	replace unempldur_l = 12 if js6 == 5
	replace unempldur_l = 24 if js6 == 6
...
*<_unempldur_u_>
	...
	replace unempldur_u = 1  if js6 == 1
	replace unempldur_u = 3  if js6 == 2
	replace unempldur_u = 6  if js6 == 3
	replace unempldur_u = 12 if js6 == 4
	replace unempldur_u = 24 if js6 == 5
...
/* <_empstat_note>
	Variables and their labels seem to be confused. They do not agree always with the questionnaire.
	MJ7 is OK but MJ8 is not what it states in he questionnaire. It is unclear which one it is. It is only
	answered by those who work own account (all) and those in a HH business (only 20%). It could MJ9 or
	MJ9B. Either way we use it as a proxy to differentiate between employers and own acocunt. With it
	we match the numbers in the ZWE report. (Whether their numbers are correct is unclear at this point)
</_empstat_note> */
	gen byte empstat=.
...
*<_ocusec_>
	...
	replace ocusec = 1 if inrange(mj9,1,2)  & empstat == 1
	replace ocusec = 3 if inrange(mj9,3,4)  & empstat == 1
	replace ocusec = 2 if inrange(mj9,5,96) & empstat == 1
...
*<_contract_>
	gen byte contract = 0 if !mi(empstat)
	replace contract = 1 if contract == 0 & mj10 == 1
...
*<_healthins_>
	capture drop healthins
	gen byte healthins = .

	/*
	MJ10c:
	1 = Yes pension only
	2 = Yes health insurance only
	3 = Yes both
	4 = No
	8 = Don't know
	*/

	replace healthins = 1 if lstatus == 1 & inlist(mj10c, 2, 3)
	replace healthins = 0 if lstatus == 1 & inlist(mj10c, 1, 4)
...
*<_socialsec_>
	capture drop socialsec
	gen byte socialsec = .

	/*
	Social security proxy: employer-paid pension contribution.
	*/

	replace socialsec = 1 if lstatus == 1 & inlist(mj10c, 1, 3)
	replace socialsec = 0 if lstatus == 1 & inlist(mj10c, 2, 4)

	label var socialsec "Employment has social security insurance primary job 7 day recall"
	label define lblsocialsec 0 "Without social security" 1 "With social security", replace
...
*<_industrycat10_2_>
	*destring industry_orig_2, gen (red_indus_2)
...
*<_occup_orig_2_>
	gen occup_orig_2 = ""
	* Info has two variables, one from Q3, another from Q4 - not the same name
	replace occup_orig_2 = sj1b_new if !mi(sj1b_new) & sj1b_new != " "
	replace occup_orig_2 = sj1b     if !mi(sj1b)     & sj1b     != " "
