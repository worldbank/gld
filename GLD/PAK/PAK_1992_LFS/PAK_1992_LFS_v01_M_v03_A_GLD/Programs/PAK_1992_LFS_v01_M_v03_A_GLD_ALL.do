...
/* -----------------------------------------------------------------------
<_Program name_>				PAK_1992_LFS_V01_M_V03_A_GLD_ALL.do </_Program name_>
...
* Date: [2023-03-29] File: [PAK_1992_LFS_V01_M_V03_A_GLD_ALL.do] - [Removed "byte" for firmsize variable coding]
...
* harmonized output in a single file
	...
	use "`path_in_stata'\lfs_9293.dta", clear
...
*<_hhid_>
	...
	tostring hhno, gen(hhno_string) format(%02.0f)
	gen hhid=process_code+hhno_string
...
*<_subnatidsurvey_>
	decode subnatid1, gen(subnatidsurvey)
...
*<_relationharm_>
	...
	gen neg_age=-(age)
	sort hhid sex neg_age serialno
	...
	* Check if hh head count is equal to one
	...
	gen head=relationharm==1
	bys hhid: egen tot_head=sum(head)
	replace relationharm=5 if hhorder2!=1 & tot_head>1 & relationharm==1
	drop head tot_head
	gen head=relationharm==1
	bys hhid: egen tot_head=sum(head)
	replace relationharm=1 if hhorder2==1&tot_head==0
	replace relationharm=5 if tot_head==2&hhorder2!=1&relationharm==1
	replace relationharm=5 if tot_head==2&male==0&relationharm==1
	drop head tot_head
	gen head=relationharm==1
	bys hhid: egen tot_head=sum(head)
	replace relationharm=5 if tot_head==2&male==0&relationharm==1
	...
	bys hhid: egen tot_head=sum(head)
	assert tot_head==1
	drop tot_head head hhorder2 age_max
...
*<_educy_>
	...
	replace educy=2 if edulevel==3 & curenrol==3
	replace educy=5 if edulevel==4
	replace educy=7 if edulevel==4 & curenrol==4
	replace educy=8 if edulevel==5
	replace educy=9 if edulevel==5 & curenrol==5
...
*<_underemployment_>
	...
	replace underemployment=1 if q15==1
	replace underemployment=0 if q15==2
...
*<_nlfreason_>
	recode q32 (4 5=1) (6=2) (7 10=3) (1 11=4) (8 9 12=5) (0=.), gen(nlfreason)
...
*<_empstat_>
	gen empstat=.
	replace empstat=1 if q10<=4 
	replace empstat=2 if q10==10
	replace empstat=3 if q10==5
	replace empstat=4 if q10>=6 & q10<=9
	replace empstat=5 if q10==11
...
*<_wage_no_compen_>
	...
	foreach v of varlist last_*{
		replace `v'=0 if q18>0&q19>0 &!mi(q18)&!mi(q19)
...
*<_unitwage_>
	gen byte unitwage=.
	replace unitwage=2 if wage_no_compen==q18
	replace unitwage=5 if wage_no_compen==q19
	replace unitwage=10 if wage_no_compen>q19&!mi(wage_no_compen)
...
q13 ; q13_1 ; q13_2 ; q13_3

Except q13 the other three variables only have 0-7, 8 categories in total, which does not make sense in terms of "working hours", as this working hours is supposed to be continuous integers.
...
*<_industrycat10_2_>
	...
	replace industrycat10_2=. if q12!=1
...
*<_njobs_>
	gen njobs=.
	replace njobs=1 if q12==2
	replace njobs=2 if q12==1
