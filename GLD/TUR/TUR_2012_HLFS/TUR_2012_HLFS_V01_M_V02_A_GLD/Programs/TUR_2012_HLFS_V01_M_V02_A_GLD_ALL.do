...
/* -----------------------------------------------------------------------
<_Program name_>				TUR_2012_HLFS_V01_M_V02_A_GLD_ALL.do </_Program name_>
...
<_Sample size (HH)_> 			145910 </_Sample size (HH)_>
<_Sample size (IND)_> 			510807</_Sample size (IND)_>
...
<_ISCED Version_>				ISCED 20177 </_ISCED Version_>
...
local path_output "Z:\GLD-Harmonization\582018_AQ\TUR\TUR_2012_HLFS\TUR_2012_HLFS_V01_M_V02_A_GLD\Data\Harmonized"
...
*<_nlfreason_>
*s29 has some replies butnot for all so missing.
	gen byte nlfreason = .
	*recode nlfreason 0=.
	*recode nlfreason 1=4 2=3 3=5 4=5 6=5 7=1 8=5 9=3 10=5
	*replace nlfreason=. if lstatus!=3
	*recode nlfreason .=5 if lstatus==3 & missing(s29)
...
*<_occup_orig_>
	gen str2 occup_orig = string(s38kod08)
	replace occup_orig="" if s38kod08==.
...
*<_occup_>
	...
	replace occup=. if s38kod08==0
	replace occup=99 if s38kod08==99
	replace occup=10 if s38kod08==1
	replace occup=1 if inrange(s38kod08,11,14)
	replace occup=2 if inrange(s38kod08,21,26)
	replace occup=3 if inrange(s38kod08,31,35)
	replace occup=4 if inrange(s38kod08,41,44)
	replace occup=5 if inrange(s38kod08,51,54)
	replace occup=6 if inrange(s38kod08,61,63)
	replace occup=7 if inrange(s38kod08,71,75)
	replace occup=8 if inrange(s38kod08,81,83)
	replace occup=9 if inrange(s38kod08,91,96)
...
save "`path_output'\TUR_2012_HLFS_V01_M_V02_A_GLD_ALL.dta", replace
