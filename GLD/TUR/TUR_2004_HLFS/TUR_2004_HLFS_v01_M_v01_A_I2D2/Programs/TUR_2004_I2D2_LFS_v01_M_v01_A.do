...
** UNIT OF ANALYSIS	
** INPUT DATABASES	Z:\GLD-Harmonization\582018_AQ\TUR\TUR_2004_HLFS\TUR_2004_HLFS_v01_M\Data\Stata\2004YIL.dta
** RESPONSIBLE	Alexandra Quiñones 
** Created	9/28/2011
** Modified	8/02/2021
** NUMBER OF HOUSEHOLDS	121622
** NUMBER OF INDIVIDUALS	472837
** EXPANDED POPULATION	66378760
**                                                                                                  **
******************************************************************************************************
*****************************************************************************************************/
...
** RUN SETTINGS
	...
	local 	main	"`year'\\`cty3'_2004_HLFS_v01_M"
	local 	stata	"`main'\data\stata"
	local 	gld 	"`year'\\`cty3'_2004_HLFS_v01_M_v01_A_GLD"
	local 	i2d2	"`year'\\`cty3'_2004_HLFS_v01_M_v01_A_I2D2"
...
** MARITAL STATUS
	gen marital=.
	replace marital=1 if s15==2
	replace marital=4 if s15==5
	replace marital=5 if s15==4
	replace marital=3 if s15==3
	replace marital=2 if s15==1
...
** CURRENTLY AT SCHOOL
	gen atschool=1 if s13==1
	replace atschool=0 if s13==2
...
** EMPLOYMENT STATUS
	gen empstat=.
	replace empstat=1 if s34==1 |s34==2
	replace empstat=2 if s34==5
	replace empstat=3 if s34==3
	replace empstat=4 if s34==4
...
** REASONS NOT IN THE LABOR FORCE
	gen nlfreason=.
	replace nlfreason=1 if s85==5
	replace nlfreason=2 if s85==6|s85==8
	replace nlfreason=3 if s85==7
	replace nlfreason=4 if s85==10
	replace nlfreason=5 if s85>=1 & s85<=4
	recode nlfreason (.=5) if s85==9 |s85>=11 & s85<=13
...
** FIRM SIZE
	gen firmsize_l=.
	replace firmsize_l=1 if s32a==1
	replace firmsize_l=10 if s32a==2
	replace firmsize_l=25 if s32a==3
	replace firmsize_l=50 if s32a==4
	replace firmsize_l=. if lstatus!=1
	label var firmsize_l "Firm size (lower bracket)"

	gen firmsize_u=.
	replace firmsize_u=9 if s32a==1
	replace firmsize_u=24 if s32a==2
	replace firmsize_u=49 if s32a==3
	replace firmsize_u=. if  s32a==4
...
** SOCIAL SECURITY
	gen socialsec=.
	replace socialsec=1 if s37==1
	replace socialsec=0 if s37==2
...
** DELETE MISSING VARIABLES
 ...
 save "`id_data'\TUR_2004_LFS_v01_M_v01_A_I2D2.dta", replace
