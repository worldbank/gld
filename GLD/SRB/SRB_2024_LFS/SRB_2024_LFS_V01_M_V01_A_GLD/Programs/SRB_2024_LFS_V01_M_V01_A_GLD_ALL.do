...
<_Program name_>				SRB_2024_LFS_V01_M_V01_A_GLD_ALL.do </_Program name_>
...
use "`path_in_stata'/lfs_data.dta", clear
...
*<_pid_>
	gen neg_age = -(age)
	bysort hhid quarter (hhlink sex neg_age): gen runner = _n
...
*<_weight_>
* I divide by 4 bec these are quarterly weights
	gen weight = coeffq/4
...
*<_potential_lf_>
	...
	* available non-seekers
	replace potential_lf = 1 if availble_a == 1 & inlist(seekwork, 3, 5) & lstatus == 3
	
	* unavailable jobseekers
	replace potential_lf = 1 if availble_a != 1 & inlist(seekwork, 1, 2, 4) & lstatus == 3
...
*<_unempldur_l_>
	* We cannot use this because there are months in the year variable and years in the month variable!
	gen byte unempldur_l=.
	   replace unempldur_l = 0   if lstatus==2 & seekdur_group==0   // 5 months group -> lower 0
    replace unempldur_l = 6   if lstatus==2 & seekdur_group==1   // 6–11 months
    replace unempldur_l = 12  if lstatus==2 & seekdur_group==2   // 12–23 months
    replace unempldur_l = 24  if lstatus==2 & seekdur_group==3   // 2+ years
	replace unempldur_l = .   if lstatus!=2 | seekdur_group==9
...
*<_unempldur_u_>
	...
	replace unempldur_u = 5   if lstatus==2 & seekdur_group==0   // 5 months group -> upper 5
    replace unempldur_u = 11  if lstatus==2 & seekdur_group==1   // 6–11 months
    replace unempldur_u = 23  if lstatus==2 & seekdur_group==2   // 12–23 months
    replace unempldur_u = .   if lstatus==2 & seekdur_group==3   // 2+ years -> open-ended
    replace unempldur_u = .   if lstatus!=2 | seekdur_group==9
...
{
*<_empstat_>
	* Here there is only self-employed, no distinction between employer and independent worker
	drop empstat
	gen empstat = stapro_group
	recode empstat (0 = 4) (1= 1) (2 = 2)
...
*<_industry_orig_>
	gen industry_orig = nace2d
	replace industry_orig= . if lstatus!=1 | missing(nace2d)
...
*<_occup_isco_>
	gen occup_isco = occup_orig + "0" if lstatus == 1
...
*<_firmsize_l_>
	gen int firmsize_l = .
    replace firmsize_l = 1  if sizefirm_a == 1
	replace firmsize_l = 10  if sizefirm_a == 2
    replace firmsize_l = 20  if sizefirm_a == 3
    replace firmsize_l = 50  if sizefirm_a == 4
    replace firmsize_l = 250 if sizefirm_a == 5
    replace firmsize_l = 1   if sizefirm_a == 6
    replace firmsize_l = 10  if sizefirm_a == 7
...
*<_firmsize_u_>
    gen int firmsize_u = .
	replace firmsize_u = 9	  if sizefirm_a == 1
    replace firmsize_u = 19   if sizefirm_a == 2
    replace firmsize_u = 49   if sizefirm_a == 3
    replace firmsize_u = 249  if sizefirm_a == 4
    replace firmsize_u = .    if sizefirm_a == 5
    replace firmsize_u = 9    if sizefirm_a == 6
    replace firmsize_u = .    if sizefirm_a == 7
...
{
*<_empstat_2_>
	gen empstat_2 = stapro2j_group
	recode empstat_2 (0 = 4) (1= 1) (2 = 2)
...
*<_industry_orig_2_>
	gen industry_orig_2 = nace2j2d_group
	replace industry_orig_2= . if lstatus!=1 | missing(nace2j2d_group)
