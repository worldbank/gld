...
<_Program name_>				BRA_1999_PNAD_V01_M_V03_A_GLD_ALL.do </_Program name_>
...
<_Study ID_> 				BRA_1999_PNAD_v01_M </_Study ID_>
<_Data collection from_> 	09/1999 </_Data collection from_>
<_Data collection to_> 		09/1999 </_Data collection to_>
...
* Date: 2024-11-04 - Update vars socialsec

* Date: 2026-05-15 - update gen gaul_adm2_code, subnatid1, subnatid2, subnatid3, subnatidsurvey
...
*----------1.2 Set directories-------------------------------------------------*
	...
	* Define path sections
	local server  "C:/Users/wb529026/Documents"
	local country "BRA"
	local year    "1999"
	local survey  "PNAD"
	local vermast "V01"
...
* Get some of what you did in the transformation and put it here
	...
	use "`path_in_stata'\individuals_1999.dta"


	preserve

	use "`path_in_stata'\households_1999.dta", clear
	...
	merge m:1 UF V0102 V0103 using `hhs', nogen keep(1 3)
...
tostring V0102,  replace format(%05.0f) force

tostring V0103, replace format(%02.0f) force

egen id_dom = concat(UF V0102 V0103)

	sort id_dom

	lab var id_dom "Household id"

	rename V301 id_pes

	lab var id_pes "Individual id"

bys id_dom: egen n_ind_1 = max(id_pes)
...
bys id_dom: gen id_pes2 = _n
	...
	replace id_pes = id_pes2 if n_ind_same != 0
...
cap gen count = 1
	...
	* population estimate of 119,7M
	tab count [iweight = V4729]
...
*----------2.2.1.2: Checking person id-----------------------------------------*
	...
	*  0 non-distinct household identifiers (never more than one duplicate)
	egen id_pnad = concat(id_dom id_pes), format(%24.2g)
	sort id_pnad
...
/* <_year_note>
	...
	1980's: V101 represents HH number
...
/* <_hhid_note>
	...
	1980's: V101 is hhid
	1983 & 1990: V101 doesn't exist; V102 & V103 do
	1999-2012: V102 has 6-digits rather than 8-digits (lacking UF digits at begining)
...
/* <_weight_note>
	...
	1999-1999: contains "-1", which should be changed to missing (.)
...
============================================================================%%*/
/* A. RIGHT STATES CODE */
* ADD TOCANTINS WITH GOIÁS

/* A.1 IN VARIABLE UF AND CREATE VARIABLE OF REGION */
destring UF, replace
recode UF (17=52)
gen region = int(UF/10)
label var region "region"
tostring UF, replace


/* B. V102 AND SERIAL NUMBER */
drop V0103

/* C. RECODE AND RENAME OF VARIABLES */
...
/* C.3 RECODE: WEIGHTS */
* BETWEEN 92 AND 2001, "-1" SEEMS TO BE == MISSING
recode weight (-1=.)

/* C.4 RECODE: TOTAL PEOPLE, TOTAL PEOPLE 10 YEARS OR MORE AND 5 YEARS OR MORE */
* BETWEEN 92 AND 2001, "-1" SEEMS TO BE == MISSING
recode V0105 (-1=.)
recode V0106 (-1=.)
* IN THE YEAR 2001 THERE IS NO TOTAL varIABLE OF PEOPLE 10 YEARS OR MORE,
* BUT TOTAL PEOPLE 5 YEARS OR MORE
replace V0106 = . if year==2001

rename V0105 tot_pess
rename V0106 tot_pess_10_more

/* C.5 RENAME: TYPE OF DOMICILE */
rename V0201 species_dom

/* C.6 RENAME: HOUSEHOLD TYPE */
rename V0202 type_dom
...
/* C.8 RECODE: COVERAGE */
recode V0204 (7=6) (9=.)
rename V0204 coverage

/* C.9 DUMMY: WATER SUPPLY */
recode V0212 (2=1) (4 6=0) (9=.)
rename V0212 water_rede
replace water_rede = 1 if V0213==1
replace water_rede = 0 if V0213==3
label var water_rede "water comes from the network"

/* C.10 RECODE: SEWER */
recode V0217 (1=0) (3=2) (5 7=6) (9=.)
rename V0217 sewer
* sewer = 0 general network
* = 2 septic tank
* = 4 rudimentary pit
* = 6 other
...
* C.11.1 DUMMY: THERE IS A TOILET
recode V0215 (3=0) (9=.)
rename V0215 sanit
label var sanit "has toilet"

/* C.11.2 DUMMY: EXCLUSIVE TOILET */
recode V0216 (2=1) (4=0) (9=.)
rename V0216 sanit_excl
label var sanit_excl "domicile sanit excl"

/* C.12 DUMMY: GARBAGE */
recode V0218 (2=1) (3/6=0) (9=.)
rename V0218 garbage
label var garbage "garbage is collected"

/* C.13 DUMMY: ELECTRIC LIGHTING */
recode V0219 (3 5=0) (9=.)
rename V0219 ilum_eletr
label var ilum_eletr "has electric ilum"

/* C.14 RECODE: NUMBER OF ROOMS AND DORMITORIES */
recode V0205 (-1 99=.)
rename V0205 comodos
recode V0206 (-1 99=.)
rename V0206 dormit

/* C.15 DUMMY: OCCUPANCY CONDITION */
recode V0207 (2 = 1) (3/6 = 0) (9 = .), gen(possession_dom)
label var possession_dom "domicile possession"
drop V0207

/* C.16 RENTAL VALUE / INSTALLMENT */
recode V0208 V0209 (-1=.)
recode V0208 V0209 (99999999/max=.)
rename V0208 rent
rename V0209 installment
...
gen stove = 1 if V0221 == 1 | V0222 == 1
replace stove = 0 if V0221 == 0 & V0222 == 0
replace stove = . if V0221 == . & V0222 == .
label var stove "has stove"
drop V0221 V0222

/* C.19 DUMMY: REFRIGERATOR */
recode V0228 (2 4=1) (6=0) (9=.)
rename V0228 refrigerator

/* C.20 DUMMY: RADIO */
recode V0225 (3=0) (9=.)
rename V0225 radio

/* C.21 DUMMY: TELEVISION */
recode V0226 (2=1) (4=0) (9=.)
recode V0227 (3=0) (9=.)
gen tv = 1 if V0226 == 1 | V0227 == 1
replace tv = 0 if V0226 == 0 & V0227 == 0
replace tv = . if V0226 == . & V0227 == .
label var tv "has television"
drop V0226 V0227
{
...
*<_subnatid1_>
	...
	Until 1999: not standardised
	
	1st digit of "UF" variable
	
	</_subnatid1_note> */
	gen subnatid1 = .
	replace subnatid1 = 1 if substr((UF), 1, 1) == "1"
	replace subnatid1 = 2 if substr((UF), 1, 1) == "2"
	replace subnatid1 = 3 if substr((UF), 1, 1) == "3"
	replace subnatid1 = 4 if substr((UF), 1, 1) == "4"
	replace subnatid1 = 5 if substr((UF), 1, 1) == "5"
...
*<_subnatid2_>
	...
	Until 1999: not standardised
	Until 1999: TO (17) part of GO (52), though created in 1988 (changes region/subnatid1)
	...
	decode subnatid2, gen(subnatid2_str)
	rename subnatid2 subnatid_num
	rename subnatid2_str subnatid2
...
*<_subnatid3_>
	...
	gen subnatid3 = .
	replace subnatid3 = 1 if UF == 15 & area_censit == 1
	replace subnatid3 = 2 if UF == 23 & area_censit == 1
	replace subnatid3 = 3 if UF == 26 & area_censit == 1
	replace subnatid3 = 4 if UF == 29 & area_censit == 1
	replace subnatid3 = 5 if UF == 31 & area_censit == 1
	replace subnatid3 = 6 if UF == 33 & area_censit == 1
	replace subnatid3 = 7 if UF == 35 & area_censit == 1
	replace subnatid3 = 8 if UF == 41 & area_censit == 1
	replace subnatid3 = 9 if UF == 43 & area_censit == 1
	...
	decode subnatid3, gen(subnatid3_str)
	drop subnatid3
	rename subnatid3_str subnatid3
	label variable subnatid3 "Subnational ID at Third Administrative Level"
*</_subnatid3_>

*<_subnatidsurvey_>
	* Hierarchy from smaller to larger:
	* 9 metropolitan areas, otherwise the state
...
*<_gaul_adm2_code_>
	gen gaul_adm2_code = subnatid_num
...
*<_migrated_binary_>
	gen migrated_binary = 1
	replace migrated_binary = 0 if V501 & V504 == 4 & V510 == 2 & V511 == 3
...
/* <_migrated_from_cat_note>
	...
	gen comp_5090 = substr(subnatid2, 1, 2)
	destring comp_5090, replace
	
	gen V5090_1 = floor(V5090/10)
	gen comp_5090_1 = floor(comp_5090/10)
	
	gen migrated_from_cat = .
	* There are two blocks / paths for those who have lived in another 
	* UF (V504 is yes) we have last UF (code V5090) that can be compared 
	* for the others, if follows they are in the same UF
	replace migrated_from_cat = 2 if migrated_binary == 1 & mi(V5090)
	replace migrated_from_cat = 5 if V5090 == 98
	replace migrated_from_cat = 4 if V5090 < 98 & comp_5090 != V5090 & V5090_1 != comp_5090_1 
	replace migrated_from_cat = 3 if V5090 < 98 & comp_5090 != V5090 & V5090_1 == comp_5090_1
	replace migrated_from_cat = 2 if V5090 < 98 & comp_5090 == V5090 
...
*<_migrated_from_code_>
	...
	replace migrated_from_code = subnatid2 if inrange(migrated_from_cat, 2, 3)
	replace migrated_from_code = "1 - North"        if migrated_from_cat == 4 & V5090_1 == 1
	replace migrated_from_code = "2 - Northeast"    if migrated_from_cat == 4 & V5090_1 == 2
	replace migrated_from_code = "3 - Southeast"    if migrated_from_cat == 4 & V5090_1 == 3
	replace migrated_from_code = "4 - South"        if migrated_from_cat == 4 & V5090_1 == 4
	replace migrated_from_code = "5 - Central-West" if migrated_from_cat == 4 & V5090_1 == 5
...
/* <_ed_mod_age_note>
	...
	As of 1999: asked to all ages

	Comparability throughout the years is compromised:
	1999: skip pattern introduced
...
/* <_school_note>
	...
	As of 1999: "Pre-school" and "School" are considered separately
...
/* <_educy_note>
	...
	Unti 1999: one set of values for each of 0-8, 9-11 and 12+
	As of 1999: 0-16
	...
	replace educy = 17 if V607 == 7 & V610 == 2
	replace educy = 18 if V607 == 7 & V610 == 3
	replace educy = 19 if V607 == 7 & V610 >= 4 & V610 <= 8
*</_educy_>
...
/* <_educat7_note>
	...
	As of 1999: "Classe de alfabetização" category is added
	1981 & 1999: "Alfabetização de adultos" exists

</_educat7_note> */
	gen educat7 = .
	replace educat7 = 1 if educy == 0
	replace educat7 = 2 if (educy >= 1 & educy <= 7) | (V603 == 1 | V603 == 3 | V603 == 6 | V603 == 7 )
	replace educat7 = 3 if educy == 8
	replace educat7 = 4 if (educy >= 9 & educy <= 10) | (V603 == 2 | V603 == 4 )
	replace educat7 = 5 if educy == 11
	replace educat7 = 7 if (educy >= 12 & educy <= 15) | (V603 == 5 | V603 == 9)
...
*<_educat_isced_>
	...
	label variable educat_isced "ISCED 1999 standardised level of education"
...
/* <_educat_orig_note>
	...
	1999: "Classe de alfabetização" category is added
	1981 & 1999: "Alfabetização de adultos" exists
...
/* <__note>
	...
	1999: A broader definition of "work" was adopted, that includes
...
/* <_lstatus_note>
	...
	Until 2006 (except 1999 & 1999): "employed" could be answered by people 5 years or older
...
/* <_empstat_note>
	...
	Comparability throughout the years is compromised:
	Until 1999: "funcionários públicos estatutários" (public servants) and "militares" (army) could not be identified
	Until 1999: "trabalhadores domésticos" (domestic workers) inexisted
	Until 1999: "empregados" (workers) contained both "com carteira" and "sem carteira" (formal and informal)
	As of 1999: "production for own consumption" and "construction for own use" exist
	As of 1999: "empregado sem declaração de carteira" (probably) became "empregados sem carteira"
	As of 1999: "trabalhador doméstico sem declaração de carteira" (probably) became "trabalhadores domésticos sem carteira"
...
gen v0504 = substr(industry_orig,3,3)
	...
	merge m:1 v0504 using "`path_in_stata'\Ind_Corresp_198090s.dta"
	drop if _merge == 2
...
/* <_occup_isco_note>
	...
	gen cbo_91 = substr(occup_orig,2,4)
	merge m:1 cbo_91 using "`path_in_stata'\isco_88_cbo_91_DD.dta"
	drop if _merge == 2
...
*<_occup_>
	...
	replace occup = 10 if lstatus == 1 & substr(occup_isco, 1, 2) == "01"
	recast byte occup
...
/* <_contract_note>
	...
	replace contract = 1 if V9034 == 2 | V9035 == 1
...
*<_socialsec_>
	rename V9059 socialsec // [1999: 4761]
	...
	label define socialsec_lab 1 "With social security" 0 "Without social security"
...
/* <_industrycat_isic_2_note>
	...
	gen v0504 = substr(industry_orig_2,3,3)
	...
	merge m:1 v0504 using "`path_in_stata'\Ind_Corresp_198090s.dta"
	drop if _merge == 2
	gen industrycat_isic_2 = floor(v9997/10) *100
	tostring industrycat_isic_2, replace
	replace industrycat_isic_2 = "0100" if industrycat_isic_2 == "100"
	replace industrycat_isic_2 = "0200" if industrycat_isic_2 == "200"
	replace industrycat_isic_2 = "0300" if industrycat_isic_2 == "300"
	replace industrycat_isic_2 = "0400" if industrycat_isic_2 == "400"
	replace industrycat_isic_2 = "0500" if industrycat_isic_2 == "500"
	replace industrycat_isic_2 = "5100" if v0504 == "410"
	replace industrycat_isic_2 = "5000" if v0504 == "419"
	replace industrycat_isic_2 = "5200" if industrycat_isic_2 == "5300"
...
/* <_occup_isco_2_note>
	...
	gen cbo_91 = substr(occup_orig_2,2,3)
	merge m:1 cbo_91 using "`path_in_stata'\isco_88_cbo_91_DD.dta"
	drop if _merge == 2
	gen occup_isco_2 = isco_88+"00"

	replace occup_isco_2 = "0100" if occup_isco_2 == "100"
...
/* <_occup_2_note>
	...
	replace occup_2 = 0 if lstatus == 1 & substr(occup_isco_2, 1, 2) == "01"
	recast byte occup_2
...
/* <_empstat_year_note>
	...
	recode empstat_year (1/8 = 1) (13 = 2) (10 = 3) (9 11/12 = 4) (14 = .)
...
/* <_industrycat_isic_year_note>
	...
	gen v0504 = substr(industry_orig_year,3,3)
	...
	merge m:1 v0504 using "`path_in_stata'\Ind_Corresp_198090s.dta"
	drop if _merge == 2
	gen industrycat_isic_year = floor(v9997/10) *100
	tostring industrycat_isic_year, replace
	replace industrycat_isic_year = "0100" if industrycat_isic_year == "100"
	replace industrycat_isic_year = "0200" if industrycat_isic_year == "200"
	replace industrycat_isic_year = "0300" if industrycat_isic_year == "300"
	replace industrycat_isic_year = "0400" if industrycat_isic_year == "400"
	replace industrycat_isic_year = "0500" if industrycat_isic_year == "500"
	replace industrycat_isic_year = "5100" if v0504 == "410"
	replace industrycat_isic_year = "5000" if v0504 == "419"
	replace industrycat_isic_year = "5200" if industrycat_isic_year == "5300"
	replace industrycat_isic_year = "" if industrycat_isic_year == "."
	replace industrycat_isic_year = "" if industrycat_isic_year == "00"
...
/* <_occup_isco_year_note>
	...
	gen cbo_91 = substr(occup_orig_year,2,3)
	merge m:1 cbo_91 using "`path_in_stata'\isco_88_cbo_91_DD.dta"
	drop if _merge == 2
	gen occup_isco_year = isco_88 +"00"
	replace occup_isco_year = "0100" if occup_isco_year == "100"
	label variable occup_isco_year "ISCO code of secondary job 12 month recall"
	drop _merge cbo_91 isco_88
	replace occup_isco_year = "" if occup_isco_year == ".00"
	replace occup_isco_year = "" if occup_isco_year == "00"
*</_occup_isco_year_>
...
/* <_occup_isco_2_year_note>
	...
	*merge m:1 cbo using "`path_in_stata'\isco_cbo.dta"
	*drop if _merge == 2
