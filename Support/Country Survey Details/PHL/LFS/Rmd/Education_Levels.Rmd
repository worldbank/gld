---
title: "Education Levels: Philippines GLD"
output: 
  github_document:
    html_preview: false
    toc: true
...
Philippine Standard Classification of Education (PSCED) document, the
education system was adjusted in 2018 in accordance with new
legislation, linked
...
Most years of education data are encoded as factor -- or numeric values
with text labels to indicate what each value means. A few years are
simply encoded as numeric data without value labels. In lieu of a yearly
codebook or something of the like, the education data values were
triangulated between the PSCED documents (linked below) and each survey
file's labels. Since the underlying numeric data values varied
considerably compared to the labelled data, the labelled information was
prioritized over the numeric data when discrepancies occurred.

Below, I have attempted to piece together two essential bits of
information for each of the education schema: the elemental stages of
education and the primary possible categorical values that may fall into
these stages. I have also recorded notes specific to rounds or years
that are worth noting in the data.

## Regarding ISCED Data

After group discussion, it was determined that mapping the Philippines
education values to the corresponding International Standard of
Classification of Education [(ISCED) from
UNESCO](http://uis.unesco.org/en/isced-mappings) would via a standard
key-value approach would not yield high quality results. The main reason
for this is the underlying difference in encoding of education data from
the labor data (industry and occupation). The education data numeric
values for the education levels they represent within each PSCED schema,
meaning that a single key cannot be created for each PSCED revision.
This is not the case with the labor data and a key can be made more
reliably in these cases. As such, we have decided to leave the `isced_*`
variables missing for the Philippines.
...
years in secondary school. It is likely due to this redefinition of
secondary in 2019 that causes an increase in observations to be coded as
`educat7 == 4` or `"secondary incomplete"` compared to previous years
where the standard for secondary completion was fewer years of
schooling.
...
2, grade 3, grade 4, and grade 5 -- and making labels for each -- the
data has been constructed with a "new" value `y` that represents "grades
1-5" with a label such as "Grades 1-5". Neither this value nor label
appear in the 2017 PSCED, nor the numeric versions of the variables in
other rounds. The original values that represent grades 1, grades 2,
grades 3, etc do not appear in the factor labelled data variables, nor
is there an "original" integer variable available.
