...
We load tidyverse and the previously appended GLD output data for all survey years,
1997 - 2019.
...
consistent with those published by the PSA. All but two, or 21 of 23 years, pass our self-imposed consistency tests. EVen so, the data for these two years that do not pass are still within reasonable margins of error. 
...
gg.pop <- ggplot(sum.15.y, aes(year, dif_pop_15up)) +
  ...
  scale_y_continuous(labels = scales::label_number(), limits = c(0,10000000)) +
...
gg.lfp <- ggplot(sum.15.y, aes(year, dif_lfp_15up)) +
  ...
  scale_y_continuous(labels = scales::label_number(), limits = c(-1, 10)) +
