...
# Define constants
base_path = 'Z:/__I2D2/Mongolia'
pattern = '*/LFS/Processed/MNG_*_I2D2_LFS.do'
vermast = 'V01'
veralt = 'V01'
server = "Y:/GLD-Harmonization/510859_AS"
GLD_path = "C:/Users/wb510859/Downloads/GLD_Harmonization_Template.do"
Stata_version = "Stata 17"
...
def update_GLD(var_name_I2D2, tag_GLD, I2D2, GLD):
    ...
    new_block = [line for line in [labelvar_line, labeldefine_line, labelvalues_line] if line is not None]
...
for I2D2_path in do_files:
    ...
    for i, line in enumerate(GLD):
        ...
        if "[Name of your do file]" in line:
            gld_do_filename = f"{gld_path_details['country']}_{gld_path_details['year']}_{gld_path_details['survey']}_{gld_path_details['vermast']}_M_{gld_path_details['veralt']}_A_GLD_ALL.do"
            GLD[i] = line.replace("[Name of your do file]", gld_do_filename)
        if "[Name of your software (STATA) and version]" in line:
            GLD[i] = line.replace("[Name of your software (STATA) and version]", Stata_version)
        if "[Country_Name (CCC)]" in line:
            GLD[i] = line.replace("[Country_Name (CCC)]", country)
