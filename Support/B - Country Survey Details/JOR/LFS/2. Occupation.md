...
The table below reports both the survey-weighted shares and the unweighted sample counts for each year. The weight variables in the Jordan LFS raw files (`pweight` in 2009, `hweight` in 2010, `wt_rel` in 2011) are normalized design weights that sum approximately to the sample size rather than expansion weights that inflate to the national population. Weighted frequencies therefore cannot be interpreted as population employment counts, but weighted percentages are valid for distributional analysis and are reported here.
...
Unweighted counts and percentages are taken directly from the raw 3-digit `tab` output. Weighted percentages are derived from the weighted raw tabs (`occ_isco88_3 [iw=pweight]` for 2009, `occ_isco08_3 [iw=hweight]` for 2010, `q222 [iw=wt_rel]` for 2011). The 2011 column confirms the series stabilizes after the classification change.
