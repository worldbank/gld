...
The official annual reports currently bundled with the GEO CSD are linked in the table above. These reports mainly cover employment and unemployment rates and their changes relative to the previous year. For 2024, the harmonized individual count shown in the table corresponds to persons aged 15 and above only, since the harmonized file does not retain observations below that age.    
...
However, without the original question-based variables, GLD variables such as `lstatus`, `potential_lf`, and `wage_no_compen` could not be constructed consistently. As noted above, we therefore obtained additional labour files from NSO containing variables pulled directly from the survey. Labour and income variables were coded using these files. The detailed discussion is provided in [this separate document](Merging_Original_Employment_Data_and_Coding_Labour_Variables.md). 

For the post-2019 rounds, an additional complication is that the delivered labour files do not always retain the questionnaire items needed to identify future starters directly. This matters because some future starters are skipped out of the job-search methods block, so unemployment would be understated if it were reconstructed only from observed search methods and availability. Under the questionnaire design, the relevant future starters are those expected to begin work within three months, which is also the group consistent with the ILO treatment used in the earlier rounds. In practice, the raw derived variable `Unemployed` appears to preserve this logic and is therefore the preferred source for coding unemployment in the post-2019 rounds. The issue is especially important in 2024, where `G1` is not retained in the delivered labour file.

For wages, the earlier rounds retain both exact earnings and bracket responses, which makes it possible to impute bracket-only cases from the distribution of exact wages among comparable workers. In 2024, by contrast, the delivered wage data retain only the bracketed earnings variable and not the exact amount. For this reason, `wage_no_compen` in 2024 is approximated directly from the reported brackets rather than imputed from exact wages.
