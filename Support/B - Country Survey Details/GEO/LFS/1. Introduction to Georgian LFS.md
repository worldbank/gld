...
The GEO LFS mainly covers detailed information on demographic details, including age, sex, marital status, and location, education, and labour market activities over the previous 7 days for all private household members aged 15 and above. Years used for harmonization range from 2017 to 2024 continuously, 8 years in total.
...
| [2024](utilities/report/GEO_2024_report.pdf) | 20,891         |  57,546     |  2,975,152  | 

The official annual reports that are currently bundled with the GEO CSD are linked in the table above. Each year's annual report mainly covers employment and unemployed rates as well as their changes compared to the previous year. For 2024, the harmonized individual count shown in the table corresponds to persons aged 15 and above only, since the received harmonized file does not retain observations below that age.    

## Where can the data be found?

Main data from 2017 to 2024 are publicly available on the [Georgia National Statistics Office website](https://www.geostat.ge/en/modules/categories/130/labour-force-survey-databases). The downloadable datasets include sub-datasets that cover different sections in the questionnaire, namely demographics, economic activities, excluded populations, and personal weights.  
...
For the classification of occupation, all harmonized years followed the International Standard Classification of Occupation (ISCO) and did not have a separate national classification. 2017 to 2019 used ISCO 1998 whereas since 2020 used ISCO 2008.  
...
However, without the un-coded original variables directly produced from the survey, GLD variables including `lstatus`, `potential_lf`, and `wage_no_compen` could not be created in a consistent way. As mentioned in the ["Where can the data be found?"](#where-can-the-data-be-found) part, we obtained additional labour files from NSO containing labour variables pulled directly from the survey. We coded labour-related variables and income variables using these files. The detailed explanations about merging extra data and coding labour variables are covered in [this separate document](Merging_Original_Employment_Data_and_Coding_Labour_Variables.md). 

For the 2020 onward rounds, an additional complication is that the received labour files retain the derived raw variable `Unemployed`, but do not always retain the questionnaire variables needed to identify future starters directly. This matters because some future starters are skipped out of the job-search methods block, so reconstructing unemployment only from observed search methods and availability would undercount unemployment. In the questionnaire, the relevant future starters are those expected to start within three months, which is the group consistent with the ILO definition used in the earlier rounds. In practice, the derived raw `Unemployed` variable appears to preserve this within-three-month future-starter logic, and is therefore the preferred source for coding unemployment in the post-2019 rounds. This issue is particularly important in 2024, where the received labour file does not retain `G1`, so future starters cannot be identified directly from the questionnaire items in the delivered data.

For wages, the earlier rounds retain both exact earnings and bracket responses, which makes it possible to impute bracket-only cases from the distribution of exact wages among comparable workers. In 2024, by contrast, the received wage data retain only the bracketed earnings variable and not the exact amount. For this reason, `wage_no_compen` in 2024 is approximated directly from the reported brackets rather than donor-imputed from exact wages.
...
The GLD codes the harmonization’s `lstatus` variable based on the concept used in the survey. In the case of the GEO LFS this change occurs after 2019, when the survey switches to the new definition. This implies that estimates for 2020-2024 may not be directly comparable with those from the previous years. However, it is possible to recode `lstatus` such that it matches the previous definition. The precise details are covered in a [separate document here](Converting%20between%20ICLS%20Definitions.md).
