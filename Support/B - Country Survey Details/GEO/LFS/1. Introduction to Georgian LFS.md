...
However, without the un-coded original variables directly produced from the survey, GLD variables including `lstat`, `potential_lf` couldn't be created. As mentioned in the ["Where can the data be found?"](#where-can-the-data-be-found) part, we obtained the original labour files from NSO which contains labour variables pulled out directly from the survey. We coded labour-related variables and income variables using these files. The detailed explanations about merging extra data and coding labour variables are covered in [this separate document](Merging_Original_Employment_Data_and_Coding_Labour_Variables.md). 

For the 2020 onward rounds, an additional complication is that the received labour files retain the derived raw variable `Unemployed`, but do not retain the questionnaire variables `G1` and `G2` that identify future starters and the expected time until starting work. This matters because some future starters are skipped out of the job-search methods block, so reconstructing unemployment only from observed search methods and availability would undercount unemployment. In the questionnaire, the relevant future starters are those expected to start within three months, which is the group consistent with the ILO definition used in the earlier rounds. In practice, the derived raw `Unemployed` variable appears to preserve this within-three-month future-starter logic, and is therefore the preferred source for coding unemployment in the post-2019 rounds.
