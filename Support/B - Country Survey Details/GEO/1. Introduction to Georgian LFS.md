...
But since 2017, the CSA adopted a new "two-two-two" rotation scheme which interviewed a sampled household in two consecutive quarters, then excluded this household for the next two quarters, and re-visited the household for the same two quarters of the next year. In this way, the newly added households account for one-fourth of the total sample size per quarter. However, the indicator of visiting record is not included in the raw data sets. Without the variable of visit, we are not able to uniquely identify households throughout time, and hence are not able to establish the panel data. An overall introduction and basic information of the GEO LFS is covered in this [offical document](utilities/GEO_intro.pdf).
