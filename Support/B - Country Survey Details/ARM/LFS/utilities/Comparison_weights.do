...
gen     lfp_new_series = .
replace lfp_new_series = weight            if inlist(lstatus,1,2) & inrange(year,2018,2023)

gen     wap_new_series = .
replace wap_new_series = weight            if inlist(lstatus,1,2,3) & inrange(year,2018,2023)
...
replace lfp_off = 1341150 if year == 2023
...
twoway (line lfp_old_series year) (line lfp_new_series year) (line lfp_off year), ///
    ...
    legend(order(1 "LFP with weight 2014-17, old weigth 2018-19" 2 "LFP with weight 2018-2023" 3 "LFP ARMSTAT Data") ///
	position(6) row(3))
