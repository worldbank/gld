...
### Wage data
In the LFS, individuals are first asked to report their exact wage. If they are unable or unwilling to provide this information, they are then asked to report their earnings using predefined wage ranges.Within GLD, wages are typically estimated using the median wage among individuals who report an exact salary. However, this approach is applied only when respondents providing exact wages represent a clear majority of the population, usually at least 75 percent.

In the case of this survey, this condition is not met. As a result, information on exact wages is retained only for individuals who reported them directly, while wages for individuals who reported income in ranges are left as missing in the harmonized dataset.

As an alternative, users may consult version 1 of the `.do` files, where a different approach is implemented. In that version, wages for individuals reporting income ranges are approximated using the midpoint of the corresponding income interval.

Finally, it is important to note that while the survey collects both exact wages and wage ranges in all years, prior to 2010 GLD only contains information on wage ranges. Both exact wages and wage ranges are available in GLD from 2010 onwards.
