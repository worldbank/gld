...
| 2023 | 122,134 | 612,650 |
| 2024 | 123,271 | 589,504 |

The Bangladesh QLFS implements a rotational sampling design, in which households may be interviewed in consecutive quarters before being replaced. Household and individual counts above are based on the harmonized annual files. Individuals interviewed in multiple quarters may therefore appear more than once in the annual individual count.
...
The raw datasets do not provide a single harmonized household identifier that can be used consistently across survey modules and rounds. In the GLD harmonization, household IDs are constructed using the available geographic and survey identifiers, including the primary sampling unit and household identification number.

For the 2024 round, a small number of duplicated household identifiers were identified in the raw data, where different households shared the same combination of quarter, PSU, enumeration area, and household number. During harmonization, the duplicated households were identified using repeated person identifiers within these household keys. The affected households were then assigned new household numbers sequentially (e.g., 999, 998, 997), and the corresponding person identifiers were reassigned in descending order (99, 98, 97, etc.). These corrections were applied consistently across the roster, socioeconomic, employment and education, and migration modules to preserve unique household-person identifiers and ensure correct merging across modules.
...
The Bangladesh QLFS employs the Bangladesh Standard Industrial Classification (BSIC), which is adapted from the International Standard Industrial Classification (ISIC) Revision 4.

| Year | Classification in survey | ISIC revision mapped to | How mapped |
|:---:|:---:|:---:|:---|
| 2015-16 | BSIC 2009 | ISIC Rev. 4 | First 3 digits; first 2 digits if the BSIC code does not exist at 3 digits in ISIC Rev. 4 |
| 2016-17 | BSIC 2009 | ISIC Rev. 4 | First 3 digits; first 2 digits if the BSIC code does not exist at 3 digits in ISIC Rev. 4 |
| 2022 | BSIC 2020 | ISIC Rev. 4 | First 2 digits retained and mapped at the ISIC division level; unidentified source codes left uncoded |
| 2023 | BSIC 2020 | ISIC Rev. 4 | First 2 digits retained and mapped at the ISIC division level; unidentified source codes left uncoded |
| 2024 | BSIC 2020 | ISIC Rev. 4 | First 2 digits retained and mapped at the ISIC division level; unidentified source codes left uncoded |

As of time of writing （Aug 2026), an update of the BSIC has been released in 2020 to use more detailed categories of industries. While the ISIC version 4 details occupational categories at four digits, the BSIC 2020 is detailed at six digits. Based on the table below, both BSIC 2020 and BSIC 2009 are comparable at most at the single digit level. BSIC is comparable with the ISIC Rev. 4 at 2 digits except for an additional two-digit code that is not in ISIC, and neither was it reported as an industry of any individual in either survey. 
...
| 2022 | BSCO 2020 | ISCO 2008 | First 4 digits used where valid; non-standard codes mapped to the most detailed valid ISCO level available, with unidentifiable codes left uncoded   |
| 2023 | BSCO 2020 | ISCO 2008 | First 4 digits used where valid; non-standard codes mapped to the most detailed valid ISCO level available, with unidentifiable codes left uncoded   |
| 2024 | BSCO 2020 | ISCO 2008 | First 4 digits used where valid; non-standard codes mapped to the most detailed valid ISCO level available, with unidentifiable codes left uncoded|


As of time of writing (Aug 2026), an update of the BSCO has been released in 2020 to use more detailed categories of occupation. While the ISCO 2008 details occupational categories at four digits, the BSCO 2020 is detailed at six digits. Based on the table below, both BSCO 2012 and 2020 are comparable at most at the 3-digit level, and both are comparable with the ISCO at 2 digits. For more information, refer to this [document](Utilities/3(v)-Bangladesh-18Feb2022.pdf).
