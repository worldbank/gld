...
The wage variable in the GLD, `wage_no_compen` uses all available information to estimate household income from primary employment. Generally, the datasets include earnings in-cash, in-kind, and net business income. Also, when households cannot provide an point estimate of income, they are made to choose where they think their income is likely to fall across different ranges of monthly income values. However, there are specific years where estimating full income is not possible. In 2018, data is only available income earned in cash among respondents aged 16 and above; and in 2019, no income data is available in the retrieved raw dataset. For more information in harmonizing `wage_no_compen`, refer to  this [separate document](Deriving%20employment%20variables.md).
