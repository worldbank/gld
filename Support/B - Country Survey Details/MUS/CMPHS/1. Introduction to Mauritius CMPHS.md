...
The official reports detail estimates by area of residence, namely urban and rural areas. However, the microdata received by the GLD team do not include the corresponding urban/rural identifier. As a result, although the survey is documented as representative at the urban/rural level, the standard GLD variables `urban` and `subnatidsurvey` could not be coded from the available files.
...
Beyond weights, the microdata received for the Mauritius CMPHS also lack several standard variables that are often used in GLD harmonization. In particular, the files available to the GLD team do not provide the urban/rural identifier used in official tabulations, nor the sampling design identifiers needed to code variables such as `psu`, `ssu`, and `strata`. As a result, these variables remain uncoded in the harmonized files. This should be kept in mind when using the data for design-based estimation or when attempting to reproduce official urban-rural breakdowns from the GLD version alone.
