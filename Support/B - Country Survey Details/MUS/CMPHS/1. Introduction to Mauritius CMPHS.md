...
The official reports detail estimates by area of residence, namely urban and rural areas. In the microdata available to the GLD team, the urban-rural identifier is available for the earlier rounds (1999-2003), allowing `urban` and `subnatidsurvey` to be coded for those years. For the later rounds (2004-2012), however, the corresponding identifier is not present in the received files. As a result, although the survey remains documented as representative at the urban-rural level, `urban` and `subnatidsurvey` could not be coded for 2004-2012 from the available data.
...
### Missing standard GLD variables

Beyond weights, the microdata received for the Mauritius CMPHS also lack several standard variables that are often used in GLD harmonization. In particular, the later files (2004-2012) do not provide the urban-rural identifier used in official tabulations, nor the sampling design identifiers needed to code variables such as `psu`, `ssu`, and `strata`. As a result, these variables remain uncoded for that part of the series. This should be kept in mind when using the data for design-based estimation or when attempting to reproduce official urban-rural breakdowns from the GLD version alone.
