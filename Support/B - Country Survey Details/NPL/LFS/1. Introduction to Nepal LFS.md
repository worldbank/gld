...
The 1998 NPL LFS is the first labor force survey in the country. The Ninth Five-Year Development Plan of Nepal prioritized poverty alleviation through employment generation and skills enhancement. The national LFS was initiated to better monitor labor market and economic activities. The 1998 LFS was carried out by the Central Bureau of Statistics from 1998 to 1999, with technical assistance from the International Labour Organization and financial support from the United Nations Development Programme.
...
As the comments in the scripts explain, `A17_corrected` was created following the questionnaire's rules and it shows that 20.6% of the sample size are not eligible for the survey. These observations are in fact not interviewed, as they do not have values for variables after `A17`. The eligibility in `A17` is related to variable `A_16`. The NPL LFS required in 2014 that each individual spend 4 nights each week in the household over the previous four weeks in order to be thought of as a LFS eligible.
...
![codes](utilities/NPL_scripts.png)
...
![NPL_wage_analysis](utilities/NPL_wage_analysis.png)
...
![NPL_wage_analysis_data](utilities/NPL_wage_analysis_data.png)
![NPL_retiree_wage](utilities/retiree_wage.png)
