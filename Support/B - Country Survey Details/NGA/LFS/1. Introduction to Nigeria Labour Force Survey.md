...
### Availability of household files in 2024

For Q3 and Q4 of 2024, the release includes individual-level files but not separate household identification files. As a result, the harmonization relies on the household and individual identifiers available in the individual files for those quarters.

Household size is derived from the individual roster in the harmonized file, rather than taken from a household-level raw file, so that the variable is constructed consistently across the appended quarters.
