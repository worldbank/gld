...
The automated quality check flags this as a discrepancy because the external reference data are from 2022. When compared against 2024 ILOSTAT data (Employment → Status in Employment), the distributions align closely, with roughly 50% self-employed and around 17% employers. The harmonization is therefore consistent with official ILO figures.
...
### Sectoral Employment and ICLS-19 Definition

The Nigeria LFS follows the ICLS-19 definition of employment, which excludes subsistence (own-consumption) farmers from employment. As a result, the agricultural employment share is lower than comparisons based on older ICLS-13 reference data would suggest.

Recoding the data to match the pre-ICLS-13 definition — by adding subsistence farmers back into employment, identified via the `agf3bclean` variable — raises the agricultural employment share from approximately 33% to around 37% (weighted), bringing it into line with the 2022 ILO reference figures. Code to replicate the old definition is provided below.

This recoding applies to both the 2024 and 2025 surveys, as the questionnaire structure and relevant variables remain unchanged.
...
The PSU is defined as the combination of id3_lga (Local Government Area) and id4b_ea (Enumeration Area identifier). Several candidate definitions were evaluated against the survey methodology, which describes approximately 860–880 Enumeration Areas per quarter. Using id4b_ea alone yields only 1,458 unique values across the full 2024 pooled dataset, far lower than expected for four quarters of a rotating sample. The combination of id3_lga and id4b_ea, by contrast, yields 3,489 unique PSUs across all four quarters, which is closest to the expected range implied by roughly 860–880 EAs per quarter. While we cannot confirm with absolute certainty that this matches the original sampling design, this combination is the closest available match in the data and is therefore used as the PSU.
