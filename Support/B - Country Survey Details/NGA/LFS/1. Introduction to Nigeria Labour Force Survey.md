...
The PSU is constructed as the combination of `id3_lga` (Local Government Area) and `id4b_ea` (Enumeration Area identifier). The EA variable alone is not unique across LGAs, so the combined key is used to uniquely identify each enumeration area in the sample.

Note that the total number of unique PSUs in the pooled data is somewhat higher than the 860–880 EAs cited in the survey methodology documentation. This is because the pooled dataset covers multiple quarters with a rotating sample design, and the per-quarter count of approximately 640 EAs is somewhat lower than the documented target, likely reflecting implementation differences.
