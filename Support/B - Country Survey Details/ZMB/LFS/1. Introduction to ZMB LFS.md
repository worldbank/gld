...
For the quarterly LFS from 2023 onward, the individual quarterly samples are intended to provide reliable estimates at the national and rural/urban levels, while the combined annual sample supports estimates at the provincial level.
...
### Household identifier limitations in the 2023 and 2024 LFS

The 2023 and 2024 source data present some limitations for constructing household and individual identifiers.

For 2023, the original household identifier is not sufficient to uniquely identify households. During master-data processing, an anonymous household grouping was therefore reconstructed within cluster using available household identifying information. The identifying information was used only for this reconstruction and was removed before the master data were stored. The resulting harmonized data contain 10,277 reconstructed households.

For 2024, the source data contain geographic and sampling identifiers that can be used to construct a household identifier, but they do not fully distinguish all household-person records. The harmonized household identifier therefore combines the available geographic and sampling identifiers with `qssignid` (Questionnaire Assign ID), which substantially improves household disambiguation. A small number of records nevertheless remain non-unique at the household-person level because no additional source identifier is available to distinguish them reliably.

Users should therefore exercise caution when conducting household-level analysis or linking records using household- or person-level identifiers, particularly for 2024.
