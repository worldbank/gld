...
Kenya uses its own occupational system called the Kenya National Occupational Classification System (KNOCS). The 2020 - 2022 rounds of KCHS uses the KNOCS 2000, which aligns with ISCO-88 at the two digit level, with some discrepancies. As a result, two-digit mapping can be approximate for certain categories and may require three-digit mapping to avoid misclassification. 

For more details on KNOCS and the recommended KNOCS-to-ISCO 2-digit mapping, refer to this section on [KNOCS-ISCO crosswalk](KNOCS-ISCO-crosswalk.md).
