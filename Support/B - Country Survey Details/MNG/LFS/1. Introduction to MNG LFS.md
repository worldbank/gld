...
The employment status categories in the Mongolia LFS are broadly comparable in the recent rounds because the main status in employment question remains the same. However, the 2024 questionnaire reorganized the employee payment block. To approximate the previous rounds as closely as possible, the GLD continues to derive `empstat` from the main status question and uses the 2024 payment-type question only to separate remunerated employee or apprentice cases from explicitly unpaid ones. The precise details are covered in this [document](Employment%20Status.md).
...
### Updating of 2019 to 2022 

Data for 2019 to 2022 has a second raw data version (`MNG_20##_V02_M') as the GLD team was alerted to an update to the weigths released by the NSO. 

2019 is a special case as the updated version does not include the location variable needed to code urban/rural status, even though it is present in the earlier version (V01_M) and documented in the DDI. Since the two raw files match one-to-one on all person identifiers, the location variable is recovered from the earlier version via a merge and saved as a new Stata file that sits alongside the original download. The harmonization do-file reads from this merged file, not directly from the downloaded source. The merge is handled by a [dedicated do-file available here]("Utilities/convert_LFS_2019_eng.do"). If you download only the latest raw file and run the harmonization do-file directly, the code will fail at the urban/rural classification step because the location variable will be absent.
