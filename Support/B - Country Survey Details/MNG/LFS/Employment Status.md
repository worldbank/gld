...
In the 2024 round, the main employment status question remains broadly comparable with the earlier recent rounds, but the employee payment block is organized differently. To keep the harmonized `empstat` as close as possible to the previous methodology, the GLD still uses the main status question as the backbone of the mapping and then uses the 2024 payment-type question `E18` only to separate paid employee or apprentice cases from explicitly unpaid ones. In practice, this means that employee or apprentice cases with any reported form of payment in `E18` are treated as paid employees, while cases coded as unpaid in `E18` are treated as non-paid employees.
