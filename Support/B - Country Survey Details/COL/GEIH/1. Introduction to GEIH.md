...
### Change in labour force concepts: ICLS 13 to ICLS 19

Until 2021, the GEIH followed the traditional labour force framework, broadly closer to the 13th ICLS standards. From January 2022 onwards, with the redesigned GEIH sample frame 2018, DANE aligned the survey with the 19th ICLS framework, where employment is restricted to work for pay or profit and other forms of work are measured separately. For further information on DANE's redesign and conceptual update of the GEIH, see the [Presentation of the new GEIH approach](https://www.dane.gov.co/files/investigaciones/fichas/empleo/Presentacion_nuevo_enfoque_GEIH.pdf)

This change affects comparability in employment-related indicators. For details on how to approximate a backward recoding from ICLS 19 to ICLS 13, see [ICLS transition note](icls19to13.md).
...
| 2020-2025 |ISIC COLOMBIA AC 4| ISIC REV 4 |
...
### Limitations of the Migration Reason Variable

GEIH identifies prior residence using two questions with different time frames: P3382 asks where the person lived five years ago, while P3384 asks where they lived twelve months ago; yet the reason for migration (P3386) is only captured for the latter. In the harmonization, we use P3386 to construct the variable `migrated_reason`, but this choice should be interpreted with caution given the structure of the questionnaire.

This creates two related problems. First, coverage loss: anyone classified as a migrant under the five-year frame but not the twelve-month frame has no recorded motive. Second, more subtly, both questions are “where did you live at time X” rather than “did you move at any point in the interval.” A person who lived in municipality A five years ago, moved to B in between, and returned to A before the twelve-month reference date will answer “this municipality” both times—appearing as a non-migrant in both frames despite having genuinely migrated. This point-in-time design, which fails to capture the full trajectory, is not unique to this survey, but combined with the asymmetric capture of motive, it leaves a gap in motive data even for movement that is otherwise detected.
