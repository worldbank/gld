...
As the comments in the scripts explain, `A17_corrected` was created following the questionnaire's rules and it shows that 20.6% of the sample size are not eligible for the survey. These observations are in fact not interviewed, as they do not have values for variables after `A17`. Yet we still chose to have them in the final result, considering dropping them will decrease the sample size and population estimate. In addition, because they have some demographic information like age and gender. One possible cause or explanation of the small sample size and large proportion of ineligible respondents may be the Ebola crisis. The outbreak started in March 2014 and lasted through the whole year. Considering the data collection is from June to August, it seems possible that the outbreak had impacts on data collection.

![codes](utilities/SLE_scripts.png)

![eligibility](utilities/eligibility.png)

However, these observations should be excluded from labor force/WAP. The two results shown in the previous section both exclude these ineligible observations. Below are their counterparts that include these observations, with the first one being 28-value weight. Surprisingly, the number **2,801,698** is exactly the same WAP provided by ILO. So we assumed that perhaps ILO used the wrong weight and included the ineligible observations, or some other institution did it this way and reported to ILO.
