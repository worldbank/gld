...
The education variables require some care in interpretation. While the survey documentation for 2012 onward references the 2008 Philippine Standard Classification of Education (PSCED), the raw survey codes do not always follow the official PSCED level numbering literally. In practice, the survey uses a compressed attainment-style coding that is broadly usable for harmonization, but not every first digit should be interpreted mechanically as the corresponding PSCED level. The precise details are covered in a separate document [here](Education_Levels.md).
