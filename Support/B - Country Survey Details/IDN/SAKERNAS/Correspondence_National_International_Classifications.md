...
In terms of categorizing level of occupation skills, using ISCO-68 is problematic as the broad skill level defined by the ILO is based on ISCO-08 and ISCO-88 whereas ISCO-68 is significantly different from the two versions after it. To solve this issue, we used 2011-2015 as "bridge years" where the raw datasets have KBJI 1982 at three-digit level and KBJI 2002 at one-digit level, creating a self-made correspondence table between KBJI 1982 and 2022. The foundation for this mapping is that KBJI 2002 is equal to ISCO 1988 at one digit, and with the correspondence between KBJI 1982 and 2002, we can then map KBJI 1982 to ISCO 88 and next, to occupation skill level. We created a [.do file](utilities/IDN_correspondence_KBJI.do) to generate the [correspondence table](utilities/occup.dta) used for categorizing KBJI 1982 into 9 main groups for 2000-2006 where the original occupation variable has three digits. Since 1994-1999 only have one-digit occupation variable, we could not apply the correspondence table to those years. Thus, 1994-199, unfortunately, do not have the GLD variables `occup` and `occup_skill`. But they still have `occup_orig` which shows what they represent in KBJI 1982. It worths mentioning that in the "bridge years", some 1982 codes have more than one counterparts from 2002 codes. In the .do file introduced above, we used `urban` as a parameter to calculate the average probability of a given 1982 code to be assigned to different 2002 codes.

*(If you know anything that might help map KBJI 1982 to occupation skill level please contact the focal point email provided at the beginning of the doc. Thanks!)*
