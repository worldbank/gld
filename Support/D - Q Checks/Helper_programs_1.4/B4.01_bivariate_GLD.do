...
*-- 01. Education: cross-categories 
	...
	* Only do this if I have all categories
	cap keep countrycode year educat7 educat5 educat4 
	if _rc == 0 {
...
*-- 05. Lstatus x age 
	...
	* Do if all vars there
	cap keep countrycode year lstatus age weight
	if _rc {
...
*-- 06. Education x age  
	...
	* do only if all vars present
	cap keep countrycode year educat4 age weight
	if _rc == 0 {
...
*-- 07. Industry x occupation 	
	use "${mydata}", clear
	* Only do if all vars present
	cap keep countrycode year industrycat4 occup weight 
	if _rc == 0 {
...
*-- 08. Occupation x education 
	...
	* Keep only relevant vars, only if working - if vars present
	cap keep countrycode year occup educat4 weight lstatus
	if _rc == 0 {
...
*-- 09. Find issues and export results 	
	...
	** Combine data 
	clear
	forvalues i = 1/8 {
		* Capture ensures only existing files are read in
		cap append using "Block4_Bivariate/01_data/biv_`i'.dta"
	}
